...
#define get16_unaligned_check(val,addr) \
	__get16_unaligned_check("ldr",val,addr)
...
#define get32_unaligned_check(val,addr) \
	__get32_unaligned_check("ldr",val,addr)
...
#define put16_unaligned_check(val,addr)  \
	__put16_unaligned_check("str",val,addr)
...
#define put32_unaligned_check(val,addr) \
	__put32_unaligned_check("str", val, addr)
...
do_alignment_ldrhstrh(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
	...
	if (LDST_L_BIT(instr)) {
		...
		put16_unaligned_check(regs->regs[rd], addr);
		      ...
		      struct pt_regs *regs)
{
	...
	if (load) {
		...
		put32_unaligned_check(regs->regs[rd], addr);
		put32_unaligned_check(regs->regs[rd2], addr + 4);
	}
 ...
 * PU = 10             A                    B
 */
int do_alignment_ldmstm(unsigned long addr, unsigned long instr, struct pt_regs *regs)
{
		...
		for (regbits = REGMASK_BITS(instr), rd = 0; regbits;
			...
			if (regbits & 1) {
				if (LDST_L_BIT(instr)) {
					...
					put32_unaligned_check(regs->regs[rd], eaddr);
				eaddr += 4;
			}
	}
...
extern void do_bad_area(unsigned long addr, unsigned int esr, struct pt_regs *regs);
