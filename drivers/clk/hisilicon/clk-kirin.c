...
#define PPLL3_EN_ACPU				SOC_CRGPERIPH_PEREN0_gt_clk_ppll3_cpu_START
...
#define PPLL2_EN_ACPU				SOC_CRGPERIPH_PEREN0_gt_clk_ppll2_cpu_START
...
static void __iomem *hs_clk_base(u32 ctrl);
...
static void __init hi3xxx_clkgate_setup(struct device_node *np)
{
	...
	if (of_property_read_string(np, "clock-friend-names", &clk_friend))
		clk_friend = NULL;
	if (of_property_read_bool(np, "clock_id")) {
		if (of_property_read_u32_array(np, "clock_id", &clock_id, 1)) {
...
static int ppll_enable_open(struct hi3xxx_ppll_clk *ppll_clk, int ppll)
{
	...
	writel(val, (ppll_clk->addr + PPLLCTRL0(ppll)));
#endif
	/*en*/
	switch (ppll) {
	case PPLL3:
...
static void __iomem __init *hs_clk_base(u32 ctrl)
{
	struct device_node *np;
	void __iomem *ret = NULL;
