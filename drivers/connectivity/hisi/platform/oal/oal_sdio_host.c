...
oal_int32 oal_sdio_send_msg(struct oal_sdio* hi_sdio, oal_uint val)
{
    ...
    {
        OAM_ERROR_LOG0(0, OAM_SF_ANY,"{oal_sdio_send_msg::host wake up device failed!}");
        sdio_release_host(func);
		return -OAL_EBUSY;
    }
#endif
...
oal_int32 oal_sdio_msg_irq(struct oal_sdio *hi_sdio)
{
...
#endif
    if(test_bit(D2H_MSG_DEVICE_PANIC, (unsigned long *)&msg))
    {
        /*Close sdio.*/
        oal_disable_sdio_state(hi_sdio, OAL_SDIO_ALL);
    }
    ...
    /*优先处理Panic消息*/
    if(test_and_clear_bit(D2H_MSG_DEVICE_PANIC, (unsigned long *)&msg))
    {
    ...
    bit = 0;
    for_each_set_bit(bit, (const unsigned long *)&msg, 15)
    {
        OAL_BUG_ON(bit > 15);
        hi_sdio->msg[bit].count++;
        hi_sdio->last_msg = bit;
...
void  oal_sdio_exception_handler(oal_work_stru *work)
{
    ...
    oal_int32 ret = OAL_SUCC;
    struct oal_sdio* hi_sdio = oal_get_sdio_default_handler();
    struct st_wifi_dump_mem_info mem_info[2];
