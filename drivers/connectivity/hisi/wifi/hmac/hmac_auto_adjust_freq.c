...
oal_uint8 hmac_set_auto_freq_mod(oal_uint8 uc_freq_enable)
{
    ...
    if(FREQ_LOCK_ENABLE == uc_freq_enable)
    {
        ...
        OAM_WARNING_LOG1(0,OAM_SF_PWR,"{hmac_set_auto_freq_mod  set device enable level[%d]}",g_freq_lock_control.uc_curr_lock_level);
    ...
    else
    {
        ...
        OAM_WARNING_LOG1(0,OAM_SF_PWR,"{hmac_set_auto_freq_mod  set device diable level[%d]}",FREQ_HIGHEST);
