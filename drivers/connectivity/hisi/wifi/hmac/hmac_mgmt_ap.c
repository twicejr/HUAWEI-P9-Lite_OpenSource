...
OAL_STATIC oal_bool_enum hmac_ap_is_olbc_present(oal_uint8 *puc_payload, oal_uint32 ul_payload_len)
{
    ...
    /* 设置偏移 */
    us_msg_offset = MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;

    while (us_msg_offset < ul_payload_len)
    {
        if (MAC_EID_ERP == puc_payload[us_msg_offset])
        {
            pst_erp_params = (mac_erp_params_stru *)(&puc_payload[us_msg_offset + MAC_IE_HDR_LEN]);
        ...
        else if (MAC_EID_RATES == puc_payload[us_msg_offset])
        {
            uc_num_rates += puc_payload[us_msg_offset + 1];
        }
        else if (MAC_EID_XRATES == puc_payload[us_msg_offset])
        {
            uc_num_rates += puc_payload[us_msg_offset + 1];
        }
...
{
    oal_uint16              us_msg_offset;
    oal_bool_enum           em_ht_cap = OAL_FALSE;  /*指示是否存在ht cap 信息元素*/
    ...
    while (us_msg_offset < ul_payload_len)
    {
        ...
        {
            pst_ht_opern  = (mac_ht_opern_stru *)(&puc_payload[us_msg_offset + MAC_IE_HDR_LEN]);
        ...
        else if (MAC_EID_HT_CAP == puc_payload[us_msg_offset])
        {
            em_ht_cap = OAL_TRUE;
        }
                ...
                oal_uint16                     *pus_msg_idx)
{
    ...
    *puc_num_rates = 0;
    ...
    else
    {
        ...
        if (MAC_EID_XRATES == puc_ext_sup_rates_ie[0] && (pst_mac_vap->en_protocol > WLAN_LEGACY_11G_MODE))
        {
...
OAL_STATIC  oal_bool_enum_uint8  hmac_ap_is_40MHz_intol_bit_set(oal_uint8 *puc_payload, oal_uint16 us_payload_len)
{
    ...
    while (us_index < us_payload_len)
    {
        if (MAC_EID_HT_CAP == puc_payload[us_index])
        {
            ...
            /* 提取HT Capabilities Info字段 */
            us_ht_cap_info = OAL_MAKE_WORD16(puc_payload[us_index], puc_payload[us_index + 1]);
