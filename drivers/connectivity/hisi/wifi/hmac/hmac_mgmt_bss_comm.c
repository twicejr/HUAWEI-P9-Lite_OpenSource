                ...
                oal_uint8                  *puc_payload)
{
    ...
    /* AMPDU使能判断，忽略ADDBA_REQ */
    if (OAL_TRUE != hmac_user_xht_support(pst_hmac_user))
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                      "{hmac_mgmt_rx_addba_req::the user is neither ht nor vht station.}");
        return OAL_SUCC;
    }
    ...
    if (!(pst_mac_vap->en_protocol >= WLAN_HT_MODE))
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA,
                      "{hmac_mgmt_rx_addba_req::ampdu is not supprot or not open,protocol mode=%d.}", pst_mac_vap->en_protocol);
        return OAL_SUCC;
    }
    ...
    {
        OAM_ERROR_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_req::addba req receive failed, tid %d overflow.}", uc_tid);
...
oal_void hmac_set_user_protocol_mode(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user)
{
    ...
    else if (OAL_TRUE == hmac_user_is_rate_support(pst_hmac_user,0x0C))   /* 判断是否支持6M(0x0C) */
    {
        ...
        else if(OAL_TRUE == hmac_user_is_rate_support(pst_hmac_user,0x16)) /* 判断是否支持11M(0x16)*/
        {
    ...
    {
        /* 兼容性问题：解决对端设备仅支持单速率且为非基本速率的问题 */
        if (WLAN_BAND_5G == pst_mac_vap->st_channel.en_band)            /* 判断是否是5G */
        {
            mac_user_set_protocol_mode(pst_mac_user, WLAN_LEGACY_11A_MODE);
        }
        else if (WLAN_BAND_2G == pst_mac_vap->st_channel.en_band)
        {
            if ((OAL_TRUE == hmac_user_is_rate_support(pst_hmac_user,0x02))
                || (OAL_TRUE == hmac_user_is_rate_support(pst_hmac_user,0x04))
                || (OAL_TRUE == hmac_user_is_rate_support(pst_hmac_user,0x0B)))
            {
                mac_user_set_protocol_mode(pst_mac_user, WLAN_LEGACY_11B_MODE);
            }
            else
        ...
        {
            OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_set_user_protocol_mode::get user protocol failed.}");
        }
    }
...
oal_void  hmac_rx_notify_channel_width(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    /* 根据发送端地找到对应AP USER结构 */
    pst_mac_user = mac_vap_get_user_by_addr(pst_mac_vap, auc_sta_addr);
    if (OAL_PTR_NULL == pst_mac_user)
    {
        OAM_WARNING_LOG0(pst_mac_vap->uc_vap_id, OAM_SF_2040, "{hmac_rx_notify_channel_width::pst_mac_user null.}");
        return;
    }
    ...
    mac_ie_proc_chwidth_field(pst_mac_vap, pst_mac_user,(puc_data[MAC_ACTION_OFFSET_ACTION + 1] & BIT0));
                                                ...
                                                oal_int          l_freq)
{
    ...
    pst_mgmt_frame->puc_buf = (oal_uint8 *)pst_rx_info->pul_mac_hdr_start_addr;
