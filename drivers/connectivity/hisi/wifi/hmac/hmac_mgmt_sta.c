...
oal_uint32 hmac_sta_get_user_protocol(mac_bss_dscr_stru *pst_bss_dscr, wlan_protocol_enum_uint8  *pen_protocol_mode)
{
    ...
    else if (OAL_TRUE == hmac_bss_is_rate_support(pst_bss_dscr,0x0C))   /* 判断是否支持6M(0x0C) */
    {
        ...
        else if(OAL_TRUE == hmac_bss_is_rate_support(pst_bss_dscr,0x16)) /* 判断是否支持11M(0x16)*/
        {
    ...
    else
    {
        ...
        else if (WLAN_BAND_2G == pst_bss_dscr->st_channel.en_band)
        {
            if ((OAL_TRUE == hmac_bss_is_rate_support(pst_bss_dscr,0x02))
                || (OAL_TRUE == hmac_bss_is_rate_support(pst_bss_dscr,0x04))
                || (OAL_TRUE == hmac_bss_is_rate_support(pst_bss_dscr,0x0B)))
            {
                *pen_protocol_mode = WLAN_LEGACY_11B_MODE;
            }
            else
...
oal_uint32  hmac_sta_wait_auth_seq2_rx(hmac_vap_stru *pst_sta, oal_void *pst_msg)
{
    ...
    if (WLAN_WITP_AUTH_SHARED_KEY == us_auth_alg)
    {
        /* 准备seq = 3的认证帧 */
        pst_auth_frame = OAL_MEM_NETBUF_ALLOC(OAL_NORMAL_NETBUF, WLAN_MEM_NETBUF_SIZE2, OAL_NETBUF_PRIORITY_MID);
                    ...
                    hmac_user_stru          *pst_hmac_user)
{
    ...
    while (us_msg_offset < us_msg_len)
    {
        /* 判断当前的ie是否是wmm ie，如果不是，继续检查下一个ie，如果是，更新EDCA参数 */
        if (OAL_TRUE == mac_is_wmm_ie(&puc_payload[us_msg_offset]))
        {
            ...
            /* 如果收到的是beacon帧，并且param_set_count没有改变，说明AP的EDCA参数没有变
               则STA也不用做任何改变，直接返回即可.
            */
...
oal_uint32  hmac_ie_proc_obss_scan_ie(mac_vap_stru *pst_mac_vap, oal_uint8 *puc_payload)
{
    ...
    ***************************************************************************/
    ...
    mac_mib_set_BSSWidthTriggerScanInterval(pst_mac_vap, OAL_MAKE_WORD16(puc_payload[6], puc_payload[7]));
...
oal_uint32  hmac_ie_proc_sec_chan_offset_2040(mac_vap_stru *pst_mac_vap, mac_sec_ch_off_enum_uint8 en_sec_chan_offset)
{
    ...
    {
        pst_mac_vap->st_channel.en_bandwidth = WLAN_BAND_WIDTH_20M;
        /*DTS2014110702152 先判断是否支持HT模式在判断40M*/
                               ...
                               oal_uint16      *pus_amsdu_maxsize)
{
    ...
    while (us_index < us_rx_len)
    {
        /* 搜索 HT Capabilities Element */
        if (MAC_EID_HT_CAP == puc_payload[us_index])
        {
            ...
            mac_ie_proc_ht_sta(pst_mac_sta, puc_payload, &us_index, pst_mac_user_ap, &us_ht_cap_info, pus_amsdu_maxsize);
            ...
            {
                OAM_WARNING_LOG0(0, OAM_SF_ANY, "{hmac_ie_check_ht_sta::AP support ht capability but support none space_stream.}");
                /* 对端ht能力置为不支持 */
                mac_user_set_ht_capable(pst_mac_user_ap, OAL_FALSE);
            }
        }
/*#ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST */
        else if (MAC_EID_EXT_CAPS == puc_payload[us_index])
        {
            /* 处理 Extended Capabilities IE */
            mac_ie_proc_ext_cap_ie(pst_mac_user_ap, &puc_payload[us_index]);
        }
        else if (MAC_EID_HT_OPERATION == puc_payload[us_index])
        {
            ul_change |= hmac_proc_ht_opern_ie(pst_mac_sta, &puc_payload[us_index], pst_mac_user_ap);
        ...
        else if (MAC_EID_OBSS_SCAN == puc_payload[us_index])
        {
            ...
            hmac_ie_proc_obss_scan_ie(pst_mac_sta, &puc_payload[us_index]);
        }
/* #endif */  /* _PRE_WLAN_FEATURE_20_40_80_COEXIST */
...
OAL_STATIC oal_void  hmac_sta_up_rx_ch_switch(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    while (us_index < us_framebody_len)
    {
        ...
        {
            //l00311403TODO - 如果通道发生改变了，不需要切信道么?
...
OAL_STATIC oal_void  hmac_sta_up_rx_ext_ch_switch(mac_vap_stru *pst_mac_vap, oal_netbuf_stru *pst_netbuf)
{
    ...
    while (us_index < us_framebody_len)
    {
        if (MAC_EID_WIDE_BW_CH_SWITCH == puc_data[us_index])
        {
            hmac_ie_proc_wide_bandwidth_ie(pst_mac_vap, &puc_data[us_index]);
        }
...
oal_uint32  hmac_sta_up_rx_mgmt(hmac_vap_stru *pst_hmac_vap_sta, oal_void *p_param)
{
    ...
    /*Bar frame proc here*/
    if (WLAN_CONTROL == mac_get_frame_type(puc_mac_hdr))
    {
        uc_mgmt_frm_type = mac_get_frame_sub_type(puc_mac_hdr);
