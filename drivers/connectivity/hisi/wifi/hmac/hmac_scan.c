                                                                     ...
                                                                     oal_uint8 *puc_ie)
{
    ...
    *puc_grp_policy = puc_ie[uc_index + MAC_OUI_LEN];
    ...
    for (us_temp = 0; us_temp < us_pcip_num; us_temp++)
    {
        ...
        {
            OAM_WARNING_LOG1(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_rsn::us_pcip_num=%d.}", us_pcip_num);
            return OAL_FALSE;
        }

        if (0 != oal_memcmp(puc_oui, puc_ie + uc_index, MAC_OUI_LEN))
        {
            OAM_WARNING_LOG0(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_rsn::invalid RSN paerwise OUI.}");
    ...
    for (us_temp = 0; us_temp < us_auth_num; us_temp++)
    {
        if (0 != oal_memcmp(puc_oui, puc_ie + uc_index, MAC_OUI_LEN))
        {
            OAM_WARNING_LOG0(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_rsn::invalid RSN auth OUI.}");
            return OAL_FALSE;
        }
                                                                      ...
                                                                      oal_uint8 *puc_ie)
{
    ...
    *puc_grp_policy = puc_ie[uc_index + MAC_OUI_LEN];
    ...
    for (us_temp = 0; us_temp < us_pcip_num; us_temp++)
    {
        if (us_pcip_num > MAC_PAIRWISE_CIPHER_SUITES_NUM)
        {
            OAM_WARNING_LOG1(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_wpa::us_pcip_num=%d.}", us_pcip_num);
            return OAL_FALSE;
        }

        if (0 != oal_memcmp(puc_oui, puc_ie + uc_index, MAC_OUI_LEN))
        {
            OAM_WARNING_LOG0(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_wpa::invalid WPA pairwise OUI}");
    ...
    for (us_temp = 0; us_temp < us_auth_num; us_temp++)
    {
        if (0 != oal_memcmp(puc_oui, puc_ie + uc_index, MAC_OUI_LEN))
        {
            OAM_WARNING_LOG0(0, OAM_SF_SCAN, "{hmac_scan_update_bss_list_wpa::invalid WPA auth OUI}");
                                                            ...
                                                            oal_uint16           us_offset)
{
    ...
    /* 11n */
    //puc_ie = mac_get_ht_cap_ie(puc_frame_body, us_frame_len, us_offset);
    ...
    if (OAL_PTR_NULL != puc_ie)
    {
        ...
        pst_bss_dscr->en_bw_cap = ((puc_ie[2] & BIT1) >> 1);            /* 取出支持的带宽 */
        pst_bss_dscr->en_ht_stbc = ((puc_ie[2] & BIT7) >> 7);                                                                /* puc_ie[2]是HT Capabilities Info的第1个字节 */
    }

    //puc_ie = mac_get_ht_opern_ie(puc_frame_body, us_frame_len, us_offset);
    puc_ie = mac_find_ie(MAC_EID_HT_OPERATION, puc_frame_body + us_offset, us_frame_len - us_offset);
    if (OAL_PTR_NULL != puc_ie)
    {
        /* 提取次信道偏移 */
        uc_sec_chan_offset = (puc_ie[3] & 0x03);                        /* puc_ie[3]是HT Operation Information的第1字节 */
    ...
    //puc_ie = mac_get_ext_cap_ie(puc_frame_body, us_frame_len);
                                                                          ...
                                                                          oal_uint16         us_frame_len)
{
    ...
    if (OAL_PTR_NULL != puc_ie)
    {
        ...
        /* DTS2015032407334 斐讯FIR304商用AP 11g模式，发送的支持速率集个数为12，不符合协议规定，为增加兼容性，修改判断分支为12 */
    ...
    pst_bss_dscr->uc_num_supp_rates = uc_num_rates + uc_num_ex_rates;
                                                          ...
                                                          oal_uint8                uc_vap_id)
{
    ...
    *****************************************************************************/
    ...
    /* 记录速率集 */
    hmac_scan_update_bss_list_rates(pst_bss_dscr, puc_frame_body, us_frame_body_len);
...
{
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION) && (_PRE_PRODUCT_ID_HI1102_HOST == _PRE_PRODUCT_ID)
