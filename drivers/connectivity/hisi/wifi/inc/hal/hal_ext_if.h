...
/*****************************************************************************
 函 数 名  : hal_get_tx_dscr_queue_total_ppdu_num
 功能描述  : 获取5个发送队列(4个EDCA队列 + 1个高优先级队列)中待发送的PPDU个数
 输入参数  : pst_hal_device: HAL to DEVICE结构体指针
 输出参数  : pus_ppdu_cnt  : 发送队列中PPDU的个数
 返 回 值  : 无
 调用函数  :
 被调函数  :
...
{
    /* d00262548 在使能pa之前，复位一下mac和phy的逻辑，防止硬件还有数据，概率性造成mac异常处理 */
...
/*****************************************************************************
 函 数 名  : hal_disable_non_frame_filter
 功能描述  : 关闭non_frame_filter
 输入参数  : 无
 输出参数  : 无
 返 回 值  : oal_void
 调用函数  :
