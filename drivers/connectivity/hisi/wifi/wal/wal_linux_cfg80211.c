                                            ...
                                            oal_cfg80211_connect_params_stru    *pst_sme)
{
    ...
    puc_ie = mac_find_ie(MAC_EID_RSN, pst_sme->ie, (oal_int32)(pst_sme->ie_len));
...
/* iw接口下发的关联请求，有可能无信道信息，此时会访问空指针，并且此处获取信道号，在入网过程中，并没有用到 */
    ...
    /* 解析内核下发的 ssid */
    st_mac_cfg80211_connect_param.puc_ssid      = pst_sme->ssid;
    st_mac_cfg80211_connect_param.uc_ssid_len   = (oal_uint8)pst_sme->ssid_len;

    /* 解析内核下发的 bssid */
    st_mac_cfg80211_connect_param.puc_bssid     = pst_sme->bssid;
    ...
    /* 设置关联P2P/WPS ie */
    st_mac_cfg80211_connect_param.puc_ie        = pst_sme->ie;
                                                   ...
                                                   oal_station_info_stru *pst_sta_info)
{
    ...
    oal_set_mac_addr(pst_query_station_info->auc_query_sta_addr, puc_mac);
    ...
    /* 固定时间内最多更新一次RSSI */
    if(OAL_FALSE == wal_cfg80211_get_station_filter(&pst_hmac_vap->st_vap_base_info, puc_mac))
    {
