...
oal_int32  wal_cfg_vap_h2d_event(oal_net_device_stru *pst_net_dev)
{
    ...
    {
        OAM_WARNING_LOG1(0,OAM_SF_ANY,"{wal_init_wlan_vap::mac_res_get_hmac_vap fail.vap_id[%u]}",pst_mac_device->uc_cfg_vap_id);
        return OAL_ERR_CODE_PTR_NULL;
    }
...
OAL_STATIC oal_int32 hwifi_config_host_global_ini_param(oal_void)
{
    ...
    if(!uc_flag)
    {
        ...
        {
            OAM_WARNING_LOG4(0, OAM_SF_ANY, "{hwifi_config_host_global_ini_param::ul_speed_level = %d,ul_min_cpu_freq = %d,uc_device_type = %d}\r\n",
...
OAL_STATIC oal_void hwifi_config_init_ini_rf(oal_net_device_stru *pst_cfg_net_dev)
{
    ...
    for (uc_idx=0; uc_idx<MAC_NUM_2G_BAND; ++uc_idx)
    {
        ...
        {
            pst_customize_rf->ac_gain_db_2g[uc_idx].c_rf_gain_db_2g_mult4 = c_mult4;
            pst_customize_rf->ac_gain_db_2g[uc_idx].c_rf_gain_db_2g_mult10 = c_mult10;
        }
        else
        {
            /* 值超出有效范围，标记置为TRUE */
...
OAL_STATIC oal_int32  _wal_netdev_open(oal_net_device_stru *pst_net_dev)
{
    ...
    oal_wireless_dev_stru   *pst_wdev;
#endif
#if ((_PRE_PRODUCT_ID == _PRE_PRODUCT_ID_HI1102_HOST)&&(_PRE_OS_VERSION_WIN32 != _PRE_OS_VERSION))
    ...
    else if ((OAL_TRUE == g_st_dfr_info.bit_ready_to_recovery_flag) && (0 == (oal_strcmp("wlan0", pst_net_dev->name))))
    {
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
...
oal_int32 wal_set_custom_process_func(oal_void)
{
...
oal_int32 wal_android_priv_cmd(oal_net_device_stru *pst_net_dev, oal_ifreq_stru *pst_ifr, oal_int32 ul_cmd)
{
    ...
    {
        OAM_WARNING_LOG2(0, OAM_SF_ANY, "{wal_android_priv_cmd::netdev not running[%d], dfr_process_status[%d]!}",
                (OAL_NETDEVICE_FLAGS(pst_net_dev) & OAL_IFF_RUNNING),
...
#ifdef _PRE_WLAN_FEATURE_HS20
    else if (0 == oal_memcmp(puc_command, CMD_SET_QOS_MAP, OAL_STRLEN(CMD_SET_QOS_MAP)))
    {
        oal_uint32 skip = OAL_STRLEN(CMD_SET_QOS_MAP) + 1;
        hmac_cfg_qos_map_param_stru st_qos_map_param;
        oal_memcopy(&st_qos_map_param, puc_command + skip, OAL_SIZEOF(hmac_cfg_qos_map_param_stru));
    ...
    else if(0 == oal_memcmp(puc_command, CMD_SET_POWER_MGMT_ON, OAL_STRLEN(CMD_SET_POWER_MGMT_ON)))
    {
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)&&(_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
        ...
        power_mgmt_flag = (oal_uint)oal_atoi((oal_int8 *)puc_command + OAL_STRLEN((oal_int8 *)CMD_SET_POWER_MGMT_ON) + 1);
        ...
        {
            OAM_WARNING_LOG0(0, OAM_SF_ANY, "{wal_android_priv_cmd::wlan_pm_get_drv return null.");
        }
#endif
...
#ifdef _PRE_WLAN_FEATURE_LTECOEX
    else if(0 == oal_memcmp(puc_command, CMD_LTECOEX_MODE, OAL_STRLEN(CMD_LTECOEX_MODE)))
    {
        oal_int32      l_ret;
        oal_int8       ltecoex_mode;
        /* 格式:LTECOEX_MODE 1 or LTECOEX_MODE 0 */
        ltecoex_mode = oal_atoi((oal_int8 *)puc_command + OAL_STRLEN((oal_int8 *)CMD_LTECOEX_MODE) + 1);
...
#endif
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
    else if (oal_memcmp(puc_command, CMD_TX_POWER, OAL_STRLEN(CMD_TX_POWER)) == 0)
    {
        oal_uint32  ul_skip = OAL_STRLEN((oal_int8 *)CMD_TX_POWER) + 1;
        oal_uint8   uc_txpwr = (oal_uint8)oal_atoi((oal_int8*)puc_command + ul_skip);
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    else if (oal_memcmp(puc_command, CMD_WPAS_GET_CUST, OAL_STRLEN(CMD_WPAS_GET_CUST)) == 0)
    {
        /* 将buf清零 */
        oal_memset(puc_command, 0, st_priv_cmd.l_total_len + 1);
        puc_command[st_priv_cmd.l_total_len] = '\0';
        ...
        /* 赋值ht40禁止位 */
        *puc_command = g_st_wlan_customize.uc_disable_capab_2ght40;

        if(oal_copy_to_user(pst_ifr->ifr_data+8, puc_command, (oal_uint32)(st_priv_cmd.l_total_len)))
        {
...
OAL_STATIC oal_uint32  wal_ioctl_set_txpower(oal_net_device_stru *pst_net_dev, oal_int8 *pc_param)
{
    ...
    *((oal_int32 *)(st_write_msg.auc_value)) = l_pwer;
    ...
    if (OAL_UNLIKELY(OAL_SUCC != l_ret))
    {
        OAM_WARNING_LOG1(0, OAM_SF_ANY, "{wal_ioctl_set_txpower::return err code %d!}", l_ret);
        return (oal_uint32)l_ret;
    }
...
oal_uint32  wal_hipriv_create_proc(oal_void *p_proc_arg)
{
    ...
    ul_ret = (oal_uint32)sysfs_create_file(gp_sys_kobject,&dev_attr_hipriv.attr);
#else
    ul_ret = OAL_SUCC;
#endif
    return ul_ret;
...
OAL_STATIC oal_int32  wal_ioctl_get_assoc_list(oal_net_device_stru *pst_net_dev, oal_iw_request_info_stru *pst_info, oal_iwreq_data_union *pst_wrqu, oal_int8 *pc_extra)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_info || OAL_PTR_NULL == pst_wrqu || OAL_PTR_NULL == pc_extra))
    {
        OAM_WARNING_LOG3(0, OAM_SF_ANY, "{wal_ioctl_get_assoc_list:: param is OAL_PTR_NULL , pst_info = %p , pst_wrqu = %p , pc_extra = %p}\n", pst_info, pst_wrqu, pc_extra);
        return -OAL_EFAIL;
    }
...
OAL_STATIC oal_int32  wal_ioctl_set_mac_filters(oal_net_device_stru *pst_net_dev, oal_iw_request_info_stru *pst_info, oal_iwreq_data_union *pst_wrqu, oal_int8 *pc_extra)
{
    ...
    if (OAL_UNLIKELY(OAL_PTR_NULL == pst_info || OAL_PTR_NULL == pst_wrqu || OAL_PTR_NULL == pc_extra))
    {
        OAM_WARNING_LOG3(0, OAM_SF_ANY, "{wal_ioctl_set_mac_filters:: param is OAL_PTR_NULL , pst_info = %p , pst_wrqu = %p , pc_extra = %p}\n", pst_info, pst_wrqu, pc_extra);
        return -OAL_EFAIL;
    }
