...
#define THRESHOLD_TASK_NUMBER	3
...
static int cpufreq_bL_hotplug_task(void *data)
{
	...
	while (1) {
		...
		ret = bL_cpufreq_set_rate(tmp_data.cpu, A7_CLUSTER, A7_CLUSTER, tmp_data.target);
		...
		unsigned int target_freq, unsigned int relation)
{
	...
	if (cur_cluster == A7_CLUSTER) {
		...
		if (nr_running < THRESHOLD_TASK_NUMBER) {
			if (bL_cpufreq_data.hotplugged_by_cpufreq)
				goto normal_set;
			pr_debug("[hotplug down]: oldfreq: %d, newfreq: %d, nr:%d\n",
					freqs.old, freqs.new, nr_running);

			spin_lock_irqsave(&(bL_cpufreq_data.hotplug_lock), flags);
			bL_cpufreq_data.target = freqs.new;
			...
			if (!bL_cpufreq_data.hotplugged_by_cpufreq)
				goto normal_set;
			spin_lock_irqsave(&(bL_cpufreq_data.hotplug_lock), flags);
			bL_cpufreq_data.target = min(freqs.new, THRESHOLD_FREQ);
			freqs.new = bL_cpufreq_data.target;
			bL_cpufreq_data.hotplug_up = true;
			spin_unlock_irqrestore(&(bL_cpufreq_data.hotplug_lock), flags);
			pr_debug("[hotplug up]: oldfreq: %d, newfreq: %d, nr:%d\n",
					freqs.old, freqs.new, nr_running);
		}
	...
	if (ret) {
		...
		if (ret) {
			...
			if (IS_ERR(hotplug_task)) {
				...
				spin_lock_init(&(bL_cpufreq_data.hotplug_lock));
