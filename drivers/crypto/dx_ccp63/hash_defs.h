...
typedef struct SepHashPrivateContext {
	/* The current length is placed at the end of the context buffer because the hash 
	   ...
	   K0 field.  The size of struct sep_ctx_hash reserved field is  88/184 bytes depend if t
	   he SHA512 is supported ( in this case teh context size is 256 bytes).
	   The size of struct sep_ctx_hash reseved field is 20 or 52 depend if the SHA512 is supported.
	...
	uint32_t reserved[(sizeof(struct sep_ctx_hash)/sizeof(uint32_t)) - SEP_HASH_LENGTH_WORDS - 3];
