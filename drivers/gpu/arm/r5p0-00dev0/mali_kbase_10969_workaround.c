	...
	if (katom->core_req & BASE_JD_REQ_FS){
		...
		if (region){
			...
			if (page_array){
				...
				/* page_1 is a u32 pointer, offset is expressed
				 * in bytes */
				page_1 += offset>>2;
				...
				/* We managed to correctly map one or two pages (in case of overflow ) */
				{
					...
					if (clamped){
						...
						dma_sync_single_for_device(katom->kctx->kbdev->dev,
									   page_private(pfn_to_page(PFN_DOWN(page_array[page_index]))) + offset,
									   copy_size,
									   DMA_TO_DEVICE);
						...
						if (copy_size < JOB_HEADER_SIZE){
							 ...
							 dma_sync_single_for_device(katom->kctx->kbdev->dev,
										    page_private(pfn_to_page(PFN_DOWN(page_array[page_index + 1]))),
										    JOB_HEADER_SIZE - copy_size,
										    DMA_TO_DEVICE);
						}
