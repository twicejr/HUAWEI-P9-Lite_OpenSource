	...
	case KBASE_FUNC_MEM_IMPORT:
		{
			...
			if (kbase_mem_import(kctx, mem_import->type, handle, &mem_import->gpu_va, &mem_import->va_pages, &mem_import->flags, mem_import->header_page_number)) {
bad_type:
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
			}
			break;
...
copy_failed:
	...
	default:
		...
		HiLOGE("DISPLAY", "GPU", "unknown ioctl %u", id);
		goto out_bad;
	}
...
static mali_error kbase_set_custom_irq_handler(struct kbase_device *kbdev, irq_handler_t custom_handler, int irq_type)
{
	...
	if (0 != request_irq(kbdev->irqs[irq_type].irq,
		...
		HiLOGE("DISPLAY", "GPU", "Can't request interrupt %d (index %d)", kbdev->irqs[irq_type].irq, irq_type);
...
static int kbase_install_interrupts(struct kbase_device *kbdev)
{
	...
	for (i = 0; i < nr; i++) {
		...
		if (err) {
			dev_err(kbdev->dev, "Can't request interrupt %d (index %d)\n", kbdev->irqs[i].irq, i);
			HiLOGE("DISPLAY", "GPU", "Can't request interrupt %d (index %d)", kbdev->irqs[i].irq, i);
#ifdef CONFIG_SPARSE_IRQ
			dev_err(kbdev->dev, "You have CONFIG_SPARSE_IRQ support enabled - is the interrupt number correct for this configuration?\n");
#endif /* CONFIG_SPARSE_IRQ */
			goto release;
...
static ssize_t set_js_softstop_always(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((items == 1) && ((softstop_always == 0) || (softstop_always == 1))) {
		...
		HiLOGE("DISPLAY", "GPU", "Couldn't process js_softstop_always write operation.\nUse format <soft_stop_always>");
        return -EINVAL;
	}
}
...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	if (!kbdev->reg_res) {
		...
		HiLOGE("DISPLAY", "GPU", "Register window unavailable");
		err = -EIO;
		goto out_region;
	}
	...
	if (!kbdev->reg) {
		...
		HiLOGE("DISPLAY", "GPU", "Can't remap register window");
		err = -EINVAL;
		goto out_ioremap;
	}
