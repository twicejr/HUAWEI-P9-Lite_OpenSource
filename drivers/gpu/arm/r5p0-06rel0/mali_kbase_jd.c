...
static mali_error kbase_jd_umm_map(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	if (reg->alloc->header_page_number)
		err = kbase_mmu_insert_pages_with_scramble_bit(kctx, reg->start_pfn, kbase_get_phy_pages(reg), kbase_reg_current_backed_size(reg), reg->flags | KBASE_REG_GPU_WR | KBASE_REG_GPU_RD,reg->alloc->header_page_number);
	else
...
#endif /* BASE_LEGACY_UK6_SUPPORT */
{
	...
	if ((kctx->jctx.sched_info.ctx.flags & KBASE_CTX_FLAG_SUBMIT_DISABLED) != 0) {
		...
		HiLOGE("DISPLAY", "GPU", "Attempt to submit to a context that has SUBMIT_DISABLED set on it");
		return MALI_ERROR_FUNCTION_FAILED;
	}
	...
	if (submit_data->stride != sizeof(base_jd_atom_v2)) {
		...
		HiLOGE("DISPLAY", "GPU", "Stride passed to job_submit doesn't match kernel");
		return MALI_ERROR_FUNCTION_FAILED;
	}
...
static enum hrtimer_restart zap_timeout_callback(struct hrtimer *timer)
{
	...
	if (kbase_prepare_to_reset_gpu(kbdev)) {
		...
		HiLOGE("DISPLAY", "GPU", "Issueing GPU soft-reset because jobs failed to be killed (within %d ms) as part of context termination (e.g. process exit)", ZAP_TIMEOUT);
