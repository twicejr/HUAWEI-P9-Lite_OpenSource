...
void kbase_job_done_slot(struct kbase_device *kbdev, int s, u32 completion_code, u64 job_tail, ktime_t *end_timestamp)
{
	...
	if (completion_code != BASE_JD_EVENT_DONE && completion_code != BASE_JD_EVENT_STOPPED)
    {
		...
		HiLOGE("DISPLAY", "GPU", "GPU fault 0x%02lx from job slot %d", (unsigned long)completion_code, s);
    }
		...
		do {
			...
			if (failed & (1u << i)) {
				...
				default:
					...
					HiLOGE("DISPLAY","GPU","error detected from slot %d, job status 0x%08x (%s)", i, completion_code, kbase_exception_name(completion_code));
				}
			}
...
static void kbasep_job_slot_soft_or_hard_stop_do_action(struct kbase_device *kbdev, int js, u32 action, u16 core_reqs, struct kbase_jd_atom *target_katom)
{
	...
	if (action == JS_COMMAND_HARD_STOP) {
		...
		if (!hard_stop_allowed) {
			...
			HiLOGE("DISPLAY","GPU","Attempt made to hard-stop a job that cannot be hard-stopped. core_reqs = 0x%X", (unsigned int)core_reqs);
            return;
		}
				...
				struct kbase_jd_atom *target_katom)
{
	...
	if (kbase_hw_has_issue(kctx->kbdev, BASE_HW_ISSUE_8401) ||
		...
		if (kbase_prepare_to_reset_gpu_locked(kbdev)) {
			...
			HiLOGE("DISPLAY", "GPU", "Issueing GPU\
			soft-reset after hard stopping due to hardware issue");
...
void kbasep_reset_timeout_worker(struct work_struct *data)
{
	...
	HiLOGE("DISPLAY", "GPU", "Resetting GPU (allowing up to %d ms)", RESET_TIMEOUT);
	...
	for (i = 0; i < kbdev->gpu_props.num_job_slots; i++) {
		...
		while (nr_done) {
			...
			HiLOGE("DISPLAY", "GPU", "Job stuck in slot %d on the GPU was cancelled", i);
...
void kbase_reset_gpu(struct kbase_device *kbdev)
{
	...
	HiLOGE("DISPLAY", "GPU", "Preparing to soft-reset GPU: Waiting (upto %d ms) for all jobs to complete soft-stop", timeout_ms);
