...
static enum hrtimer_restart timer_callback(struct hrtimer *timer)
{
		...
		if (atom != NULL) {
			...
			if (!kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_5736)) {
				...
				if (ticks == soft_stop_ticks) {
					...
					HiLOGE("DISPLAY","GPU","JS: Job Hard-Stopped (took more than %lu ticks at %lu ms/tick)", (unsigned long)ticks, (unsigned long)(js_devdata->scheduling_tick_ns / 1000000u));
	...
	if (reset_needed) {
		...
		HiLOGE("DISPLAY", "GPU", "JS: Job has been on the GPU for too long (KBASE_CONFIG_ATTR_JS_RESET_TICKS_SS/NSS timeout hit). Issueing GPU soft-reset to resolve.");
