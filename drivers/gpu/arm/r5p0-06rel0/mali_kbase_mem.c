		...
		size_t nr_pages, size_t align)
{
	...
	if (gpu_pfn) {
		...
		if (!tmp) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_GPU_MEMORY, Enclosing region not found: 0x%08llx gpu_pfn, %zu nr_pages",
                    gpu_pfn, nr_pages);
		...
		if ((!kbase_region_tracker_match_zone(tmp, reg)) ||
			...
			err = MALI_ERROR_OUT_OF_GPU_MEMORY;
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_GPU_MEMORY, (%p, %p, 0x%llx, 0x%zx, 0x%zx)",
		...
		if (err) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_GPU_MEMORY:Failed to insert va region");
			err = MALI_ERROR_OUT_OF_GPU_MEMORY;
			goto exit;
		}
	...
	/* Path 2: Map any free address which meets the requirements.  */
	{
		...
		if (!tmp) {
			err = MALI_ERROR_OUT_OF_GPU_MEMORY;
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_GPU_MEMORY");
			goto exit;
		}
...
struct kbase_va_region *kbase_alloc_free_region(struct kbase_context *kctx, u64 start_pfn, size_t nr_pages, int zone)
{
	...
	if (!new_reg) {
		...
		HiLOGE("DISPLAY","GPU","kzalloc failed");
		return NULL;
	}
...
static mali_error kbase_do_syncset(struct kbase_context *kctx, struct base_syncset *set, kbase_sync_kmem_fn sync_fn)
{
	...
	if (!reg) {
		...
		HiLOGE("DISPLAY","GPU","MALI_ERROR_FUNCTION_FAILED:Can't find region at VA 0x%016llX",sset->mem_handle);
	...
	if (!map) {
		...
		HiLOGE("DISPLAY","GPU","MALI_ERROR_FUNCTION_FAILED:Can't find CPU mapping 0x%016lX for VA 0x%016llX", start, sset->mem_handle);
        err = MALI_ERROR_FUNCTION_FAILED;
		goto out_unlock;
	}
...
mali_error kbase_mem_free_region(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	if (err) {
		...
		HiLOGE("DISPLAY","GPU","Could not unmap from the GPU...");
		goto out;
	}
