...
struct kbase_va_region *kbase_mem_alloc(struct kbase_context *kctx, u64 va_pages, u64 commit_pages, u64 extent, u64 *flags, u64 *gpu_va, u16 *va_alignment)
{
	...
	*gpu_va = 0; /* return 0 on failure */
	...
	if (!kbase_check_alloc_flags(*flags)) {
		...
		HiLOGE("DISPLAY","GPU","kbase_mem_alloc called with bad flags (%llx)",
				(unsigned long long)*flags);
		goto bad_flags;
	}
	...
	if (!reg) {
		...
		HiLOGE("DISPLAY", "GPU", "Failed to allocate free region");
		goto no_region;
	}

	if (MALI_ERROR_NONE != kbase_reg_prepare_native(reg, kctx)) {
		dev_err(dev, "Failed to prepare region");
		HiLOGE("DISPLAY", "GPU", "Failed to prepare region");
		goto prepare_failed;
	}
	...
	if (kbase_alloc_phy_pages(reg, va_pages, commit_pages)) {
		...
		HiLOGE("DISPLAY","GPU","Failed to allocate %lld pages (va_pages=%lld)",
				(unsigned long long)commit_pages,
				(unsigned long long)va_pages);
		goto no_mem;
	}
	...
	if (*flags & BASE_MEM_SAME_VA) {
		...
		if (!kctx->cookies) {
			...
			HiLOGE("DISPLAY", "GPU", "No cookies available for allocation! \n");
			goto no_cookie;
		}
		/* return a cookie */
		...
		*gpu_va <<= PAGE_SHIFT;
		...
		if (MALI_ERROR_NONE != kbase_gpu_mmap(kctx, reg, 0, va_pages, 1)) {
			...
			HiLOGE("DISPLAY","GPU","Failed to map memory on GPU");
			goto no_mmap;
		}
		/* return real GPU VA */
...
static struct kbase_va_region *kbase_mem_from_umm(struct kbase_context *kctx, int fd, u64 *va_pages, u64 *flags, u64 header_page_number)
{
	...
	*flags &= ~BASE_MEM_SAME_VA;
	...
	reg->extent = 0;
	reg->alloc->header_page_number = header_page_number;
	...
	*flags &= (BASE_MEM_PROT_GPU_RD | BASE_MEM_PROT_GPU_WR |
	...
	if (!(*flags & (BASE_MEM_PROT_GPU_RD | BASE_MEM_PROT_GPU_WR))) {
		dev_warn(kctx->kbdev->dev,
				...
				(unsigned long long)*flags);
		HiLOGE("DISPLAY","GPU","kbase_mem_alias called with bad flags (%llx)",
				(unsigned long long)*flags);
		goto bad_flags;
	}
...
int kbase_mem_import(struct kbase_context *kctx, enum base_mem_import_type type, int handle, mali_addr64 *gpu_va, u64 *va_pages, u64 *flags, u64 header_page_number)
{
	struct kbase_va_region *reg;
	...
	case BASE_MEM_IMPORT_TYPE_UMM:
		reg = kbase_mem_from_umm(kctx, handle, va_pages, flags, header_page_number);
		break;
#endif /* CONFIG_DMA_SHARED_BUFFER */
	default:
		reg = NULL;
	...
	case PFN_DOWN(BASE_MEM_COOKIE_BASE) ...
		...
		if (NULL != reg) {
			...
			if (MALI_ERROR_NONE != kbase_gpu_mmap(kctx, reg,
							      ...
							      1)) {
				dev_err(dev, "%s:%d\n", __FILE__, __LINE__);
				HiLOGE("DISPLAY", "GPU", "Unable to map in GPU space.");
...
out:
	if (err)
    {
		...
		HiLOGE("DISPLAY", "GPU", "mmap failed %d", err);
    }
