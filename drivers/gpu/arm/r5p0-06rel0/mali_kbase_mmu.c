...
static u64 mmu_phyaddr_to_ate_with_scramble_bit(phys_addr_t phy, u64 flags)
{
	return (phy & ~0xFFF) | (flags & ENTRY_FLAGS_MASK) | ENTRY_IS_ATE | ENTRY_SCRAMBLE_BIT;
}
...
static phys_addr_t mmu_get_next_pgd(struct kbase_context *kctx, phys_addr_t pgd, u64 vpfn, int level)
{
	...
	if (NULL == page) {
		...
		HiLOGE("DISPLAY","GPU","mmu_get_next_pgd: kmap failure");
		return 0;
	}
	...
	if (!target_pgd) {
		...
		if (!target_pgd) {
			...
			HiLOGE("DISPLAY","GPU","mmu_get_next_pgd: kbase_mmu_alloc_pgd failure");
			kunmap(p);
			return 0;
		}
...
static phys_addr_t mmu_get_bottom_pgd(struct kbase_context *kctx, u64 vpfn)
{
	...
	for (l = MIDGARD_MMU_TOPLEVEL; l < 3; l++) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY","GPU","mmu_get_bottom_pgd: mmu_get_next_pgd failure");
			return 0;
		}
	}
					...
					unsigned long flags)
{
	...
	while (nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_FUNCTION_FAILED:kbase_mmu_insert_pages: mmu_get_bottom_pgd failure");
			if (recover_required) {
		...
		if (!pgd_page) {
			dev_warn(kctx->kbdev->dev,
					       "kbase_mmu_insert_pages: "
					       "kmap failure\n");
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_MEMORY:kbase_mmu_insert_pages: kmap failure");
			if (recover_required) {
				/* Invalidate the pages we have partially
				 * completed */
				mmu_insert_pages_failure_recovery(kctx,
...
mali_error kbase_mmu_insert_pages_with_scramble_bit(struct kbase_context *kctx, u64 vpfn,
				phys_addr_t *phys, size_t nr,
				unsigned long flags, u64 header_page_number)
{
	...
	while (nr) {
		...
		for (i = 0; i < count; i++) {
			...
			{
				page_table_entry_set(kctx->kbdev, &pgd_page[ofs],
							mmu_phyaddr_to_ate_with_scramble_bit(phys[i],
								mmu_flags)
							);
			}
	...
	if (ctx_is_in_runpool) {
		...
		if (kbdev->js_data.runpool_irq.per_as_data[kctx->as_nr].as_busy_refcount >= 2) {
			...
			if (ret) {
				...
				HiLOGE("DISPLAY", "GPU", "Flush for GPU page table update did not complete. Issueing GPU soft-reset to recover");
...
mali_error kbase_mmu_teardown_pages(struct kbase_context *kctx, u64 vpfn, size_t nr)
{
	...
	while (nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_FUNCTION_FAILED:kbase_mmu_teardown_pages: mmu_get_bottom_pgd failure");
			return MALI_ERROR_FUNCTION_FAILED;
		}
		...
		if (!pgd_page) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_MEMORY:kbase_mmu_teardown_pages: kmap failure");
			return MALI_ERROR_OUT_OF_MEMORY;
		}
...
mali_error kbase_mmu_update_pages(struct kbase_context *kctx, u64 vpfn, phys_addr_t* phys, size_t nr, unsigned long flags)
{
	...
	while(nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_FUNCTION_FAILED:mmu_get_bottom_pgd failure");
			return MALI_ERROR_FUNCTION_FAILED;
		}
		...
		if (!pgd_page) {
			...
			HiLOGE("DISPLAY","GPU","MALI_ERROR_OUT_OF_MEMORY:kmap failure");
			return MALI_ERROR_OUT_OF_MEMORY;
		}
...
static size_t kbasep_mmu_dump_level(struct kbase_context *kctx, phys_addr_t pgd, int level, char ** const buffer, size_t *size_left)
{
	...
	if (!pgd_page) {
		...
		HiLOGE("DISPLAY","GPU","kbasep_mmu_dump_level: kmap failure");
		return 0;
	}
...
static void bus_fault_worker(struct work_struct *data)
{
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8245)) {
		...
		HiLOGE("DISPLAY", "GPU", "GPU bus error occurred. For this GPU version we now soft-reset as part of bus error recovery");
		...
		struct kbase_as *as, const char *reason_str)
{
	...
	dev_err(kbdev->dev,
		...
		source_id);
	HiLOGE("DISPLAY", "GPU", "Unhandled Page fault in AS%d at VA 0x%016llX"
		"Reason: %s\n"
		"raw fault status 0x%X\n"
		"decoded fault status: %s\n"
		"exception type 0x%X: %s\n"
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8245)) {
		...
		HiLOGE("DISPLAY", "GPU", "Unhandled page fault. For this GPU version we now soft-reset the GPU as part of page fault recovery.");
        reset_status = kbase_prepare_to_reset_gpu(kbdev);
	}
#endif /* KBASE_GPU_RESET_EN */
	/* switch to UNMAPPED mode, will abort all jobs and stop any hw counter dumping */
...
void kbase_mmu_interrupt_process(struct kbase_device *kbdev, struct kbase_context *kctx, struct kbase_as *as)
{
	...
	if (kctx == NULL) {
		...
		HiLOGE("DISPLAY","GPU","%s in AS%d at 0x%016llx with no context present! Suprious IRQ or SW Design Error?",
				 kbase_as_has_bus_fault(as) ? "Bus error" : "Page fault",
				 as->number, as->fault_addr);
	}
	...
	if (kbase_as_has_bus_fault(as)) {
		if (kctx) {
			...
			dev_warn(kbdev->dev, "Bus error in AS%d at 0x%016llx\n",
					 as->number, as->fault_addr);
			HiLOGE("DISPLAY","GPU","Bus error in AS%d at 0x%016llx",
					 as->number, as->fault_addr);
		}
