...
static void kbase_pm_hw_issues(struct kbase_device *kbdev)
{
	...
	value = kbdev->gpu_outstanding ? kbdev->gpu_outstanding : value;
...
mali_error kbase_pm_init_hw(struct kbase_device *kbdev, mali_bool enable_irqs)
{
...
#ifdef CONFIG_ARCH_HI3650
#if 0
	/* Ignore soft reset if hi3650 platform */
	kbase_pm_reset_done(kbdev);
#endif
	/* In freya T880 r0p2, fix reset problem if lower voltage */
	...
	if (kbase_reg_read(kbdev, GPU_CONTROL_REG(GPU_IRQ_RAWSTAT), NULL) & RESET_COMPLETED) {
		...
		HiLOGE("DISPLAY","GPU","Reset interrupt didn't reach CPU. Check interrupt assignments.");
	...
	HiLOGE("DISPLAY", "GPU", "Failed to soft-reset GPU (timed out after %d ms), now attempting a hard reset", RESET_TIMEOUT);
	...
	dev_err(kbdev->dev, "Failed to hard-reset the GPU (timed out after %d ms)\n", RESET_TIMEOUT);
 	HiLOGE("DISPLAY", "GPU", "Failed to hard-reset the GPU (timed out after %d ms)", RESET_TIMEOUT);
