		...
		u16 hierarchy_mask, u32 default_weight)
{
	...
	if (!fbd_tiler) {
		...
		HiLOGE("DISPLAY", "GPU", "kbasep_replay_reset_fbd: failed to map fbd");
		...
		u16 hierarchy_mask, u32 default_weight,	mali_bool job_64)
{
	...
	if (job_64) {
		...
		if (!job_ext) {
			...
			HiLOGE("DISPLAY", "GPU", "kbasep_replay_reset_tiler_job: failed to map jc");
			return MALI_ERROR_FUNCTION_FAILED;
		}
		...
		mali_bool first_in_chain, mali_bool fragment_chain)
{
	...
	if (!job) {
		...
		HiLOGE("DISPLAY", "GPU", "kbasep_replay_parse_jc: failed to map jc");
		return MALI_ERROR_FUNCTION_FAILED;
	}
	...
	if (job->status == JOB_NOT_STARTED && !fragment_chain) {
		...
		HiLOGE("DISPLAY", "GPU", "Job already not started");
		goto out_unmap;
	}
	...
	if ((job->flags & JOB_TYPE_MASK) == JOB_TYPE_FUSED) {
		...
		HiLOGE("DISPLAY", "GPU", "Fused jobs can not be replayed");
		goto out_unmap;
	}
	...
	if ((job->dependencies[0] + hw_job_id_offset) > JOB_HEADER_ID_MAX ||
		...
		HiLOGE("DISPLAY", "GPU", "Job indicies/dependencies out of valid range");
		goto out_unmap;
	}
		...
		mali_addr64 jc,	u16 *hw_job_id)
{
	while (jc) {
		...
		if (!job) {
			...
			HiLOGE("DISPLAY", "GPU", "failed to map jc");
			return MALI_ERROR_FUNCTION_FAILED;
		}
		...
		mali_bool fragment_chain)
{
	...
	while (jc) {
		...
		if (fragment_chain &&
			...
			HiLOGE("DISPLAY", "GPU", "Exceeded maximum number of jobs in fragment chain");
			return MALI_ERROR_FUNCTION_FAILED;
		}
	}
					      ...
					      struct base_jd_atom_v2 *f_atom)
{
	...
	if (!next) {
		...
		HiLOGE("DISPLAY", "GPU", "Invalid tiler JC list");
		goto out;
	}
	...
	while (next) {
		...
		if (!jc_struct) {
			...
			HiLOGE("DISPLAY", "GPU", "Failed to map jc struct");
			goto out;
		}
...
bool kbase_replay_process(struct kbase_jd_atom *katom)
{
	...
	if (katom->retry_count > BASEP_JD_REPLAY_LIMIT) {
		...
		HiLOGE("DISPLAY", "GPU", "Replay exceeded limit - failing jobs");
		kbase_disjoint_state_down(kctx->kbdev);
