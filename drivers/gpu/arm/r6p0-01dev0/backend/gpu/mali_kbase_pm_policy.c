				...
				bool tiler_required, u64 shader_cores)
{
	...
	if (tiler_required) {
		...
		/* For tiler jobs, we must make sure that core 0 is not turned
		 * off if it's already on. However, it's safe for core 0 to be
		 * left off and turned on later whilst a tiler job is running.
		 * Hence, we don't need to update the cores state immediately.
		 * Also, attempts to turn off cores will always check the
		 * tiler_needed/inuse state first anyway.
		 *
		 * Finally, kbase_js_choose_affinity() ensures core 0 is always
		 * requested for tiler jobs anyway. Hence when there's only a
		 * tiler job in the system, this will still cause
		 * kbase_pm_update_cores_state_nolock() to be called.
		 *
		 * Note that we still need to keep track of
		 * tiler_needed/inuse_cnt, to ensure that
		 * kbase_pm_update_cores_state_nolock() can override the core
		 * availability policy and force core 0 to be powered when a
		 * tiler job is in the system. */
	}
				...
				bool tiler_required, u64 shader_cores)
{
	...
	if (tiler_required) {
		...
		/* Whilst tiler jobs must not allow core 0 to be turned off, we
		 * don't need to make an extra call to
		 * kbase_pm_update_cores_state_nolock() to ensure core 0 is
		 * turned off when the last tiler job unrequests cores:
		 * kbase_js_choose_affinity() ensures core 0 was originally
		 * requested for tiler jobs. Hence when there's only a tiler job
		 * in the system, this will still cause
		 * kbase_pm_update_cores_state_nolock() to be called. */
	}
				...
				bool tiler_required, u64 shader_cores)
{
	...
	if (tiler_required) {
		 ...
		 * don't need to make an extra call to
		 * kbase_pm_update_cores_state_nolock() to ensure core 0 is
		 * turned off when the last tiler job finishes:
		 * kbase_js_choose_affinity() ensures core 0 was originally
		 * requested for tiler jobs. Hence when there's only a tiler job
		 * in the system, this will still cause
		 * kbase_pm_update_cores_state_nolock() to be called */
