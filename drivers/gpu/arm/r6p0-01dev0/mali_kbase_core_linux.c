...
copy_failed:
	...
	case KBASE_FUNC_HWCNT_CLEAR:
		{
			/* args ignored */
			if (kbase_instr_hwcnt_clear(kctx) != 0)
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
			break;
		}
...
static int kbase_common_device_init(struct kbase_device *kbdev)
{
	...
	err = device_create_file(kbdev->dev, &dev_attr_core_availability_policy);
	if (err) {
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(kbdev->mali_debugfs_directory)) {
		...
		err = IS_ERR(kbdev->mali_debugfs_directory) ?
				PTR_ERR(kbdev->mali_debugfs_directory) :
				-ENOMEM;
		goto out_no_debugfs_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_ctx_defaults_directory)) {
		dev_err(kbdev->dev,
				"Couldn't create context defaults directory\n");
		err = -ENOMEM;
		goto out_debugfs_remove;
	}
...
#if 1 //GPU reset disable for FPGA, must remove on chip
    ...
    writel(0x007001c, crg+0x84);
    writel(0x007001c, crg+0x88);
