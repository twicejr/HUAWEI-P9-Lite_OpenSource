					...
					int irq_type)
{
	...
	if (0 != request_irq(kbdev->irqs[irq_type].irq,
		...
		HiLOGE("DISPLAY", "GPU", "Can't request interrupt %d (index %d)",
					kbdev->irqs[irq_type].irq, irq_type);
	...
	if (kbdev->irqs[tag].irq) {
		...
		if (kbase_set_custom_irq_handler(kbdev, test_handler, tag) != 0) {
			...
			if (kbasep_irq_test_data.timeout != 0) {
				...
				HiLOGE("DISPLAY", "GPU", "Interrupt %d (index %d) didn't reach CPU.",
						kbdev->irqs[tag].irq, tag);
				err = -EINVAL;
			} else {
		...
		if (request_irq(kbdev->irqs[tag].irq, kbase_handler_table[tag],
			...
			dev_err(kbdev->dev, "Can't restore original interrupt %d (index %d)\n",
						kbdev->irqs[tag].irq, tag);
			HiLOGE("DISPLAY", "GPU", "Can't restore original interrupt %d (index %d)",
						kbdev->irqs[tag].irq, tag);
			err = -EINVAL;
		}
	}
					...
					struct kbase_device * const kbdev)
{
	...
	if (err) {
		...
		HiLOGE("DISPLAY", "GPU", "Interrupt JOB_IRQ didn't reach CPU. Check interrupt assignments.");
		goto out;
	}
	...
	if (err) {
		...
		HiLOGE("DISPLAY", "GPU", "Interrupt MMU_IRQ didn't reach CPU. Check interrupt assignments.");
		goto out;
	}
