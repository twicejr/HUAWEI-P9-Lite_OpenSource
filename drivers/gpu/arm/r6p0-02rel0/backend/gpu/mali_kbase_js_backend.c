...
static enum hrtimer_restart timer_callback(struct hrtimer *timer)
{
	...
	for (s = 0; s < kbdev->gpu_props.num_job_slots; s++) {
		...
		KBASE_DISJOINT_STATE_INTERLEAVED_CONTEXT_COUNT_THRESHOLD;
					...
					dev_warn(kbdev->dev, "JS: Job Hard-Stopped (took more than %lu ticks at %lu ms/tick)",
							(unsigned long)ticks,
							(unsigned long)ms);
					HiLOGE("DISPLAY", "GPU", "JS: Job Hard-Stopped (took more than %lu ticks at %lu ms/tick)",
							(unsigned long)ticks,
							(unsigned long)ms);
	...
	if (reset_needed) {
		...
		HiLOGE("DISPLAY", "GPU", "JS: Job has been on the GPU for too long (JS_RESET_TICKS_SS/DUMPING timeout hit). Issueing GPU soft-reset to resolve.");
