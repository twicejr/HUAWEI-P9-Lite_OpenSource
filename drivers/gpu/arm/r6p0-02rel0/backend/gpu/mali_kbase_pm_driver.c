...
static void kbase_pm_reset_instead(struct kbase_device *kbdev)
{
	...
	if (max_loop == 0) {
		...
		HiLOGE("DISPLAY", "GPU", "Failed to wait l2 cache power off complete");
	}

	kbase_pm_reset_toplevel(kbdev);
}
#endif /* CONFIG_MALI_PLATFORM_HI3650 */
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
	...
	dev_err(kbdev->dev, "Failed to soft-reset GPU (timed out after %d ms), now attempting a hard reset\n",
								RESET_TIMEOUT);
	HiLOGE("DISPLAY", "GPU", "Failed to soft-reset GPU (timed out after %d ms), now attempting a hard reset",
								RESET_TIMEOUT);
	...
	HiLOGE("DISPLAY", "GPU", "Failed to 2nd external reset GPU, do GPU poweroff and poweron sequence.");
	...
	dev_err(kbdev->dev, "Failed to hard-reset the GPU (timed out after %d ms)\n",
								RESET_TIMEOUT);
	HiLOGE("DISPLAY", "GPU", "Failed to hard-reset the GPU (timed out after %d ms)",
								RESET_TIMEOUT);
#endif	/* CONFIG_MALI_PLATFORM_HI3650 */
