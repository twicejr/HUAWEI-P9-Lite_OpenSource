	...
	case KBASE_FUNC_MEM_COMMIT:
		{
			...
			if (commit->gpu_addr & ~PAGE_MASK) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_MEM_COMMIT: commit->gpu_addr: passed parameter is invalid");
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
	...
	case KBASE_FUNC_MEM_QUERY:
		{
			...
			if (query->gpu_addr & ~PAGE_MASK) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_MEM_QUERY: query->gpu_addr: passed parameter is invalid");
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
			if (query->query != KBASE_MEM_QUERY_COMMIT_SIZE &&
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_MEM_QUERY: query->query = %lld unknown", (unsigned long long)query->query);
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
	...
	case KBASE_FUNC_MEM_FLAGS_CHANGE:
		{
			...
			if ((fc->gpu_va & ~PAGE_MASK) && (fc->gpu_va >= PAGE_SIZE)) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_MEM_FLAGS_CHANGE: mem->gpu_va: passed parameter is invalid");
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
	...
	case KBASE_FUNC_MEM_FREE:
		{
			...
			if ((mem->gpu_addr & ~PAGE_MASK) && (mem->gpu_addr >= PAGE_SIZE)) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_MEM_FREE: mem->gpu_addr: passed parameter is invalid");
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
	...
	case KBASE_FUNC_SYNC:
		{
			...
			if (sn->sset.basep_sset.mem_handle & ~PAGE_MASK) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_SYNC: sn->sset.basep_sset.mem_handle: passed parameter is invalid");
				ukh->ret = MALI_ERROR_FUNCTION_FAILED;
				break;
			}
	...
	case KBASE_FUNC_FIND_CPU_OFFSET:
		{
			...
			if (find->gpu_addr & ~PAGE_MASK) {
				...
				HiLOGE("DISPLAY", "GPU", "kbase_dispatch case KBASE_FUNC_FIND_CPU_OFFSET: find->gpu_addr: passed parameter is invalid");
				goto out_bad;
			}
	...
	case KBASE_FUNC_DEBUGFS_MEM_PROFILE_ADD:
		{
			...
			if (add_data->len > KBASE_MEM_PROFILE_MAX_BUF_SIZE) {
				...
				HiLOGE("DISPLAY", "GPU", "buffer too big");
				goto out_bad;
			}
...
static long kbase_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (0 != copy_from_user(&msg, (void __user *)arg, size)) {
		...
		HiLOGE("DISPLAY", "GPU", "failed to copy ioctl argument into kernel space");
		return -EFAULT;
	}
	...
	if (0 != copy_to_user((void __user *)arg, &msg, size)) {
		...
		HiLOGE("DISPLAY", "GPU", "failed to copy results of UK call back to user space");
		return -EFAULT;
	}
	return 0;
}
...
static ssize_t set_policy(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!new_policy) {
		...
		HiLOGE("DISPLAY", "GPU", "power_policy: policy not found");
		return -EINVAL;
	}
...
static ssize_t set_ca_policy(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!new_policy) {
		...
		HiLOGE("DISPLAY", "GPU", "core_availability_policy: policy not found");
		return -EINVAL;
	}
...
static ssize_t set_split(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	HiLOGE("DISPLAY", "GPU", "sc_split: invalid value");
	dev_err(dev, "  Possible settings: mp[1|2|4], mp[1|2]_vf\n");
	HiLOGE("DISPLAY", "GPU", "  Possible settings: mp[1|2|4], mp[1|2]_vf\n");
	return -ENOENT;
}
...
static ssize_t set_js_timeouts(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	HiLOGE("DISPLAY", "GPU", "Couldn't process js_timeouts write operation.\n"
			"Use format <soft_stop_ms> <soft_stop_ms_cl> <hard_stop_ms_ss> <hard_stop_ms_cl> <hard_stop_ms_dumping> <reset_ms_ss> <reset_ms_cl> <reset_ms_dumping>\n"
			"Write 0 for no change, -1 to restore default timeout");
	return -EINVAL;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (ret || !js_scheduling_period) {
		...
		HiLOGE("DISPLAY", "GPU", "Couldn't process js_scheduling_period write operation.\n"
				"Use format <js_scheduling_period_ms>");
		return -EINVAL;
	}
...
static ssize_t set_force_replay(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	HiLOGE("DISPLAY", "GPU", "Couldn't process force_replay write operation.\nPossible settings: limit=<limit>, random_limit, norandom_limit, core_req=<core_req>");
	return -EINVAL;
}
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
...
#endif
	kbdev->clock = clk_get(kbdev->dev, "clk_g3d"); //clk_mali
...
#ifdef CONFIG_MALI_PLATFORM_HI6250 //GPU reset disable for FPGA, must remove on chip
    ...
    msleep(1);
    writel(0x00000038, crg+0x34);
    writel(0x00030000, crg+0xf0);
    msleep(1);
    writel(0x00000020, crg+0x148);
    writel(0x00000000, crg+0x234);
    writel(0x0070001c, crg+0x84);
    writel(0x0070001c, crg+0x88);
    writel(0x00000001, crg+0x234);
    msleep(1);
    writel(0x00030003, crg+0xf0);
    writel(0x00000038, crg+0x30);
    writel(0xc0008000, crg+0x50);
    msleep(1);
