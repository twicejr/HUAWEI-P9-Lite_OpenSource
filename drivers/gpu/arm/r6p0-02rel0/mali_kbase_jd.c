...
static int kbase_jd_umm_map(struct kbase_context *kctx, struct kbase_va_region *reg)
{
	...
	if (reg->gpu_alloc->header_page_number)
		err = kbase_mmu_insert_pages_with_scramble_bit(kctx, reg->start_pfn, kbase_get_gpu_phy_pages(reg), kbase_reg_current_backed_size(reg), reg->flags | KBASE_REG_GPU_WR | KBASE_REG_GPU_RD,reg->gpu_alloc->header_page_number);
	else
			 ...
			 struct kbase_jd_atom *katom)
{
	...
	if (!katom->jc && (katom->core_req & BASEP_JD_REQ_ATOM_TYPE) != BASE_JD_REQ_DEP) {
		...
		HiLOGE("DISPLAY", "GPU", "Rejecting atom with jc = NULL");
	...
	if ((katom->core_req & BASE_JD_REQ_SPECIFIC_COHERENT_GROUP) &&
		...
		dev_warn(kctx->kbdev->dev,
				...
				katom->device_nr);
		HiLOGE("DISPLAY", "GPU", "Rejecting atom with invalid device_nr %d",
				katom->device_nr);
		katom->event_code = BASE_JD_EVENT_JOB_INVALID;
		ret = jd_done_nolock(katom);
		goto out;
		...
		dev_err(kbdev->dev,
			"t6xx: GPU fault 0x%02lx from job slot %d\n",
					(unsigned long)katom->event_code,
								katom->slot_nr);
		HiLOGE("DISPLAY", "GPU", "t6xx: GPU fault 0x%02lx from job slot %d",
					(unsigned long)katom->event_code,
								katom->slot_nr);
	}
