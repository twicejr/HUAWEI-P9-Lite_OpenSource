		...
		size_t nr_pages, size_t align)
{
	...
	if (gpu_pfn) {
		...
		if (!tmp) {
			...
			HiLOGE("DISPLAY", "GPU", "Enclosing region not found: 0x%08llx gpu_pfn, %zu nr_pages", gpu_pfn, nr_pages);
			err = -ENOMEM;
			goto exit;
		}
		...
		if (err) {
			...
			HiLOGE("DISPLAY", "GPU", "Failed to insert va region");
			err = -ENOMEM;
			goto exit;
		}
		...
		struct base_syncset *set, enum kbase_sync_type sync_fn)
{
	...
	if (!reg) {
		dev_warn(kctx->kbdev->dev, "Can't find region at VA 0x%016llX",
				sset->mem_handle);
		HiLOGE("DISPLAY", "GPU", "Can't find region at VA 0x%016llX",
				sset->mem_handle);
		err = -EINVAL;
		goto out_unlock;
	}
	...
	if (!map) {
		...
		HiLOGE("DISPLAY", "GPU", "Can't find CPU mapping 0x%016lX for VA 0x%016llX",
				start, sset->mem_handle);
		err = -EINVAL;
		goto out_unlock;
	}
...
int kbase_mem_free(struct kbase_context *kctx, u64 gpu_addr)
{
	...
	if (0 == gpu_addr) {
		...
		HiLOGE("DISPLAY", "GPU", "gpu_addr 0 is reserved for the ringbuffer and it's an error to try to free it using kbase_mem_free");
		return -EINVAL;
	}
	kbase_gpu_vm_lock(kctx);
	...
	if (gpu_addr >= BASE_MEM_COOKIE_BASE &&
		...
		if (!reg || (reg->flags & KBASE_REG_FREE)) {
			dev_warn(kctx->kbdev->dev, "kbase_mem_free called with nonexistent gpu_addr 0x%llX",
					gpu_addr);
			HiLOGE("DISPLAY", "GPU", "kbase_mem_free called with nonexistent gpu_addr 0x%llX",
					gpu_addr);
			err = -EINVAL;
			goto out_unlock;
		}
		...
		if ((reg->flags & KBASE_REG_ZONE_MASK) == KBASE_REG_ZONE_SAME_VA) {
			...
			dev_warn(kctx->kbdev->dev, "%s called on SAME_VA memory 0x%llX", __func__,
					gpu_addr);
			HiLOGE("DISPLAY", "GPU", "%s called on SAME_VA memory 0x%llX", __func__,
					gpu_addr);
			err = -EINVAL;
			goto out_unlock;
		}
