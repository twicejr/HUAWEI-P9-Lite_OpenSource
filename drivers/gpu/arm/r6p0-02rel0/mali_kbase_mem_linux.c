...
struct kbase_va_region *kbase_mem_alloc(struct kbase_context *kctx, u64 va_pages, u64 commit_pages, u64 extent, u64 *flags, u64 *gpu_va, u16 *va_alignment)
{
	...
	*gpu_va = 0; /* return 0 on failure */
	...
	if (0 == va_pages) {
		...
		HiLOGE("DISPLAY", "GPU", "kbase_mem_alloc called with 0 va_pages!");
		goto bad_size;
	}
...
int kbase_mem_import(struct kbase_context *kctx, enum base_mem_import_type type, int handle, u64 *gpu_va, u64 *va_pages, u64 *flags, u64 header_page_number)
{
	struct kbase_va_region *reg;
...
out:
	if (err) {
		...
		HiLOGE("DISPLAY", "GPU", "mmap failed %d\n", err);
	}
