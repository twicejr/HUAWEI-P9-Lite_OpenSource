					...
					unsigned long flags)
{
	...
	while (nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY", "GPU", "kbase_mmu_insert_pages: mmu_get_bottom_pgd failure");
			if (recover_required) {
		...
		if (!pgd_page) {
			...
			HiLOGE("DISPLAY", "GPU", "kbase_mmu_insert_pages: kmap failure");
			if (recover_required) {
				/* Invalidate the pages we have partially
				 * completed */
				mmu_insert_pages_failure_recovery(kctx,
				...
				unsigned long flags, u64 header_page_number)
{
    ...
    //printk("--------header_page_number is %llx \n",header_page_number);
...
int kbase_mmu_teardown_pages(struct kbase_context *kctx, u64 vpfn, size_t nr)
{
	...
	while (nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY", "GPU", "kbase_mmu_teardown_pages: mmu_get_bottom_pgd failure");
			return -EINVAL;
		}
		...
		if (!pgd_page) {
			...
			HiLOGE("DISPLAY", "GPU", "kbase_mmu_teardown_pages: kmap failure");
			return -ENOMEM;
		}
...
int kbase_mmu_update_pages(struct kbase_context *kctx, u64 vpfn, phys_addr_t *phys, size_t nr, unsigned long flags)
{
	...
	while (nr) {
		...
		if (!pgd) {
			...
			HiLOGE("DISPLAY", "GPU", "mmu_get_bottom_pgd failure");
			return -EINVAL;
		}
		...
		if (!pgd_page) {
			...
			HiLOGE("DISPLAY", "GPU", "kmap failure");
			return -ENOMEM;
		}
