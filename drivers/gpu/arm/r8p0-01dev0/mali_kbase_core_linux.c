...
#if 0
//#ifdef CONFIG_MALI_PLATFORM_HI3660
	dev_err(kbdev->dev, "Power on in probe for chicago platform.\n");
	void __iomem *crg = ioremap(0xfff35000, 0xfff);
	void __iomem *pmu = ioremap(0xfff36000, 0xfff);
	writel(0x00000003, pmu+0x938);
	dev_err(kbdev->dev, "power on sequence 1.\n");
	msleep(5);
	writel(0x00030003, crg+0xf0);
	writel(0x00000038, crg+0x30);
	dev_err(kbdev->dev, "power on sequence 2.\n");
	msleep(1);
	...
	dev_err(kbdev->dev, "power on sequence 3.\n");
	writel(0x0070001c, crg+0x84);
	writel(0x0070001c, crg+0x88);
	writel(0x00000001, crg+0x234);
	dev_err(kbdev->dev, "power on sequence 4.\n");
	dev_err(kbdev->dev, "power on sequence, 0xfff35234 = 0x%x\n", readl(crg+0x234));
	msleep(1);
	writel(0x00030003, crg+0xf0);
	writel(0x00000038, crg+0x30);
	writel(0xc0008000, crg+0x50);
	msleep(1);
	dev_err(kbdev->dev, "finish power on.\n");
	dev_err(kbdev->dev, "gpu id is 0x%x\n", readl(kbdev->reg));
#endif
