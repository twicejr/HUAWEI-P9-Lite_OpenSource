...
#define BIT_DMC_PERF_EN		SOC_DDRC_DMC_DDRC_CFG_PERF_flux_en_START
...
#endif
#ifndef DDR_REG_DMC
#define DDR_REG_DMC(m,n) (SOC_ACPU_DMCPACK0_BASE_ADDR + 0x4000 + 0x20000 * (m*2) + 0x20000 * (n))
#endif
#ifndef DDR_REG_QOSBUF
#define DDR_REG_QOSBUF(m) (SOC_ACPU_DMSS_CFG_BASE_ADDR + 0x8000 + 0x4000 * (m))
#endif
...
static struct ddrflux_pull ddrflux_lookups[] = {
	 ...
	 DDR_FLUX(DMSS , (0x70c+0x800*(8)) ,  (1*4), "ASI_QOS_OSTD_ST_8,"),
	 DDR_FLUX(DMSS , (0x70c+0x800*(9)) ,  (1*4), "ASI_QOS_OSTD_ST_9,"),
	 DDR_FLUX(DMSS , (0x70c+0x800*(10)) ,  (1*4), "ASI_QOS_OSTD_ST_10,"),
	 ...
	 DDR_FLUX(DMSS , (0x218+0x800*(8)) ,  (1*4), "ASI_FLUX_STAT_WR_8,"),
	 DDR_FLUX(DMSS , (0x218+0x800*(9)) ,  (1*4), "ASI_FLUX_STAT_WR_9,"),
	 DDR_FLUX(DMSS , (0x218+0x800*(10)) , (1*4), "ASI_FLUX_STAT_WR_10,"),
	 ...
	 DDR_FLUX(DMSS , (0x21c+0x800*(8)) ,  (1*4), "ASI_FLUX_STAT_RD_8,"),
	 DDR_FLUX(DMSS , (0x21c+0x800*(9)) ,  (1*4), "ASI_FLUX_STAT_RD_9,"),
	 DDR_FLUX(DMSS , (0x21c+0x800*(10)) , (1*4), "ASI_FLUX_STAT_RD_10,"),
...
void __ddrflux_init(void)
{
	...
	/*if ddr is secos */
	if (usr[ddrc_unsec_pass] == 0) {
		pr_err("Welcome enter sec arm-trustframeworld!\n");
		/*dram type get*/
		...
		*((unsigned long *)dfdev->flux_staid_va) = usr[sta_id];
		(void)atfd_hisi_service_access_register_smc(ACCESS_REGISTER_FN_MAIN_ID, dfdev->flux_staid_pa,
					usr[sta_id_mask], ACCESS_REGISTER_FN_SUB_ID_DDR_STDID_W);
	...
	/*if ddr is unsecos */
		...
		if (usr[dmss_enable]) {
			...
			if (!dfdev->dmss_base)
			    dfdev->dmss_base = VIRT(REG_BASE_DMSS, 8);
			if (!dfdev->dmss_base)
				goto map_err;
		}
		if (usr[qosbuf_enable]) {
			...
			if (!dfdev->qosb_base[0])
			    dfdev->qosb_base[0] = VIRT(DDR_REG_QOSBUF(0), 1);
			if (!dfdev->qosb_base[0])
				goto map_err1;
			if (!dfdev->qosb_base[1])
			    dfdev->qosb_base[1] = VIRT(DDR_REG_QOSBUF(1), 1);
			if (!dfdev->qosb_base[1])
				goto map_err2;
		}

		dfdev->dram_type = readl(SOC_DDRC_DMC_DDRC_CFG_DDRMODE_ADDR
						(VIRT(DDR_REG_DMC(0,0), 1)));
		...
		if (usr[dmc_enable]) {
			...
			if (LPDDR4 == dfdev->dram_type) {
				if (!dfdev->dmc_base[0])
					dfdev->dmc_base[0] = VIRT(DDR_REG_DMC(0,0), 1);
				if (!dfdev->dmc_base[0])
					goto map_err3;
				if (!dfdev->dmc_base[1])
					dfdev->dmc_base[1] = VIRT(DDR_REG_DMC(1,0), 1);
				if (!dfdev->dmc_base[1])
					goto map_err4;
				if (!dfdev->dmc_base[2])
					dfdev->dmc_base[2] = VIRT(DDR_REG_DMC(0,1), 1);
				if (!dfdev->dmc_base[2])
					goto map_err5;
				if (!dfdev->dmc_base[3])
					dfdev->dmc_base[3] = VIRT(DDR_REG_DMC(1,1), 1);
				if (!dfdev->dmc_base[3])
					goto map_err6;
			}
...
/*
				for(i = 0; i < 11;i ++){
					writel(usr[sta_id], SOC_DMSS_ASI_STAT_FLUX_FILTER0_ADDR(dfdev->dmss_base,i));
					writel(usr[sta_id_mask], SOC_DMSS_ASI_STAT_FLUX_FILTER1_ADDR(dfdev->dmss_base,i));
					}
*/
			} 
			else {
...
static int ddrflux_data_seq_show(struct seq_file *m, void *v)
{
	...
	if (i == 0) {
		seq_printf(m, "sta_id:%d,sta_id_mask:%d,dram_type:%s,interval:%dus\n",
			usr[sta_id], usr[sta_id_mask], ("LPDDR4"), usr[interval]);
...
static void dmss_flux_enable_ctrl(int en)
{
	...
	if (!usr[ddrc_unsec_pass])
		return;
	if (en) {
		val = readl(SOC_DMSS_GLB_STAT_CTRL_ADDR
					(VIRT(REG_BASE_DMSS, 8)));
		val |= 0xFFF;
		writel(val, SOC_DMSS_GLB_STAT_CTRL_ADDR
				(VIRT(REG_BASE_DMSS, 8)));
	}else{
		val = readl(SOC_DMSS_GLB_STAT_CTRL_ADDR
				(VIRT(REG_BASE_DMSS, 8)));
...
static void qosbuf_flux_enable_ctrl(int en)
{
	...
	if (en) {
		val0 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR
					(VIRT(DDR_REG_QOSBUF(0), 1)));
		val1 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR
					(VIRT(DDR_REG_QOSBUF(1), 1)));
