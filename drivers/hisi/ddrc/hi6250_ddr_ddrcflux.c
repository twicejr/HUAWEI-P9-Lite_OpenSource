...
#define MAX_DMSS_REG_NUM			21
#define MAX_QOSB_REG_NUM			5
#define MAX_DMC_REG_NUM				20
...
#define DDRFLUX_STAID_MASK_OOFFSET	0x190
	...
	/*if ddr is unsecos */
		...
		if (usr[qosbuf_enable]) {
			...
			if (!dfdev->qosb_base[0])
				dfdev->qosb_base[0] = VIRT(SOC_ACPU_DDRC_QOSB_BASE_ADDR, 1);
			if (!dfdev->qosb_base[0])
				goto map_err1;
		}

		dfdev->dram_type = readl(SOC_DDRC_DMC_DDRC_CFG_DDRMODE_ADDR
				(VIRT(SOC_ACPU_DDRC_DMC_BASE_ADDR, 1)));
		dfdev->dram_type &= DDRC_DRAM_TYPE_MASK;
		pr_info("[%s] mask=0x%x ,dramtype=%d,%d\n", __func__, DDRC_DRAM_TYPE_MASK, dfdev->dram_type, __LINE__);
		if (usr[dmc_enable]) {
			dmc_flux_enable_ctrl(1);
			if (!dfdev->dmc_base[0])
				dfdev->dmc_base[0] = VIRT(SOC_ACPU_DDRC_DMC_BASE_ADDR, 1);
			if (!dfdev->dmc_base[0])
				goto map_err2;
			writel(usr[sta_id], SOC_DDRC_DMC_DDRC_CFG_STAID_ADDR(dfdev->dmc_base[0]));
			writel(usr[sta_id_mask], SOC_DDRC_DMC_DDRC_CFG_STAIDMSK_ADDR(dfdev->dmc_base[0]));
...
static void dmss_flux_enable_ctrl(int en)
{
	...
	if (en) {
		val = readl(SOC_DMSS_GLB_STAT_CTRL_ADDR
				(VIRT(SOC_ACPU_DMSS_BASE_ADDR, 8)));
		val |= 0x7F;
...
static void qosbuf_flux_enable_ctrl(int en)
{
	...
	if (en) {
		val0 = readl(SOC_DDRC_QOSB_QOSB_CFG_PERF_ADDR
				(VIRT(SOC_ACPU_DDRC_QOSB_BASE_ADDR, 1)));
...
static void dmc_flux_enable_ctrl(int en)
{
	...
	if (en) {
		...
		writel(1<<30, SOC_DDRC_DMC_DDRC_CFG_STADAT_ADDR
				(VIRT(SOC_ACPU_DDRC_DMC_BASE_ADDR, 1)));
		...
		writel(1<<30, SOC_DDRC_DMC_DDRC_CFG_STACMD_ADDR
				(VIRT(SOC_ACPU_DDRC_DMC_BASE_ADDR, 1)));

		writel(1<<29, SOC_DDRC_DMC_DDRC_CFG_PERF_ADDR
				(VIRT(SOC_ACPU_DDRC_DMC_BASE_ADDR, 1)));


	} else {
		/*DMC_0_0, offset 0x254, bit30 to 0*/
		...
		/*DMC_0_0, offset 0x260, bit30 to 0*/
