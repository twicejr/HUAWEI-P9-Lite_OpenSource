...
static int cyttsp5_debug_probe(struct device *dev)
{
    ...
    if (rc) {
        TS_LOG_ERR("%s: Error, could not create int_count\n",
                __func__);
    ...
    if (rc) {
        TS_LOG_ERR("%s: Error, could not create formated_output\n",
                __func__);
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error, could not subscribe attention cb\n",
                __func__);
...
static int cyttsp5_debug_release(struct device *dev)
{
    ...
    rc = cmd->unsubscribe_attention(dev, CY_ATTEN_IRQ, CY_MODULE_DEBUG,
        cyttsp5_debug_attention, CY_MODE_OPERATIONAL);
    if (rc < 0) {
        TS_LOG_ERR("%s: Error, could not un-subscribe attention\n",
                __func__);
...
static int __init cyttsp5_debug_init(void)
{
    ...
    for (i = 0; i < num_core_ids; i++) {
        ...
        TS_LOG_INFO("%s: Registering debug module for core_id: %s\n",
            __func__, core_ids[i]);
    ...
    TS_LOG_INFO("%s: Cypress TTSP Debug Driver (Built %s) rc=%d\n",
...
fail_unregister_devices:
    for (i--; i >= 0; i--) {
        ...
        TS_LOG_INFO("%s: Unregistering debug module for core_id: %s\n",
            __func__, core_ids[i]);
    }
    return rc;
}
