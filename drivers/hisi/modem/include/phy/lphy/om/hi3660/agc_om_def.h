...
typedef enum
{
    ...
    LPHY_REQ_THRL_DC_RPT_ENA,
    LPHY_REQ_THRL_FIX_FREQ_THR_ENA,
    LPHY_REQ_THRL_FIX_DCEN_ENA,
    LPHY_REQ_THRL_FIX_DCNUM_ENA,
    LPHY_REQ_THRL_FIX_ELINUM_ENA,    
    LPHY_REQ_THRL_FIX_ALPHA_ENA,
    LPHY_REQ_THRL_FIX_FRATIO_ENA,
    LPHY_REQ_THRL_FIX_BYPASS_ENA,
    LPHY_REQ_THRL_STATE_PRT_ENA
}LPHY_REQ_AGC_ENUM;
...
typedef UINT8 HRL_SWITCH_ENUM_UINT8;
...
typedef enum
{
    ...
    LPHY_TRACE_THRL_MODE_REQ_INFO,
    LPHY_TRACE_THRL_HAR_DC_STATE_INFO,
    LPHY_TRACE_THRL_NV_LOAD_INFO,
    LPHY_TRACE_THRL_STATUS_INFO    
}LPHY_TRACE_AGC_ENUM;
...
{
    UINT8  ucTHrlSwitch;
    UINT8  ucTHarInRealBw;
    UINT8  ucTHrlDcEn;                     /*bbp消除DC指示*/
    UINT8  ucTHrlNvFlag;
    UINT16 usSpurFreqInfo; 
    NV_HRL_SPUR_STRU stNvThrlInfo;
}THRL_CTRL_STRU;
...
typedef enum
{
	THRL_CALLED_AFC = 0,
	THRL_CALLED_RFE 
}THRL_CTRL_CALLED_ENUM;
typedef  UINT32 THRL_CTRL_CALLED_ENUM_UINT32;
...
typedef enum
{
	...
	THRL_MODE_DEM,               /* 解调模式 */ 
	THRL_MODE_SCAN,              /* 扫档模式 */ 
	THRL_MODE_INTER              /* 异频模式 */ 
}THRL_MODE_ENUM;
typedef UINT8 THRL_MODE_ENUM_UINT08;
typedef UINT32 THRL_MODE_ENUM_UINT32;
typedef UINT16 THRL_MODE_ENUM_UINT16;

typedef enum
{
    THRL_16_SAMPLE_POINT = 0,
    THRL_32_SAMPLE_POINT,
    THRL_64_SAMPLE_POINT,    
    THRL_128_SAMPLE_POINT,
    THRL_256_SAMPLE_POINT,
    THRL_512_SAMPLE_POINT,
    THRL_1024_SAMPLE_POINT,
    THRL_2048_SAMPLE_POINT,
    THRL_4096_SAMPLE_POINT,
    THRL_8192_SAMPLE_POINT
}THRL_DC_NUM_ENUM;
typedef UINT8 THRL_SAMPLE_POINT_ENUM_UINT08;

typedef struct
{
    UINT16                          usFreqInfo; 	/*频点*/
    UINT16                          usBand;         /*频段*/
    THRL_MODE_ENUM_UINT08       	enThrlMode;     /*时域谐波工作模式 */
    UINT8                           ucValidFlg;     /*接口生效标志*/
    UINT16                          usDualPssFlag;
}THRL_MODE_CFG_CC_INFO_STRU;

typedef struct
{
    THRL_MODE_CFG_CC_INFO_STRU   astThrlModeCcInfo[AGC_MAX_CC_NUM]; 
    THRL_CTRL_CALLED_ENUM_UINT32 enTHRLCalledFun;
}THRL_MODE_CFG_STRU;
...
typedef struct 
{
    LPHY_CA_CC_ENUM_UINT16 enCC;    
    UINT16 usRsv;
    UINT32 ulHarPha[2];      /*扫档模式目标谐波相位步长上报值*/
    UINT32 ulHarCnt[2];      /*扫档模式频偏档位估计统计上报*/
    UINT32 ulHarState[2];    /*扫档模式状态上报*/          
    UINT32 ulDcState[2];  /*DC估计状态上报*/
    UINT32 ulDcA[2];      /*RXA天线DC上报值*/
    UINT32 ulDcB[2];      /*RXB天线DC上报值*/
}THRL_BBP_DC_STATE_RPT_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enReportEn;
}LPHY_REQ_THRL_DC_REPORT_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT16    enReportEn;
    INT16 sFreqThr;
}LPHY_THRL_FIX_FREQ_THR_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT16    enEnable;
    HRL_SWITCH_ENUM_UINT8        enDcEn[4]; /*4cc*/
}LPHY_THRL_FIX_DCEN_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT16    enEnable;
    THRL_SAMPLE_POINT_ENUM_UINT08      enDcNum[4]; /*4cc*/ 
}LPHY_THRL_FIX_DCNUM_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT16    enEnable;
    THRL_SAMPLE_POINT_ENUM_UINT08      enEliNum[4]; /*4cc*/ 
}LPHY_THRL_FIX_ELINUM_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enEnable;
    UINT32   ulAlpha[4]; /*4cc*/ 
}LPHY_THRL_FIX_ALPHA_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enEnable;
    UINT32   ulFRatio[4]; /*4cc*/ 
}LPHY_THRL_FIX_FRATIO_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enEnable;
    UINT32   ulBypassEn[4]; /*4cc*/ 
}LPHY_THRL_BYPASS_ENA_STRU;

typedef struct
{
    AGC_OM_SWITCH_ENUM_UINT32    enEnable;
}LPHY_THRL_STATUS_RPT_ENA_STRU;

typedef struct 
{
    LPHY_CA_CC_ENUM_UINT16 enCc;
    THRL_MODE_ENUM_UINT16 enWorkMode;
    THRL_MODE_ENUM_UINT16 enWorkModeBbp[2];
    INT32 lShiftFreq;
    INT32 lPhaseStep;
    INT32 lPhaseStepBBp[2];
    INT32 ulDcReqBBp[2];
    INT32 lPhaseStepMinus10kHz;
    INT32 lPhaseStepPlus10kHz;    
}THRL_STATUS_RPT_STRU;

typedef struct
{
    UINT8 ucHarSwitch;
    UINT8 ucTHarSwitch;     /*时域谐波开关*/
    UINT8 ucDcSwitch;
    UINT8 ucHarSpurFreqNum; /*谐波干扰频点个数  小于AGC_HRL_MAX_HAR_FREQ_NUM*/
    UINT8 aucTharAntSel[4];    /*4R */
    UINT16 ausTharDcAntSel[2]; /*DC天线选择 2通道 每通道 0 主集，1 分集*/
    UINT32 ulHarSource;
    UINT16 ausHarSpurFreqInfo[6];    
}THRL_NV_RPT_STRU;
