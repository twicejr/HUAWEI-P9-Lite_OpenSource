...
#define LPHY_FEATURE_MAX_CARRIER_NUM_TRU  (4)
#define LPHY_MAX_UL_CARRIER_NUM_TRU       (2)

#define SCC_TIMING_SYNC_PCC_EVT        ( 1 )
/************************************************************
                             数据结构定义
************************************************************/
...
typedef enum __LPHY_REQ_MSGID_TIMING__
{
    ...
    LPHY_REQ_TIMING_EMBMS_OM_CUT                /* 多播OM cut 参数 */
...
{
    OM_REQ_ENABLE_ENUM_UINT16   enEna;          /*0,不使能定时模块; 1,使能定时*/
    UINT16                      usReportPeriod; /* 上报周期 */
    INT16                      usRTTAFCBiasHz;
    UINT16                      usRTTAFCSet;
    INT16                      usRTTTRUBiasTs;
    UINT16                      usRTTTRUSet;
...
typedef struct __TIMING_NOISE_STRU__
{
	...
	INT16     sTotalGaindB[LPHY_MAX_RX_ANTENNA_NUM];     //子帧的AGC总增益;
	UINT32     ulInitFlag;
...
typedef struct __TRU_PARA_CONFIG_BBP__
{
    ...
    UINT32 ulSquRightSftBit;/* 3'd0:右移10bit/d1:11bit/d2:12bit/d3:13bit/d4:14bit/d5:15bit */
    UINT32 ulPdpSftTarBit; /* 5 bit default 19 */
    UINT32 ulpdpSftValue:6;/* pdp_shift_mode为1时有效 */
    UINT32 ulRsv3:10;
    UINT32 ulPdpSftMode:1;/* 1'b0:自适应移位// 1'b1:固定移位（软件配置移位bit数）*/
    UINT32 ucRsv4:15;
...
typedef struct __TIMING_DTA_STRU__
{
    ...
    UINT16                          usReserved;
    UINT32							pFpu;
    UINT32                          usSccConfigState;      /*Scc定时配置状态，0表示按着自己调整配置，1表示跟着Pcc配置 */
}TIMING_DTA_STRU;
...
typedef struct __timing_controller_stru__
{
    ...
    LTE_CELL_INFO_STRU              stCurCellInfo[LPHY_FEATURE_MAX_CARRIER_NUM_TRU];      /*当前小区的参数信息*/
...
typedef struct __TIMING_MODULE_STRU__
{
    ...
    TIMING_NORMAL_STRU               stNormal[LPHY_FEATURE_MAX_CARRIER_NUM_TRU];           /*单播定时跟踪控制*/
    TIMING_CONTROLLER_STRU           stCtrl;             /*定时控制*/
    TIMING_MBMS_STRU                 stMbms;             /*MBMS的定时跟踪控制*/
    TIMING_STT_STRU                  stStt[LPHY_MAX_UL_CARRIER_NUM_TRU];              /*系统定时跟踪*/
    TIMING_STA_STRU                  stSta[LPHY_MAX_UL_CARRIER_NUM_TRU];              /*系统定时调整*/
    TIMING_INFO_STRU                 stMaintainedInfo[CELL_TOTAL_NUM];/*位ANR增加的维护定时小区*/
}TIMING_MODULE_STRU;

typedef struct __TIMING_MODULE_SIG_QUALITY_STRU__
{
    INT16                          asCmuSnr[CMU_RX_MAX_NUM];                       /* CMU */
	UINT32                          ulCmuRSrpdBm[CMU_RX_MAX_NUM];
    UINT32                          aulCmuServCrsRsrp[CMU_RX_MAX_NUM];
    UINT32                          aulCmuFinalNoisePower[CMU_RX_MAX_NUM];

	UINT16                          usAfcUnLockedFlag;  /* TIMING_MBMS_STRU.TIMING_EST_STRU AFC锁定标记*/
	UINT16                          usSavedFilterPAR;   /*滤波后的峰均比值*/
    UINT16                          usSavedOncePAR;     /*单次峰均比值*/
    UINT16                          usLowPARFlagSuccNum;/*连续低信噪比个数*/
    UINT16                          usLowPARHoldFlag[2];   /*低信噪比HoldFlag指示*/
    UINT32                          ulMbmsPenulDemodTime;       /*倒数第二个子帧的解调定时, 单位: 0.25Ts*/
    UINT32                          ulMbmsLastDemodTime;        /*倒数第一个子帧的解调定时, 单位: 0.25Ts*/
    UINT32                          ulMbmsCurDemodTime;         /*当前子帧的解调定时, 单位: 0.25TS*/
    UINT32                          ulMbmsNextDemodTime;        /*下一个子帧的解调定时, 单位: 0.25TS*/
    ...
    UINT16                          usRSCCIndMax;       /*两天线相关系数的最大值*/
    UINT16                          usMRSCCIndMax;      /*两天线内环环CCInd中的最大值*/
    INT16                          sFilterInFOest;     /* AFC Embms */
    INT16                          sMFilterInFOest;    /* AFC Embms */
    INT32                          lCurFOAdjHz;          /* 当前频偏调整量 */
    INT32                          lMCurFOAdjHz;         /* 当前频偏调整量 */
    INT32                          lAccFOHz;             /* 频偏调整量累加值 */
    INT32                          lMAccFOHz;            /* 频偏调整量累加值 */

}TIMING_MODULE_SIG_QUALITY_STRU;

typedef struct __AFC_EMBMS_PHASE_HZ_STRU__
{
    INT32    lCrsInitAccFoHz;
    INT32    lEmbmaThetaInitialHz;
    INT32    lEmbmaPhaseStep;
    INT32    lEmbmsOutPhase;
    INT32    lEmbmaBaseBandFoHz;
}AFC_EMBMS_PHASE_HZ_STRU;

typedef struct __TIMING_MODULE_STATUS_STRU__
{
    UINT16                          usSib2Valid;        /* 信道控制接口 */
    ...
    AFC_LOOP_STATE_ENUM_UINT16      enAFCLoopCurState;  /* 环路状态，内环失锁，内环锁定 */
...
typedef struct __TIMING_MODULE_EMBMS_STRU_CUT__
{
    ...
    UINT16                              usSysFrmNum;        /* 上报时候的系统帧号0~1023 */
    UINT16                              usSubFrmNum;        /* 上报时刻的子帧号 */
