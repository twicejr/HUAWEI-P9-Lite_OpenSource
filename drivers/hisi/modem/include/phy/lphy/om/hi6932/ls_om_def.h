...
typedef enum
{
	...
	LPHY_REQ_LS_CELL_INFO_REPORT_ENA
...
typedef enum
{
	...
	LPHY_TRACE_LS_PCELL_INFO_REPORT,
	LPHY_TRACE_LS_SCELL0_INFO_REPORT,
	LPHY_TRACE_LS_SCELL1_INFO_REPORT,
	LPHY_TRACE_LS_SCELL2_INFO_REPORT
}LPHY_TRACE_LS_ENUM;
typedef struct
{
	UINT32	 ulDagcGain;
...
typedef struct
{
   OM_REQ_ENABLE_ENUM_UINT16    enLsCellInfoReportEn;
   UINT16                       usReseve;
}LPHY_REQ_LS_CELL_INFO_REPORT_ENA_STRU;
...
typedef struct
{
    ...
    UINT16  usMbmsMainAreaIdFlag;
    UINT8   ucLastInterFlag;
    UINT8   ucInterFlag;
    UINT8   ucNextInterFlag;
    UINT8   ucRxAntNum;
    UINT8   ucCarrierNum;
    UINT8   ucCarrierNumBak;

    UINT16  ausCellId;
    UINT16  ausVShift;
	UINT16  ausVindex;
    UINT16  ausTxAntNum;
    UINT16  ausLsTxAntNum;
    LTE_BAND_WIDTH_ENUM_UINT16          aenBandWidth;
    LTE_CP_TYPE_ENUM_UINT16             aenCpMode;

    LTE_FRAME_TYPE_ENUM_UINT16          aenDuplex;
    LTE_TDD_SUBFRAME_TYPE_ENUM_UINT16   aenTddSfType;
    LTE_TDD_SUBFRAME_TYPE_ENUM_UINT16   aenLastTddSfType;

    LTE_TDD_UD_CFG_ENUM_UINT16          aenTddUDConfig;
    LTE_TDD_SPEC_SUBFRM_CFG_ENUM_UINT16 aenSpecSubFormat;
    UINT16  ausCrsDataValid;//解调通路CRS是否有效(包括S子帧)，M/U/GAP均为无效

    UINT16  ausLastDlCrsDataValid;//上一子帧是否有效
    UINT16  ausDlCrsDataValid;//解调通路CRS是否有效(只包括D子帧)，S/M/U/GAP均为无效
    UINT16  ausNextDlCrsDataValid;//下一子帧是否有效

	UINT16  ausSCrsDataValid;
	UINT16  ausNextSCrsDataValid;
    UINT16  ausMframeValid;

    UINT16  ausReserve;
}LPHY_TRACE_LS_PCELL_INFO_OM_STRU;
...
typedef struct
{
    ...
    UINT16  ausReserve;
}LPHY_TRACE_LS_SCELL_INFO_OM_STRU;
...
void LS_SccStart(LTE_CELL_INFO_STRU *pstCellInfo,UINT16 usCcIdx);
void LS_SccStop(UINT16 usCcIdx);
void   LS_HoldCcCurSubFrame(UINT16 usCcIdx);
