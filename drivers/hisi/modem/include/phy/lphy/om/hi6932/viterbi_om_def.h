...
typedef struct
{
    UINT32 ulDlDciPower[DL_MAX_CELL_NUM][MAX_DL_CHAN_NUM];    /*[CC数][信道数]下行DCI软件度量值*/
    UINT32 ulUlDciPower[OM_UL_MAX_CELL_NUM];       /*上行DCI软件度量值*/
    UINT32 ulRsNoisePower[DL_MAX_CELL_NUM][MAX_RX_NUM];  /*[CC数][天线数]天线0\1\2\3的平均宽带噪声功率*/
    INT32  lDlDciSINR[DL_MAX_CELL_NUM];         /*[CC数]下行DCI信干噪比*/
    INT32  lUlDciSINR[OM_UL_MAX_CELL_NUM];                          /*上行DCI信干噪比*/
...
typedef struct
{
    UINT8  ucMcs;
    UINT8  ucNdi;
    UINT8  ucNewReTrans;       /*路测中是保留字*/
    UINT8  ucRv;
    PDSCH_TRANS_SCHEME_OM_ENUM_UINT16  usTransScheme;          /*下行每码字传输方式，上行此参数无效*/
...
{
    UINT32 ulPdcchErrorNum[DL_MAX_CELL_NUM];         /*定义为无任何有效的DCI(包括上下行)就算一次 -> 算法需求, 但不清楚定义, 故DSP自己定义*/
    UINT32 ulPdcchTotalNum[DL_MAX_CELL_NUM];         /*定义为只要VDL浮动中断上来就算一次, 注意对于DTX情况总数要累加, 但错误数亦累加*/
    UINT32 ulPdcchAccuErrorNum[DL_MAX_CELL_NUM];     /*累积量*/
    UINT32 ulPdcchAccuTotalNum[DL_MAX_CELL_NUM];     /*累积量*/
