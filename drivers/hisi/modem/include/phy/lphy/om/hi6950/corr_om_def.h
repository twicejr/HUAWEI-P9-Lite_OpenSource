...
typedef enum __LPHY_REQ_MSGID_CORR__
{
    ...
    LPHY_REQ_CORR_RPT_ENA,                       //???ì?¨¨1?D???ì|???§?è??¨??a1?
    LPHY_REQ_CORR_ALPHA_POSITION,                       //Q???ì? ??2??¨?????
    LPHY_REQ_CORR_RESULT_MODE,                              //???ì?¨¨1?D?|?§?????ì?¨|1???ìo????§?è???ì???ìo?3??1??ìo???ìa3???????ì??
    LPHY_REQ_CORR_NONWHITE_ALPHA,                              //??¨¨?????§?é???ê?è??2??¨???ì????ì????§?é??ì?
    LPHY_REQ_CORR_WHITE_ALPHA,                           //????§?é???ê?è|?§????2??¨???ì????ì????§?é??ì?
    LPHY_REQ_CORR_ALL_THRESH,                           //????ì????§?é????T
...
{
	OM_REQ_ENABLE_ENUM_UINT16	enEn;
	UINT16						usAlphaPosition; //Q???¨?? |??ì????2???§?????
}LPHY_REQ_CORR_ALPHA_POSITION_STRU;

typedef struct
{
	OM_REQ_ENABLE_ENUM_UINT16	enEn;
	UINT16						usCorrResultMode;//???¨???§?§1?D?|??ì?????¨???§|1???¨?o?????ì?¨¨???¨????¨?o?3??1??¨?o???¨?a3???????¨???,0?a????ì?¨¨???¨????¨?o?3????ìo?1?a??¨?a3???????¨???
...
typedef struct __LPHY_SG_CORR_OM_STAT_STRU__
{
    ...
    UINT16      ulWhtnRecCorr[3];                /*???Dμí?à1??ù??°ù·?±è*/
    UINT16      ulWhtnCorr[3];                   /*???Dμí?à1??ù??°ù·?±è*/
    UINT16      ulNonWhtnRecCorr[3];             /*???Dμí?à1??ù??°ù·?±è*/
    UINT16      ulNonWhtnCorr[3];                /*???Dμí?à1??ù??°ù·?±è*/
