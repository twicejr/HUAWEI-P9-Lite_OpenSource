...
extern slimbus_channel_property_t audio_debug[SLIMBUS_DEBUG_CHANNELS];
...
static uint16_t sd_table[SLIMBUS_RF_MAX][SLIMBUS_TRACK_MAX][SLIMBUS_CHANNELS_MAX] ={
			/* 24.576M */
			{
				{0xC90, 0xC96}, 				/* audio playback */
				{0xCB0, 0xCB6, 0xCD0, 0xCD6},			/* audio capture */
				{0x124, 0x12A}, 				/* voice down 8k */
				{0x144, 0x14A, 0x150, 0x156},			/* voice up 8k */
				{0xC0B},					/* image download */
				{0xC88, 0xC8C}, 				/* EC_REF */
				{0xC3C}, 					/* sound trigger */
				{0xCF0, 0xCF6}, 				/* debug 48k*/
			},
			/* 6.144M */
			{
...
static uint16_t sd_voice_down_16k[SLIMBUS_RF_MAX][SLIMBUS_CHANNELS_MAX] = {
			{0xA4,	0xAA},						/* 24.576M voice down 16k*/
			{0x38,	0x3C},						/* 6.144M voice down 16k*/
};

static uint16_t sd_voice_up_16k[SLIMBUS_RF_MAX][SLIMBUS_CHANNELS_MAX] = {
			{0xC4, 0xCA, 0xD0, 0xD6},				/* 24.576M voice up 16k*/
			{0x438, 0x43c, 0x838, 0x83c},				/* 6.144M voice up 16k*/
};

static uint16_t sd_soundtrigger_48k[SLIMBUS_CHANNELS_MAX] = {0xC9C};

static uint16_t sd_soundtrigger_16k[SLIMBUS_CHANNELS_MAX] = {0x9C};

static uint16_t sd_debug_192k[SLIMBUS_CHANNELS_MAX] = {0xC24};
static uint16_t sd_debug_16k[SLIMBUS_CHANNELS_MAX] = {0xF0, 0xF6};
static uint16_t sd_debug_8k[SLIMBUS_CHANNELS_MAX] = {0x170, 0x176};
				...
				slimbus_track_param_t *params)
{
	...
	} else if(SLIMBUS_TRACK_DEBUG == track_type) {
		track = SLIMBUS_TRACK_DEBUG;
		pchprop = &audio_debug[0];
		for (ch=0; ch < chnum_table[track]; ch++) {
			if (params->rate == 48000) {
				pchprop->sd 	   = sd_table[rf][track][ch];
				pchprop->pr 	   = pr_table[rf][track];
			} else if (params->rate == 192000) {
				pchprop->sd 	   = sd_debug_192k[ch];
				pchprop->pr 	   = SLIMBUS_PR_192K;
			} else if (params->rate == 16000) {
				pchprop->sd 	   = sd_debug_16k[ch];
				pchprop->pr 	   = SLIMBUS_PR_16K;
			} else if (params->rate == 8000) {
				pchprop->sd 	   = sd_debug_8k[ch];
				pchprop->pr 	   = SLIMBUS_PR_8K;
			}
			pchprop++;
		}
	} else {
