 ...
 * send_tui_msg_running:send message to TUI in running state only.
 * send_tui_msg_config:send message to TUI in config state only.
 * send_tui_msg_adapt:send message to TUI in both states.
 */
...
#else
static inline int init_tui(void)
{
	return 0;
}
...
static inline int send_tui_msg_running(int type, int val, void *data)
{
	return 0;
}
