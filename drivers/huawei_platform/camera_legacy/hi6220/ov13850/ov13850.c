...
#include "cherryplus/ov13850_cherryplus.h"
#include "cherrypro/ov13850_cherrypro.h"
#include "chm/ov13850_chm.h"
#include "alice/ov13850_alice.h"

/*#include "../isp/k3_isp_io.h"*/
#include <asm/bug.h>
#include <linux/device.h>

#define LOG_TAG "OV13850"
...
#define OV13850_DIG_GAIN_REG_H   0x5502
#define OV13850_DIG_GAIN_REG_L   0x5503

#define OV13850_VTS_REG_H		0x380e
#define OV13850_VTS_REG_L		0x380f

#define OV13850_EQUIVALENT_FOCUS        27// 27mm
...
/******************vcm begin*************************/
static vcm_info_s vcm_dw9714a_ov13850_cherryplus = {
    #include "cherryplus/vcm_dw9714_liteon_cherryplus.h"
};

static vcm_info_s vcm_dw9714a_ov13850_chm = {
    #include "chm/vcm_dw9714_liteon_chm.h"
};

static vcm_info_s vcm_dw9714a_ov13850_alice = {
    #include "alice/vcm_dw9714_liteon_alice.h"
};
...
/******************effect begin*************************/
static effect_params effect_ov13850_cherryplus = {
    #include "cherryplus/effect_ov13850_liteon_cherryplus.h"
};

static effect_params effect_ov13850_chm = {
    #include "chm/effect_ov13850_liteon_chm.h"
};

static effect_params effect_ov13850_alice = {
    #include "alice/effect_ov13850_liteon_alice.h"
};
...
static const sensor_config_s ov13850_config_settings[]= {
    {
        "cherryplus",
        {ov13850_cherryplus_init_array,  ARRAY_SIZE(ov13850_cherryplus_init_array)},
        {ov13850_cherryplus_framesizes, ARRAY_SIZE(ov13850_cherryplus_framesizes)},
        &effect_ov13850_cherryplus,
        &vcm_dw9714a_ov13850_cherryplus,
    },

    {
        "chm",
        {ov13850_chm_init_array,  ARRAY_SIZE(ov13850_chm_init_array) },
        {ov13850_chm_framesizes, ARRAY_SIZE(ov13850_chm_framesizes)},
        &effect_ov13850_chm,
        &vcm_dw9714a_ov13850_chm,
    },

    {
        "alice",
        {ov13850_alice_init_array,  ARRAY_SIZE(ov13850_alice_init_array) },
        {ov13850_alice_framesizes, ARRAY_SIZE(ov13850_alice_framesizes)},
        &effect_ov13850_alice,
        &vcm_dw9714a_ov13850_alice,
    },

    {
        "cherrypro",
        {ov13850_cherrypro_init_array,  ARRAY_SIZE(ov13850_cherrypro_init_array) },
        {ov13850_cherrypro_framesizes, ARRAY_SIZE(ov13850_cherrypro_framesizes)},
        &effect_ov13850_cherryplus,
        &vcm_dw9714a_ov13850_cherryplus,
    },
};
static const sensor_config_s* ov13850_config = ov13850_config_settings;

static camera_sensor ov13850_sensor;
...
{
	return k3_ispio_read_reg(ov13850_sensor.i2c_config.index, ov13850_sensor.i2c_config.addr,
                reg, (u16*)val, ov13850_sensor.i2c_config.val_bits, ov13850_sensor.i2c_config.addr_bits );
}
...
{
	return k3_ispio_write_reg(ov13850_sensor.i2c_config.index, ov13850_sensor.i2c_config.addr,
	reg, val, ov13850_sensor.i2c_config.val_bits, mask,  ov13850_sensor.i2c_config.addr_bits);
}
...
static int ov13850_write_seq(const struct _sensor_reg_t *seq, u32 size, u8 mask)
{
	...
	return k3_ispio_write_seq(ov13850_sensor.i2c_config.index, ov13850_sensor.i2c_config.addr,
                seq, size, ov13850_sensor.i2c_config.val_bits, mask,  ov13850_sensor.i2c_config.addr_bits);
}
...
static int ov13850_get_format(struct v4l2_fmtdesc *fmt)
{
	if (fmt->type == V4L2_BUF_TYPE_VIDEO_OVERLAY) {
		fmt->pixelformat = ov13850_sensor.fmt[STATE_PREVIEW];
	} else {
		fmt->pixelformat = ov13850_sensor.fmt[STATE_CAPTURE];
	}
	return 0;
}
...
static int ov13850_enum_framesizes(struct v4l2_frmsizeenum *framesizes)
{
	...
	u32 this_max_index = ov13850_config->framesizes.size - 1;
	framesize_s * ov13850_framesizes = ov13850_config->framesizes.framesize_setting;
	assert(framesizes);
...
{
	u32 max_index = ov13850_config->framesizes.size - 1;
	framesize_s * ov13850_framesizes =  ov13850_config->framesizes.framesize_setting;
				 ...
				 struct v4l2_frmsize_discrete *fs, int flag, camera_setting_view_type view_type,bool zsl_preview)
{
	...
	bool match = false;
	u32 size =  0;
	framesize_s * ov13850_framesizes = NULL;
	assert(fs);
	...
	size =  ov13850_config->framesizes.size;
	ov13850_framesizes =  ov13850_config->framesizes.framesize_setting;
	...
	if (state == STATE_PREVIEW) {
		ov13850_sensor.preview_frmsize_index = i;
	} else {
		ov13850_sensor.capture_frmsize_index = i;
	}
	print_info("Enter Function:%s  preview index =%d, capture=%d ", __func__, ov13850_sensor.preview_frmsize_index, ov13850_sensor.capture_frmsize_index);
	return 0;
}
				     ...
				     struct v4l2_frmsize_discrete *fs)
{
	...
	ov13850_framesizes =  ov13850_config->framesizes.framesize_setting;
	if (state == STATE_PREVIEW) {
		frmsize_index = ov13850_sensor.preview_frmsize_index;
	} else if (state == STATE_CAPTURE) {
		frmsize_index = ov13850_sensor.capture_frmsize_index;
	} else {
		return -EINVAL;
	}
...
{
	int size = 0;
	camera_sensor *sensor = &ov13850_sensor;
...
{
	int size = 0;
	const sensor_reg_t* ov13850_init_regs = NULL;

	print_debug("Enter Function:%s  , initsize=%d",
		    __func__, ov13850_config->init_setting.settings[0].seq_size);


	if (0 != k3_ispio_init_csi(ov13850_sensor.mipi_index,
			ov13850_sensor.mipi_lane_count, ov13850_sensor.lane_clk)) {
		return -EFAULT;
	}
	...
	size = ov13850_config->init_setting.settings[0].seq_size;
	ov13850_init_regs = ov13850_config->init_setting.settings[0].setting;
...
static int ov13850_get_hflip(void)
{
	...
	return ov13850_sensor.hflip;
}
static int ov13850_set_vflip(int flip)
{
	print_debug("enter %s flip=%d", __func__, flip);

	ov13850_sensor.vflip = flip;
...
{
	print_debug("enter %s", __func__);
	return ov13850_sensor.vflip;
}

 
static int ov13850_update_flip(u16 width, u16 height)
{
	u8 new_flip = ((ov13850_sensor.vflip << 1) | ov13850_sensor.hflip);
 
	print_debug("Enter %s  ", __func__);
	if(ov13850_sensor.old_flip != new_flip) 
	{

		k3_ispio_update_flip((ov13850_sensor.old_flip ^ new_flip) & 0x03, width, height, PIXEL_ORDER_NO_CHANGED);

		ov13850_sensor.old_flip = new_flip;

 		ov13850_write_reg(OV13850_FLIP_REG, ov13850_sensor.vflip ? 0x00 : 0x04,  ~0x04);
		ov13850_write_reg(OV13850_MIRROR_REG, ov13850_sensor.hflip ? 0x04 : 0x00, ~0x04);
...
static int ov13850_framesize_switch(camera_state state)
{
	...
	u32 size = ov13850_config->framesizes.size;

	if (state == STATE_PREVIEW)
		next_frmsize_index = ov13850_sensor.preview_frmsize_index;
	else
		next_frmsize_index = ov13850_sensor.capture_frmsize_index;
	...
	if (0 != ov13850_write_seq(ov13850_sensor.frmsize_list[next_frmsize_index].sensor_setting.setting
		,ov13850_sensor.frmsize_list[next_frmsize_index].sensor_setting.seq_size, 0x00)) {
...
static int ov13850_check_sensor(void)
{
	...
	if(ret < 0) {
		print_error("%s ov13850_sensor fail to get gpio value!!!so return -1!\n",__func__);
		return -1;
	}

	if(ret != MODULE_LITEON){
		print_info("%s this moudle is not ov13850 moudle",__func__);
		return -1;
	}

	//strncpy(ov13850_sensor.info.name, "ov13850_liteon", sizeof(ov13850_sensor.info.name));
	...
	ov13850_sensor.vcm->get_vcm_otp = ov13850_otp_get_vcm;
#endif
...
static int ov13850_power(camera_power_state power)
{
	...
	if (power == POWER_ON) {
		...
		k3_ispio_ioconfig(&ov13850_sensor, power);
		k3_ispgpio_power_sensor(&ov13850_sensor, power);
		msleep(3);

	} else {
		k3_ispio_deinit_csi(ov13850_sensor.mipi_index);
...
static void ov13850_set_exposure_gain(u32 exposure, u32 gain)
{
    ...
    ov13850_write_reg(OV13850_GAIN_REG_L, analog_gain & 0xff, 0x00);

    ov13850_write_reg(OV13850_DIG_GAIN_REG_H, (digital_gain >> 8) & 0x07, 0x00);
    ov13850_write_reg(OV13850_DIG_GAIN_REG_L, digital_gain & 0xff, 0x00);
...
/* **************************************************************************
  * FunctionName: ov13850_dump_reg_debug;
  ...
  * Other       : NA;
  ***************************************************************************/
static void ov13850_dump_reg_debug()
{
      u16 reg = 0;
      u8 val = 0;

      reg=0x0100;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);

      reg=0x0300;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x0302;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3614;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x370a;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x372a;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x372f;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //H crop start L
      reg=0x3801;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //V crop start L
      reg=0x3803;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //H crop end L
      reg=0x3805;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //V crop end L
      reg=0x3807;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //H output size H
      reg=0x3808;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3809;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380a;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380b;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380c;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380d;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380e;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x380f;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3811;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3813;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3814;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3815;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3820;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3821;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3836;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3837;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4020;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4021;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4022;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4023;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4024;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4025;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4026;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4027;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4501;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x4601;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x5401;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x5405;ov13850_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
}
...
static int ov13850_reset(camera_power_state power_state)
{
	...
	if (POWER_ON == power_state) {
		k3_isp_io_enable_mclk(MCLK_ENABLE, ov13850_sensor.sensor_index);
		udelay(100);
		k3_ispgpio_reset_sensor(ov13850_sensor.sensor_index, power_state,
			      ov13850_sensor.power_conf.reset_valid);
		mdelay(5);
	} else {
		k3_ispgpio_reset_sensor(ov13850_sensor.sensor_index, power_state,
			      ov13850_sensor.power_conf.reset_valid);
		udelay(10);
		k3_isp_io_enable_mclk(MCLK_DISABLE, ov13850_sensor.sensor_index);
	}
	ov13850_sensor.old_flip = 0;
	return 0;
}
...
static int ov13850_init(void)
{
    ...
    if (false == ov13850_check) {
        if (video_check_suspensory_camera("ov13850_sensor") != 1) {
            print_error("%s: product not have this sensor", __func__);
            return -ENODEV;
        }

        ret = camera_get_matching_sensor_config(ov13850_config_settings, ARRAY_SIZE(ov13850_config_settings),&ov13850_config);
        if( ret < 0 ){
            print_error("%s: fail to match sensor config.", __func__);
            return -ENODEV;
        } else {
            print_info("%s: choose the %s's setting.",__func__,ov13850_config->product);
        }

        ov13850_sensor.frmsize_list = ov13850_config->framesizes.framesize_setting;
        ov13850_sensor.vcm = ov13850_config->vcm_info;
        ov13850_sensor.effect = ov13850_config->effect;
        ov13850_sensor.lane_clk = ov13850_config->framesizes.framesize_setting[0].lane_clk;
        ov13850_check = true;
    }

    if (ov13850_sensor.owner && !try_module_get(ov13850_sensor.owner)) {
...
static void ov13850_exit(void)
{
	...
	if (ov13850_sensor.owner) {
		module_put(ov13850_sensor.owner);
	}
...
{
	print_debug("enter %s", __func__);
	k3_ispgpio_power_sensor(&ov13850_sensor, POWER_OFF);
}
...
static void ov13850_config_dphy_clk(camera_state state)
{
	...
	if (state == STATE_PREVIEW) {
		lane_clk =ov13850_sensor.frmsize_list[ov13850_sensor.preview_frmsize_index].lane_clk;
	} else {
		lane_clk =ov13850_sensor.frmsize_list[ov13850_sensor.capture_frmsize_index].lane_clk;
	}

	print_info("%s lane_clk = 0x%x state = %d",__func__, lane_clk, state);
	k3_ispio_config_lane_clk(ov13850_sensor.mipi_index, ov13850_sensor.mipi_lane_count, lane_clk);
}
...
static void ov13850_read_otp(u8 i2c_addr,u16 reg,u8 *buf,u16 count)
{
	...
	for(;i<count;i++) {
		ret =ispv1_read_sensor_byte_addr8(ov13850_sensor.i2c_config.index,i2c_addr,reg+i,&val,ov13850_sensor.i2c_config.val_bits);
		if(ret !=0)
...
static void ov13850_otp_get_vcm(u16 *vcm_start, u16 *vcm_end)
{
	if (0 == ov13850_otp.vcm_start) {
		*vcm_start = ov13850_sensor.vcm->infiniteDistance;
	} else {
        if (ov13850_otp.vcm_start > ov13850_sensor.vcm->startCurrentOffset)
            *vcm_start = ov13850_otp.vcm_start - ov13850_sensor.vcm->startCurrentOffset;
        else
            *vcm_start = 0;
	}

	if (0 == ov13850_otp.vcm_end) {
		*vcm_end = ov13850_sensor.vcm->normalDistanceEnd;
	} else {
...
static int ov13850_get_equivalent_focus(void)
{
	print_debug("enter %s", __func__);
	return OV13850_EQUIVALENT_FOCUS;
}
/*
 **************************************************************************
 * FunctionName: ov13850_set_default;
 * Description : init ov13850_sensor;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 * Other       : NA;
...
static void ov13850_set_default(void)
{
	ov13850_sensor.init = ov13850_init;
	ov13850_sensor.exit = ov13850_exit;
	ov13850_sensor.shut_down = ov13850_shut_down;
	ov13850_sensor.reset = ov13850_reset;
	ov13850_sensor.check_sensor = ov13850_check_sensor;
	ov13850_sensor.power = ov13850_power;
	ov13850_sensor.init_reg = ov13850_init_reg;
	ov13850_sensor.stream_on = ov13850_stream_on;

	ov13850_sensor.get_format = ov13850_get_format;
	ov13850_sensor.set_flash = NULL;
	ov13850_sensor.get_flash = NULL;
	ov13850_sensor.set_scene = NULL;
	ov13850_sensor.get_scene = NULL;

	ov13850_sensor.enum_framesizes = ov13850_enum_framesizes;
	ov13850_sensor.try_framesizes = ov13850_try_framesizes;
	ov13850_sensor.set_framesizes = ov13850_set_framesizes;
	ov13850_sensor.get_framesizes = ov13850_get_framesizes;

	ov13850_sensor.enum_frame_intervals = ov13850_enum_frame_intervals;
	ov13850_sensor.try_frame_intervals = NULL;
	ov13850_sensor.set_frame_intervals = NULL;
	ov13850_sensor.get_frame_intervals = NULL;

	ov13850_sensor.get_vts_reg_addr = ov13850_get_vts_reg_addr;

	ov13850_sensor.get_capability = ov13850_get_capability;

	ov13850_sensor.set_hflip = ov13850_set_hflip;
	ov13850_sensor.get_hflip = ov13850_get_hflip;
	ov13850_sensor.set_vflip = ov13850_set_vflip;
	ov13850_sensor.get_vflip = ov13850_get_vflip;
	ov13850_sensor.update_flip = ov13850_update_flip;

	strncpy(ov13850_sensor.info.name,"ov13850",sizeof(ov13850_sensor.info.name));
	ov13850_sensor.interface_type = MIPI1;
	ov13850_sensor.mipi_lane_count = CSI_LINES_4;
	ov13850_sensor.mipi_index = CSI_INDEX_0;
	ov13850_sensor.sensor_index = CAMERA_SENSOR_PRIMARY;
	ov13850_sensor.skip_frames = 2;

	ov13850_sensor.power_conf.pd_valid = LOW_VALID;
	ov13850_sensor.power_conf.reset_valid = LOW_VALID;
	ov13850_sensor.power_conf.vcmpd_valid = LOW_VALID;

	ov13850_sensor.i2c_config.index = I2C_PRIMARY;
	ov13850_sensor.i2c_config.speed = I2C_SPEED_400;
	ov13850_sensor.i2c_config.addr = OV13850_SLAVE_ADDRESS;
	ov13850_sensor.i2c_config.addr_bits = I2C_16BIT;
	ov13850_sensor.i2c_config.val_bits = I2C_8BIT;

	ov13850_sensor.preview_frmsize_index = 0;
	ov13850_sensor.capture_frmsize_index = 0;
	ov13850_sensor.frmsize_list = ov13850_config_settings[0].framesizes.framesize_setting;
	ov13850_sensor.fmt[STATE_PREVIEW] = V4L2_PIX_FMT_RAW10;
	ov13850_sensor.fmt[STATE_CAPTURE] = V4L2_PIX_FMT_RAW10;

#ifdef OV13850_AP_WRITEAE_MODE /* just an example and test case for AP write AE mode */
	ov13850_sensor.aec_addr[0] = 0;
	ov13850_sensor.aec_addr[1] = 0;
	ov13850_sensor.aec_addr[2] = 0;
	ov13850_sensor.agc_addr[0] = 0;
	ov13850_sensor.agc_addr[1] = 0;
	ov13850_sensor.ap_writeAE_delay = 6000; /* 5 expo and gain registers, 6000 us is enough */
#else
	ov13850_sensor.aec_addr[0] = 0x3500;
	ov13850_sensor.aec_addr[1] = 0x3501;
	ov13850_sensor.aec_addr[2] = 0x3502;
	ov13850_sensor.agc_addr[0] = 0x350a; 
	ov13850_sensor.agc_addr[1] = 0x350b;
#endif
	...
	ov13850_sensor.sensor_type = SENSOR_SONY;
	ov13850_sensor.sensor_rgb_type = SENSOR_BGGR;

	ov13850_sensor.set_gain = NULL;//ov13850_set_gain;
	ov13850_sensor.set_exposure = NULL;//ov13850_set_exposure;
	ov13850_sensor.set_exposure_gain = ov13850_set_exposure_gain;
	ov13850_sensor.set_vts = ov13850_set_vts;

	ov13850_sensor.sensor_dump_reg = ov13850_dump_reg_debug;

	ov13850_sensor.get_override_param = ov13850_get_override_param;

	ov13850_sensor.sensor_gain_to_iso = ov13850_gain_to_iso;
	ov13850_sensor.sensor_iso_to_gain = ov13850_iso_to_gain;

	ov13850_sensor.get_sensor_aperture = ov13850_get_sensor_aperture;
	ov13850_sensor.get_equivalent_focus = ov13850_get_equivalent_focus;
	ov13850_sensor.set_effect = NULL;

	ov13850_sensor.isp_location = CAMERA_USE_K3ISP;
	ov13850_sensor.sensor_tune_ops = NULL;

	ov13850_sensor.af_enable = 1;
	ov13850_sensor.vcm = &vcm_dw9714;
	ov13850_sensor.get_flash_awb = ov13850_get_flash_awb;

	ov13850_sensor.image_setting.lensc_param = ov13850_lensc_param;
	ov13850_sensor.image_setting.ccm_param = ov13850_ccm_param;
	ov13850_sensor.image_setting.awb_param = ov13850_awb_param;

	ov13850_sensor.fps_max = 30;
	ov13850_sensor.fps_min = 10;
	ov13850_sensor.fps = 25;


	ov13850_sensor.owner = THIS_MODULE;

	ov13850_sensor.info.facing = CAMERA_FACING_BACK;
	ov13850_sensor.info.orientation = 90;
	ov13850_sensor.info.focal_length = 379;	/* 3.79mm */
	ov13850_sensor.info.h_view_angle = 63;//63.2
	ov13850_sensor.info.v_view_angle = 49;

	ov13850_sensor.lane_clk = CLK_500M;

	ov13850_sensor.hflip = 0;
	ov13850_sensor.vflip = 0;
	ov13850_sensor.old_flip = 0;

	ov13850_sensor.get_gain     				= ov13850_get_gain;
	ov13850_sensor.get_exposure   			= ov13850_get_exposure;
	ov13850_sensor.init_isp_reg 				= ov13850_init_isp_reg;

	ov13850_sensor.support_summary		= false;
	ov13850_sensor.isp_idi_skip = false;

	ov13850_sensor.stream_off_setting = ov13850_stream_off_regs;
	ov13850_sensor.stream_off_setting_size = ARRAY_SIZE(ov13850_stream_off_regs);
	...
	ov13850_sensor.config_dphy_clk = ov13850_config_dphy_clk;

	ov13850_sensor.get_sensor_reg = ov13850_get_sensor_reg;
	ov13850_sensor.set_sensor_reg = ov13850_set_sensor_reg;
	ov13850_sensor.check_otp_status = ov13850_check_otp;

}

/*
 **************************************************************************
 * FunctionName: ov13850_module_init;
 * Description : NA;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 ...
 **************************************************************************
 */
static __init int ov13850_module_init(void)
{
	ov13850_set_default();
	return register_camera_sensor(ov13850_sensor.sensor_index, &ov13850_sensor);
}

/*
 **************************************************************************
 * FunctionName: ov13850_module_exit;
 * Description : NA;
 * Input       : NA;
 * Output      : NA;
 * ReturnValue : NA;
 ...
 **************************************************************************
 */
static void __exit ov13850_module_exit(void)
{
	unregister_camera_sensor(ov13850_sensor.sensor_index, &ov13850_sensor);
}

MODULE_AUTHOR("Hisilicon");
module_init(ov13850_module_init);
module_exit(ov13850_module_exit);
MODULE_LICENSE("GPL");
