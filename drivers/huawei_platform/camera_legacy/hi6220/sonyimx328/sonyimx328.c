...
#define SONYIMX328_EQUIVALENT_FOCUS		27 // 27mm
...
#define SONYIMX328_BLC_CLAMP_MODE_REG   (0x30B1)
...
#define SONYIMX328_MODULE_HUAWEI_ID    0xA7//23060167

awb_gain awb_gain_typical[] = {
...
/******************vcm begin*************************/
static vcm_info_s vcm_dw9714_sonyimx328_cherryplus = {
    #include "cherryplus/vcm_dw9714_sunny_cherryplus.h"
};

static vcm_info_s vcm_dw9714_sonyimx328_chm = {
    #include "chm/vcm_dw9714_sunny_chm.h"
};
...
/******************effect begin*************************/
static effect_params effect_imx328_cherryplus = {
#include "cherryplus/effect_imx328_sunny_cherryplus.h"
};

static effect_params effect_imx328_chm = {
#include "chm/effect_imx328_sunny_chm.h"
};
...
static const sensor_config_s sonyimx328_config_settings[]= {
    {
        "cherryplus",
        {sonyimx328_cherryplus_init_array,  ARRAY_SIZE(sonyimx328_cherryplus_init_array) },
        {sonyimx328_cherryplus_framesizes, ARRAY_SIZE(sonyimx328_cherryplus_framesizes)},
    ...
    {
        "chm",
        {sonyimx328_chm_init_array,  ARRAY_SIZE(sonyimx328_chm_init_array) },
        {sonyimx328_chm_framesizes, ARRAY_SIZE(sonyimx328_chm_framesizes)},
        &effect_imx328_chm,
        &vcm_dw9714_sonyimx328_chm,
    },

    {
        "alice",
        {sonyimx328_alice_init_array,  ARRAY_SIZE(sonyimx328_alice_init_array) },
        {sonyimx328_alice_framesizes, ARRAY_SIZE(sonyimx328_alice_framesizes)},
        &effect_imx328_alice,
        &vcm_dw9714_sonyimx328_alice,
    },
...
static void sonyimx328_reset_dphy(void);
...
static int sonyimx328_enum_framesizes(struct v4l2_frmsizeenum *framesizes)
{
	...
	u32 this_max_index =   sonyimx328_config->framesizes.size-1;
	framesize_s * sonyimx328_framesizes = sonyimx328_config->framesizes.framesize_setting;
...
{
	u32 max_index = sonyimx328_config->framesizes.size - 1;
	framesize_s * sonyimx328_framesizes =  sonyimx328_config->framesizes.framesize_setting;
                ...
                struct v4l2_frmsize_discrete *fs, int flag, camera_setting_view_type view_type,bool zsl_preview)
{
    ...
    bool match = false;
    int size = 0;
    framesize_s * sonyimx328_framesizes = NULL;
...
{
	int size = 0;
	const sensor_reg_t* sonyimx328_init_regs = NULL;
	print_debug("Enter Function:%s  , initsize=%d",
		    __func__, sonyimx328_config->init_setting.settings[0].seq_size);
	...
	size = sonyimx328_config->init_setting.settings[0].seq_size;
	sonyimx328_init_regs = sonyimx328_config->init_setting.settings[0].setting;
...
/* **************************************************************************
 * FunctionName: sonyimx328_foxconn_dump_reg_debug;
 ...
 * Other       : NA;
 ***************************************************************************/
static void sonyimx328_dump_reg_debug(void)
{
      u16 reg = 0;
      u8 val = 0;

      reg=0x0100;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //frame count
      reg=0x0005;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x0307;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //frame lenght
      reg=0x0340;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x0341;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //x output size
      reg=0x034c;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x034d;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      //y output size
      reg=0x034e;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x034f;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x0383;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x0387;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x3048;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x309b;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x30d5;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
      reg=0x30d6;sonyimx328_read_reg(reg,&val);print_info("0x%0x=0x%0x", reg, val);
...
static int sonyimx328_framesize_switch(camera_state state)
{
	...
	u32 size = sonyimx328_config->framesizes.size;
...
static int sonyimx328_init(void)
{
    ...
    if (false == sonyimx328_check) {
        if (video_check_suspensory_camera("sonyimx328_sensor") != 1) {
            print_error("%s: product not have this sensor", __func__);
            return -ENODEV;
        }

        ret = camera_get_matching_sensor_config(sonyimx328_config_settings, ARRAY_SIZE(sonyimx328_config_settings),&sonyimx328_config);
        if( ret < 0 ){
            print_error("%s: fail to match sensor config.", __func__);
            return -ENODEV;
        } else {
            print_info("%s: choose the %s's setting.",__func__,sonyimx328_config->product);
        }

        sonyimx328_sensor.frmsize_list = sonyimx328_config->framesizes.framesize_setting;
        sonyimx328_sensor.vcm =  sonyimx328_config->vcm_info;
        sonyimx328_sensor.effect = sonyimx328_config->effect;
        sonyimx328_sensor.lane_clk = sonyimx328_config->framesizes.framesize_setting[0].lane_clk;
...
/*
 **************************************************************************
 * FunctionName: sonyimx328_set_blc_normal_mode_clamp;
 * Description : set blc clamp mode to normal tracking mode
 * Input       	 NA;
 * Output      	 NA
 * ReturnValue : NA;
 * Other       : only sonyimx328 sensor need to define this func;
 **************************************************************************
*/
static void sonyimx328_set_blc_normal_mode_clamp(void)
{
    print_debug("AEC:enter %s", __func__);
    sonyimx328_write_reg(SONYIMX328_BLC_CLAMP_MODE_REG, 0x03, 0x00);
}
...
/*
 **************************************************************************
 * FunctionName: sonyimx328_reset_dphy;
...
static void sonyimx328_reset_dphy(void)
{
       k3_ispio_reset_phy(sonyimx328_sensor.mipi_index, sonyimx328_sensor.mipi_lane_count);
}

static int sonyimx328_get_equivalent_focus(void)
{
	print_debug("enter %s", __func__);
	return SONYIMX328_EQUIVALENT_FOCUS;
}
/*
 **************************************************************************
...
static void sonyimx328_set_default(void)
{
	...
	strncpy(sonyimx328_sensor.info.name, "sonyimx328_sunny",sizeof(sonyimx328_sensor.info.name));
	...
	sonyimx328_sensor.frmsize_list = sonyimx328_config_settings[0].framesizes.framesize_setting;
	...
	sonyimx328_sensor.get_equivalent_focus = sonyimx328_get_equivalent_focus;
	sonyimx328_sensor.isp_location = CAMERA_USE_K3ISP;
	sonyimx328_sensor.sensor_tune_ops = NULL;

	sonyimx328_sensor.af_enable = 1;
	sonyimx328_sensor.vcm = sonyimx328_config_settings[0].vcm_info;
    ...
    /*sonyimx328 sensor's optical black level normal tracking mode setting.
      Set clamping mode back to normal tracking mode after 2 frames.
      1.Do ignore this ptr or set to be NULL if the sensor drv needn't.
    */
	sonyimx328_sensor.set_blc_normal_mode_clamp = sonyimx328_set_blc_normal_mode_clamp;
	sonyimx328_sensor.capture_skip_frames = CAPTURE_SKIP_2;
	...
	sonyimx328_sensor.config_dphy_clk = sonyimx328_config_dphy_clk;
       sonyimx328_sensor.reset_dphy = sonyimx328_reset_dphy;
 ...
 **************************************************************************
 */
static __init int sonyimx328_module_init(void)
{
 ...
 **************************************************************************
 */
static void __exit sonyimx328_module_exit(void)
{
	unregister_camera_sensor(sonyimx328_sensor.sensor_index, &sonyimx328_sensor);
}

MODULE_AUTHOR("Hisilicon");
module_init(sonyimx328_module_init);
module_exit(sonyimx328_module_exit);
MODULE_LICENSE("GPL");
