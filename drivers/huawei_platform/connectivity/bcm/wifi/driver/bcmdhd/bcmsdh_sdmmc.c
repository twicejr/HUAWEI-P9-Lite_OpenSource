...
#ifdef HW_SDIO_RW_RETRY
			if (retry_times > 0) {
				sd_err(("bcmsdh_sdmmc: retry for %s error. left time: %d\n", rw ? "Write" : "Read", retry_times));
				retry_times--;
				err_ret = 0;
				goto retry;
			}
...
#ifdef HW_WIFI_DMD_LOG
			hw_wifi_dsm_client_notify(DSM_WIFI_SDIO_RESET_COMM_ERROR,
			                  "sdio_reset_comm failed, ret = %d\n", ret);
#endif
			return ret;
		}
		else {
...
int sdioh_power_off(sdioh_info_t *sd)
{
	int ret;
	struct mmc_host *host = NULL;

	if( (sd != NULL) && (sd->func[0] != NULL) && (sd->func[0]->card != NULL) && (sd->func[0]->card->host != NULL) ) {
        host = sd->func[0]->card->host;
    }else{
	...
	sdio_claim_host(sd->func[0]);
	ret = mmc_power_save_host_for_wifi(host);
	sdio_release_host(sd->func[0]);
...
int sdioh_power_on(sdioh_info_t *sd)
{
	int ret;
	struct mmc_host *host = NULL;
	...
	sdio_claim_host(sd->func[0]);
	ret = mmc_power_restore_host_for_wifi(host);
	sdio_release_host(sd->func[0]);
...
int sdioh_wakelog_switch(sdioh_info_t *sd, bool enable)
{
	int ret;
	struct mmc_host *host = NULL;
	if( (sd != NULL) && (sd->func[0] != NULL) && (sd->func[0]->card != NULL) && (sd->func[0]->card->host != NULL) ) {
	...
	dw_mci_sdio_wakelog_switch(host, enable);
}
#endif
