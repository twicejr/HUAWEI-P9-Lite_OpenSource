...
static void *dhd_wlan_mem_prealloc(int section, unsigned long size)
{
	...
	if (section == DHD_PREALLOC_OSL_BUF) {
		if (size > DHD_PREALLOC_OSL_BUF_SIZE) {
			HW_PRINT_HI("request OSL_BUF(%lu) is bigger than static size(%ld).\n",
	...
	if (section == DHD_PREALLOC_DHD_INFO) {
		if (size > DHD_PREALLOC_DHD_INFO_SIZE) {
			HW_PRINT_HI("request DHD_INFO size(%lu) is bigger than static size(%d).\n",
	...
	if (section == DHD_PREALLOC_IF_FLOW_LKUP)  {
		if (size > DHD_PREALLOC_IF_FLOW_LKUP_SIZE) {
			HW_PRINT_HI("request DHD_IF_FLOW_LKUP size(%lu) is bigger than static size(%d).\n",
	...
	if ((section < 0) || (section > DHD_PREALLOC_MAX))
		HW_PRINT_HI("request section id(%d) is out of max index %d\n",
...
static int dhd_init_wlan_mem(void)
{
	...
	if (!wlan_static_dhd_info_buf) {
		HW_PRINT_HI("Failed to alloc wlan_static_dhd_info_buf\n");
		goto err_mem_alloc;
	}
#ifdef CONFIG_BCMDHD_PCIE
	wlan_static_if_flow_lkup = kmalloc(DHD_PREALLOC_IF_FLOW_LKUP_SIZE, GFP_KERNEL);
	if (!wlan_static_if_flow_lkup) {
		HW_PRINT_HI("Failed to alloc wlan_static_if_flow_lkup\n");
		goto err_mem_alloc;
	}
#endif /* CONFIG_BCMDHD_PCIE */
...
#endif
	HW_PRINT_HI("Failed to mem_alloc for WLAN\n");

	i = WLAN_SKB_BUF_NUM;

err_skb_alloc:
	HW_PRINT_HI("Failed to skb_alloc for WLAN\n");
	for (j = 0; j < i; j++) {
...
int dhd_wifi_init_gpio(void)
{
	...
	if (gpio_request(gpio_wl_reg_on, "WL_REG_ON"))
		HW_PRINT_HI("%s: Faiiled to request gpio %d for WL_REG_ON\n",
			__func__, gpio_wl_reg_on);
	else
		HW_PRINT_HI("%s: gpio_request WL_REG_ON done\n", __func__);

	if (gpio_direction_output(gpio_wl_reg_on, 1))
		HW_PRINT_HI("%s: WL_REG_ON failed to pull up\n", __func__);
	else
...
{
	HW_PRINT_HI("%s Enter: power %s\n", __func__, on ? "on" : "off");

	if (on) {
		if (gpio_direction_output(gpio_wl_reg_on, 1)) {
			HW_PRINT_HI("%s: WL_REG_ON didn't output high\n", __func__);
			return -EIO;
		}
		if (!gpio_get_value(gpio_wl_reg_on))
			HW_PRINT_HI("[%s] gpio didn't set high.\n", __func__);
	} else {
		if (gpio_direction_output(gpio_wl_reg_on, 0)) {
			HW_PRINT_HI("%s: WL_REG_ON didn't output low\n", __func__);
			return -EIO;
		}
	}
	return 0;
