...
int wifi_platform_get_nvram_path(wifi_adapter_info_t *adapter, unsigned char *buf, int len)
{
	struct wifi_platform_data *plat_data;

	if (!buf || !adapter || !adapter->wifi_plat_data || len <= 0) {
		DHD_ERROR(("%s: the input param is invalid. len=%d\n", __FUNCTION__, len));
		return -EINVAL;
	}

	plat_data = adapter->wifi_plat_data;
	if (plat_data->get_nvram_path) {
		return plat_data->get_nvram_path(buf, len);
	}

	DHD_ERROR(("%s: plat_data->get_nvram_path is null\n", __FUNCTION__));
	return -EOPNOTSUPP;
}
#endif
