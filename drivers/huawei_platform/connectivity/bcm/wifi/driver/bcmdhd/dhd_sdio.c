...
#define MAX_NVRAMBUF_SIZE  (2*4096)	/* max nvram buf size */
#else
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	int err = 0;
	int try_cnt = 0;
#ifdef HW_SKIP_CMD52_TIMEOUT_LOG
	bcmsdh_wakelog_switch(bus->sdh, true);
#endif
...
#endif
#ifdef HW_SKIP_CMD52_TIMEOUT_LOG
	bcmsdh_wakelog_switch(bus->sdh, false);
#endif
	return err;
}
...
#ifdef HW_DNS_DHCP_PARSE
	hwdump_data = PKTDATA(osh, pkt);
	hwdump_len = PKTLEN(osh, pkt);
	hwdump_data += 4;
	hwdump_protocal = ((hwdump_data[12] << 8) | hwdump_data[13]);
	if (hwdump_protocal == ETH_P_IP && hwdump_len > (ETH_HLEN + 4)) {
		hw_parse_special_ipv4_packet(hwdump_data, hwdump_len - 4);
	}
...
#ifdef HW_KSO_EXCEPTION_PANIC_FIX
		return 0;
#else
	...
	if (bus->glom) {
		...
		if (errcode < 0) {
			...
			if (bus->glomerr++ < 3) {
				...
				if (errcode == BCME_SDIO_ERROR){
					DHD_ERROR(("%s:sdio error found, errcode=%d\n",__FUNCTION__,errcode));
				}
				bus->glomerr = 0;
	...
	if (bus->activity)
		...
		if ((bus->idletime > 0) && (bus->idlecount >= bus->idletime)) {
			...
			if (SLPAUTO_ENAB(bus)) {
				...
				dhdsdio_clkctl(bus, CLK_NONE, FALSE);
#ifdef HW_SOFTAP_NOT_SUSPEND_BUG
				dhd_os_wd_timer(bus->dhd, 0);
#endif
			}
			bus->idlecount = 0;
...
static int hw_load_calibrate_data(char * calbufp, uint size)
{
	int caldatalen = 0;
	int ret = 0, sum = 0;
...
static int hw_calibrate_nvram_vars(struct dhd_bus *bus, char * bufp, uint len)
{
	...
	CalDateLen = hw_load_calibrate_data(CalDateBuf, sizeof(CalDateBuf));
	if (CalDateLen<=0) {
	...
	if (flag == TRUE) {
		...
		if (bus->dhd->dongle_reset) {
			...
			if (dhdsdio_probe_attach(bus, bus->dhd->osh, bus->sdh,
				...
				bcmerror = BCME_SDIO_ERROR;
#ifdef HW_WIFI_DMD_LOG
				hw_wifi_dsm_client_notify(DSM_WIFI_SDIO_PROBE_ATTACH_ERROR,
				             "dhdsdio_probe_attach failed\n");
#endif
			}
...
int dhd_bus_power_up(dhd_pub_t *dhdpub)
{
    return bcmsdh_power_on(dhdpub->bus->sdh);
}

int dhd_bus_power_down(dhd_pub_t *dhdpub)
{
    return bcmsdh_power_off(dhdpub->bus->sdh);
}
#endif
