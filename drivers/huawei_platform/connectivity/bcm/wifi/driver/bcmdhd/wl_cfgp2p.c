...
wl_cfgp2p_add_p2p_disc_if(struct bcm_cfg80211 *cfg)
{
	...
	if (!cfg->p2p) {
		CFGP2P_ERR(("p2p_info is not initialized.\n"));
...
wl_cfgp2p_stop_p2p_device(struct wiphy *wiphy, struct wireless_dev *wdev)
{
    ...
    //wifi cause system panic when do wl_cfgp2p_disable_discovery.
    if (!g_wifi_on) {
