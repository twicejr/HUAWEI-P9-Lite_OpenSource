...
extern struct mutex mutex_sensorhub_rdr;
/*begin huangwen 20120706*/
static int isSensorMcuMode = 0; /*mcu power mode: 0 power off;  1 power on*/
...
int inputhub_mcu_recv(const char *buf, unsigned int length)
{
    ...
    if (api_inputhub_mcu_recv != NULL) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_READ, __func__, "---->error: api_inputhub_mcu_recv == NULL\n");
        return -1;
    }
}
...
int iom3_need_recovery(int modid)
{
	...
	if(old_state == IOM3_RECOVERY_IDLE){//prev state is IDLE start recovery progress
			...
			dmd_log_report(DSM_SHB_ERR_IOM7_WDG, __func__, "sensorhub crash, trigger rdr dump\n");
		}
