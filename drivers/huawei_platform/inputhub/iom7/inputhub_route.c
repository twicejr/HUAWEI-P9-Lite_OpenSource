...
static uint64_t sensors_tm[TAG_SENSOR_END] = {0,};
...
static inline bool really_do_enable_disable(int *ref_cnt, bool enable, int bit)
{
    bool ret = false;
...
static int inputhub_route_item(unsigned short port, struct inputhub_route_table **route_item)
{
    ...
    dmd_log_report(DSM_SHB_ERR_MCU_SYS_MAIN_INIT, __func__, "unknown port\n");
    return -EINVAL;
...
ssize_t inputhub_route_read(unsigned short port, char __user *buf, size_t count)
{
    ...
    if (reader.buffer_size > ROUTE_BUFFER_MAX_SIZE) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_READ, __func__, "error reader.buffer_size in port\n");
        goto clean_buffer;
    }
    ...
    if (full_pkg_length + LENGTH_SIZE > reader.buffer_size || full_pkg_length > count) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_READ, __func__, "full_pkg_length is too large in port\n");
        goto clean_buffer;
    }
    ...
    if (buffer_end - reader.pos >= full_pkg_length) {
        if (0 == copy_to_user(buf, reader.pos, full_pkg_length)) {
            ...
            dmd_log_report(DSM_SHB_ERR_IOM7_READ, __func__, "copy to user failed\n");
            return 0;
        }
    } else {
        tail_half_len = buffer_end - reader.pos;
...
static int inputhub_sensor_enable_internal_nolock(int tag, bool enable, close_param_t *param)
{
    if (NULL == param) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "NULL pointer param\n");
        return -EINVAL;
    }
    ...
    if (TAG_SENSOR_BEGIN <= tag && tag < TAG_SENSOR_END) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "unknown sensor type\n");
        return -EINVAL;
    }
}
...
static void update_motion_info(obj_cmd_t cmd, motion_type_t type)
{
    ...
    switch (cmd) {
        ...
        default:
            ...
            dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "unknown cmd type\n");
            break;
    }
}
...
int inputhub_mcu_write_cmd(const void *buf, unsigned int length)
{
    ...
    if (length > MAX_PKT_LENGTH) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "length is too large\n");
        return -EINVAL;
    }
...
int send_motion_cmd_internal(int tag, obj_cmd_t cmd, motion_type_t type, bool use_lock)
{
    ...
    if (CMD_CMN_OPEN_REQ == cmd) {
        ...
        if (really_do_enable_disable(&motion_ref_cnt, true, type)) {
           ...
           // i_pkt.param.reserved[0] = type;
            ...
            if (likely((type > MOTION_TYPE_START)&&(type < MOTION_TYPE_END))){
                ...
                hwlog_info("send_motion_cmd open cmd:%d motion: %d !", cmd, type);
            }
            if (use_lock) {
        ...
        hwlog_info("send_motion_cmd config cmd:%d motion: %s !motion_ref_cnt= %x", cmd, motion_type_str[type],print_count);
        ...
        if (really_do_enable_disable(&motion_ref_cnt, false, type)) {
            ...
            if (likely((type > MOTION_TYPE_START)&&(type < MOTION_TYPE_END))){
                ...
                hwlog_info("send_motion_cmd close cmd:%d motion: %d !", cmd, type);
            }
            if (use_lock) {
...
int send_motion_cmd(unsigned int cmd, unsigned long arg)
{
    ...
    if (sizeof(motions_cmd_map_tab) / sizeof(motions_cmd_map_tab[0]) == i) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "send_motion_cmd unknown cmd in parse_motion_cmd!\n");
        return -EFAULT;
    }
...
int send_apr_log(void)
{
    ...
    if(ret < 0 )
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "logexception sysfs err\n");
    return ret;
}
    ...
    if (tag == TAG_STEP_COUNTER) {
        ...
        if (!enable) {
	     ...
	     app_config[1] = TYPE_STANDARD;
            send_app_config_cmd(TAG_STEP_COUNTER, app_config, true);
            if(really_do_enable_disable(&step_ref_cnt, enable, TYPE_STANDARD))
...
bool ap_sensor_flush(int tag)
{
    ...
    if (work_on_ap) {
        ...
        return inputhub_route_write_batch(ROUTE_SHB_PORT, (char *)&event, event.length + OFFSET_OF_END_MEM(struct sensor_data, length), 0);
    }

    return work_on_ap;
}
 int send_sensor_batch_flush_cmd(unsigned int cmd, struct ioctl_para *para, int tag)
{
...
static int send_sensor_cmd(unsigned int cmd, unsigned long arg)
{
    ...
    case SHB_IOCTL_APP_SENSOR_FLUSH:
        if(tag == TAG_STEP_COUNTER)
        {
            if(0 == (ret = send_sensor_batch_flush_cmd(cmd, &para, tag)))
            {
                //success
                app_config[0] = 1;
	          app_config[1] = TYPE_STANDARD;
...
int write_customize_cmd(const struct write_info *wr, struct read_info *rd)
{
    ...
    if (NULL == wr) {
        ...
        dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "NULL pointer\n");
        return -EINVAL;
    }
    //[TAG_BEGIN, TAG_END)
    if (wr->tag < TAG_BEGIN || wr->tag >= TAG_END) {
        hwlog_err("tag = %d error in %s\n", wr->tag, __func__);
		dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "tag error\n");
        return -EINVAL;
    }
...
{
     return (TAG_FINGERSENSE == head->tag) && (CMD_ACCEL_FINGERSENSE_DATA_REPORT == head->cmd);
}
...
int report_sensor_event_batch(int tag, int value[], int length, uint64_t timestamp)
{
    struct sensor_data event;
    ...
    return inputhub_route_write_batch(ROUTE_SHB_PORT, (char *)&event, event.length + OFFSET_OF_END_MEM(struct sensor_data, length), ltimestamp);
}
    ...
    if (is_fingersense_zaxis_data_report(head)) {
	  ...
	  sensor_event = (pkt_batch_data_req_t *)head;
        if (TAG_STEP_COUNTER == head->tag) {//extend step counter date
            ...
            report_sensor_event(head->tag, (int *)(&((pkt_step_counter_data_req_t *)head)->step_count)
                , head->length);
            if((flush_flag = ((pkt_step_counter_data_req_t *)head)->data_flag) == 1)
            {
                goto flush_event;
            }
        }
        ...
        //hwlog_debug("[%lld] Kernel get event %d cnt %d! ele_cnt %d\n",getTimestamp(), head->tag, sensor_event->cnt, sensor_event->len_element);
        ...
        }else if(sensor_event->cnt < 1)
        {
            ...
            head_timestamp = timestamp - (sensor_event->cnt - 1) * delta;
            //hwlog_warn("timestamp is %lld delta is %lld\n",timestamp,delta);
            ...
            for(i = 0; i < sensor_event->cnt; i ++) {
                report_sensor_event_batch(head->tag, (int *)((char *)head + OFFSET(pkt_batch_data_req_t, xyz)
                                    + i * sensor_event->len_element), sensor_event->len_element, timestamp);
                //hwlog_debug("[%lld] Kernel get timestamp =%ld , tag=%d\n",getTimestamp(), timestamp, head->tag);
                timestamp += delta;
            }
            timestamp -= delta;
            goto flush_event;
        ...
        if (head->tag == TAG_PHONECALL) {//recieve phonecall event
            ...
            if(sensor_event->xyz[0].y == 1 && ps_value != 0)
            {
                ...
                dmd_log_report(DSM_SHB_ERR_MCU_PS, __func__, "gesture worked\n");
            }
        }
       report_sensor_event_batch(head->tag, (int *)(sensor_event->xyz), sensor_event->len_element, timestamp);
   flush_event:
       sensors_tm[head->tag] = timestamp;
        if(sensor_event->flush_flag == 1 || flush_flag == 1)
        {
            return report_sensor_event_batch(TAG_FLUSH_META, (int *)head, sizeof(pkt_header_t), 0);
        }
    } else if (is_dmd_log_data_report(head)) {
        struct iom7_log_work *work = kmalloc(sizeof(struct iom7_log_work), GFP_ATOMIC);
	 ...
	 memset(work->log_p,0,head->length + sizeof(pkt_header_t));
        ...
        if (((int)motion_data[0]) == MOTIONHUB_TYPE_ROTATION) {
            hwlog_info("%s rotation motion Type = %d, status = %d \n", __func__, motion_data[0],motion_data[2]);
        }
...
int inputhub_route_cmd(unsigned short port, unsigned int cmd, unsigned long arg)
{
    switch (port) {
        ...
        default:
            ...
            dmd_log_report(DSM_SHB_ERR_IOM7_CMD, __func__, "unimplement port\n");
            return -EINVAL;
            break;
    }
    return 0;
