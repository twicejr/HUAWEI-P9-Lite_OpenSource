...
bool chip_detfail_dsm = false;
#endif

struct ts_data g_ts_data;
atomic_t g_data_report_over = ATOMIC_INIT(1);
u8 g_ts_log_cfg = 0;
volatile bool gesture_func = false;
volatile bool g_lcd_control_tp_power = false;
volatile int g_tp_power_ctrl = 0;

static struct ts_cmd_node ping_cmd_buff;
static struct ts_cmd_node pang_cmd_buff;
//tui_tp_work is used for tp to do init asynchronous when tui start to display
static struct work_struct tui_tp_work;
int tp_gpio_num = 0;
...
static void ts_tui_secos_exit(void)
{
	...
	if (g_ts_data.chip_data->report_tui_enable) {
		...
		if (g_ts_data.chip_data->tui_set_flag & 0x1) {
			...
			ts_power_control_notify(TS_BEFORE_SUSPEND, SHORT_SYNC_TIMEOUT);
		}

		if (g_ts_data.chip_data->tui_set_flag & 0x2) {
			TS_LOG_INFO("TUI exit, do suspend\n");
			ts_power_control_notify(TS_SUSPEND_DEVICE, NO_SYNC_TIMEOUT);
		}
...
static int tui_tp_init_async(void)
{
	TS_LOG_DEBUG("before do tui work in tui_tp_work\n");
    INIT_WORK(&tui_tp_work, ts_tui_secos_init);
    schedule_work(&tui_tp_work);

    TS_LOG_DEBUG("After do tui tui_tp_init_async in tui_tp_work\n");
...
static ssize_t ts_register_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < g_ts_data.chip_data->reg_num; i++) {
		ret = snprintf(buf_value, TS_MAX_REG_VALUE_NUM, "0x%02x", g_ts_data.chip_data->reg_values[i]);
		buf_value += ret;
...
static ssize_t ts_register_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for(i = 0; i < num; i++) {
		...
		}else if('a' <= _tolower(value[4*i+2]) && _tolower(value[4*i+2]) <= 'f') {
			value_u8[2*i] = _tolower(value[4*i+2]) - 'a' + 10;
		}
		if('0' <= value[4*i+3] && value[4*i+3] <= '9'){
			value_u8[2*i+1] = value[4*i+3] - '0';
		}else if('a' <= _tolower(value[4*i+3]) && _tolower(value[4*i+3]) <= 'f') {
			value_u8[2*i+1] = _tolower(value[4*i+3]) - 'a' + 10;
		}
	}

	/*convert input value to reg_values*/
	for (i = 0; i < num; i++) {
		info->values[i] = (value_u8[2*i] << 4) | (value_u8[2*i+1]);
	}
...
static ssize_t ts_roi_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (info->roi_switch == value) {
		TS_LOG_INFO("%s, there is no need to send same cmd twice. roi_switch valie is %u", __func__, value);
		error = count;
		goto out;
	}
...
static void ts_special_hardware_test_switch(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	struct ts_device_data *dev = g_ts_data.chip_data;
...
static ssize_t touch_special_hardware_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cmd->cmd_param.prv_params = (void *)info;
	put_one_cmd(cmd, NO_SYNC_TIMEOUT);
...
int ts_power_control_notify(enum ts_pm_type pm_type, int timeout)
{
	...
	if (g_ts_data.chip_data->report_tui_enable && TS_BEFORE_SUSPEND == pm_type) {
		g_ts_data.chip_data->tui_set_flag |= 0x1;
		TS_LOG_INFO("TUI is working, later do before suspend\n");
		return NO_ERR;
	}

	if (g_ts_data.chip_data->report_tui_enable && TS_SUSPEND_DEVICE == pm_type) {
...
void ts_charger_switch(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	int error = -EIO;
	struct ts_device_data *dev = g_ts_data.chip_data;
	struct ts_charger_info *info = (struct ts_charger_info *)in_cmd->cmd_param.prv_params;
...
static int ts_charger_detect_cmd(enum hisi_charger_type charger_type)
{
	...
	TS_LOG_INFO("%s called, charger type: %d, [0 in, other out], supported: %d\n", __func__, charger_type, g_ts_data.feature_info.charger_info.charger_supported);
...
static int charger_detect_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	TS_LOG_INFO("%s, charger type:%ld, [0 in, other out]. charger switch supported: %d\n", __func__, event, g_ts_data.feature_info.charger_info.charger_supported);
...
static irqreturn_t ts_irq_handler(int irq, void *dev_id)
{
	...
	if (put_one_cmd(&cmd, NO_SYNC_TIMEOUT) && (TS_UNINIT != atomic_read(&g_ts_data.state)))
		enable_irq(g_ts_data.irq_id);
...
static inline int ts_calibrate_wakeup_gesture(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	int error = NO_ERR;
	struct ts_device_data *dev = g_ts_data.chip_data;
...
static inline int ts_read_debug_data(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	int error = NO_ERR;
	struct ts_device_data *dev = g_ts_data.chip_data;
...
#if defined (CONFIG_TEE_TUI)
	register_tui_driver(tui_tp_init, "tp", g_ts_data.chip_data->tui_data, 0);
#endif
...
#if defined(HUAWEI_CHARGER_FB)
	g_ts_data.charger_detect_notify.notifier_call = charger_detect_notifier_callback;
	error = hisi_charger_type_notifier_register(&g_ts_data.charger_detect_notify);
	if (error < 0) {
		TS_LOG_ERR("%s, hisi_charger_type_notifier_register failed\n", __func__);
		g_ts_data.charger_detect_notify.notifier_call = NULL;
	} else {
		TS_LOG_INFO("%s, already registe charger_detect_notifier_callback\n", __func__);
...
out:
	...
	if (NULL != g_ts_data.charger_detect_notify.notifier_call) {
		hisi_charger_type_notifier_unregister(&g_ts_data.charger_detect_notify);
