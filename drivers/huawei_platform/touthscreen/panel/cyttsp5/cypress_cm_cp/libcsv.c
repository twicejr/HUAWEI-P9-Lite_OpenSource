...
csv_free(struct csv_parser *p)
{
  ...
  if (p->entry_buf)
    p->free_func((const void *)p->entry_buf);
...
void
csv_set_realloc_func(struct csv_parser *p, void *(*f)(const void *, size_t, gfp_t))
{
...
void
csv_set_free_func(struct csv_parser *p, void (*f)(const void *))
{
...
csv_increase_buffer(struct csv_parser *p)
{
  ...
  while ((vp = p->realloc_func((const void *)p->entry_buf, p->entry_size + to_add, GFP_KERNEL)) == NULL) {
    to_add /= 2;
    if (!to_add) {
      p->status = CSV_ENOMEM;
      return -1;
