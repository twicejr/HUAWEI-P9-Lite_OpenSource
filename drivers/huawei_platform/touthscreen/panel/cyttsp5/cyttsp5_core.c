        ...
        struct device_attribute *attr, char *buf)
{
    ...
    len += snprintf(buf, PAGE_SIZE,"%s_cyttsp5_%s.%d\n", cd->cpdata->product_name,
                    IS_ERR_OR_NULL(panel_name) ? "unknown" : panel_name, cydata->fw_ver_conf);
...
static struct device_attribute attributes[] = {
    ...
    __ATTR(tp_capacitance_test_type, 0664, ts_capacitance_test_type_show, ts_capacitance_test_type_store),
};
