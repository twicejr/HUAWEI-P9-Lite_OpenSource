...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
    ...
    if (!rc) {
        pdata->x_max = value;
        TS_LOG_INFO("%s %d:pdata->x_max is %d\n", __func__, __LINE__, value);
    } else {
        TS_LOG_ERR("%s %d:[cy,x_max] not found,will cause input dev work error,rc = %d\n",
    ...
    if (!rc) {
        pdata->y_max = value;
        TS_LOG_INFO("%s %d:pdata->y_max is %d\n", __func__, __LINE__, value);
    } else {
        TS_LOG_ERR("%s %d:[cy,y_max] not found,will cause input dev work error,rc = %d\n",
                    __func__, __LINE__, rc);
        goto fail_free_pdata;
    }
    ...
    if (!rc) {
        pdata->p_max = value;
        TS_LOG_INFO("%s %d:pdata->p_max is %d\n", __func__, __LINE__, value);
    } else {
        TS_LOG_ERR("%s %d:[cy,p_max] not found,will cause input dev work error,rc = %d\n",
                    __func__, __LINE__, rc);
        goto fail_free_pdata;
    }
    ...
    if (!rc) {
        ...
        TS_LOG_INFO("%s %d:pdata->slots_max is %d\n", __func__, __LINE__, value);
    } else {
        TS_LOG_ERR("%s %d:[cy,slots_max] not found,will cause input dev work error, rc = %d\n",
                    __func__, __LINE__, rc);
        goto fail_free_pdata;
    }
    /* Required fields */
        ...
        struct device_node *core_node)
{
    ...
    rc = of_property_read_u32(core_node, "cy,mmi_test_support", &value);
    if (!rc) {
        pdata->mmi_test_support = value;
    } else {
        pdata->mmi_test_support = 0xFFFF;
    }
    ...
    if (rc) {
        TS_LOG_INFO("get device tp test type  not exit,use default value\n");
    ...
    TS_LOG_INFO("%s: " "product_name:%s chip_name:%s irq_gpio:%d rst_gpio:%d"
        ...
        " easy_wakeup_supported_gestures=%d double_tap_enabled=%d upgrade_ttconfig=%d has_touchplus=%d has_holster=%d mmi_test_support=%d\n",
        __func__,
        ...
        pdata->upgrade_ttconfig,pdata->has_touchplus,pdata->has_holster,pdata->mmi_test_support);
