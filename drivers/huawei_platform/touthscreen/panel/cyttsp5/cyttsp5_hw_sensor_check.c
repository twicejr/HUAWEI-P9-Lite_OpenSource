...
void mmi_info_limit_print(struct cyttsp5_sensor_info_limit *mmi_info)
{
    ...
    TS_LOG_INFO("%s,  selflocalPWC.min data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);
...
static int cyttsp5_get_rawdata(struct device *dev)
{
    ...
    for(i = 0;i < length;i++){
        ...
        || dad->data_buf[i] < m_rawdata.min[j]){
            TS_LOG_ERR( "%s check is failed. dad->data_buf[%d] is %d, rawdata min[%d]:max[%d]\n",__func__, i, dad->data_buf[i], m_rawdata.min[j], m_rawdata.max[j] );
            check_flag = -1;
            break;
        }
...
static int cyttsp5_get_mutual_noise(struct device *dev)
{
    ...
    min_raw = kzalloc(sizeof(int) * length,GFP_KERNEL);
    if(min_raw == NULL){
        TS_LOG_ERR( "%s: Error, kzalloc\n", __func__);
        kfree(max_raw);
        return -1;
    ...
    for (k = 0; k < 5; k++) {
        ...
        if (k ==0 ) {
            ...
            memcpy(min_raw,dad->data_buf,sizeof(int) * length);
        } else {
            for (i = 0; i < length; i++) {
                max_raw[i] = max(dad->data_buf[i],max_raw[i]);
                min_raw[i] = min(dad->data_buf[i],min_raw[i]);
            }
        }
    }
...
static int cyttsp5_get_SelfLocalPWC(struct device *dev)
{
   ...
   if( (dad->data_buf[0] > sPWC.self_gidac_max_rx ) || (dad->data_buf[0] < sPWC.self_gidac_min_rx) ) {
        TS_LOG_ERR( "%s  Self-cap Global iDAC check rx is failed. value is %d, min[%d]-max[%d]\n",__func__, dad->data_buf[0], sPWC.self_gidac_min_rx, sPWC.self_gidac_max_rx);
        rx_flag = -1;
        goto exit;
    }

    if( (dad->data_buf[1] > sPWC.self_gidac_max_tx) || (dad->data_buf[1] < sPWC.self_gidac_min_tx) ) {
        TS_LOG_ERR( "%s  Self-cap Global iDAC check tx is failed. value is %d, min[%d]-max[%d]\n",__func__, dad->data_buf[1], sPWC.self_gidac_min_tx, sPWC.self_gidac_max_tx);
        tx_flag = -1;
        goto exit;
    }
    ...
    for(i = 0;i< length-1 ;i++) {  //差值需要考虑
        ...
        if( abs(opens_diff[i]) > rx_allowed_max ) {
            ...
            if( iCount > rx_allowed_max_num) {
                TS_LOG_ERR( "%s rx allowed max data num check failed, data is %d, allowed num is %d\n",__func__, rx_allowed_max, rx_allowed_max_num);
                goto exit;
            }
        }
    ...
    for(i = 0; i< rx_num * (tx_num -1); i++) {
        ...
        if( abs(opens_diff[i]) > tx_allowed_max ) {
            ...
            if( iCount > tx_allowed_max_num) {
                TS_LOG_ERR( "%s tx allowed max data num check failed, data is %d, allowed num is %d\n",__func__, tx_allowed_max, tx_allowed_max_num);
                goto exit;
            }
        }
...
static int cyttsp5_proc_data_print(struct device *dev,int *data_buf,struct seq_file *m)
{
    ...
    case CYTTSP_SELF_LOCALPWC:
        ...
        seq_printf(m, "Self RX LocalPWC:\n");
        for (index = 2;index < rx_num + 2; index++) {
            seq_printf(m, "%4d", data_buf[index]);
        }

        seq_printf(m, "\nRX Diff:\n");
        for (index = 2;index < rx_num + 1; index++) {
            seq_printf(m, "%4d", data_buf[index] - data_buf[index + 1]);
        }

        seq_printf(m, "\nSelf TX LocalPWC:\n");
        for (index = rx_num + 2;index < rx_num + tx_num + 2; index++) {
            seq_printf(m, "%4d", data_buf[index]);
        }

        seq_printf(m, "\nTX Diff:\n");
        for (index = rx_num + 2;index < rx_num + tx_num + 1; index++) {
            seq_printf(m, "%4d", data_buf[index] - data_buf[index + 1]);
        }
        seq_printf(m, "\n");
        break;
...
static void cyttsp5_sensor_check(struct device *dev,int *buf[CYTTSP_CHECK_MAX],int *check_count)
{
    ...
    //Mutual noise check
    dad->check_index = CYTTSP_MUTUAL_NOISE;
    test_flag = cyttsp5_check_items(dev,CYTTSP_MUTUAL_NOISE,1,buf[CYTTSP_MUTUAL_NOISE]);
    CHECK_RESULT_SET(dev,test_flag,"8");
                    ...
                    struct seq_file *m)
{
    ...
    //Mutual noise check
    dad->check_index = CYTTSP_MUTUAL_NOISE;
    cyttsp5_proc_data_print(dev,data_buf[CYTTSP_MUTUAL_NOISE],m);
