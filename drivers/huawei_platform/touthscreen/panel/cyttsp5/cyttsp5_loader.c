...
static struct cyttsp5_fw_info hw_fw_info[] = {
    ...
    {"Nemo","CS445A", 0,"oflim","ts/Nemo_CS445A_default.bin"},
    {"Nemo","CS445A", 18,"toptouch","ts/Nemo_CS445A_default.bin"},
};
        ...
        u32 fw_ver_new, u32 fw_revctrl_new)
{
    ...
    if( FW_UPDATE_DIFF == cd->cpdata->fw_update_logic ) {
        ...
        TS_LOG_INFO("%s: update logic is update to high ver\n",__func__);
    }
        ...
        void *context)
{
    ...
    if (!fw) {
        /* fw file not exist will not report dmd */
        ld->builtin_bin_fw_status = 0;
        TS_LOG_INFO("%s: No builtin firmware image found, update skiped!\n", __func__);
