    ...
    struct cyttsp5_touch *touch)
{
    ...
    if (md->pdata->flags & CY_MT_FLAG_INV_X) {
        if (flipped)
            touch->abs[CY_TCH_X] = md->pdata->y_max -
                touch->abs[CY_TCH_X];
        else
            touch->abs[CY_TCH_X] = md->pdata->x_max -
                touch->abs[CY_TCH_X];
        touch->abs[CY_TCH_OR] *= -1;
    }
    if (md->pdata->flags & CY_MT_FLAG_INV_Y) {
        if (flipped)
            touch->abs[CY_TCH_Y] = md->pdata->x_max -
                touch->abs[CY_TCH_Y];
        else
            touch->abs[CY_TCH_Y] = md->pdata->y_max -
                touch->abs[CY_TCH_Y];
        touch->abs[CY_TCH_OR] *= -1;
    }

    /* Convert MAJOR/MINOR from mm to resolution */
    tmp = touch->abs[CY_TCH_MAJ] * 100 * md->pdata->x_max;
    touch->abs[CY_TCH_MAJ] = tmp / si->sensing_conf_data.len_x;
    tmp = touch->abs[CY_TCH_MIN] * 100 * md->pdata->x_max;
        ...
        struct cyttsp5_touch *tch, int num_cur_tch)
{
    ...
    int sig;
    int i, j, t = 0;
    DECLARE_BITMAP(ids, md->pdata->slots_max);
    int mt_sync_count = 0;
    u8 *tch_addr;

    bitmap_zero(ids, md->pdata->slots_max);
...
cyttsp5_get_mt_touches_pr_tch:
    ...
    if (md->mt_function.final_sync)
        md->mt_function.final_sync(md->input,
                md->pdata->slots_max, mt_sync_count, ids);
...
static int cyttsp5_check_and_request_si(struct device *dev, struct cyttsp5_mt_data *md)
{
    if(IS_ERR_OR_NULL(dev) || IS_ERR_OR_NULL(md)) {
        TS_LOG_ERR("%s:input ptr err", __func__);
        return -EINVAL;
    }
    ...
    if (!md->si) {
        ...
        if (!md->si) {
            TS_LOG_ERR("%s:md->si request again fail\n", __func__);
            return -EAGAIN;
        }
    }
...
static int cyttsp5_mt_attention(struct device *dev)
{
    ...
    rc = cyttsp5_check_and_request_si(dev, md);
    if(rc) {
        TS_LOG_ERR("%s:check si fail, rc=%d\n",__func__, rc);
        return -EINVAL;
    }
...
static int cyttsp5_setup_input_device(struct device *dev)
{
    ...
    TS_LOG_INFO("%s:set input dev p_max=%d slots_max=%d x_max=%d y_max=%d\n",
                    __func__, max_p, slots_max, max_x_tmp, max_y_tmp);
...
int cyttsp5_mt_probe(struct device *dev)
{
    ...
    if (!md->si)
            TS_LOG_INFO("%s,si is null\n", __func__);

    rc = cyttsp5_setup_input_device(dev);
    if (rc) {
        TS_LOG_ERR("%s %d:cyttsp5_setup_input_device fail, rc = %d\n",
