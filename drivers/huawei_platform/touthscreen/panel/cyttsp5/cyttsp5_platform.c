...
static const char * cyttsp5_panel_name_list[PANLE_ID_MAX + 1] = {
    [OFILM]     = "ofilm",
    [EELY]      = "eely",
    [TRULY]     = "truly",
    [MOTTO]     = "mutto",
    ...
    [AUO]       = "auo",
    [TOPTOUCH]  = "toptouch",
    [PANLE_ID_MAX]  = "unkonwn",
};

int cyttsp5_get_panel_name(int panle_id, char **panel_name)
{
    int ret = 0;
    if (panle_id >= PANLE_ID_MAX || panle_id < 0) {
        TS_LOG_ERR("%s:panle id out of range %d\n", __func__, panle_id);
        *panel_name = cyttsp5_panel_name_list[PANLE_ID_MAX];
        ret = -EINVAL;
    } else {
        TS_LOG_INFO("%s panel_id = %d, panel_name:%s\n", __func__, panle_id, cyttsp5_panel_name_list[panle_id]);
        *panel_name = cyttsp5_panel_name_list[panle_id];
    }
    return ret;
}
