    ...
    struct cyttsp5_touch *touch, u8 *xy_data)
{
    ...
    TS_LOG_DEBUG("%s: x=%04X(%d) y=%04X(%d)\n", __func__,
...
static int cyttsp5_xy_worker(struct cyttsp5_proximity_data *pd)
{
    ...
    TS_LOG_DEBUG("%s: extract data num_cur_rec=%d\n", __func__,
        num_cur_tch);
    if (num_cur_tch)
...
static int _cyttsp5_proximity_enable(struct cyttsp5_proximity_data *pd)
{
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on request enable proximity scantype r=%d\n",
                __func__, rc);
        goto exit_release;
    }
        ...
        bool force)
{
    ...
    if (rc < 0) {
        TS_LOG_ERR("%s: Error on request disable proximity scan r=%d\n",
                __func__, rc);
        goto exit_release;
    }
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (value) {
        if (pd->enable_count++) {
            TS_LOG_DEBUG("%s: '%s' already enabled\n", __func__,
                pd->input->name);
        } else {
        ...
        if (--pd->enable_count) {
            if (pd->enable_count < 0) {
                TS_LOG_ERR("%s: '%s' unbalanced disable\n",
...
static int cyttsp5_setup_input_device_and_sysfs(struct device *dev)
{
    ...
    if (rc) {
        TS_LOG_ERR("%s: Error, could not create enable\n",
                __func__);
        goto exit;
    }
