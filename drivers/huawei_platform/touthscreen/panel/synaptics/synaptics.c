...
static int synaptics_get_debug_data(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd);
...
static int synaptics_vddio_enable(void)
{
	...
	if(rmi4_data->synaptics_chip_data->regulator_ctr.need_set_vddio_value) {
		...
		retval = regulator_set_voltage(rmi4_data->tp_vddio, vddio_value, vddio_value);
		if (retval < 0) {
...
static void synaptics_parse_chip_specific_dts(struct ts_device_data *chip_data)
{
	...
	TS_LOG_INFO("product_id_string = %s\n", rmi4_data->rmi4_mod_info.product_id_string);
	strncat(string_id_buf, rmi4_data->rmi4_mod_info.project_id_string, PROJECT_ID_FW_LEN);
	...
	retval = of_property_read_u32(device, "supported_func_indicater", &read_val);
	if (!retval) {
	...
	if (1 == chip_data->unite_cap_test_interface) {
		...
		if (array_len > 0 && array_len <= RAWDATA_NUM) {
			for(index = 0; index < array_len; index++){
				...
				if (retval) {
					TS_LOG_INFO("read index = %d,raw_data_limit = %s,retval = %d error,\n", index, raw_data_dts, retval);
					break;
				}
	...
	if (!retval) {
		...
		g_ts_data.feature_info.charger_info.charger_switch_bit = (u16)read_val;
	}

	retval = of_property_read_u32(device, "charger_switch_addr", &read_val);
	if (!retval) {
		TS_LOG_INFO("get chip specific charger_switch_addr  = 0x%04x\n", read_val);
		g_ts_data.feature_info.charger_info.charger_switch_addr = (u16)read_val;
	}
#endif

	retval = of_property_read_u32(device, SYNAPTICS_HOSTLER_SWITCH_BIT, &read_val);
	if (!retval) {
		TS_LOG_INFO("get chip specific holster_switch_bit = %d\n", read_val);
		g_ts_data.feature_info.holster_info.holster_switch_bit = (u16)read_val;
	}

	retval = of_property_read_u32(device, SYNAPTICS_HOSTLER_SWITCH_ADDR, &read_val);
	if (!retval) {
		TS_LOG_INFO("get chip specific holster_switch_addr = 0x%04x\n", read_val);
		g_ts_data.feature_info.holster_info.holster_switch_addr = (u16)read_val;
	}

	retval = of_property_read_u32(device, SYNAPTICS_GLOVE_SWITCH_BIT, &read_val);
	if (!retval) {
		TS_LOG_INFO("get chip specific glove_switch_bit = %d\n", read_val);
		g_ts_data.feature_info.glove_info.glove_switch_bit = (u16)read_val;
	}

	retval = of_property_read_u32(device, SYNAPTICS_GLOVE_SWITCH_ADDR, &read_val);
	if (!retval) {
	...
	retval = of_property_read_u32(device, SYNAPTICS_FW_UPDATE_LOGIC, &chip_data->fw_update_logic);
	if (retval) {
...
static int synaptics_parse_dts(struct device_node *device,struct ts_device_data *chip_data)
{
	...
	retval = of_property_read_u32(device, SYNAPTICS_VCI_LDO_VALUE, &chip_data->regulator_ctr.vci_value);
	if (retval) {
		TS_LOG_INFO("Not define Vci value in Dts, use fault value\n");
		chip_data->regulator_ctr.vci_value = 3100000;
	}

	retval = of_property_read_u32(device, SYNAPTICS_NEED_SET_VDDIO_VALUE, &chip_data->regulator_ctr.need_set_vddio_value);
	if (retval) {
		TS_LOG_INFO("Not define need set Vddio value in Dts, use fault value\n");
		chip_data->regulator_ctr.need_set_vddio_value = 0;
	} else {
		retval = of_property_read_u32(device, SYNAPTICS_VDDIO_LDO_VALUE, &chip_data->regulator_ctr.vddio_value);
		if (retval) {
	...
	if (retval) {
		TS_LOG_INFO("get device SYNAPTICS_COVER_FORCE_GLOVE failed,use default!\n");
	...
	retval = of_property_read_u32(device, "bootloader_update_enable", &chip_data->bootloader_update_enable);
	if (retval) {
		chip_data->bootloader_update_enable = 0;
		TS_LOG_INFO("get device bootloader_update_enable fail\n");
	}
	retval = of_property_read_u32(device, "disable_reset_bit", &chip_data->disable_reset);
	if (!retval) {
	...
	TS_LOG_INFO("reset_gpio = %d, irq_gpio = %d, irq_config = %d, algo_id = %d, ic_type = %d, x_max = %d, y_max = %d, x_mt = %d,y_mt = %d, bootloader_update_enable = %d\n", \
		chip_data->reset_gpio, chip_data->irq_gpio, chip_data->irq_config, chip_data->algo_id, chip_data->ic_type,
		chip_data->x_max, chip_data->y_max, chip_data->x_max_mt, chip_data->y_max_mt, chip_data->bootloader_update_enable);
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	synaptics_tui_data.device_name[strlen("synaptics")+1] = '\0';
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (rmi4_data->force_update || need_update) {
			...
			if (retval) {
				...
				strncpy(config_id_string, rmi4_data->rmi4_mod_info.device_config_id, 
				...
				strncpy(config_id_string, rmi4_data->rmi4_mod_info.image_config_id, 
...
static int synaptics_regs_operate(struct ts_regs_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		if((1 == info->num)&&(8 > info->bit)) {	
			retval = synaptics_rmi4_i2c_read(rmi4_data, info->addr, value, info->num);
			if (retval < 0) {
	...
	case TS_ACTION_READ:
		...
		if((1 == info->num)&&(8 > info->bit)){			
			rmi4_data->synaptics_chip_data->reg_values[0] = (value[0] >>  info->bit)&0x01;
		}else{
			for (i = 0; i < info->num; i++) {
...
static int synaptics_set_wakeup_gesture_enable_switch(u8 enable)
{
	...
	holster_enable_addr = g_ts_data.feature_info.holster_info.holster_switch_addr;
	holster_bit_num = g_ts_data.feature_info.holster_info.holster_switch_bit;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, holster_enable_addr, &holster_temp_value, 1);
	if (retval < 0) {
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, holster_enable_addr, &holster_temp_value, 1);
	if (retval < 0) {
...
static int synaptics_set_charger_switch(u8 charger_switch)
{
	...
	charger_enable_addr = g_ts_data.feature_info.charger_info.charger_switch_addr;
	charger_bit_num = g_ts_data.feature_info.charger_info.charger_switch_bit;
	TS_LOG_INFO("synaptics charger_enable_addr=0x%04x, charger_bit_num=%d\n", charger_enable_addr, charger_bit_num);

	if (!charger_enable_addr) {
		TS_LOG_ERR("charger_enable_addr is null, charger feature is not supported.\n");
		goto out;
	}

	retval = synaptics_rmi4_i2c_read(rmi4_data, charger_enable_addr, &charger_temp_value, 1);
	if (retval < 0) {
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, charger_enable_addr, &charger_temp_value, 1);
	if (retval < 0) {
		TS_LOG_ERR("write charger information failed: %d\n", retval);
	}
out:
	TS_LOG_INFO("%s, write TP IC, addr=0x%04x, value=0x%04x\n", __func__, charger_enable_addr, charger_temp_value);
	return retval;
}
#endif
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	holster_enable_addr = g_ts_data.feature_info.holster_info.holster_switch_addr;
	holster_bit_num = g_ts_data.feature_info.holster_info.holster_switch_bit;
	TS_LOG_INFO("synaptics holster_enable_addr=0x%04x, holster_bit_num=%d\n", holster_enable_addr, holster_bit_num);

	if (!holster_enable_addr) {
		TS_LOG_ERR("holster_enable_addr is null, hoslter feature is not supported.\n");
		goto out;
	}
...
static int synaptics_set_roi_switch(u8 roi_switch)
{
	...
	TS_LOG_INFO("roi_ctrl_write_addr=0x%04x, roi_control_bit=%d, roi_switch=%d\n", roi_ctrl_addr, roi_control_bit, roi_switch);

	if (!roi_ctrl_addr) {
		TS_LOG_ERR("roi_ctrl_addr is null, roi feature is not supported.\n");
		goto out;
	}

	retval = synaptics_rmi4_i2c_read(rmi4_data, roi_ctrl_addr, &temp_roi_switch, 1);
	if (retval < 0) {
	...
	if (retval < 0) {
		TS_LOG_ERR("set roi switch failed: %d, roi_ctrl_write_addr=0x%04x\n", retval, roi_ctrl_addr);
		return retval;
	}
...
static int synaptics_do_calibrate(unsigned char write_value, int count)
{
	...
	addr = synaptics_f54_get_calibrate_addr(rmi4_data, synaptics_sett_param_regs->module_name);
	if(!addr) {
...
static int synaptics_get_glove_switch(u8 *glove_switch)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, glove_enable_addr, &value, sizeof(value));
	if (retval < 0) {
...
static int synaptics_set_glove_switch(u8 glove_switch)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, glove_enable_addr, &glove_temp_value, 1);
	if (retval < 0) {
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, glove_enable_addr, &glove_temp_value, 1);
	if (retval < 0) {
...
static int synaptics_chip_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	switch (info->op_action) {
		case TS_ACTION_READ:
			memcpy(info->tp_test_type, rmi4_data->synaptics_chip_data->tp_test_type,TS_CAP_TEST_TYPE_LEN);
...
static int synaptics_set_palm_switch(u8 palm_switch)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		palm_enable_offset = rmi4_data->rmi4_feature.geswakeup_feature.f12_2d_ctrl22_palm;
		palm_enable_addr = rmi4_data->rmi4_feature.f12_ctrl_base_addr + palm_enable_offset;
		TS_LOG_INFO("set palm addr : 0x%02x , value(%d) (1:close) (0:open)\n", palm_enable_addr, palm_switch);
...
static void synaptics_put_device_into_easy_wakeup(void)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		TS_LOG_DEBUG(" read from  f11_ctrl_base_addr92  rmi4_data->f11_ctrl_base_addr  = 0x%04x rmi4_data->f11_ctrl_base_addr+92 =0x%04x\n",rmi4_data->rmi4_feature.f11_ctrl_base_addr,
		...
		if (retval < 0) {
			...
			TS_LOG_DEBUG("read Wakeup Gesture Only reg F12_2D_CTRL20(00)/00 Motion Suppression : CTRL20(00)/00 : 0x%02x  CTRL20(00)/01 : 0x%02x CTRL20(01)/00 : 0x%02x CTRL20(04)/00 : 0x%02x\n", device_ctrl_data[0], device_ctrl_data[1], device_ctrl_data[2], device_ctrl_data[3]);
		}
...
static void synaptics_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	else
	{
		...
		device_ctrl = (device_ctrl | NORMAL_OPERATION  | SENSOR_SLEEP_BIT1);
	}
...
static void synaptics_sleep_mode_out(struct synaptics_rmi4_data *rmi4_data)
{
	...
	{
		device_ctrl = (device_ctrl & ~MASK_2BIT);
		device_ctrl = (device_ctrl | SENSOR_SLEEP_BIT0 | NORMAL_OPERATION);
	}
...
rescan:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		...
		if (done) {
			TS_LOG_DEBUG("Find F01 in page description table 0x%04x\n", rmi4_data->rmi4_feature.f01_cmd_base_addr);
			break;
		}
	}
...
static int synaptics_rmi4_status_resume(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_set_holster_switch(info->holster_info.holster_switch);
	if (retval < 0) {
	...
	if (info->charger_info.charger_supported) {
		retval = synaptics_set_charger_switch(info->charger_info.charger_switch);
		if (retval < 0) {
...
static int synaptics_rmi4_device_status_check(struct synaptics_rmi4_f01_device_status status){
	...
	switch(status.status_code){
		...
		case STATUS_FIRMWARE_CRC_FAILURE:
			TS_LOG_ERR("status value = 0x%02x(STATUS_FIRMWARE_CRC_FAILURE)\n", status.status_code);
			ret = -1;
			break;
		...
		struct synaptics_rmi4_fn *fhandler, struct ts_fingers *info)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		TS_LOG_DEBUG("%s:###read### f11_2d_data39[7] = 0x%02x, f11_2d_data39[6] = 0x%02x\n",__func__,f11_2d_data39[7], f11_2d_data39[6]);
		...
		struct ts_fingers *info)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (retval < 0) {
			...
			TS_LOG_DEBUG("read plam object_type_and_status OK object_type_and_status = 0x%04x\n",palm_status);
		}
