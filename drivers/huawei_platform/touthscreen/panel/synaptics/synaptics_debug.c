...
retry:
	msleep(100);
	result = f54->fn_ptr->read(f54->rmi4_data, f54->command_base_addr, &ready_report, 1);
...
int synaptics_get_cap_data(struct ts_rawdata_info *info)
{
	...
	switch(f54->rmi4_data->synaptics_chip_data->ic_type) {
		...
		case SYNAPTICS_S3718:
			...
			strncat(buf_f54test_result, f54->rmi4_data->rmi4_mod_info.project_id_string,
				strlen(f54->rmi4_data->rmi4_mod_info.project_id_string));
			break;
		default:
...
static int synaptics_read_f34(void)
{
	...
	if (1== f54->rmi4_data->synaptics_chip_data->support_3d_func) {
		...
		f54->rmi4_data->num_of_tx = (f54->rmi4_data->num_of_tx) -FORCE_NUMER;
	}
...
int synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data,const char *module_name)
{
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			if (rmi_fd.fn_number == SYNAPTICS_RMI4_F54) {
				...
				TS_LOG_DEBUG("f54->control_base_addr = 0x%04x, f54->data_base_addr = 0x%04x, f54->query_base_addr = 0x%04x\n", f54->control_base_addr, f54->data_base_addr, f54->query_base_addr);
	...
	TS_LOG_INFO("f54->control_base_addr = 0x%04x, f54->data_base_addr = 0x%04x\n", f54->control_base_addr, f54->data_base_addr);
...
unsigned short synaptics_f54_get_calibrate_addr(struct synaptics_rmi4_data *rmi4_data,const char *module_name)
{
	int rc = NO_ERR;
	u8 value = 0;
	...
	rc = f54->fn_ptr->read(rmi4_data, f54->query_base_addr + 0x0B, &value, sizeof(value));
	if (rc) {
