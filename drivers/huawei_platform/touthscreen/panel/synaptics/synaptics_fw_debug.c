		...
		size_t count, loff_t *f_pos)
{
	...
	retval = rmidev->fn_ptr->read(rmidev->rmi4_data,
			*f_pos,
			...
			count);
	if (retval < 0){
		TS_LOG_ERR("read:read error, %d", retval);
		goto clean_up;
	}
	...
	in_cmd->cmd_param.pub_params.algo_param.algo_order = rmidev->rmi4_data->synaptics_chip_data->algo_id;

	if (!list_empty(&rmidev->rmi4_data->rmi4_mod_info.support_fn_list)) {
		list_for_each_entry(fhandler, &rmidev->rmi4_data->rmi4_mod_info.support_fn_list, link) {
			if (fhandler->num_of_data_sources) {
				if (fhandler->intr_mask & intr_status) {
					rmidev->rmi4_data->report_touch(rmidev->rmi4_data, fhandler, info);
				}
			}
		}
	}

	put_one_cmd(in_cmd, SHORT_SYNC_TIMEOUT);
