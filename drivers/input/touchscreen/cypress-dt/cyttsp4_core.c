...
static void cyttsp4_watchdog_work(struct work_struct *work)
{
	struct cyttsp4_core_data *cd =
		container_of(work, struct cyttsp4_core_data, watchdog_work);
	struct cyttsp4_core_platform_data *pdata = cd->pdata;
	u8 mode[2];
	bool restart = false;
	int rc;
	...
	if (pdata == NULL) {
		dev_err(cd->dev, "%s: NULL pdata pointer\n", __func__);
		return;
	}

	rc = gpio_request(pdata->en_gpio,  "CYPRESS enable");
	if (rc) {
		pr_err("cyttsp4_watchdog_work: GPIO Request CYPRESS enable failed.\n");
		return;
	}
	rc = gpio_direction_output(pdata->en_gpio, 1);
	if(rc < 0){
		pr_err("cyttsp4_watchdog_work: gpio CYPRESS enable operate failed\n");
		return;
	}
	...
	/* request the gpio */
	rc = gpio_request(pdata->en_2v85_gpio,  "CYPRESS 2V85");
	if (rc) {
		pr_err("cyttsp4_watchdog_work: GPIO Request CYPRESS_2V85_EN failed.\n");
		return;
	}
	rc = gpio_direction_output(pdata->en_2v85_gpio, 1);
	if(rc < 0){
		pr_err("cyttsp4_watchdog_work: gpio CYPRESS_2V85_EN operate failed\n");
		return;
	}

	rc = gpio_request(pdata->en_1v80_gpio,  "CYPRESS 1V80");
	if (rc) {
		pr_err("cyttsp4_watchdog_work: GPIO Request CYPRESS_1V80_EN failed.\n");
		return;
	}
	rc = gpio_direction_output(pdata->en_1v80_gpio, 1);
	if(rc < 0)
	{
		pr_err("cyttsp4_watchdog_work: gpio CYPRESS_1V80_EN operate failed\n");
		return;
	}
...
exit:
	...
	gpio_free(pdata->en_2v85_gpio);
	gpio_free(pdata->en_1v80_gpio);
}
#endif
...
reset:
	...
	if (rc < 0) {
		...
		mdelay(100);
#else
        rc = gpio_direction_output(pdata->en_1v80_gpio, 0);
        if(rc < 0)
        {
