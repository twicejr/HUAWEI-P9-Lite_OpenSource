		...
		const char *core_id)
{
	...
	{
		pdata = (struct cyttsp4_loader_platform_data*)info.platform_data;
	}
		...
		struct device_node *core_node)
{
	...
	pdata->en_gpio = of_get_named_gpio(core_node, "cy,gpio_en", 0);
	pdata->en_2v85_gpio = of_get_named_gpio(core_node, "cy,2v85_en", 0);
	pdata->en_1v80_gpio = of_get_named_gpio(core_node, "cy,1v80_en", 0);
