	...
	const struct i2c_device_id *i2c_id)
{
...
#else
	gpio_pin = of_get_named_gpio(dev->of_node, "cy,gpio_en", 0);
	rc = gpio_request(gpio_pin,  "CYPRESS enable");
	if (rc) {
		pr_err("cyttsp4_i2c_probe: GPIO Request CYPRESS enable failed.\n");
		goto add_adapter_err;
	}

	rc = gpio_direction_output(gpio_pin, 1);
	if(rc < 0){
	...
	gpio_pin = of_get_named_gpio(dev->of_node, "cy,2v85_en", 0);
	/* request the gpio */
	rc = gpio_request(gpio_pin,  "CYPRESS 2V85");
	if (rc) {
	...
	gpio_pin = of_get_named_gpio(dev->of_node, "cy,1v80_en", 0);
	rc = gpio_request(gpio_pin,  "CYPRESS 1V80");
	if (rc) {
...
#endif
	/* init irq gpio */
	gpio_pin = of_get_named_gpio(dev->of_node, "cy,irq_gpio", 0);
	rc = gpio_request(gpio_pin, "cyttsp4_irq");
	if (rc < 0) {
		gpio_free(gpio_pin);
		rc = gpio_request(gpio_pin,"cyttsp4_irq");
	}
