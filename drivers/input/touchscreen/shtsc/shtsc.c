...
#define LINUX_VERSION_CODE (KERNEL_VERSION(3,10,0)) //dp qualcomm
...
char VersionMonth = 1;
char VersionDay = 06;
char VersionSerialNumber = 13; // reset on another day
...
#ifdef SHTSC_ICON_KEY_NUM
#error Icon key is not supported by LR388K4
#endif
...
#define SHTSC_COORDS_ARR_SIZE	4
...
static void GetTouchReport(void *ts, u8 u8Num)
{
  ...
  error
	...
	continue;
      ...
      if(((struct shtsc_i2c *)ts)->swap_xy){
	int tmp;
	tmp = touch[i].x;
	touch[i].x = touch[i].y;
	touch[i].y = tmp;
  ...
  input_report_key(input_dev, BTN_TOUCH, touchNum?true:false );
  input_sync(input_dev);
}
#endif /* DEBUG_IRQ_DISABLE */
				     ...
				     const struct i2c_device_id *id)
{
  ...
  ts->touch_num_max = pdata->ts_touch_num_max;
  ...
  input_dev->open = shtsc_input_open;
  input_dev->close = shtsc_input_close;
...
static struct i2c_driver shtsc_i2c_driver = {
  .driver		= {
    ...
    .of_match_table = shtsc_match_table,
  },
