...
static int sdhci_bcm_kona_sd_card_emulate(struct sdhci_host *host, int insert)
{
	...
	if (insert) {
		...
		if (ret >= 0)
			val = (val & ~KONA_SDHOST_WP) | ((ret) ? KONA_SDHOST_WP : 0);
...
static void sdhci_bcm_kona_card_event(struct sdhci_host *host)
{
	if (mmc_gpio_get_cd(host->mmc) > 0) {
		dev_dbg(mmc_dev(host->mmc), "card inserted\n");
		sdhci_bcm_kona_sd_card_emulate(host, 1);
	} else {
		dev_dbg(mmc_dev(host->mmc), "card removed\n");
...
static void sdhci_bcm_kona_init_74_clocks(struct sdhci_host *host, u8 power_mode)
{
	/*
...
static struct sdhci_pltfm_data sdhci_pltfm_data_kona = {
	...
	.quirks = SDHCI_QUIRK_NO_CARD_NO_RESET | SDHCI_QUIRK_BROKEN_TIMEOUT_VAL
			| SDHCI_QUIRK_32BIT_DMA_ADDR | SDHCI_QUIRK_32BIT_DMA_SIZE | SDHCI_QUIRK_32BIT_ADMA_SIZE
				| SDHCI_QUIRK_FORCE_BLK_SZ_2048 | SDHCI_QUIRK_CAP_CLOCK_BASE_BROKEN,
};
...
static int sdhci_bcm_kona_probe(struct platform_device *pdev)
{
	...
	host = sdhci_pltfm_init(pdev, &sdhci_pltfm_data_kona, sizeof(*kona_dev));
	if (IS_ERR(host))
		return PTR_ERR(host);
