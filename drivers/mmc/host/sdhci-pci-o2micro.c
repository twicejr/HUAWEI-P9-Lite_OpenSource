...
void sdhci_pci_o2_fujin2_pci_init(struct sdhci_pci_chip *chip)
{
	...
	ret = pci_read_config_dword(chip->pdev, O2_SD_TUNING_CTRL, &scratch_32);
	if (ret)
		return;
	...
	/* Set UHS2 T_EIDLE */
	ret = pci_read_config_dword(chip->pdev, O2_SD_UHS2_L1_CTRL, &scratch_32);
	if (ret)
		return;
	...
	ret = pci_read_config_dword(chip->pdev, O2_SD_MISC_CTRL4, &scratch_32);
	if (ret)
		return;
	...
	case PCI_DEVICE_ID_O2_FUJIN2:
		...
		if (chip->pdev->device == PCI_DEVICE_ID_O2_FUJIN2) {
			...
			if ((scratch_32 == 0x11) || (scratch_32 == 0x12)) {
				...
				ret = pci_read_config_dword(chip->pdev, O2_SD_FUNC_REG4, &scratch_32);
				...
				pci_write_config_byte(chip->pdev, O2_SD_TUNING_CTRL, 0x44);
		...
		/* Set timeout CLK */
		ret = pci_read_config_dword(chip->pdev, O2_SD_CLK_SETTING, &scratch_32);
		if (ret)
			return ret;

		scratch_32 &= ~(0xFF00);
		scratch_32 |= 0x07E0C800;
		pci_write_config_dword(chip->pdev, O2_SD_CLK_SETTING, scratch_32);

		ret = pci_read_config_dword(chip->pdev, O2_SD_CLKREQ, &scratch_32);
		if (ret)
			return ret;
		scratch_32 |= 0x3;
		pci_write_config_dword(chip->pdev, O2_SD_CLKREQ, scratch_32);

		ret = pci_read_config_dword(chip->pdev, O2_SD_PLL_SETTING, &scratch_32);
		if (ret)
			return ret;
		...
		ret = pci_read_config_dword(chip->pdev, O2_SD_CAP_REG2, &scratch_32);
		if (ret)
			return ret;
		scratch_32 &= ~(0xE0);
