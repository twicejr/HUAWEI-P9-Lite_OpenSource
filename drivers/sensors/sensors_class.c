...
extern char *sensor_binder_input[SENSOR_MAX];
...
static ssize_t sensors_event_path_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    struct sensors_classdev *sensors_cdev = dev_get_drvdata(dev);
    int ret;
    ...
    {
        dev_err(dev, "Invalid sensor class sensors_classdev\n");
        return -EINVAL;
    }
    printk("sensor type =%d.\n",sensors_cdev->type);
    switch(sensors_cdev->type)
    {
...
static ssize_t sensors_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
    struct sensors_classdev *sensors_cdev = dev_get_drvdata(dev);
    int ret;
...
static struct device_attribute sensors_class_attrs[] = {
	...
	__ATTR(set_delay, 0664, sensors_delay_show, sensors_delay_store),
	...
	__ATTR(event_path, 0444, sensors_event_path_show, NULL),
	__ATTR(info, 0444, sensors_info_show, NULL),
	__ATTR_NULL,
};
				...
				struct sensors_classdev *sensors_cdev)
{
	sensors_cdev->dev = device_create(sensors_class, parent, 0,
				      sensors_cdev, "%s", sensors_cdev->path_name);
