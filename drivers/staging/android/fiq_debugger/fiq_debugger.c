...
#ifdef CONFIG_HI6220_FIQ_DEBUGGER
static int fiq_debugger_wdt_int_type(struct fiq_debugger_state *state)
{
	unsigned int mask = 0;

	if (state->pdata->get_signal_irq_type) {
		mask = state->pdata->get_signal_irq_type(state->pdev, state->signal_irq);
	}
	
	return mask;
}

static void fiq_debugger_wdt_int(struct fiq_debugger_state *state, unsigned int is_irq)
{
	if (state->pdata->get_signal_irq_type) {
		state->pdata->set_signal_irq_type(state->pdev, state->signal_irq, is_irq);
	}
...
static void fiq_debugger_help(struct fiq_debugger_state *state)
{
	...
	fiq_debugger_printf(&state->output,
#ifdef CONFIG_HI6220_FIQ_DEBUGGER		
				" wdt_int       current wdt interrupts type\n"
				" wdt_int <c>   set wdt interrupts to i(irq)/f(fiq)\n"
#endif				
				" ps            Process list\n"
				" sysrq         sysrq options\n"
...
static void fiq_debugger_switch_cpu(struct fiq_debugger_state *state, int cpu)
{
	...
	if (!fiq_debugger_have_fiq(state)) {
		...
		state->pdata->fiq_switch_cpu(state->pdev, state->fiq, cpu);
	}
			...
			void *svc_sp)
{
	...
	if (!strcmp(cmd, "help") || !strcmp(cmd, "?")) {
		...
		fiq_debugger_printf(&state->output, "cpu %d\n", state->current_cpu);
#ifdef CONFIG_HI6220_FIQ_DEBUGGER
	} else if (!strcmp(cmd, "wdt_int")) {
		unsigned int irq;
		irq = fiq_debugger_wdt_int_type(state);
		fiq_debugger_printf(&state->output, "wdt_int %s\n", irq?"irq":"fiq");
	} else if (!strncmp(cmd, "wdt_int ", 8)) {
		if (cmd[8] == 'f') {
			fiq_debugger_wdt_int(state,0);
			fiq_debugger_printf(&state->output, "wdt_int fiq\n");
		} else if (cmd[8] == 'i') {
			fiq_debugger_wdt_int(state,1);
			fiq_debugger_printf(&state->output, "wdt_int irq\n");
		} else {
			fiq_debugger_printf(&state->output, "invalid args\n");
		}
#endif	
	} else {
		if (state->debug_busy) {
			...
			int this_cpu, const struct pt_regs *regs, void *svc_sp)
{
	...
	while ((c = fiq_debugger_getc(state)) != FIQ_DEBUGGER_NO_CHAR) {
		...
		} else if ((c == FIQ_DEBUGGER_BREAK) || (esc_count==3)) {
			esc_count = 0;
		...
		const struct pt_regs *regs, void *svc_sp)
{
	...
	state->pdata->fiq_handle(state->pdev, regs, svc_sp);
...
static irqreturn_t fiq_debugger_uart_irq_null(int irq, void *dev)
{
...
static int __init fiq_debugger_console_setup(struct console *co, char *options)
{
	struct fiq_debugger_state *state;
	int baud;
	...
	if (options) {
		...
		if (state->pdata->console_setup)
			state->pdata->console_setup(state->pdev, baud);
	}
...
static struct console fiq_debugger_console = {
	...
	.setup = fiq_debugger_console_setup,
...
static int fiq_debugger_dev_suspend(struct device *dev)
{
	...
	if (state->pdata->fiq_enable) 
		state->pdata->fiq_enable(pdev, state->fiq, 0);
...
static int fiq_debugger_dev_resume(struct device *dev)
{
	...
	if (state->pdata->uart_dev_resume) 
		ret = state->pdata->uart_dev_resume(pdev);

	if (state->pdata->fiq_enable) 
		state->pdata->fiq_enable(pdev, state->fiq, 1);
	
	return ret;
}

static int fiq_idle_notifier(struct notifier_block *self, unsigned long cmd,	void *v)
{
	switch (cmd) {
		case CPU_PM_ENTER:
			local_fiq_disable();
...
static struct notifier_block fiq_idle_block = {
	.notifier_call = fiq_idle_notifier,
};

#define HI6220_FIQ_COMP    "hisilicon,hi6220-fiq"

#ifdef CONFIG_OF
static const struct of_device_id fiq_debugger_match[] = {
    {	  .compatible = HI6220_FIQ_COMP,
        .data = NULL,},
    {},
};
MODULE_DEVICE_TABLE(of, fiq_debugger_match);
#endif
...
static int fiq_debugger_probe(struct platform_device *pdev)
{
	...
	if ((match = of_match_node(fiq_debugger_match, pdev->dev.of_node)) == NULL) {
		pr_err("%s:dev node is not match. exiting.\n", __func__);
		return -ENODEV;
	}

	if (!strcmp(match->compatible, HI6220_FIQ_COMP)) {
		pr_err("%s:hi6220_fiq_comp.\n", __func__);
		hisi_serial_debug_init(pdev);
		pdata = dev_get_platdata(&pdev->dev);
		pdev->id = 0;
	} else {
		pr_err("%s:match not support. exiting.\n", __func__);
		return -ENODEV;
	}
	...
	state->current_cpu = FIQ_DEFAULT_CPU;
...
#ifdef CONFIG_FIQ_GLUE
	if (fiq_debugger_have_fiq(state) && 
		check_himntn(HIMNTN_FIQ_DEBUGGER)) {
		ret = request_irq(state->fiq, fiq_debugger_uart_irq_null,
				  IRQF_NO_SUSPEND, "debugnull", state);
		if (ret) {
	...
	if (check_himntn(HIMNTN_FIQ_DEBUGGER))  {
		fiq_debugger_printf_nfiq(state,
	...
	cpu_pm_register_notifier(&fiq_idle_block);
	
#if defined(CONFIG_FIQ_DEBUGGER_CONSOLE)
	spin_lock_init(&state->console_lock);
	state->console = fiq_debugger_console;
	state->console.index = pdev->id;
	if (get_uart0_config() && check_himntn(HIMNTN_FIQ_DEBUGGER)) {
