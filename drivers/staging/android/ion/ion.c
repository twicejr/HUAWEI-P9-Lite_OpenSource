			     ...
			     unsigned int flags)
{
	...
	if (IS_ERR(buffer)) {
		pr_err("%s: ion buffer create error! buffer: %p\n", __func__, buffer);
...
int ion_change_flags(struct ion_client *client, struct ion_handle *handle, int flags)
{
	struct ion_buffer *buffer;
	...
	case ION_IOC_ALLOC:
	{
		...
		if (IS_ERR(handle)) {
			pr_err("%s: ion alloc failed!"
				" len: %lx, align: %lx, heap_id_mask: %x, flags: %x\n",
...
{
	return single_open(file, ion_debug_memtrack_show,
