...
static gfp_t high_order_gfp_flags = (GFP_USER | __GFP_NOWARN | __GFP_COMP |
				     __GFP_NO_KSWAPD | __GFP_NORETRY) & ~__GFP_WAIT;
static gfp_t low_order_gfp_flags  = (GFP_USER | __GFP_NOWARN | __GFP_COMP);
...
struct ion_heap *ion_system_heap_create(struct ion_platform_heap *unused)
{
	...
	if(hisi_iommu_page_size() == SZ_8K) {
		printk(KERN_INFO"cur size %x orders %x\n",hisi_iommu_page_size(),num_orders);
