...
unsigned int g_ipa_freq_limit[IPA_ACTOR_MAX] ={IPA_FREQ_MAX,IPA_FREQ_MAX,IPA_FREQ_MAX};
...
unsigned int ipa_freq_limit(enum ipa_actor actor,unsigned int target_freq) {
	if(actor>=IPA_ACTOR_MAX) return target_freq;

	if(g_ipa_freq_limit_debug)
	{
		pr_err("actor[%d]target_freq[%u]IPA:[%u][%u][%u]min[%u]\n",
		actor,target_freq,
		g_ipa_freq_limit[IPA_CLUSTER0],g_ipa_freq_limit[IPA_CLUSTER1],g_ipa_freq_limit[IPA_GPU],
...
void thermal_unregister_governor(struct thermal_governor *governor)
{
	...
	list_for_each_entry(pos, &thermal_tz_list, node) {
		if (!strnicmp(pos->governor->name, governor->name,
						THERMAL_NAME_LENGTH)) {
			ret = thermal_set_governor(pos, NULL);
			if (ret)
				dev_err(&pos->device, "%s thermal_set_governor fail!\n", __func__);
		}
	}
	   ...
	   const char *buf, size_t count)
{
	...
	trace_IPA_boost(tz,tz->tzp->boost,tz->tzp->boost_timeout);
...
#define BOOST_TIMEOUT_THRES	500
static ssize_t
...
static int create_trip_attrs(struct thermal_zone_device *tz, int mask)
{
	...
	tz->trip_type_attrs = devm_kzalloc(&tz->device, size, GFP_KERNEL);
	if (!tz->trip_type_attrs)
		return -ENOMEM;

	tz->trip_temp_attrs = devm_kzalloc(&tz->device, size, GFP_KERNEL);
	if (!tz->trip_temp_attrs) {
		devm_kfree(&tz->device, tz->trip_type_attrs);
		return -ENOMEM;
	}

	if (tz->ops->get_trip_hyst) {
		tz->trip_hyst_attrs = devm_kzalloc(&tz->device, size, GFP_KERNEL);
		if (!tz->trip_hyst_attrs) {
			devm_kfree(&tz->device, tz->trip_type_attrs);
			devm_kfree(&tz->device, tz->trip_temp_attrs);
			return -ENOMEM;
		}
	}
	...
	for (indx = 0; indx < tz->trips; indx++) {
...
#endif
		err = device_create_file(&tz->device,
				   &tz->trip_type_attrs[indx].attr);
		if (err) {
			pr_err("[%s]trip_type_attrs [%d]err[%d]\n",__func__,indx,err);
			return err;
		}
		...
		if (err) {
			pr_err("[%s]trip_temp_attrs [%d]err[%d]\n",__func__,indx,err);
			return err;
		}
		...
		if (err) {
			pr_err("[%s]trip_hyst_attrs [%d]err[%d]\n",__func__,indx,err);
			return err;
		}
...
static void remove_trip_attrs(struct thermal_zone_device *tz)
{
	...
	devm_kfree(&tz->device, tz->trip_type_attrs);
	devm_kfree(&tz->device, tz->trip_temp_attrs);
	devm_kfree(&tz->device, tz->trip_hyst_attrs);
}
