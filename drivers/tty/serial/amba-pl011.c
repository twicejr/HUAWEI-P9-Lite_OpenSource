...
static unsigned int get_fifosize_arm(unsigned int periphid)
{
	...
	return rev < 3 ? 16 : 32;/*[false alarm]*/
}
...
static void pl011_dma_rx_irq(struct uart_amba_port *uap)
{
	...
	if (dmastat != DMA_PAUSED)
		dev_err(uap->port.dev, "unable to pause DMA transfer dmastat[%d] dmarx->cookie[%d] completed_cookie[%d] cookie[%d]\n",
        dmastat, dmarx->cookie, rxchan->completed_cookie, rxchan->cookie);
	...
	if (status) {
		do {
			...
			if (status & UART011_TXIS)
				pl011_tx_chars(uap);
			/*to Solve the deadlock for uart0*/
			/*
...
static unsigned int pl011_tx_empty(struct uart_port *port)
{
	...
	return status & (UART01x_FR_BUSY|UART01x_FR_TXFF) ? 0 : TIOCSER_TEMT;/*[false alarm]*/
}
