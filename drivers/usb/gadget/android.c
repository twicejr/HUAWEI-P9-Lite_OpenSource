					...
					const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE) << 8) | 0xa2:
		...
		cdev->req->complete = acm_complete_nop;
		break;
	default:
		break;
	}
...
#define MAX_BALONG_ACM_INSTANCES 10
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= sizeof(config->manufacturer))
		return -EINVAL;
	if (sscanf(buf, "%s", config->manufacturer) == 1)     /* [false alarm]:this is original code */
		return size;
	return -1;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",                  /* [false alarm]:this is original code */
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= sizeof(config->common->inquiry_string))
		return -EINVAL;
	if (sscanf(buf, "%s", config->common->inquiry_string) != 1) /* [false alarm]:this is original code */
		return -EINVAL;
	return size;
}
...
DESCRIPTOR_STRING_ATTR(iManufacturer, manufacturer_string) /* [false alarm]:this is original code */
DESCRIPTOR_STRING_ATTR(iProduct, product_string) /* [false alarm]:this is original code */
DESCRIPTOR_STRING_ATTR(iSerial, serial_string) /* [false alarm]:this is original code */
...
#ifdef CONFIG_USB_F_BALONG_ACM
	device_desc.bcdDevice = cpu_to_le16(0x0200 | 0x0099);
#endif
