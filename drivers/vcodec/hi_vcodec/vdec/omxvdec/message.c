...
msg_queue_s* message_queue_init(HI_S32 chan_id, HI_U32 max_msg_num)
{
    ...
    memset(&mem_buf, 0, sizeof(MEM_BUFFER_S));
    mem_buf.u32Size = sizeof(msg_queue_s) + sizeof(msg_data_s)*max_msg_num;
    snprintf(buf_name, sizeof(buf_name), "Chan%d_MSG", chan_id);
    ret = VDEC_MEM_KAlloc(buf_name, OMXVDEC_ZONE, &mem_buf, 1);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_FATAL, "%s malloc for queue failed.\n", __func__);
        return HI_NULL;
    }
...
HI_VOID message_queue_deinit(msg_queue_s *queue)
{
    ...
    memset(&mem_buf, 0, sizeof(MEM_BUFFER_S));
    mem_buf.pStartVirAddr = (HI_VOID *)queue;
    mem_buf.u32Size       = sizeof(msg_queue_s) + sizeof(msg_data_s)*queue->msg_num;
    VDEC_MEM_KFree(&mem_buf);
