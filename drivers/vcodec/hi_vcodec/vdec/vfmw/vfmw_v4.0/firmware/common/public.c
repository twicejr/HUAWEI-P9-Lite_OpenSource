...
hU2r3z6w3YJVkA73kZvaQukJ0aPf6/qtwoVI/HlZ4FYrVQsS8VzX2/+Ri9aJQ4FGAWQcA4w+
D1DKx3dSaY55dyClbxbSCO2L5Gx7HRQG9sAbN8JZGSr6cN0yvkFGL71JMgk/ww==*/
/*--------------------------------------------------------------------------------------------------------------------------*/
...
MEMPRINT      *g_pMemPrint=0;                        /* 内存打印设备描述 */
...
SINT32 DBG_CreateTracer(VOID)
{
    ...
    if ( MEM_MAN_OK == MEM_AllocMemBlock("TRACER", TRACE_MEM_SIZE, &MemRecord, 0) )
    {
...
VOID DBG_AddTrace( SINT8 *pFuncName, SINT32 nLine, SINT32 Data )
{
    if( NULL != s_pstTracer )
    {
		...
		pstTraceObj->u32Time = VFMW_OSAL_GetTimeInUs();
...
#################################################################################################*/
inline SINT32 IsDprintTypeEnable(UINT32 type)
{
...
inline VOID vfmw_dprint_nothing(VOID)
{
    return;
}
       ...
       /*这里初始化的loguy 和 higuy两个指针，是在循环中用于移动来指示需要交换的两个元素的。higuy递减，loguy递增，所以下面的for循环总是可以终止。*/
        ...
        for (;;)
        {
           ...
           /*开始移动loguy指针，直到A[loguy]>A[mid]*/
            ...
            /*如果移动到loguy>=mid的时候，就继续向后移动，使得A[loguy]>a[mid]。这一步实际上作用就是使得移动完loguy之后，loguy指针之前的元素都是不大于划分值的元素。*/
            if (mid <= loguy)
            {
                do
                {
