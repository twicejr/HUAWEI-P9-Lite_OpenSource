...
#define  DEFAULT_PRINT_ENABLE   (0x0)          // 默认 'PRN_ERROR' & 'PRN_FATAL' 打开，其余关闭
...
#define REPORT_NEED_ARRANGE_L(chan_id, FrmNum, FrmSize, PmvSize, Width, Height, Stride)                     	\
do{ 														\
	if( NULL != g_event_report )							\
	{														\
        UINT32  para[6];                                    \
        ...
        para[3] = (UINT32)(Width);                          \
        para[4] = (UINT32)(Height);                         \
        para[5] = (UINT32)(Stride);                         \
        g_event_report(chan_id, EVNT_NEED_ARRANGE_L, (UINT8*)para, sizeof(para));  \
	}														\
}while(0)
