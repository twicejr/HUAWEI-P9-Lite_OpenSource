...
#if !defined (CFG_HEVC_MAX_TEMPLAYER)
#define HEVC_MAX_TEMPLAYER             (8)             //max number of temporal layer
#else
#define HEVC_MAX_TEMPLAYER             (CFG_HEVC_MAX_TEMPLAYER)
#endif
...
#if HM11_SYNTAX
#if !defined (CFG_HEVC_MAX_VPS_OP_SETS_PLUS1)
#define HEVC_MAX_VPS_OP_SETS_PLUS1     (1024)
#else
#define HEVC_MAX_VPS_OP_SETS_PLUS1     (CFG_HEVC_MAX_VPS_OP_SETS_PLUS1)
#endif
#define HEVC_MAX_VPS_NUH_RESERVED_ZERO_LAYER_ID_PLUS1  (1)
#endif

#if !defined (CFG_HEVC_MAX_STRPS_NUM)
#define HEVC_MAX_STRPS_NUM             (64)
#else
#define HEVC_MAX_STRPS_NUM             (CFG_HEVC_MAX_STRPS_NUM)
#endif
//macros
...
typedef struct
{
    ...
    UINT8    scaling_list_pred_matrix_id_delta[SCALING_LIST_SIZE_NUM][HEVC_SCALING_LIST_NUM];
    UINT8    refMatrixId[SCALING_LIST_SIZE_NUM][HEVC_SCALING_LIST_NUM]; //!< RefMatrixID
    UINT8    predMatrixId[SCALING_LIST_SIZE_NUM][HEVC_SCALING_LIST_NUM]; //!< reference list index
...
typedef struct
{
    ...
    UINT8   refIdc[HEVC_MAX_NUM_REF_PICS + 1];
...
typedef struct
{
    ...
    UINT32 nesting_max_temporal_id_plus1[HEVC_MAX_NESTING_NUM_LAYER];
...
typedef struct
{
    ...
    //record pic_parameter_set_id for tmp
    SINT32                     Slice[HEVC_MAX_SLICE_NUM];
