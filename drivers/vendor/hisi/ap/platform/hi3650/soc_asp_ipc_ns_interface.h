 ...
 ******************************************************************************
  文 件 名   : soc_asp_ipc_ns_interface.h
  版 本 号   : 初稿
  作    者   : Excel2Code
  生成日期   : 2015-03-10 11:18:14
  最近修改   :
  功能描述   : 接口头文件
  函数列表   :
  修改历史   :
  1.日    期   : 2015年3月10日
    作    者   : fWX220878
    修改内容   : 从《Hi3650V100 SOC寄存器手册_ASP_IPC_NS.xml》自动生成
 ...
 ****************************************************************************/
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_SOURCE_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  source   : 4;  /* bit[0-3] : 邮箱通道占用源处理器配置，可配置多个处理器共同占用此邮箱通道，每一bit对应一个占用通道的处理器；
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_DSET_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  dset     : 4;  /* bit[0-3] : 邮箱通道目的处理器配置寄存器，即通信中断的接收处理器设置，可配置多个处理器同时接收中断，每一bit对应一个处理器。
                                                   0：无影响；
                                                   1：设置为目的处理器。 */
        unsigned int  reserved : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_DCLEAR_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  dclear   : 4;  /* bit[0-3] : 邮箱通道目的处理器清除寄存器，每一bit对应一个处理器。
                                                   0：无影响；
                                                   1：清除。 */
        unsigned int  reserved : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_DSTATUS_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  dstatus  : 4;  /* bit[0-3] : 邮箱通道目的处理器状态寄存器，每一bit对应一个处理器。 */
        unsigned int  reserved : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_MODE_UNION
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_IMASK_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  int_mask : 4;  /* bit[0-3] : 邮箱通道到处理器的中断屏蔽设置，每一bit对应一个处理器。
                                                   0：不屏蔽；
                                                   1：屏蔽。 */
        unsigned int  reserved : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_ICLR_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  int_clear : 4;  /* bit[0-3] : 邮箱通道到处理器屏蔽后中断状态，每一bit对应一个处理器，写1清除。
                                                    0：无中断；
                                                    1：有中断。 */
        unsigned int  reserved  : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_SEND_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  send     : 4;  /* bit[0-3] : 中断发送寄存器，每一bit对应一个处理器。通信过程中，配置此寄存器，只置位bit有效。
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_DATA0_UNION
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_MBX_DATA1_UNION
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_CPU_IMST_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  int_mask_status : 4;  /* bit[0-3] : 处理器j屏蔽后的中断状态寄存器，每个bit对应一个邮箱通道。
                                                          0：无中断；
                                                          1：有中断。 */
        unsigned int  reserved        : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_CPU_IRST_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  int_raw_status : 4;  /* bit[0-3] : 处理器j原始中断状态寄存器，每个bit对应一个邮箱通道。
                                                         0：无中断；
                                                         1：有中断。 */
        unsigned int  reserved       : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_IPC_MBX_ACTIVE_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  mbx_active_status : 4;  /* bit[0-3] : 邮箱通道有效状态，每个bit对应一个邮箱通道。
                                                            0：空闲；
                                                            1：有效。 */
        unsigned int  reserved          : 28; /* bit[4-31]: reserved */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_ASP_IPC_NS_IPC_LOCK_UNION
