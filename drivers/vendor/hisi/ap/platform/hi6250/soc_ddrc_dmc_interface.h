...
/* 寄存器说明：DDRC_CFG_SFC_WDATA是SFC写数据配置寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_CFG_SFC_WDATA_UNION */
#define SOC_DDRC_DMC_DDRC_CFG_SFC_WDATA_ADDR(base)    ((base) + (0x21C))

/* 寄存器说明：DDRC_CFG_SFC_WDATA是SFC写数据配置寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_CFG_SFC_WCTRL_UNION */
#define SOC_DDRC_DMC_DDRC_CFG_SFC_WCTRL_ADDR(base)    ((base) + (0x220))

/* 寄存器说明：DDRC_CFG_SFCMASK0是DDRC软件配置DDR命令MPC MASK寄存器。
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_CFG_SFC_MASK0_UNION */
#define SOC_DDRC_DMC_DDRC_CFG_SFC_MASK0_ADDR(base)    ((base) + (0x224))

/* 寄存器说明：DDRC_CFG_SFCMASK1是DDRC软件配置DDR命令MPC MASK寄存器。
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_CFG_SFC_MASK1_UNION */
#define SOC_DDRC_DMC_DDRC_CFG_SFC_MASK1_ADDR(base)    ((base) + (0x228))
...
/* 寄存器说明：DDRC_CFG_ECC_CTRL是DDRC单/多比特第一次和最后一次结果读取控制寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_UNION */
#define SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_ADDR(base)     ((base) + (0x2A8))

/* 寄存器说明：DDRC_HIS_SERR_ID是DDRC EC单比特错误ID寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_HIS_SERR_ID_UNION */
#define SOC_DDRC_DMC_DDRC_HIS_SERR_ID_ADDR(base)      ((base) + (0x2AC))
...
/* 寄存器说明：DDRC_HIS_SFC_RDATA是SFC读数据寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA_UNION */
#define SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA_ADDR(base)    ((base) + (0x4A8))

/* 寄存器说明：DDRC_SFC_RCTRL是SFC读数据指针寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_SFC_RCTRL_UNION */
#define SOC_DDRC_DMC_DDRC_SFC_RCTRL_ADDR(base)        ((base) + (0x4AC))

/* 寄存器说明：DDRC_HIS_SFC_RDATA0_DBI是读数据对应的DBI寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA0_DBI_UNION */
#define SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA0_DBI_ADDR(base) ((base) + (0x4C8))

/* 寄存器说明：DDRC_HIS_SFC_RDATA1_DBI是读数据对应的DBI寄存器
   位域定义UNION结构:  SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA1_DBI_UNION */
#define SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA1_DBI_ADDR(base) ((base) + (0x4CC))
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  wsram_err_inj  : 2;  /* bit[2-3]  : WSRAM ECC注入模式
                                                          00：正常模式；
                                                          01：1bit错误注入；
                                                          10：1bit错误注入；
                                                          11：2bit错误注入。
...
#define SOC_DDRC_DMC_DDRC_CFG_FIFO_ERRINJ_wsram_err_inj_START   (2)
#define SOC_DDRC_DMC_DDRC_CFG_FIFO_ERRINJ_wsram_err_inj_END     (3)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  rank         : 2;  /* bit[20-21]: 控制器Rank配置。
                                                        ...
                                                        10：3个rank；
                                                        11：4个rank。
                                                        注意：当外接DDR器件大于4个RANK时，此控制位同时用于0-3,4-7,8-11,12-16四组RANK的控制,即每个rank group的配置相同（不支持不同rank group不同配置）。当软件配置的rank数小于设计最大支持的rank个数时，可以通过rank_disable来禁止不使用的rank.比如每个rank group为4个rank,但需要配置出每个rank group中仅为一个rank有效。可以可配置该寄存器为rank==1,同时配置rank_disable=16'hEEEE.由于这种配置会存在空洞，需要Master保证访问到有效空间。 */
        ...
        unsigned int  asref_zqc_en : 1;  /* bit[23]   : asref退出时的SDRAM ZQ使能。
                                                        1：使能；
                                                        0：禁止。 */
        unsigned int  bank_xor     : 3;  /* bit[24-26]: BANK地址与ROW地址加扰使能。
                                                        0：禁止；
                                                        1：BANK与偏移地址0加扰；
                                                        2：BANK与偏移地址1加扰；
                                                        3：BANK与偏移地址2加扰；
        ...
        unsigned int  bank_mode    : 4;  /* bit[28-31]: BANK交织模式。
                                                        存储总线位宽为8bit时：
                                                        0000:8byte交织；
                                                        0001:16byte交织；
                                                        …
                                                        ...
                                                        1000:16Kbyte交织；
                                                        1001:32Kbyte交织。
                                                        注意：当AXI的配置为非零时，这个域必须配置为0000,以AXI配置模式工作。当AXI配置为0000时，以DMC配置模式工作。另外，需要注意，这里的交织粒度不能大于器件的Page容量。即由RRCBC移成RRBC后，Bank的位置就不能再向左移动了。 */
    } reg;
...
#define SOC_DDRC_DMC_DDRC_CFG_DDRMODE_bank_xor_END        (26)
#define SOC_DDRC_DMC_DDRC_CFG_DDRMODE_bank_mode_START     (28)
#define SOC_DDRC_DMC_DDRC_CFG_DDRMODE_bank_mode_END       (31)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  rodt : 16; /* bit[16-31]: 当前RANK发送读命令时，其他RANK的ODT配置。
                                                1：使能读ODT；
                                                0：禁止读ODT。
                                                注意：rodt不能与asref功能并存。即如果打开了rodt功能，则要将asref功能关闭。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_CFG_SFC_WDATA_UNION
 结构说明  : DDRC_CFG_SFC_WDATA 寄存器结构定义。地址偏移量:0x21C，初值:0x00000000，宽度:32
 寄存器说明: DDRC_CFG_SFC_WDATA是SFC写数据配置寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  wdata : 32; /* bit[0-31]: 该寄存器为sfc的写数据。其中一个ddr burst的数据都通过该寄存器写入，每写入32bit数据，sfc_wdata_ptr会自动加1。另外注意，其中ECC数据统一放在正常数据之后。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_CFG_SFC_WCTRL_UNION
 结构说明  : DDRC_CFG_SFC_WCTRL 寄存器结构定义。地址偏移量:0x220，初值:0x00000000，宽度:32
 寄存器说明: DDRC_CFG_SFC_WDATA是SFC写数据配置寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  sfc_wdata_ptr : 32; /* bit[0-31]: sfc写入指针控制寄存器。当每写一次DDRC_CFG_SFC_WDATA时，该寄存器会自动加1.在全部写入后，需要通过写该寄存器来将指针归位。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_CFG_SFC_MASK0_UNION
 结构说明  : DDRC_CFG_SFC_MASK0 寄存器结构定义。地址偏移量:0x224，初值:0x00000000，宽度:32
 寄存器说明: DDRC_CFG_SFCMASK0是DDRC软件配置DDR命令MPC MASK寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  sfc_mask0 : 32; /* bit[0-31]: 针对写命令每个byte数据所对应的mask配置。
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_CFG_SFC_MASK1_UNION
 结构说明  : DDRC_CFG_SFC_MASK1 寄存器结构定义。地址偏移量:0x228，初值:0x00000000，宽度:32
 寄存器说明: DDRC_CFG_SFCMASK1是DDRC软件配置DDR命令MPC MASK寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  sfc_mask1 : 32; /* bit[0-31]: 当一个burst为64byte时，该寄存器用于指示后32个byte是否需要mask. */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : DDRC_INTMSK 寄存器结构定义。地址偏移量:0x280，初值:0xF11F77FF，宽度:32
 寄存器说明: DDRC_INTMSK是DDRC中断屏蔽寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  sbram_serr_int_mask   : 1;  /* bit[20]   : SBRAM单比特错误中断屏蔽使能。
                                                                 1：屏蔽中断；
                                                                 0：使能中断。 */
        unsigned int  wsram_serr_int_mask   : 1;  /* bit[21]   : WSRAM单比特错误中断屏蔽使能。
                                                                 1：屏蔽中断；
                                                                 0：使能中断。 */
        unsigned int  wsram_merr_int_mask   : 1;  /* bit[22]   : WSRAM多比特错误中断屏蔽使能。
                                                                 1：屏蔽中断；
                                                                 0：使能中断。 */
...
#define SOC_DDRC_DMC_DDRC_INTMSK_wsram_serr_int_mask_START    (21)
#define SOC_DDRC_DMC_DDRC_INTMSK_wsram_serr_int_mask_END      (21)
#define SOC_DDRC_DMC_DDRC_INTMSK_wsram_merr_int_mask_START    (22)
#define SOC_DDRC_DMC_DDRC_INTMSK_wsram_merr_int_mask_END      (22)
...
typedef union
{
    ...
    struct
    {
        unsigned int  serr_rint         : 1;  /* bit[0]    : DDR ECC单比错误特原始中断。写1中断清除
                                                             注意：该寄存器不带复位端，如果要使用该中断，上电后需通过写1来清0。 */
        ...
        unsigned int  dimm_parity_rint  : 1;  /* bit[8]    : DDR DIMM奇偶校验错误原始中断。写1中断清除。
                                                             注意：该寄存器不带复位端，如果要使用该中断，上电后需通过写1来清0。在使能CRC功能不使能CA Parity功能将不能产生这个中断，此时可以查询rec_int。 */
        ...
        unsigned int  sbram_serr_rint   : 1;  /* bit[20]   : SBRAM单比特错误中断。写1中断清除。
                                                             注意：该寄存器不带复位端，如果要使用该中断，上电后需通过写1来清0。 */
        unsigned int  wsram_serr_rint   : 1;  /* bit[21]   : WSRAM单比特错误中断。写1中断清除。
                                                             注意：该寄存器不带复位端，如果要使用该中断，上电后需通过写1来清0。 */
        unsigned int  wsram_merr_rint   : 1;  /* bit[22]   : WSRAM多比特错误中断。写1中断清除。
                                                             注意：该寄存器不带复位端，如果要使用该中断，上电后需通过写1来清0。 */
        unsigned int  reserved_2        : 1;  /* bit[23]   : 保留。 */
...
#define SOC_DDRC_DMC_DDRC_RINT_wsram_serr_rint_START    (21)
#define SOC_DDRC_DMC_DDRC_RINT_wsram_serr_rint_END      (21)
#define SOC_DDRC_DMC_DDRC_RINT_wsram_merr_rint_START    (22)
#define SOC_DDRC_DMC_DDRC_RINT_wsram_merr_rint_END      (22)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  dimm_parity_intsts  : 1;  /* bit[8]    : DDR DIMM奇偶校验错误中断。
                                                               注意：在使能CRC功能不使能CA Parity功能将不能产生这个中断，此时可以查询rec_int */
        ...
        unsigned int  wsram_serr_intsts   : 1;  /* bit[21]   : WSRAM单比特错误中断。 */
        unsigned int  wsram_merr_intsts   : 1;  /* bit[22]   : WSRAM多比特错误中断。 */
        unsigned int  reserved_2          : 1;  /* bit[23]   : 保留。 */
...
#define SOC_DDRC_DMC_DDRC_INTSTS_wsram_serr_intsts_START    (21)
#define SOC_DDRC_DMC_DDRC_INTSTS_wsram_serr_intsts_END      (21)
#define SOC_DDRC_DMC_DDRC_INTSTS_wsram_merr_intsts_START    (22)
#define SOC_DDRC_DMC_DDRC_INTSTS_wsram_merr_intsts_END      (22)
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_UNION
 结构说明  : DDRC_CFG_ECC_CTRL 寄存器结构定义。地址偏移量:0x2A8，初值:0x00000000，宽度:32
 寄存器说明: DDRC_CFG_ECC_CTRL是DDRC单/多比特第一次和最后一次结果读取控制寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  err_buf_sel : 1;  /* bit[0]   : 单/多比特ECC错误信息第一次和最后一次结果读取控制寄存器
                                                      1：读取最后一次ECC单/多比特错误信息；
                                                      0：读取第一次ECC单/多比特错误信息。 */
        unsigned int  reserved    : 31; /* bit[1-31]: 保留。 */
    } reg;
} SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_UNION;
#endif
#define SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_err_buf_sel_START  (0)
#define SOC_DDRC_DMC_DDRC_CFG_ECC_CTRL_err_buf_sel_END    (0)


/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_HIS_SERR_ID_UNION
 结构说明  : DDRC_HIS_SERR_ID 寄存器结构定义。地址偏移量:0x2AC，初值:0x00000000，宽度:32
 寄存器说明: DDRC_HIS_SERR_ID是DDRC EC单比特错误ID寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  serr_id : 32; /* bit[0-31]: DDRC ECC单比特错误ID寄存器。
                                                  注意：此寄存器记录第一次发生单比特错误的ID，当清楚多比特错误中断时，解除锁定。 */
    } reg;
} SOC_DDRC_DMC_DDRC_HIS_SERR_ID_UNION;
#endif
#define SOC_DDRC_DMC_DDRC_HIS_SERR_ID_serr_id_START  (0)
#define SOC_DDRC_DMC_DDRC_HIS_SERR_ID_serr_id_END    (31)
...
typedef union
{
    ...
    struct
    {
        unsigned int  serr_cnt : 32; /* bit[0-31]: 单比特ECC数据错误统计值饱和计数 。
                                                   注意：此寄存器为读清。同时，由于该寄存器不带复位端，上电后需通过读取清0。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  crcerr_cnt : 16; /* bit[0-15] : 供发生CRC错误的次数，在crc_en使能时，Alert_n拉低一次的时间小于设定的阈值则判定为一次CRC错误。
                                                      注意：该寄存器没有复位，上电和读取该寄存器状态后，需要通过写DDRC_HIS_REC_ERR0寄存器清零 */
        unsigned int  parerr_cnt : 16; /* bit[16-31]: 共发生CA Parity错误的次数，在parity_en使能时，Alert_n拉低一次的时间超过设定的阈值则判定为一次CA Parity错误.
                                                      注意：该寄存器没有复位，上电和读取该寄存器状态后，需要通过写DDRC_HIS_REC_ERR0寄存器清零 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  overnum_cnt   : 8;  /* bit[0-7]  : 判定为CRC错误但是错误窗口中没有写命令错误的次数.
                                                         
                                                         注意：该寄存器没有复位，上电和读取该寄存器状态后，需要通过写DDRC_HIS_REC_ERR1寄存器清零 */
        unsigned int  crc_no_wr_cnt : 8;  /* bit[8-15] : 判定为CRC错误但是错误窗口中没有写命令错误的次数.
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA_UNION
 结构说明  : DDRC_HIS_SFC_RDATA 寄存器结构定义。地址偏移量:0x4A8，初值:0x00000000，宽度:32
 寄存器说明: DDRC_HIS_SFC_RDATA是SFC读数据寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  rdata : 32; /* bit[0-31]: DDR的一个burst数据都从该寄存器读取，每读一次，sfc_rptr会自动加1。其中rdata[7:0]为读模式寄存器的值。另外，ecc相关的数据跟在正常数据之后。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_SFC_RCTRL_UNION
 结构说明  : DDRC_SFC_RCTRL 寄存器结构定义。地址偏移量:0x4AC，初值:0x00000000，宽度:32
 寄存器说明: DDRC_SFC_RCTRL是SFC读数据指针寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  sfc_rdata_ptr : 32; /* bit[0-31]: 该寄存器为SFC读数据的指针寄存器。每读一次DDRC_HIS_SFC_RDATA，该寄存器自动加1。数据读取完毕，需要通过写入来将指针复位为0.该指针的位宽由实际配置决定。 */
    } reg;
} SOC_DDRC_DMC_DDRC_SFC_RCTRL_UNION;
#endif
#define SOC_DDRC_DMC_DDRC_SFC_RCTRL_sfc_rdata_ptr_START  (0)
#define SOC_DDRC_DMC_DDRC_SFC_RCTRL_sfc_rdata_ptr_END    (31)


/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA0_DBI_UNION
 结构说明  : DDRC_HIS_SFC_RDATA0_DBI 寄存器结构定义。地址偏移量:0x4C8，初值:0x00000000，宽度:32
 寄存器说明: DDRC_HIS_SFC_RDATA0_DBI是读数据对应的DBI寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  rdata0_dbi : 32; /* bit[0-31]: SFC读数据每个Byte对应的DBI信号。
...
/*****************************************************************************
 结构名    : SOC_DDRC_DMC_DDRC_HIS_SFC_RDATA1_DBI_UNION
 结构说明  : DDRC_HIS_SFC_RDATA1_DBI 寄存器结构定义。地址偏移量:0x4CC，初值:0x00000000，宽度:32
 寄存器说明: DDRC_HIS_SFC_RDATA1_DBI是读数据对应的DBI寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  rdata1_dbi : 32; /* bit[0-31]: SFC读数据每个Byte对应的DBI信号。
                                                     SFC读数据[7:0]对应rdata_dbi[0];
                                                     SFC读数据[15:8]对应rdata_dbi[1];
                                                     ….
                                                     SFC读数据[255:248]对应rdata_dbi[31];
...
/*****************************************************************************
 ...
 结构说明  : DDRC_FUNC_STAT6 寄存器结构定义。地址偏移量:0x4EC，初值:0x0300000F，宽度:32
 寄存器说明: DDRC_FUNC_STAT6是fun的握手状态寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  rec_farb_creq      : 1;  /* bit[26]   : rec_farb_creq的状态信息 */
        unsigned int  farb_rec_cmd_idle  : 1;  /* bit[27]   : farb_rec_cmd_idle的状态信息 */
        unsigned int  sch_rec_func_grant : 1;  /* bit[28]   : sch_rec_func_grant的状态信息 */
...
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_rec_farb_creq_START       (26)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_rec_farb_creq_END         (26)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_farb_rec_cmd_idle_START   (27)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_farb_rec_cmd_idle_END     (27)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_sch_rec_func_grant_START  (28)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT6_sch_rec_func_grant_END    (28)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  rec_state        : 4;  /* bit[17-20]: rec的模块的状态机 */
        unsigned int  phy_func_alert_n : 1;  /* bit[21]   : phy_func_alert_n的状态 */
        unsigned int  reserved         : 10; /* bit[22-31]: 保留。 */
    } reg;
...
#define SOC_DDRC_DMC_DDRC_FUNC_STAT10_rec_state_START         (17)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT10_rec_state_END           (20)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT10_phy_func_alert_n_START  (21)
#define SOC_DDRC_DMC_DDRC_FUNC_STAT10_phy_func_alert_n_END    (21)
...
/*****************************************************************************
 ...
 结构说明  : DDRC_TEST_RAM_TMOD 寄存器结构定义。地址偏移量:0x508，初值:0x00000000，宽度:32
 寄存器说明: DDRC_TEST_TMOD是DDR 中RAM的测试模式配置寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 ...
 结构说明  : DDRC_TEST_RTL_CFG0 寄存器结构定义。地址偏移量:0x510，初值:0x00000000，宽度:32
 寄存器说明: DDRC_TEST_RTL_CFG0是DDRC RTL参数信息寄存器0
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 ...
 结构说明  : DDRC_TEST_RTL_CFG1 寄存器结构定义。地址偏移量:0x514，初值:0x00000000，宽度:32
 寄存器说明: DDRC_TEST_RTL_CFG1是DDRC RTL参数信息寄存器1
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 ...
 结构说明  : DDRC_TEST_RTL_CFG2 寄存器结构定义。地址偏移量:0x518，初值:0x00000000，宽度:32
 寄存器说明: DDRC_TEST_RTL_CFG2是DDRC RTL参数信息寄存器2
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
