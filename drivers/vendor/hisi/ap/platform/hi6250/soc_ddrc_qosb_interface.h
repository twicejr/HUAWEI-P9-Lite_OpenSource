...
typedef union
{
    ...
    struct
    {
        unsigned int  flux_id  : 25; /* bit[0-24] : 用于流量统计的匹配ID，一共有0~6个ID，每个ID对应一个统计组；如果与这7个ID都不相同，则将流量归入默认的一组。 */
        unsigned int  reserved : 7;  /* bit[25-31]: 保留。 */
    } reg;
} SOC_DDRC_QOSB_QOSB_FLUX_ID_UNION;
#endif
#define SOC_DDRC_QOSB_QOSB_FLUX_ID_flux_id_START   (0)
#define SOC_DDRC_QOSB_QOSB_FLUX_ID_flux_id_END     (24)
...
typedef union
{
    ...
    struct
    {
        unsigned int  flux_id_mask : 25; /* bit[0-24] : 与QOS_FLUX_ID寄存器对应，用于ID的MASK。
                                                        0：表示对应的ID的bit被忽略；
                                                        1：表示对应的ID的bit参与比较。 */
        unsigned int  reserved     : 7;  /* bit[25-31]: 保留。 */
    } reg;
...
#define SOC_DDRC_QOSB_QOSB_FLUX_ID_MASK_flux_id_mask_END    (24)
...
/*****************************************************************************
 ...
 结构说明  : QOSB_RAM_TMOD 寄存器结构定义。地址偏移量:0x104，初值:0x00000000，宽度:32
 寄存器说明: QOS_RAM_TMOD是QosBuf/RDR的RAM的端口配置信号寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
