...
/* 寄存器说明：CMDLIST中断状态寄存器。
   位域定义UNION结构:  SOC_DSS_CMD_CMD_AXI_DBG_UNION */
#define SOC_DSS_CMD_CMD_AXI_DBG_ADDR(base)            ((base) + (0x0424))
...
typedef union
{
    ...
    struct
    {
        unsigned int  buf_dbg_cnt_clr : 1;  /* bit[0]   : 调试用寄存器 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : CMDLIST_CTRL 寄存器结构定义。地址偏移量:0x0410，初值:0x00000404，宽度:32
 寄存器说明: CMDLIST控制寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
typedef union
{
    ...
    struct
    {
        unsigned int  ch_secu  : 14; /* bit[0-13] : CMDLIST通道起始地址寄存器的起始地址寄存器安全属性。1: 安全, 0: 非安全
                                                    ...
                                                    bit8：ch6起始地址寄存器安全属性；
                                                    bit9：保留；
                                                    bit10：ch7起始地址寄存器安全属性；
                                                    bit11：保留；
                                                    bit12：ch8起始地址寄存器安全属性；
                                                    bit13：保留。 */
        unsigned int  reserved : 18; /* bit[14-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DSS_CMD_CMD_AXI_DBG_UNION
 结构说明  : CMD_AXI_DBG 寄存器结构定义。地址偏移量:0x0424，初值:0x00000000，宽度:32
 寄存器说明: CMDLIST中断状态寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  cmd_axi_dbg : 32; /* bit[0-31]: 保留。 */
    } reg;
} SOC_DSS_CMD_CMD_AXI_DBG_UNION;
#endif
#define SOC_DSS_CMD_CMD_AXI_DBG_cmd_axi_dbg_START  (0)
#define SOC_DSS_CMD_CMD_AXI_DBG_cmd_axi_dbg_END    (31)
