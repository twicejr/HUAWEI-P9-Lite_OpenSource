...
/* 寄存器说明：FLUX_REQ阈值寄存器
   位域定义UNION结构:  SOC_DSS_DBUF_THD_FLUX_REQ_UNION */
#define SOC_DSS_DBUF_THD_FLUX_REQ_ADDR(base)          ((base) + (0x0048))
...
typedef union
{
    ...
    struct
    {
        unsigned int  sram_valid_num : 2;  /* bit[0-1] : 对于DBUF0，有效memory的个数
                                                         0:DFS最大深度为1008x144bits
                                                         1:DFS最大深度为2016x144bits
                                                         2,3:保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  wbe_en   : 1;  /* bit[0]   : 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  thd_wqos_in  : 14; /* bit[0-13] : 保留 */
        unsigned int  reserved_0   : 2;  /* bit[14-15]: 保留 */
        unsigned int  thd_wqos_out : 14; /* bit[16-29]: 保留 */
        unsigned int  reserved_1   : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  thd_wr_wait : 14; /* bit[0-13] : 保留 */
        unsigned int  reserved_0  : 2;  /* bit[14-15]: 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  online_fill_lev : 14; /* bit[0-13] : 在线通路填充水限寄存器。 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  wb_fill_lev : 14; /* bit[0-13] : 保留。 */
        unsigned int  reserved    : 18; /* bit[14-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  online_empty  : 1;  /* bit[0]   : dbuf空，高有效。 */
        unsigned int  online_full   : 1;  /* bit[1]   : dbuf满，高有效。 */
        unsigned int  offline_empty : 1;  /* bit[2]   : 保留。 */
        unsigned int  offline_full  : 1;  /* bit[3]   : 保留。 */
        unsigned int  dfs_cg        : 1;  /* bit[4]   : 时钟门控信号，高有效。 */
        unsigned int  dfs_wqos      : 1;  /* bit[5]   : 保留。 */
        unsigned int  dfs_rqos      : 1;  /* bit[6]   : dfs_qos信号，高有效。 */
        unsigned int  dfs_wb_error  : 1;  /* bit[7]   : 保留。 */
...
/*****************************************************************************
 结构名    : SOC_DSS_DBUF_THD_FLUX_REQ_UNION
 结构说明  : THD_FLUX_REQ 寄存器结构定义。地址偏移量:0x0048，初值:0x00000000，宽度:32
 寄存器说明: FLUX_REQ阈值寄存器
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  thd_flux_req_in  : 14; /* bit[0-13] : 在线通道填充水限小于此阈值时，dss_flux_req信号置1，此处填充水限的单位为memory深度，例如深度1对应与1x144bits = 6像素 */
        unsigned int  reserved_0       : 2;  /* bit[14-15]: 保留 */
        unsigned int  thd_flux_req_out : 14; /* bit[16-29]: 在线通道填充水限大于此阈值时，dss_flux_req信号置0，此处填充水限的单位为memory深度，例如深度1对应与1x144bits = 6像素 */
        unsigned int  reserved_1       : 2;  /* bit[30-31]: 保留 */
    } reg;
} SOC_DSS_DBUF_THD_FLUX_REQ_UNION;
#endif
#define SOC_DSS_DBUF_THD_FLUX_REQ_thd_flux_req_in_START   (0)
#define SOC_DSS_DBUF_THD_FLUX_REQ_thd_flux_req_in_END     (13)
#define SOC_DSS_DBUF_THD_FLUX_REQ_thd_flux_req_out_START  (16)
#define SOC_DSS_DBUF_THD_FLUX_REQ_thd_flux_req_out_END    (29)
