...
/* 寄存器说明：子图header pointer和总图header pointer之间的offset
   位域定义UNION结构:  SOC_DSS_RCH_G_AFBCD_HEADER_POINTER_OFFSET_UNION */
#define SOC_DSS_RCH_G_AFBCD_HEADER_POINTER_OFFSET_ADDR(base) ((base) + (0x0944))

/* 寄存器说明：可维可测寄存器1。
   位域定义UNION结构:  SOC_DSS_RCH_G_AFBCD_MONITOR_REG1_UNION */
#define SOC_DSS_RCH_G_AFBCD_MONITOR_REG1_ADDR(base)   ((base) + (0x0948))

/* 寄存器说明：可维可测寄存器2。
   位域定义UNION结构:  SOC_DSS_RCH_G_AFBCD_MONITOR_REG2_UNION */
#define SOC_DSS_RCH_G_AFBCD_MONITOR_REG2_ADDR(base)   ((base) + (0x094C))

/* 寄存器说明：可维可测寄存器3。
   位域定义UNION结构:  SOC_DSS_RCH_G_AFBCD_MONITOR_REG3_UNION */
...
typedef union
{
    ...
    struct
    {
        unsigned int  scf_output_height : 13; /* bit[0-12] : 实际输出高度 - 1；该寄存器配置值大于等于15； */
        unsigned int  reserved_0        : 3;  /* bit[13-15]: 保留 */
        unsigned int  scf_output_width  : 13; /* bit[16-28]: 实际输出宽度 - 1;该寄存器配置值大于等于15，scf_h_v_order按照推荐配置时，在水平缩小的情况下小于等于1199； */
        unsigned int  reserved_1        : 3;  /* bit[29-31]: 保留 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : SCF_DB_V1 寄存器结构定义。地址偏移量:0x028C，初值:0x30000000，宽度:32
 寄存器说明: 垂直滤波器的内部状态寄存器1
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 结构名    : SOC_DSS_RCH_G_AFBCD_HEADER_POINTER_OFFSET_UNION
...
/*****************************************************************************
 结构名    : SOC_DSS_RCH_G_AFBCD_MONITOR_REG1_UNION
...
/*****************************************************************************
 结构名    : SOC_DSS_RCH_G_AFBCD_MONITOR_REG2_UNION
...
/*****************************************************************************
 结构名    : SOC_DSS_RCH_G_AFBCD_MONITOR_REG3_UNION
