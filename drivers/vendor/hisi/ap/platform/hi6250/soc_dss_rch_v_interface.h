...
/* 寄存器说明：子图header pointer和总图header pointer之间的offset
   位域定义UNION结构:  SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_UNION */
#define SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_ADDR(base) ((base) + (0x0944))

/* 寄存器说明：可维可测寄存器1。
   位域定义UNION结构:  SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_UNION */
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_ADDR(base)   ((base) + (0x0948))

/* 寄存器说明：可维可测寄存器2。
   位域定义UNION结构:  SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_UNION */
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_ADDR(base)   ((base) + (0x094C))

/* 寄存器说明：可维可测寄存器3。
   位域定义UNION结构:  SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_UNION */
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_ADDR(base)   ((base) + (0x0950))
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  dma_cross_128       : 1;  /* bit[23]   : 保留 */
        unsigned int  dma_tile_128byte_en : 1;  /* bit[24]   : 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  dma_stride1           : 13; /* bit[0-12] : RDMA读入图像或WDMA写出图像的V平面行地址偏移量。行地址偏移量为图像在内存中相邻两行首像素存储地址之差。（1代表128bit）
                                                                 非旋转时Linear格式Stride按128bit对齐，即最小配置值为1，表示Stride为128bit；
                                                                 旋转时Linear格式Stride按4*128bit对齐，即最小配置值为4，表示Stride为4*128bit；
                                                                 Tile格式Stride按2的幂次方Byte对齐，最小256Byte，即最小配置值是256Byte/128bit = 16 = 10000。
                                                                 注：
        ...
        unsigned int  dma_l2t_interleave_n1 : 4;  /* bit[16-19]: Video Tile: Linear VA转Tile VA时交织的位号；
                                                                 CPU Tile：Linear PA转Tile PA 交织的位号；
                                                                 计算方法是：l2t_interleave_n=7 + log2(dma_stride1*16/256)。合法范围是：7~15。
                                                                 （需要注意的是，当Stride=256Byte时，其图像宽度就是一个tile的宽度，此时n=7，Linear地址和Tile地址实际是一致的，并不需要转换） */
        unsigned int  reserved_1            : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  dma_data_num1 : 30; /* bit[0-29] : 保留 */
        unsigned int  reserved      : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  dma_l2t_interleave_n2 : 4;  /* bit[16-19]: Video Tile: Linear VA转Tile VA时交织的位号；
                                                                 CPU Tile：Linear PA转Tile PA 交织的位号；
                                                                 计算方法是：l2t_interleave_n=7 + log2(dma_stride2*16/256)。合法范围是：7~15。
                                                                 （需要注意的是，当Stride=256Byte时，其图像宽度就是一个tile的宽度，此时n=7，Linear地址和Tile地址实际是一致的，并不需要转换） */
        unsigned int  reserved_1            : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  dma_data_num2 : 30; /* bit[0-29] : 保留 */
        unsigned int  reserved      : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  dfc_dither_en : 1;  /* bit[0]   : 抖动处理使能 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  scf_a_core_gt : 1;  /* bit[0]   : 通道alpha单独门控使能：
                                                        1：表示在通道alpha不存在或不工作时，可以关闭通道alpha相关逻辑时钟,硬件自动时钟门控；
                                                        0：通道alpha使能，时钟不可以关闭,软件强制打开时钟； */
        unsigned int  scf_r_core_gt : 1;  /* bit[1]   : 通道R或Y单独门控使能：
                                                        1：表示在通道R或Y不存在或不工作时，可以关闭通道R或Y相关逻辑时钟,硬件自动时钟门控；
                                                        0：通道R或Y使能，时钟不可以关闭,软件强制打开时钟； */
        unsigned int  scf_g_core_gt : 1;  /* bit[2]   : 通道G或U单独门控使能：
                                                        1：表示在通道G或U不存在或不工作时，可以关闭通道G或U相关逻辑时钟,硬件自动时钟门控；
                                                        0：通道G或U使能，时钟不可以关闭,软件强制打开时钟； */
        unsigned int  scf_b_core_gt : 1;  /* bit[3]   : 通道B或V单独门控使能：
                                                        1：表示在通道B或V不存在或不工作时，可以关闭通道B或V相关逻辑时钟,硬件自动时钟门控；
                                                        0：通道B或V使能，时钟不可以关闭,软件强制打开时钟； */
...
typedef union
{
    ...
    struct
    {
        unsigned int  scf_input_height : 13; /* bit[0-12] : 实际输入高度 - 1；该寄存器配置值大于等于15 */
        unsigned int  reserved_0       : 3;  /* bit[13-15]: 保留 */
        unsigned int  scf_input_width  : 13; /* bit[16-28]: 实际输入宽度 - 1;该寄存器配置值大于等于15,scf_h_v_order按照推荐配置时，在水平放大的情况下小于等于1199； */
        unsigned int  reserved_1       : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  scf_output_height : 13; /* bit[0-12] : 实际输出高度 - 1；该寄存器配置值大于等于15；同时作为sharp的尺寸寄存器 */
        unsigned int  reserved_0        : 3;  /* bit[13-15]: 保留 */
        unsigned int  scf_output_width  : 13; /* bit[16-28]: 实际输出宽度 - 1;该寄存器配置值大于等于15，scf_h_v_order按照推荐配置时，在水平缩小的情况下小于等于1199；同时作为sharp的尺寸寄存器 */
        unsigned int  reserved_1        : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  scf_inc_hscl : 24; /* bit[0-23] : 水平缩放比例，计算公式为
                                                        ...
                                                        inc_hscl的低18比特为ratio的小数部分，高6比特为整数部分。整数部分6比特，支持60倍以下（含60倍）的缩小。放大的倍数，如果SCF_H_V_ORDER按照推荐设置，放大的最大倍数为60倍（含60倍）。放大场景，SCF_H_V_ORDER设为0，最大放大倍数为min(60,floor(1200/实际的输入图像宽度）)
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_flt_c0 : 8;  /* bit[0-7]  : RCH_V1：1D sharp
                                                        sharpness7阶滤波器系数0，8bit无符号数 */
        unsigned int  sharp_flt_c1 : 8;  /* bit[8-15] : RCH_V1：1D sharp
                                                        sharpness7阶滤波器系数1，8bit有符号数 */
        unsigned int  sharp_flt_c2 : 8;  /* bit[16-23]: RCH_V1：1D sharp
                                                        sharpness7阶滤波器系数2，8bit有符号数 */
        unsigned int  sharp_flt_c3 : 7;  /* bit[24-30]: RCH_V1：1D sharp
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_mode : 2;  /* bit[0-1] : RCH_V1：1D sharp
                                                     sharpness模式选择寄存器 */
        unsigned int  sharp_en   : 1;  /* bit[2]   : sharpness功能是否打开:
                                                     1：作sharp功能，使能时需要配置scf_output_width和scf_output_height作为sharp的尺寸寄存器
                                                     0：bypass */
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_softthx : 8;  /* bit[0-7]  : RCH_V1：1D sharp
                                                         软阈值中用到阈值 */
        unsigned int  sharp_kgain   : 8;  /* bit[8-15] : RCH_V1：1D sharp
                                                         锐度提升增益 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_grayweight : 4;  /* bit[0-3]  : RCH_V1：1D sharp
                                                            灰度控制中blending权重 */
        unsigned int  reserved_0       : 4;  /* bit[4-7]  : 保留。 */
        unsigned int  sharp_k4posjaggy : 8;  /* bit[8-15] : RCH_V1：1D sharp
                                                            梯度控制中梯度增益 */
        unsigned int  sharp_k4negjaggy : 8;  /* bit[16-23]: RCH_V1：1D sharp
                                                            梯度控制中梯度增益 */
        unsigned int  sharp_intercept  : 4;  /* bit[24-27]: RCH_V1：1D sharp
                                                            增益曲线的横截距 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_flt1_c0 : 8;  /* bit[0-7]  : 保留。 */
        unsigned int  sharp_flt1_c1 : 8;  /* bit[8-15] : 保留。 */
        unsigned int  sharp_flt1_c2 : 8;  /* bit[16-23]: 保留。 */
        unsigned int  reserved      : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharp_flt2_c0 : 8;  /* bit[0-7]  : 保留。 */
        unsigned int  sharp_flt2_c1 : 8;  /* bit[8-15] : 保留。 */
        unsigned int  sharp_flt2_c2 : 8;  /* bit[16-23]: 保留。 */
        unsigned int  reserved      : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  linedet__linethd1  : 5;  /* bit[0-4]  : 保留。 */
        unsigned int  reserved_0         : 3;  /* bit[5-7]  : 保留。 */
        unsigned int  linedet__linethd2  : 5;  /* bit[8-12] : 保留。 */
        unsigned int  reserved_1         : 3;  /* bit[13-15]: 保留。 */
        unsigned int  linedet__linedeten : 1;  /* bit[16-16]: 保留。 */
        unsigned int  linedet__lineamt1  : 12; /* bit[17-28]: 保留。 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  sharpcfg__sharpamt1    : 12; /* bit[0-11] : 保留。 */
        unsigned int  sharpcfg__sharpthd1mul : 12; /* bit[12-23]: 保留。 */
        unsigned int  sharpcfg__sharpthd1    : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  edgecfg__edgeamt1    : 12; /* bit[0-11] : 保留。 */
        unsigned int  edgecfg__edgethd1mul : 12; /* bit[12-23]: 保留。 */
        unsigned int  edgecfg__edgethd1    : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : SHARP_DBG0 寄存器结构定义。地址偏移量:0x0330，初值:0x00000011，宽度:32
 寄存器说明: sharpness调试寄存器0
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
typedef union
{
    ...
    struct
    {
        unsigned int  post_clip_right : 6;  /* bit[0-5]  : 图像右边界需CLIP掉的像素数 */
        unsigned int  reserved_0      : 10; /* bit[6-15] : 保留 */
        unsigned int  post_clip_left  : 6;  /* bit[16-21]: 图像左边界需CLIP掉的像素数 */
        unsigned int  reserved_1      : 10; /* bit[22-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  post_clip_bot : 6;  /* bit[0-5]  : 图像底边界需CLIP掉的像素数 */
        unsigned int  reserved_0    : 10; /* bit[6-15] : 保留 */
        unsigned int  post_clip_top : 6;  /* bit[16-21]: 图像顶边界需CLIP掉的像素数 */
        unsigned int  reserved_1    : 10; /* bit[22-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_hist_reset : 1;  /* bit[0]   : 保留。 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_hist_ctl : 3;  /* bit[0-2] : 保留。 */
        unsigned int  reserved    : 29; /* bit[3-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_hist_frame_cnt : 6;  /* bit[0-5] : 保留。 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_height : 13; /* bit[0-12] : 保留。 */
        unsigned int  reserved_0: 3;  /* bit[13-15]: 保留。 */
        unsigned int  ce_width  : 13; /* bit[16-28]: 保留。 */
        unsigned int  reserved_1: 3;  /* bit[29-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_no_stat_lines : 8;  /* bit[0-7] : 保留。 */
        unsigned int  reserved         : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_black_region_thre : 8;  /* bit[0-7] : 保留。 */
        unsigned int  reserved             : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_white_region_thre : 8;  /* bit[0-7] : 保留。 */
        unsigned int  reserved             : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_lut_sel : 1;  /* bit[0]   : 保留。 */
        unsigned int  reserved   : 31; /* bit[1-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_lut_enable  : 1;  /* bit[0]   : 保留。 */
        unsigned int  ce_hist_enable : 1;  /* bit[1]   : 保留。 */
        unsigned int  reserved       : 30; /* bit[2-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_lut_using_ind : 1;  /* bit[0]   : 保留。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_state_ind : 1;  /* bit[0]   : 保留。 */
        unsigned int  reserved     : 31; /* bit[1-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_lut_mem_ctrl : 4;  /* bit[0-3] : 保留。 */
        unsigned int  ce_his_mem_ctrl : 4;  /* bit[4-7] : 保留。 */
        unsigned int  reserved        : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_totalgray : 32; /* bit[0-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_totalpixl : 24; /* bit[0-23] : 保留。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_mean  : 8;  /* bit[0-7] : 保留。 */
        unsigned int  reserved : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_pec0 : 8;  /* bit[0-7]  : 保留。 */
        unsigned int  ce_pec1 : 8;  /* bit[8-15] : 保留。 */
        unsigned int  ce_pec2 : 8;  /* bit[16-23]: 保留。 */
        unsigned int  ce_pec3 : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_pec4 : 8;  /* bit[0-7]  : 保留。 */
        unsigned int  ce_pec5 : 8;  /* bit[8-15] : 保留。 */
        unsigned int  ce_pec6 : 8;  /* bit[16-23]: 保留。 */
        unsigned int  ce_pec7 : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_pec8  : 8;  /* bit[0-7] : 保留。 */
        unsigned int  reserved : 24; /* bit[8-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_black_num : 24; /* bit[0-23] : 保留。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_white_num : 24; /* bit[0-23] : 保留。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ce_hist_rpt_ind : 1;  /* bit[0]   : 保留。 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_6            : 24; /* bit[8-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_UNION
...
} SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_UNION;
#endif
#define SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_afbcd_header_pointer_offset_START  (0)
#define SOC_DSS_RCH_V_AFBCD_HEADER_POINTER_OFFSET_afbcd_header_pointer_offset_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_UNION
...
} SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_UNION;
#endif
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_afbcd_monitor_reg1_START  (0)
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG1_afbcd_monitor_reg1_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_UNION
...
} SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_UNION;
#endif
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_afbcd_monitor_reg2_START  (0)
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG2_afbcd_monitor_reg2_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_UNION
...
} SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_UNION;
#endif
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_afbcd_monitor_reg3_START  (0)
#define SOC_DSS_RCH_V_AFBCD_MONITOR_REG3_afbcd_monitor_reg3_END    (31)
