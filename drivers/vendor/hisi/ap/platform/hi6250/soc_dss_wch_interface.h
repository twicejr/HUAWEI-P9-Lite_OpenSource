...
/* 寄存器说明：子图header pointer和总图header pointer之间的offset
   位域定义UNION结构:  SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_UNION */
#define SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_ADDR(base) ((base) + (0x0934))

/* 寄存器说明：可维可测寄存器1。
   位域定义UNION结构:  SOC_DSS_WCH_AFBCE_MONITOR_REG1_UNION */
#define SOC_DSS_WCH_AFBCE_MONITOR_REG1_ADDR(base)     ((base) + (0x0938))

/* 寄存器说明：可维可测寄存器2。
   位域定义UNION结构:  SOC_DSS_WCH_AFBCE_MONITOR_REG2_UNION */
#define SOC_DSS_WCH_AFBCE_MONITOR_REG2_ADDR(base)     ((base) + (0x093C))

/* 寄存器说明：可维可测寄存器3。
   位域定义UNION结构:  SOC_DSS_WCH_AFBCE_MONITOR_REG3_UNION */
#define SOC_DSS_WCH_AFBCE_MONITOR_REG3_ADDR(base)     ((base) + (0x0940))
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  dma_counter_inc_cnum : 4;  /* bit[1-4]  : WDMA输出counter_inc（表示一行已完全写入dram）拉高的cycle数，为实际值-1
                                                                注：
                                                                1、RDMA不关注此项 */
        unsigned int  dma_counter_rst_cnum : 4;  /* bit[5-8]  : WDMA输出counter_rst（表示一帧开始写入dram）拉高的cycle数，为实际值-1
                                                                注：
                                                                1、RDMA不关注此项 */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_1            : 5;  /* bit[16-20]: 
...
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_threshold : 3;  /* bit[0-2] : afbce数据输出的水线寄存器
                                                          使用缺省值2, 允许配置2,3,4,5,6.（如果需要修改配置，需要按照芯片方调试好的值配置） 0和1是非法值，不允许配置。 */
        unsigned int  reserved        : 29; /* bit[3-31]:  */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_scramble_mode : 2;  /* bit[0-1] : afbce地址交织寄存器，在DALLAS里只能配成2'b00
...
/*****************************************************************************
 结构名    : SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_UNION
 结构说明  : AFBCE_HEADER_POINTER_OFFSET 寄存器结构定义。地址偏移量:0x0934，初值:0x00000000，宽度:32
 寄存器说明: 子图header pointer和总图header pointer之间的offset
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_header_pointer_offset : 32; /* bit[0-31]: 子图header pointer和总图header pointer之间的offset */
    } reg;
} SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_UNION;
#endif
#define SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_afbce_header_pointer_offset_START  (0)
#define SOC_DSS_WCH_AFBCE_HEADER_POINTER_OFFSET_afbce_header_pointer_offset_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_WCH_AFBCE_MONITOR_REG1_UNION
 结构说明  : AFBCE_MONITOR_REG1 寄存器结构定义。地址偏移量:0x0938，初值:0x00000000，宽度:32
 寄存器说明: 可维可测寄存器1。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_monitor_reg1 : 32; /* bit[0-31]: 调试用。
                                                             Payload pro 
                                                             [31]os_zero 
                                                             [30] frm_pl_send_done 
                                                             [29]frm_pl_rcv_done 
                                                             [28:24]data_burst_cnt
                                                             [23:14]cmd_v_blk_cnt 
                                                             [13:4]cmd_h_blk_cnt 
                                                             [3]cmd_fifo_empty 
                                                             [2]cmd_fifo_full 
...
} SOC_DSS_WCH_AFBCE_MONITOR_REG1_UNION;
#endif
#define SOC_DSS_WCH_AFBCE_MONITOR_REG1_afbce_monitor_reg1_START  (0)
#define SOC_DSS_WCH_AFBCE_MONITOR_REG1_afbce_monitor_reg1_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_WCH_AFBCE_MONITOR_REG2_UNION
 结构说明  : AFBCE_MONITOR_REG2 寄存器结构定义。地址偏移量:0x093C，初值:0x00000000，宽度:32
 寄存器说明: 可维可测寄存器2。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_monitor_reg2 : 32; /* bit[0-31]: 调试用。
                                                             Header pro 
                                                             [31:8] reserved 
                                                             [7:5]current_state(header pro FSM) 
                                                             [4:2]vbif_cmd_cur（cmd FSM） 
                                                             [1]:burst4_wfull(异步fifo写满) 
                                                             [0]: b4_done_rempty(异步fifo读空) */
    } reg;
} SOC_DSS_WCH_AFBCE_MONITOR_REG2_UNION;
#endif
#define SOC_DSS_WCH_AFBCE_MONITOR_REG2_afbce_monitor_reg2_START  (0)
#define SOC_DSS_WCH_AFBCE_MONITOR_REG2_afbce_monitor_reg2_END    (31)


/*****************************************************************************
 结构名    : SOC_DSS_WCH_AFBCE_MONITOR_REG3_UNION
 结构说明  : AFBCE_MONITOR_REG3 寄存器结构定义。地址偏移量:0x0940，初值:0x00000000，宽度:32
 寄存器说明: 可维可测寄存器3。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  afbce_monitor_reg3 : 32; /* bit[0-31]: 调试用。
                                                             [31:16] reserved 
                                                             [15]frame_start_internal 
                                                             [14]end_req_internal 
                                                             [13]active 
                                                             [12]shutdown 
                                                             [11:8]frame_cur(afbce FSM) 
                                                             [7:2]Afbce_monitor_reg_os 
                                                             [1:0]Afbce_monitor_reg_1pto2p */
    } reg;
} SOC_DSS_WCH_AFBCE_MONITOR_REG3_UNION;
#endif
#define SOC_DSS_WCH_AFBCE_MONITOR_REG3_afbce_monitor_reg3_START  (0)
#define SOC_DSS_WCH_AFBCE_MONITOR_REG3_afbce_monitor_reg3_END    (31)
