...
/* 寄存器说明：DDR子系统的APB安全权限配置。
   位域定义UNION结构:  SOC_EXMBIST_DDRSYS_APB_SECURE_UNION */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  apb_clken_0         : 1;  /* bit[3]    : DDRC0 APB时钟门控使能：
                                                               0：关闭该模块的时钟。
                                                               1：使能该模块的时钟。 */
        unsigned int  reserved_1          : 6;  /* bit[4-9]  : 保留 */
        unsigned int  exmbist_wrap_clk_en : 1;  /* bit[10]   : exmbist_wrap时钟门控使能：
                                                               0：关闭该模块的时钟。
                                                               1：使能该模块的时钟。 */
        unsigned int  reserved_2          : 5;  /* bit[11-15]: 保留。 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  cfg_chan_type : 2;  /* bit[0-1] : 对接DDR为单通道、双通道或者四通道。
                                                        0：对接DDR为单通道。
                                                        1：对接DDR为双通道。
                                                        2：对接DDR为四通道。 */
        unsigned int  cfg_test_chan : 2;  /* bit[2-3] : 当前选择扫描的channel。
                                                        00：选择channel 0。
                                                        01：选择channel 1。
                                                        10：选择channel 2。
                                                        11：选择channel 3。 */
...
/*****************************************************************************
 ...
 结构说明  : STATUS 寄存器结构定义。地址偏移量:0x0058，初值:0x00005002，宽度:32
 寄存器说明: EXMBIST模块的状态输出寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 ...
 结构说明  : DFX_INFO 寄存器结构定义。地址偏移量:0x005C，初值:0x00000007，宽度:32
 寄存器说明: DFX信息寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
typedef union
{
    ...
    struct
    {
        unsigned int  slp      : 1;  /* bit[0]    : 保留。 */
        unsigned int  dslp     : 1;  /* bit[1]    : 保留。 */
        unsigned int  sd       : 1;  /* bit[2]    : 保留。 */
        unsigned int  rct      : 2;  /* bit[3-4]  : 保留。 */
        unsigned int  wct      : 2;  /* bit[5-6]  : 保留。 */
        unsigned int  kp       : 3;  /* bit[7-9]  : 保留。 */
...
/*****************************************************************************
 ...
 结构说明  : COMMON_CTRL0 寄存器结构定义。地址偏移量:0x0068，初值:0x00000010，宽度:32
 寄存器说明: 通用控制寄存器0。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  sel_clk_ddrphy_rs_wr : 2;  /* bit[0-1] : 各级RS的写时钟正反沿配置。
                                                               0：正常时钟。
                                                               1：反相时钟。 */
        unsigned int  sel_clk_ddrphy_rs_rd : 2;  /* bit[2-3] : 各级RS的读时钟正反沿配置。
                                                               0：正常时钟。
                                                               1：反相时钟。 */
        unsigned int  apb_wr_buf_bypass    : 1;  /* bit[4]   : APB write buf bypass。
                                                               0：no bypass。
                                                               1：bypass。 */
        unsigned int  common_ctrl          : 27; /* bit[5-31]: DDRPHY的预留控制接口。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  common_ctrl2 : 32; /* bit[0-31]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  common_ctrl3 : 32; /* bit[0-31]: 保留。 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : RW_MID 寄存器结构定义。地址偏移量:0x0078，初值:0x00000000，宽度:32
 寄存器说明: 读写mid配置寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 结构名    : SOC_EXMBIST_DDRSYS_APB_SECURE_UNION
 结构说明  : DDRSYS_APB_SECURE 寄存器结构定义。地址偏移量:0x0100，初值:0x00000001，宽度:32
 寄存器说明: DDR子系统的APB安全权限配置。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  ddrsys_apb_secure : 1;  /* bit[0]   : DDR子系统内配置寄存器的APB安全权限配置：
                                                            0：non-secure，允许安全或非安全APB访问；
                                                            1：secure，只允许安全APB访问。
                                                            注意：
                                                            1.此寄存器本身，及DMSS中SEC功能相关的配置寄存器，始终是只允许安全APB访问；
                                                            2.当无权限的非法APB访问时，将丢弃写数据或返回全零读数据，并且返回pslverr为1。 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留。 */
    } reg;
