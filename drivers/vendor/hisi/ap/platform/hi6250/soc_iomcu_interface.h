...
/* 寄存器说明：对比特写1，使能IOMCU_CLKSTAT0对应的时钟；写0不影响时钟使能的状态寄存器,IOMCU分区时钟使能_0寄存器。
   位域定义UNION结构:  SOC_IOMCU_CLKEN0_UNION */
#define SOC_IOMCU_CLKEN0_ADDR(base)                   ((base) + (0x010))

/* 寄存器说明：对比特写1，使能IOMCU_CLKSTAT0对应的时钟；写0不影响时钟使能的状态的寄存器,IOMCU分区时钟去使能_0寄存器。
   位域定义UNION结构:  SOC_IOMCU_CLKDIS0_UNION */
#define SOC_IOMCU_CLKDIS0_ADDR(base)                  ((base) + (0x014))

/* 寄存器说明：由IOMCU_CLKEN0和IOMCU_CLKDIS0配置进行状态修改。
            外设时钟控制信号需经过在CRG中进行同步处理。因此，这里的寄存器值在延迟相应时钟域的几个周期后才真正反应时钟状态。
            此寄存器还影响到防挂死逻辑，当时钟关闭时且防挂死不bypass情况下，防挂死逻辑生效，内部总线返回Pslverr。
            每个Bit含义：
            0：时钟关闭；
            1：时钟打开；
            IOMCU分区时钟状态_0寄存器。
...
/* 寄存器说明：用于使能IOMCU_RSTSTAT0复位。对比特写1，对应IP进入复位状态；写0不影响复位状态。IOMCU分区复位使能_0寄存器。
   位域定义UNION结构:  SOC_IOMCU_RSTEN0_UNION */
#define SOC_IOMCU_RSTEN0_ADDR(base)                   ((base) + (0x020))

/* 寄存器说明：用于使能IOMCU_RSTSTAT0复位。对比特写1，对应IP进入解复位状态；写0不影响复位状态。IOMCU分区复位使能_0寄存器。
   位域定义UNION结构:  SOC_IOMCU_RSTDIS0_UNION */
#define SOC_IOMCU_RSTDIS0_ADDR(base)                  ((base) + (0x024))

/* 寄存器说明：由SC_PERIPH_RSTEN4和SC_PERIPH_RSTDIS4配置进行状态修改。解复位控制信号需经过同步处理，因此，这里的寄存器值在延迟相应时钟域的几个周期后才真正反应解复位状态。
            此寄存器还影响到防挂死逻辑，当相关逻辑处于复位状态时且防挂死不bypass情况下，防挂死逻辑生效,内部总线返回Pslverr。
            每个Bit含义：
            0：复位不使能；
            1：复位使能的寄存器。ALWAYSON分区设复位状态_4寄存器。
...
/* 寄存器说明：NOC 相关信号的控制寄存器。
   位域定义UNION结构:  SOC_IOMCU_NOC_CTRL_UNION */
#define SOC_IOMCU_NOC_CTRL_ADDR(base)                 ((base) + (0x088))

/* 寄存器说明：M7访问HIFI_DTCM的滑动窗口设置寄存器。
   位域定义UNION结构:  SOC_IOMCU_HIFI_DTCM_ACCESS_WINDOW_UNION */
#define SOC_IOMCU_HIFI_DTCM_ACCESS_WINDOW_ADDR(base)  ((base) + (0x08C))

/* 寄存器说明：对比特写1，使能IOMCU_CLKSTAT0对应的时钟；写0不影响时钟使能的状态。IOMCU分区时钟使能_1寄存器。
...
/* 寄存器说明：M7访问OCRAM的滑动窗口设置寄存器。
   位域定义UNION结构:  SOC_IOMCU_OCRAM_ACCESS_WINDOW_UNION */
#define SOC_IOMCU_OCRAM_ACCESS_WINDOW_ADDR(base)      ((base) + (0x09C))

/* 寄存器说明：安全TZPC位（绝对安全区域）0表示安全，1表示非安全寄存器。
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  iomcu_clken0_28mmbuf       : 1;  /* bit[28]: 28mmbuf_bus */
        unsigned int  iomcu_clken0_29ao_bus      : 1;  /* bit[29]: 29ao_bus */
...
#define SOC_IOMCU_CLKEN0_reverse_START                     (4)
#define SOC_IOMCU_CLKEN0_reverse_END                       (4)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reversedis                  : 1;  /* bit[4] :  */
        ...
        unsigned int  iomcu_clkdis0_28mmbuf       : 1;  /* bit[28]: 28mmbuf_bus */
        unsigned int  iomcu_clkdis0_29ao_bus      : 1;  /* bit[29]: 29ao_bus */
...
#define SOC_IOMCU_CLKDIS0_reversedis_START                   (4)
#define SOC_IOMCU_CLKDIS0_reversedis_END                     (4)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reversestat                  : 1;  /* bit[4] :  */
        ...
        unsigned int  iomcu_clkstat0_28mmbuf       : 1;  /* bit[28]: 28mmbuf_bus */
        unsigned int  iomcu_clkstat0_29ao_bus      : 1;  /* bit[29]: 29ao_bus */
...
#define SOC_IOMCU_CLKSTAT0_reversestat_START                   (4)
#define SOC_IOMCU_CLKSTAT0_reversestat_END                     (4)
...
/*****************************************************************************
 ...
 结构说明  : RSTSTAT0 寄存器结构定义。地址偏移量:0x028，初值:0xC8847FFF，宽度:32
 寄存器说明: 由SC_PERIPH_RSTEN4和SC_PERIPH_RSTDIS4配置进行状态修改。解复位控制信号需经过同步处理，因此，这里的寄存器值在延迟相应时钟域的几个周期后才真正反应解复位状态。
            此寄存器还影响到防挂死逻辑，当相关逻辑处于复位状态时且防挂死不bypass情况下，防挂死逻辑生效,内部总线返回Pslverr。
            每个Bit含义：
            0：复位不使能；
...
/*****************************************************************************
 ...
 结构说明  : M7_CFG0 寄存器结构定义。地址偏移量:0x030，初值:0x0000000F，宽度:32
 寄存器说明: IOMCU M7配置寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  hclken_sleeping_bypass : 1;  /* bit[1]   : 针对于hclk_en:当M7处于sleeping态时，是否自动关闭M7的hclk时钟
                                                                 0：自动关闭M7的hclk时钟
                                                                 1：不自动关闭M7的hclk时钟 */
        unsigned int  hifi_dtcm_remap_bypass : 1;  /* bit[2]   : 对hifi_dtcm访问的remap操作是否bypass
                                                                 0：不bypass,对hifi_dtcm的访问remap到ocram。
                                                                 1：bypass,对hifi_dtcm的访问remap到DDR。 */
        unsigned int  ocram_remap_bypass     : 1;  /* bit[3]   : 对ocram访问的remap操作是否bypass
                                                                 0：不bypass,对ocram的访问remap到ocram。
                                                                 1：bypass,对ocram的访问remap到DDR。 */
        ...
        unsigned int  cpu_iadbgprot          : 1;  /* bit[31]  : ??? */
    } reg;
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  m7_axi_bypass        : 1;  /* bit[3]    : m7 axi口投票给Sctrl决定是否关闭AO_BUS/MMBuf对应的bypass信号，0代表Bypass,即不使用Nopending信号，直接AO降频MMBUF关时钟。 */
        unsigned int  m7_ahb_bypass        : 1;  /* bit[4]    : m7 ahb口投票给Sctrl决定是否关闭AO_BUS/MMBuf对应的bypass信号，0代表Bypass,即不使用Nopending信号，直接AO降频MMBUF关时钟。 */
        unsigned int  m7_dma_bypass        : 1;  /* bit[5]    : m7 dma口投票给Sctrl决定是否关闭AO_BUS/MMBuf对应的bypass信号，0代表Bypass,即不使用Nopending信号，直接AO降频MMBUF关时钟。 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  mem_ctrl_bank0     : 3;  /* bit[0-2]  : bank0 mem功耗控制
                                                              ...
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank1     : 3;  /* bit[3-5]  : bank1 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank2     : 3;  /* bit[6-8]  : bank2 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank3     : 3;  /* bit[9-11] : bank3 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank4     : 3;  /* bit[12-14]: bank4 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank5     : 3;  /* bit[15-17]: bank5 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_bank6     : 3;  /* bit[18-20]: bank6 mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
        unsigned int  mem_ctrl_icache    : 3;  /* bit[21-23]: icache mem功耗控制
                                                              [2]:SD
                                                              [1]:DSLP
                                                              [0]:SLP */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  gt_clk_mmbuf : 1;  /* bit[6]   : 指示mmbuf是否已经被门控
                                                       1'b1表示MMBUF门控使能
                                                       1'b0表示MMBUF门控不使能 */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  sys_wakeup_int_raw : 1;  /* bit[9]    : 外部系统从其他状态进入到Wake状态的中断状态 */
        unsigned int  sys_sleep_int_raw  : 1;  /* bit[10]   : 外部系统从其他状态进入到Sleep状态的中断状态 */
        unsigned int  st_harq_int_raw    : 1;  /* bit[11]   : Harq Mem状态的原始中断状态
                                                              1'b1 表示Harq被MDM占用
                                                              1'b0 表示Harq未被占用 */
...
#define SOC_IOMCU_COMB_INT_RAW_st_harq_int_raw_START     (11)
#define SOC_IOMCU_COMB_INT_RAW_st_harq_int_raw_END       (11)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  sys_wakeup_int_msk : 1;  /* bit[11]   : 外部系统从其他状态进入到Wake状态的中断屏蔽 */
        unsigned int  sys_wakeup_int_clr : 1;  /* bit[12]   : 外部系统从其他状态进入到Wake状态的中断清除 */
        unsigned int  sys_sleep_int_msk  : 1;  /* bit[13]   : 外部系统从其他状态进入到Sleep状态的中断屏蔽 */
        unsigned int  sys_sleep_int_clr  : 1;  /* bit[14]   : 外部系统从其他状态进入到Sleep状态的中断清除 */
        unsigned int  st_harq_int_msk    : 1;  /* bit[15]   : Harq Mem状态的原始中断状态屏蔽 */
        unsigned int  st_harq_int_clr    : 1;  /* bit[16]   : Harq Mem状态的原始中断状态清除 */
...
#define SOC_IOMCU_COMB_INT_MSK_st_harq_int_msk_START     (15)
#define SOC_IOMCU_COMB_INT_MSK_st_harq_int_msk_END       (15)
#define SOC_IOMCU_COMB_INT_MSK_st_harq_int_clr_START     (16)
#define SOC_IOMCU_COMB_INT_MSK_st_harq_int_clr_END       (16)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  sys_wakeup_int_mskd : 1;  /* bit[9]    : 外部系统从到其他状态进入Wake屏蔽后的中断状态 */
        unsigned int  sys_sleep_int_mskd  : 1;  /* bit[10]   : 外部系统从到其他状态进入Sleep屏蔽后的中断状态 */
        unsigned int  st_harq_int_mskd    : 1;  /* bit[11]   : Harq Mem状态的屏蔽后的中断状态 */
        unsigned int  reserved            : 20; /* bit[12-31]: reserved */
    } reg;
...
#define SOC_IOMCU_COMB_INT_STAT_st_harq_int_mskd_START     (11)
#define SOC_IOMCU_COMB_INT_STAT_st_harq_int_mskd_END       (11)
...
/*****************************************************************************
 结构名    : SOC_IOMCU_HIFI_DTCM_ACCESS_WINDOW_UNION
 结构说明  : HIFI_DTCM_ACCESS_WINDOW 寄存器结构定义。地址偏移量:0x08C，初值:0x00000000，宽度:32
 寄存器说明: M7访问HIFI_DTCM的滑动窗口设置寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  hifi_dtcm_access_window : 32; /* bit[0-31]: M7对96KB HIFI_DTCM空间访问时，所加的滑动访问窗口，配置范围在0x6000_0000~0x7FFF_FFFF。 */
    } reg;
...
/*****************************************************************************
 结构名    : SOC_IOMCU_OCRAM_ACCESS_WINDOW_UNION
 结构说明  : OCRAM_ACCESS_WINDOW 寄存器结构定义。地址偏移量:0x09C，初值:0x00000000，宽度:32
 寄存器说明: M7访问OCRAM的滑动窗口设置寄存器。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  ocram_access_window : 32; /* bit[0-31]: M7对128KB OCRAM空间访问时，所加的滑动访问窗口，配置范围在0x6000_0000~0x7FFF_FFFF。 */
    } reg;
