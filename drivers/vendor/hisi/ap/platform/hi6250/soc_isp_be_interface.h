...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  ccir_seq         : 2;  /* bit[2-3]  : indicates the Y,Cb,Cr sequencing to manage upsampling in the UVDEC module 
                                                            ...
                                                            10- CbYCrY 
                                                            11- CrYCbY
                                                            [Note] Only 0b00 is supported in current v100 integration[end] */
        ...
        unsigned int  uvup_enable      : 1;  /* bit[10]   : uvup ON/OFF. Note that when Disabled the accept is valid on the on input video port
                                                            0 OFF
                                                            1 ON */
        unsigned int  reserved_0       : 1;  /* bit[11]   : Reserved - input stream on ISP_BE can be selected to come from ISP_FE/STREAM_ROUTER or from DRAM (VPM_33_x)
...
typedef union
{
    ...
    struct
    {
        unsigned int  ratio    : 5;  /* bit[0-4] : The throughput ratio between the ISP capability of the pipe and the use case requirement is indicated as a decimal value on 4 bit. 1 = 0.h10.the increment as 1/16 granularity
                                                   ...
                                                   0x1x : The effect of ISP_SLOWDOW is enabled. The system behaves as a bypass until the ISP_SLOWDOW becomes active. Then, the bandwidth is reduced with the ratio defined by x. Note that if x = 0 there is no reduction.
                                                   [Note] The behavior of the PTREG depends on the input ISP_SLOWDOWN signal[end] */
...
/*****************************************************************************
 ...
 结构说明  : FORCE_CLK_ON_CFG 寄存器结构定义。地址偏移量:0x00A0，初值:0x0001E7FF，宽度:32
 寄存器说明: used to force the clock which is generally controlled by HW
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_0      : 1;  /* bit[11]   : TNR_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON */
        unsigned int  reserved_1      : 1;  /* bit[12]   : IE_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON */
        ...
        unsigned int  force_clk_on_14 : 1;  /* bit[14]   : LSC top logic clock is controlled by idle_detector by default. If set to '1', this will force the clock ON (clock also used by idle_detector)  */
        unsigned int  force_clk_on_15 : 1;  /* bit[15]   : DPCC top logic clock is controlled by idle_detector by default. If set to '1', this will force the clock ON (clock also used by idle_detector)  */
        unsigned int  force_clk_on_16 : 1;  /* bit[16]   : RAWNF_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON */
...
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_14_START  (14)
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_14_END    (14)
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_15_START  (15)
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_15_END    (15)
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_16_START  (16)
#define SOC_ISP_BE_FORCE_CLK_ON_CFG_force_clk_on_16_END    (16)
