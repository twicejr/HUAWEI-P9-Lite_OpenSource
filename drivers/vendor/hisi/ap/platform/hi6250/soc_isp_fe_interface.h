...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0     : 1;  /* bit[0]   : reserved - Select if the stream comes from the Bayer Scaler BAS_(x) or from the RAWNF_(x). Used to drive the SELECT input of VPM2TO1_31_(x)
                                                         0 -> From BAS_(x)
                                                         1 -> From RAWNF_(x) */
        unsigned int  reserved_1     : 2;  /* bit[1-2] : Reserved for extra VP mux related to ISP_FE */
        ...
        unsigned int  sensor_ir      : 1;  /* bit[6]   : Sensor bayer pattern has a IR channel in place of one of the green GR or GB
                                                         0: No IR channel
                                                         1: One IR channel  */
        unsigned int  ff_before_pdaf : 1;  /* bit[7]   : Select connection from FE to DRAM for the Full Frame before/after PDAF of ISP_FE_x (VPM_52_1)
                                                         0 -> DPAF output connected to FBCRAW_1_1 ( Front end outputs full frame raw image with PDAF pixel correction)
                                                         1 -> GAMMA output connected to FBCRAW_1_1 ( Front end outputs full frame raw image without PDAF pixel correction ) */
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicate position of first pixel on the left for the acquisition window, 0 means no left crop.)
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicates position of first line on the top for the acquisition window,, 0 means no top crop)
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE inform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicates the number of horizontal samples minus 1 of the acquisition window (one = 2 samples = 2 raw pixels))
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE inform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Indicates the number of vertical samples minus 1 of the acquisition window (one = 2 samples = 2 raw pixels))
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE inform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicate position of first pixel on the left for the effective output window, 0 means no left crop.)
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicates position of first line on the top for the effective output window,, 0 means no top crop)
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE outform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Indicates the number of horizontal minus 1 samples of the effective output window (one = 2 samples = 2 raw pixels))
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE outform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  reserved_0: 13; /* bit[0-12] : Reserved (Indicates the number of vertical samples minus 1 of the effective output window (one = 2 sample = 2 raw pixels))
                                                     SHALL BE SHADOWED INSIDE THE SUB_MODULE outform.v */
        unsigned int  reserved_1: 19; /* bit[13-31]: Reserved */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : FORCE_CLK_ON_CFG 寄存器结构定义。地址偏移量:0x0050，初值:0x000003b3，宽度:32
 寄存器说明: used to force the clock which is generally controlled by HW.
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_0     : 1;  /* bit[2]    : Reserved (LSC_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON) */
        unsigned int  reserved_1     : 1;  /* bit[3]    : Reserved (DPCC_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON) */
        ...
        unsigned int  reserved_2     : 1;  /* bit[6]    : Reserved (RAWNF_(x) clock is controlled by idle_detector by default. If set to '1', this will force the clock ON) */
        ...
        unsigned int  force_clk_on_9 : 1;  /* bit[9]    : PDAF clock is controlled by idle_detector by default. If set to '1', this will force the clock ON (clock also used by idle_detector)  */
        unsigned int  reserved_3     : 22; /* bit[10-31]: Reserved */
    } reg;
