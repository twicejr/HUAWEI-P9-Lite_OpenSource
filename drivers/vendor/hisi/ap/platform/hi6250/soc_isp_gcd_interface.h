...
/*****************************************************************************
 ...
 结构说明  : GKM_IHLEFT 寄存器结构定义。地址偏移量:0x0004，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  gkm_crop_left : 13; /* bit[0-12] : Crop Left
                                                         [Note]gkm_crop_left must be even[end] */
...
/*****************************************************************************
 ...
 结构说明  : GKM_IVTOP 寄存器结构定义。地址偏移量:0x0008，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  gkm_crop_top : 13; /* bit[0-12] : Crop Top
                                                        [Note]gkm_crop_top must be even[end] */
        unsigned int  reserved     : 19; /* bit[13-31]:  */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  gkm_crop_right : 13; /* bit[0-12] : Crop Right
                                                          [Note]gkm_crop_right minus gkm_crop_left must be greater than 14[end] */
        unsigned int  reserved       : 19; /* bit[13-31]:  */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  gkm_crop_bottom : 13; /* bit[0-12] : Crop Bottom
                                                           [Note]gkm_crop_bottom minus gkm_crop_top must be greater than 6[end] */
        unsigned int  reserved        : 19; /* bit[13-31]:  */
    } reg;
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  fcr_cho  : 3;  /* bit[3-5]  : range[0,6] */
...
typedef union
{
    ...
    struct
    {
        unsigned int  cac_cen1 : 14; /* bit[0-13] : s14.0[Noteconstraint is as follows:
                                                    if(cac_cen1>=8192) { (16384-cac_cen1+img_width)<=8191; }[end] */
        unsigned int  reserved_0: 2;  /* bit[14-15]:  */
        unsigned int  cac_cen2 : 14; /* bit[16-29]: s14.0[Note]constraint is as follows:
                                                    if(cac_cen2>=8192) { (16384-cac_cen2+img_height)<=8191; }[end] */
...
#define SOC_ISP_GCD_CAC_CEN_cac_cen2_END    (29)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  cac_cho2 : 2;  /* bit[2-3]  : [Note]if(cac_ena3==1) cac_cho2<=2;[end] */
        unsigned int  cac_cho3 : 3;  /* bit[4-6]  : range[0,6] */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  x_ns     : 4;  /* bit[8-11] : range[0,8][Note]
                                                    some constraints are as follows:
                                                    int cen1_max;
                                                    if(cac_cen1<8192) //it means that the optical center on the right side of current pipe's orgin
                                                    { 
                                                     if(cac_cen1>=img_width/2) //it means that the optical center on the right side of the center of current pipe.
                                                     cen1_max = cac_cen1;
                                                     else //it means that the optical center on the left side of the center of current pipe. 
                                                     cen1_max = img_width - cac_cen1;
                                                    } 
                                                    else //it means that the optical center on the left side of current pipe's orgin.
                                                    {
                                                     cen1_max = 16384 - cac_cen1 + img_width;
                                                    }
                                                    
                                                    if(cen1_max>4095) x_ns = 8;
                                                    else if(cen1_max>2047) x_ns >= 7;
                                                    else if(cen1_max>1023) x_ns >= 6;
                                                    else if(cen1_max>511) x_ns >= 5;
                                                    else if(cen1_max>255) x_ns >= 4;
                                                    else if(cen1_max>127) x_ns >= 3;
                                                    else if(cen1_max>63) x_ns >= 2;
                                                    else if(cen1_max>31) x_ns >= 1;
                                                    else x_ns = 0;
                                                    [end] */
        unsigned int  reserved_1: 4;  /* bit[12-15]:  */
        unsigned int  y_nf     : 5;  /* bit[16-20]:  */
        unsigned int  reserved_2: 3;  /* bit[21-23]:  */
        unsigned int  y_ns     : 4;  /* bit[24-27]: range[0,8][Note]
                                                    some constraints are as follows:
                                                    [Note]
                                                    some constraints are as follows:
                                                    int cen2_max;
                                                    if(cac_cen2<8192) //it means that the optical center under current pipe's orgin
                                                    { 
                                                     if(cac_cen2>=img_height/2) //it means that the optical center under the center of current pipe.
                                                     cen2_max = cac_cen2;
                                                     else //it means that the optical center above the center of current pipe. 
                                                     cen2_max = img_height - cac_cen2;
                                                    } 
                                                    else //it means that the optical center above current pipe's orgin.
                                                    {
                                                     cen2_max = 16384 - cac_cen2 + img_height;
                                                    }
                                                    
                                                    if(cen2_max>4095) x_ns = 8;
                                                    else if(cen2_max>2047) x_ns >= 7;
                                                    else if(cen2_max>1023) x_ns >= 6;
                                                    else if(cen2_max>511) x_ns >= 5;
                                                    else if(cen2_max>255) x_ns >= 4;
                                                    else if(cen2_max>127) x_ns >= 3;
                                                    else if(cen2_max>63) x_ns >= 2;
                                                    else if(cen2_max>31) x_ns >= 1;
                                                    else x_ns = 0;
                                                    [end] */
...
typedef union
{
    ...
    struct
    {
        unsigned int  lsc_table_active : 1;  /* bit[0-0] : 0: LSC_LUT_0 is used by the HW
                                                           1: LSC_LUT_1 is used by the HW */
        unsigned int  reserved         : 31; /* bit[1-31]:  */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  lcsgain  : 11; /* bit[0-10] : range[512,2047] */
        unsigned int  reserved : 21; /* bit[11-31]:  */
    } reg;
