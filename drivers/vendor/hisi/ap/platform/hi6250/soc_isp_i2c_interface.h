...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  RX_FIFO_FULL_HLD_CTRL     : 1;  /* bit[9]    : This bit controls whether DW_apb_i2c should hold the bus when the Rx FIFO is physically full to its RX_BUFFER_DEPTH, as described in the IC_RX_FULL_HLD_BUS_EN parameter.
                                                                     ...
                                                                     Reset value: 0x0 */
        unsigned int  STOP_DET_IF_MASTER_ACTIVE : 1;  /* bit[10]   : In Master mode
                                                                     ■ 1’b1: Issues the STOP_DET interrupt only when the
                                                                     master is active
...
#define SOC_ISP_I2C_CON_STOP_DET_IF_MASTER_ACTIVE_START  (10)
#define SOC_ISP_I2C_CON_STOP_DET_IF_MASTER_ACTIVE_END    (10)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  FIRST_DATA_BYTE : 1;  /* bit[11]   : Indicates the first data byte received after the address phase for receive
...
#define SOC_ISP_I2C_DATA_CMD_FIRST_DATA_BYTE_START  (11)
#define SOC_ISP_I2C_DATA_CMD_FIRST_DATA_BYTE_END    (11)
...
typedef union
{
    ...
    struct
    {
        unsigned int  R_RX_UNDER    : 1;  /* bit[0]    : Set if the processor attempts to read the receive buffer when it is empty by
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_RX_OVER     : 1;  /* bit[1]    : Set if the receive buffer is completely filled to IC_RX_BUFFER_DEPTH and
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_RX_FULL     : 1;  /* bit[2]    : Set when the receive buffer reaches or goes above the RX_TL threshold in the
                                                         ...
                                                         continues.
                                                         Reset value: 0x0 */
        unsigned int  R_TX_OVER     : 1;  /* bit[3]    : Set during transmit if the transmit buffer is filled to IC_TX_BUFFER_DEPTH
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_TX_EMPTY    : 1;  /* bit[4]    : The behavior of the TX_EMPTY interrupt status differs based on the
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_RD_REQ      : 1;  /* bit[5]    : This bit is set to 1 when DW_apb_i2c is acting as a slave and another I2C
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_TX_ABRT     : 1;  /* bit[6]    : This bit indicates if DW_apb_i2c, as an I2C transmitter, is unable to complete
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_RX_DONE     : 1;  /* bit[7]    : When the DW_apb_i2c is acting as a slave-transmitter, this bit is set to 1 if the
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_ACTIVITY    : 1;  /* bit[8]    : This bit captures DW_apb_i2c activity and stays set until it is cleared. There
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_STOP_DET    : 1;  /* bit[9]    : Indicates whether a STOP condition has occurred on the I2C interface
                                                         ...
                                                         Reset value: 0x0 */
        unsigned int  R_START_DET   : 1;  /* bit[10]   : Indicates whether a START or RESTART condition has occurred on the I2C
                                                         interface regardless of whether DW_apb_i2c is operating in slave or master
                                                         mode. */
        unsigned int  R_GEN_CALL    : 1;  /* bit[11]   : Set only when a General Call address is received and it is acknowledged. It
        ...
        unsigned int  R_RESTART_DET : 1;  /* bit[12]   : Indicates whether a RESTART condition has occurred on the I2C interface
        ...
        unsigned int  R_MST_ON_HOLD : 1;  /* bit[13]   : Indicates whether a master is holding the bus and the Tx FIFO is empty.
...
typedef union
{
    ...
    struct
    {
        unsigned int  CLR_RX_OVER : 1;  /* bit[0]   : Read this register to clear the RX_OVER interrupt (bit 1) of the
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  ABORT        : 1;  /* bit[1]   : When set, the controller initiates the transfer abort.
                                                       ...
                                                       Reset value: 0x0 */
        unsigned int  TX_CMD_BLOCK : 1;  /* bit[2]   : In Master mode
                                                       ■ 1’b1: Blocks the transmission of data on I2C bus even if Tx FIFO has data
                                                       to transmit.
...
#define SOC_ISP_I2C_ENABLE_TX_CMD_BLOCK_START  (2)
#define SOC_ISP_I2C_ENABLE_TX_CMD_BLOCK_END    (2)
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  SLV_ACTIVITY           : 1;  /* bit[6]    : Slave FSM Activity Status. When the Slave Finite State Machine (FSM) is not in the IDLE state, this bit is set.
                                                                  ...
                                                                  Reset value: 0x0 */
        unsigned int  MST_HOLD_TX_FIFO_EMPTY : 1;  /* bit[7]    : If the IC_EMPTYFIFO_HOLD_MASTER_EN parameter is set to 1,
        ...
        unsigned int  MST_HOLD_RX_FIFO_FULL  : 1;  /* bit[8]    : This bit indicates the BUS Hold in Master mode due to Rx FIFO is
                                                                  Full and additional byte has been received (This kind of Bus hold is
                                                                  applicable if IC_RX_FULL_HLD_BUS_EN is set to 1).
                                                                  Reset value: 0x0
                                                                  Dependencies: This Register bit value is applicable only when
                                                                  IC_STAT_FOR_CLK_STRETCH=1 */
        unsigned int  SLV_HOLD_TX_FIFO_EMPTY : 1;  /* bit[9]    : This bit indicates the BUS Hold in Slave mode for the Read request
                                                                  when the Tx FIFO is empty. The Bus is in hold until the Tx FIFO has
                                                                  data to Transmit for the read request.
                                                                  Reset value: 0x0
                                                                  Dependencies: This Register bit value is applicable only when
                                                                  IC_STAT_FOR_CLK_STRETCH=1. */
        unsigned int  SLV_HOLD_RX_FIFO_FULL  : 1;  /* bit[10]   : This bit indicates the BUS Hold in Slave mode due to the Rx FIFO
...
#define SOC_ISP_I2C_STATUS_MST_HOLD_TX_FIFO_EMPTY_START  (7)
#define SOC_ISP_I2C_STATUS_MST_HOLD_TX_FIFO_EMPTY_END    (7)
#define SOC_ISP_I2C_STATUS_MST_HOLD_RX_FIFO_FULL_START   (8)
#define SOC_ISP_I2C_STATUS_MST_HOLD_RX_FIFO_FULL_END     (8)
#define SOC_ISP_I2C_STATUS_SLV_HOLD_TX_FIFO_EMPTY_START  (9)
#define SOC_ISP_I2C_STATUS_SLV_HOLD_TX_FIFO_EMPTY_END    (9)
#define SOC_ISP_I2C_STATUS_SLV_HOLD_RX_FIFO_FULL_START   (10)
#define SOC_ISP_I2C_STATUS_SLV_HOLD_RX_FIFO_FULL_END     (10)
...
/*****************************************************************************
 ...
 结构说明  : COMP_VERSION 寄存器结构定义。地址偏移量:0x00f8，初值:0x3132322A，宽度:32
 寄存器说明: I2C Component Version Register.
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
