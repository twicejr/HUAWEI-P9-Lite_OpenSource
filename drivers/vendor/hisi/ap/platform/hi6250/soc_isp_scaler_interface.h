...
typedef union
{
    ...
    struct
    {
        unsigned int  ihright  : 29; /* bit[0-28] : Position of last pixel in active area. Qu13.16 (Qu n.m )
                                                    0x1FFFFFFF means no crop,otherwise:
                                                    0:first pixel in input stream (n=0, m=0)
                                                    0x8000: first half pixel (n=0,m=0,5)
                                                    0x10000:second pixel in input stream
                                                    n.m:(n+1),m th pixel in input stream
                                                    [Note]For correct rescaling we should have ihright = ihleft+hinc ( ohsize-1)[end] */
        unsigned int  reserved : 3;  /* bit[29-31]: reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ivbottom : 29; /* bit[0-28] : Position of last line in active area. Qu13.16
                                                    ...
                                                    n.m:(n+1),m th line in input stream
                                                    [Note]For correct rescaling we should have ivbottom=ivtop+vinc(ovsize-1)[end] */
        unsigned int  reserved : 3;  /* bit[29-31]: reserved */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  ihinc    : 20; /* bit[0-19] : Horizontal increment (Qu4.16) for Luma.Ratio= Input Size/output Size. IHINC = 2^16 * ratio
                                                    0x8000 means upscaling by 2 and ratio = 0.5
                                                    0x10000 means ratio 1
                                                    0x20000 means downscaling by 2 and ratio=2  */
...
typedef union
{
    ...
    struct
    {
        unsigned int  ivinc        : 20; /* bit[0-19] : Vertical increment (Qu4.16) for Luma.Ratio= Input Size/output size. IVINC = 2^16 * ratio
                                                        0x8000 means upscaling by 2 and ratio = 0.5
                                                        0x10000 means ratio 1
                                                        0x20000 means downscaling by 2 and ratio=2 */
        unsigned int  halflinemode : 1;  /* bit[20-20]: 0 : half line mode not selected, 4-tap is used for vertical scaling
                                                        1: half line mode is selected, 6-tap is used for vertical scaling
                                                        [Note]recommended value is 1 when vertical scaling ratio is in range [1/6 1/2] ( 1/6<= ratio <= 1/2 ) AND output hsize <= LINE_SIZE/2. ( LINE_SIZE= 1024 for scaler#1, LINE_SIZE= 1024 for scaler#2 ). Otherwise half line mode = 0 [end] */
...
/*****************************************************************************
 ...
 结构说明  : FORMAT 寄存器结构定义。地址偏移量:0x0028，初值:0x000000000，宽度:32
 寄存器说明: format configuration.
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  outformat : 2;  /* bit[0-1] : Output format
                                                    ...
                                                    [Note] Scaling YUV420 semi-planar data from Memory to Memory should be performed in two passes and configured output is YUV422:
                                                    -Y is scaled in one pass with swapin=swapout=1
                                                    -UV is scaled in the other pass with swapin=swapout=0
                                                    The result is YUV420 semi-planar while configured output format is YUV422.
                                                    [end] */
        unsigned int  swapout   : 1;  /* bit[2]   : swap Y and UV at output
                                                    0: no swap
                                                    1:swap enabled
                                                    P0 pixfrag0 can be either Y or UV
                                                    P1: pixfrag1 can be either Y or UV
                                                    In fmt swapin format[0] swapout out fmt
                                                    422 P1P0 0 0:Yuv422 0 422 P1P0
                                                    422 P1P0 1 0:Yuv422 0 422 P0P1
                                                    422 P1P0 0 0:Yuv422 1 422 P0P1
                                                    422 P1P0 1 0:Yuv422 1 422 P1P0
                                                    420 Y 1 0:Yuv422 1 420 Y/monochrome
                                                    420 UV 0 0:Yuv422 0 420 UV
                                                    422 P1P0 0 1:Yuv420 0 420 P1P0
                                                    422 P1P0 1 1:Yuv420 0 420 P0P1
                                                    422 P1P0 0 1:Yuv420 1 420 P0P1
                                                    422 P1P0 1 1:Yuv420 1 420 P1P0
                                                      */
        unsigned int  swapin    : 1;  /* bit[3]   : swap Y and UV at input
                                                    0: no swap
                                                    1: swap enabled
                                                    P0 pixfrag0 can be either Y or UV
                                                    P1: pixfrag1 can be either Y or UV
