...
#define SOC_ISP_STATDIS_SRAM2_ADDR(base, Range1152)   ((base) + (0x1000+0x4*(Range1152)))

/* 寄存器说明：Y projection of current frame
   位域定义UNION结构:  SOC_ISP_STATDIS_SRAM4_UNION */
#define SOC_ISP_STATDIS_SRAM4_ADDR(base, Range648)    ((base) + (0x4000+0x4*(Range648)))
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  nb_zone  : 1;  /* bit[2-2] : 0-work on 2 zones(not supported in V110),
                                                   1-work on 4 zones */
        unsigned int  scm      : 1;  /* bit[3-3] : 0 - normal mode, 1 - not supported in V110 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  restoredone : 1;  /* bit[0-0] : 1- after memory is fully restored (not used in V110) */
...
/*****************************************************************************
 ...
 结构说明  : SRAM2 寄存器结构定义。地址偏移量:0x1000+0x4*(Range1152)，初值:0x00000000，宽度:32
 寄存器说明: X projection of current frame
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
...
/*****************************************************************************
 ...
 结构说明  : SRAM4 寄存器结构定义。地址偏移量:0x4000+0x4*(Range648)，初值:0x00000000，宽度:32
 寄存器说明: Y projection of current frame
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
