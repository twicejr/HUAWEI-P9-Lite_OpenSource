...
typedef union
{
    ...
    struct
    {
        unsigned int  mdm_dbg_ctrl : 8;  /* bit[0-7]  : modem debug（mdm2ap_status）控制信号，来自pctrl，default为8'h0；
                                                         8'h00：mdm2ap_status输出为0；
                                                         8'h01：mdm2ap_status复用模式1；
                                                         8'h02：mdm2ap_status复用模式2；
                                                         8'h03：mdm2ap_status复用模式3；
                                                         others：预留。 */
        unsigned int  spi1_cs      : 2;  /* bit[8-9]  : spi1_cs:
                                                        spi_cs[0]，片选0；
                                                        0:片选无效；
                                                        1:片选有效；
                                                        spi_cs[1]，片选1：
...
#endif
#define SOC_PCTRL_PERI_CTRL17_mdm_dbg_ctrl_START  (0)
#define SOC_PCTRL_PERI_CTRL17_mdm_dbg_ctrl_END    (7)
#define SOC_PCTRL_PERI_CTRL17_spi1_cs_START       (8)
#define SOC_PCTRL_PERI_CTRL17_spi1_cs_END         (9)
...
typedef union
{
    ...
    struct
    {
        unsigned int  g3d_drm_mode_cfg                  : 1;  /* bit[0]    : bit[0]:g3d drm mode config default:0X1
                                                                             g3d_drm_mode上的默认值为1'b0，为g3d非保护模式。
                                                                             g3d工作时，可以通过配置g3d_drm_mode_cfg从1'b0变为1'b1，使得g3d_drm_mode变为1'b1，g3d进入保护模式；
                                                                             g3d reset时，g3d_drm_mode自动变为1'b0，退出g3d保护模式。
                                                                             注意：软件需要先配置g3d_drm_mode_cfg为1'b0，再配置g3d_drm_mode_cfg为1'b1，才能使得g3d_drm_mode变为1'b1，进入g3d保护模式。 */
        ...
        unsigned int  g3d_pw_sel                        : 1;  /* bit[4]    : bit[4]:g3d_pw_sel default:0X1
                                                                             1：硬件自动控制g3d memory低功耗模式。
                                                                             0：软件配置PERI_CTRL26寄存器控制g3d memory低功耗模式。 */
        unsigned int  g3d_slp_sel                       : 1;  /* bit[5]    : bit[5]:g3d_slp_sel default:0X0
                                                                             1：在g3d_pw_sel为1的情况下，硬件自动控制g3d_memory进入deepsleep低功耗模式。
                                                                             0：在g3d_pw_sel为1的情况下，硬件控制g3d_memory始终不进入deepsleep低功耗模式。 */
        unsigned int  reserved_0                        : 3;  /* bit[6-8]  : 保留。 */
        unsigned int  gpu_striping_granule              : 3;  /* bit[9-11] : bit[11:9]:gpu_striping_granule default:0X1 ，dallas不使用，
                                                                             gpu_1to2_axi总线的交织粒度
                                                                             000：128B；001：256B；010：512B；
                                                                             011：1KB；100：2KB；101：4KB；others：reserved。 */
        unsigned int  mediabus_interleave_stripe_select : 3;  /* bit[12-14]: bit[14:12]:mediabus_interleave_stripe_select default:0X1 ，dallas不使用，
                                                                             000：128B；001：256B；010：512B；
                                                                             011：1KB；100：2KB；101：4KB；others：reserved。 */
        unsigned int  vivobus_interleave_stripe_select  : 3;  /* bit[15-17]: bit[17:15]:vivobus_interleave_stripe_select default:0X1 ，dallas不使用，
                                                                             000：128B；001：256B；010：512B；
                                                                             011：1KB；100：2KB；101：4KB；others：reserved。 */
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  pmussi1_mst_cnt          : 3;  /* bit[16-18]: bit[18:16]:pmussi1_mst_cnt default:0X1
                                                                    ...
                                                                    110：4.5 clk；
                                                                    111：5.0 clk。 */
        unsigned int  usb2_hresp_ctrl          : 1;  /* bit[19]   : usb2_hresp_ctrl
                                                                    0：no response； 1：response valid。 */
...
#define SOC_PCTRL_PERI_CTRL24_usb2_hresp_ctrl_START           (19)
#define SOC_PCTRL_PERI_CTRL24_usb2_hresp_ctrl_END             (19)
...
typedef union
{
    ...
    struct
    {
        unsigned int  peri_stat0                : 30; /* bit[0-29]: bit[29]:保留
                                                                    ...
                                                                    bit[16]:int dss0 wr transaction probe mainstatalarm ，dallas不使用，
...
typedef union
{
    ...
    struct
    {
        unsigned int  peri_stat2                 : 31; /* bit[0-30]: bit[30]:vdec service target mainnopendingtrans
                                                                     bit[29]:noc_ipf_i_mainnopendingtrans
                                                                     bit[28]:noc mmc0bus sdio0 I mainnopendingtrans ，dallas不使用
                                                                     ...
                                                                     bit[12]:noc dss0 wr i mainnopendingtrans，dallas不使用
...
typedef union
{
    ...
    struct
    {
        unsigned int  peri_stat3 : 32; /* bit[0-31]: bit[31]:intr_a7_rd_transaction_probe_mainstatalarm
                                                     ...
                                                     bit[16]:保留
                                                     bit[15]:保留
                                                     bit[14]:sc noc emmc1 i mainnopendingtrans,dallas不使用
...
typedef union
{
    ...
    struct
    {
        unsigned int  peri_stat4 : 23; /* bit[0-22] : bit[25:23]:modem_ipc_s clk state[2:0]
                                                      000：start 001：wait 010：access 011：busy
                                                      bit[22]: intr_cci400_err 
                                                      0：error 1：normal；
                                                      ...
                                                      bit[12:10]:保留
                                                      bit[9]:保留
                                                      bit[8]:g3d_drm_mode 0:g3d non secure mode 1:g3d secure mode;
                                                      bit[7]:保留
                                                      bit[6]:socp idle
                                                      0：busy 1：idle；
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  peri_stat15   : 21; /* bit[11-31]: others:reserved
                                                         [13]:vdm_clk_en_2，dallas不使用
                                                         0：clk disable 1：clk enable；
                                                         [12]:vdm_clk_en_1，dallas不使用
                                                         0：clk disable 1：clk enable；
                                                         [11]:vdm_clk_en_0，dallas不使用
                                                         0：clk disable 1：clk enable。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  peri_stat17 : 32; /* bit[0-31]: [2]:asp_dwaxi_m3_st,0:idle,1:busy
                                                      [1]:asp_dwaxi_m2_st,0:idle,1:busy
                                                      [0]:asp_dwaxi_m1_st,0:idle,1:busy
                                                      其他：保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  sdio0_mid   : 5;  /* bit[20-24]: bit[24:20]:sc_perf_stat_mid default:00011 具体配置参考总线相关说明。 */
        unsigned int  dmac_mid    : 5;  /* bit[25-29]: bit[29:25]:cssys_mid default:01110 具体配置参考总线相关说明。 */
        unsigned int  reserved    : 2;  /* bit[30-31]: 保留。 */
    } reg;
...
#define SOC_PCTRL_PERI_CTRL6_sdio0_mid_START    (20)
#define SOC_PCTRL_PERI_CTRL6_sdio0_mid_END      (24)
#define SOC_PCTRL_PERI_CTRL6_dmac_mid_START     (25)
#define SOC_PCTRL_PERI_CTRL6_dmac_mid_END       (29)
...
/*****************************************************************************
 ...
 结构说明  : PERI_CTRL10 寄存器结构定义。地址偏移量:0xC14，初值:0xCD52064F，宽度:32
 寄存器说明: 通用外设控制寄存器10。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  ddrc7_mid        : 1;  /* bit[7]    : bit[7]:ddrc7_mid default:0 具体配置参考总线相关说明,dallas 删除不用。 */
        unsigned int  ddrc8_mid        : 1;  /* bit[8]    : bit[8]:ddrc8_mid default:0 具体配置参考总线相关说明,dallas 删除不用。 */
        unsigned int  ddrc9_mid        : 1;  /* bit[9]    : bit[9]:ddrc9_mid default:1 具体配置参考总线相关说明,dallas 删除不用。 */
        unsigned int  ddrc10_mid       : 1;  /* bit[10]   : bit[10]:ddrc10_mid default:1 具体配置参考总线相关说明,dallas 删除不用。 */
        unsigned int  reserved_0       : 4;  /* bit[11-14]: 保留。 */
        unsigned int  sdio1_mid        : 5;  /* bit[15-19]: bit[19:15]:sdio1_mid default:00100 具体配置参考总线相关说明，dallas未使用。 */
...
/*****************************************************************************
 ...
 结构说明  : PERI_CTRL11 寄存器结构定义。地址偏移量:0xC18，初值:0x001B0000，宽度:32
 寄存器说明: 通用外设控制寄存器11。
*****************************************************************************/
#ifndef __SOC_H_FOR_ASM__
typedef union
{
    ...
    struct
    {
        unsigned int  peri_ctrl11_cmd                 : 16; /* bit[0-15] : mcpu boot addr 
                                                                           MODEM启动地址配置寄存器。 */
        unsigned int  ipf_mid                         : 5;  /* bit[16-20]: ipf mid 具体配置参考总线相关说明。 */
        unsigned int  sc_modem_ipc_s_auto_clk_gate_en : 1;  /* bit[21]   : sc_modem_ipc_s_auto_clk_gate_en default:0x0
                                                                           0：auto clkgate disable；1：auto clkgate enable。 */
        unsigned int  sc_modem_ipc_s_soft_gate_clk_en : 1;  /* bit[22]   : sc_modem_ipc_s_soft_gate_clk_en default:0x0
                                                                           0：clk enable；1：clk gate。 */
...
#define SOC_PCTRL_PERI_CTRL11_ipf_mid_START                          (16)
#define SOC_PCTRL_PERI_CTRL11_ipf_mid_END                            (20)
#define SOC_PCTRL_PERI_CTRL11_sc_modem_ipc_s_auto_clk_gate_en_START  (21)
#define SOC_PCTRL_PERI_CTRL11_sc_modem_ipc_s_auto_clk_gate_en_END    (21)
#define SOC_PCTRL_PERI_CTRL11_sc_modem_ipc_s_soft_gate_clk_en_START  (22)
#define SOC_PCTRL_PERI_CTRL11_sc_modem_ipc_s_soft_gate_clk_en_END    (22)
