...
#define SOC_SOCP_DEC_SRC_BUFX_WPTR_ADDR(base, x)      ((base) + (0xA00+0x40*(x)))

/* 寄存器说明：解码通路源bufferx读指针寄存器
   位域定义UNION结构:  SOC_SOCP_DEC_SRC_BUFX_RPTR_UNION */
#define SOC_SOCP_DEC_SRC_BUFX_RPTR_ADDR(base, x)      ((base) + (0xA04+0x40*(x)))

/* 寄存器说明：解码通路源bufferx起始地址寄存器
   位域定义UNION结构:  SOC_SOCP_DEC_SRC_BUFX_ADDR_UNION */
#define SOC_SOCP_DEC_SRC_BUFX_ADDR_ADDR(base, x)      ((base) + (0xA08+0x40*(x)))

/* 寄存器说明：解码通路源bufferx配置寄存器
   位域定义UNION结构:  SOC_SOCP_DEC_SRC_BUFX_CFG0_UNION */
#define SOC_SOCP_DEC_SRC_BUFX_CFG0_ADDR(base, x)      ((base) + (0xA0C+0x40*(x)))

/* 寄存器说明：解码通路状态寄存器0
   位域定义UNION结构:  SOC_SOCP_DEC_BUFX_STATUS0_UNION */
#define SOC_SOCP_DEC_BUFX_STATUS0_ADDR(base, x)       ((base) + (0xA20+0x40*(x)))

/* 寄存器说明：解码通路状态寄存器1
   位域定义UNION结构:  SOC_SOCP_DEC_BUFX_STATUS1_UNION */
#define SOC_SOCP_DEC_BUFX_STATUS1_ADDR(base, x)       ((base) + (0xA24+0x40*(x)))
