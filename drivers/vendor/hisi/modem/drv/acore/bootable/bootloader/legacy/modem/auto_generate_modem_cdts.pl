...
sub my_main()
{
    ...
    foreach(@file_s)
    {
		...
		if(($_ =~ m/tools/)||($_ =~ m/auto-generate/)||($_ =~ m/\./)||($_ =~ m/\.\./)||($_ =~ m/$ARGV[1]/))
		{
			next;
		}
		#open sub dir
		...
		foreach(@file_sub)
		{
			...
			{
				$found = 1;
				system("sed -i \"s\/$ARGV[1]_modem.dtsi\/$ARGV[1]_modem_${product}.dtsi\/g\" ${auto_generate}/${new_file}.dts");
				system("sed -i \"s\/$ARGV[1]_modem.dtsi\/$ARGV[1]_modem_${product}.dtsi\/g\" ${auto_generate}/${new_file}.dtsi");
			}
		}
		closedir(SUB_DIR);
