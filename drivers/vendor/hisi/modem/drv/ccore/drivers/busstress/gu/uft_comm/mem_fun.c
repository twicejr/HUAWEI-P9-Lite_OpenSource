/**********************************************************************
 Copyright (C), 1988-1999, Huawei Tech. Co., Ltd.
  FileName: test.cpp
  Author:段立明 	 ID: 56989      Version : 1.0         Date: 2009-11-5
  Description:     function dealing with memory       
  Version:         1.0
  Function List:   // 主要函数及其功能
	1. mem_init () ; //初始化指定内存段
	2. mem_clr ()  ; //对指定内存段清零
	3. mem_copy () ; //复制指定内存段到另一位置
	4. mem_comp () ; //对比指定的两段内存段
	5. mem_test () ; //使用随机数据检查内存读写是否有错误
...
/***********************************************************
  ...
  Calls:          set32 ; set8 ; read32 ; read8 ; printf // 被本函数调用的函数清单（建议描述）
...
/***********************************************************
  ...
  Calls:          set32; read32 ; srand; rand; printf // 被本函数调用的函数清单（建议描述）
  Called By:      // 调用本函数的函数清单（建议描述）
  Table Accessed: 无// 被访问的表（此项仅对于牵扯到数据库操作的程序）
  Table Updated:  无// 被修改的表（此项仅对于牵扯到数据库操作的程序）
