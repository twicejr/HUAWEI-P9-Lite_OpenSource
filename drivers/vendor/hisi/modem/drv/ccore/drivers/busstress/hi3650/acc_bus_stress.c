...
int acc_stress_test_routine(void * par)
{
	...
	total_len = PKT_LEN * (ACC_FIFO_DEEP - 2);
	for(i = 0; i < ACC_FIFO_DEEP - 2; i++)
    {
...
#endif
int acc_stress_test_start(int task_priority,int test_type,int test_rate)
{
	int s32ret = 0;
	...
	{
		s32ret = acc_ddr_data_init(PKT_LEN, ACC_FIFO_DEEP, &pInAddr, 
													&pOutAddr,&pstCfg);
		if(s32ret)
		{
		    return ERROR;
	...
	if((DDR == test_type) ||(DDR_BBE16_AXIMEM == test_type))
	{
		s32ret = osl_task_init("accTestInDdr", task_priority, 1024, 
...
void acc_stress_test_debug(void)
{
