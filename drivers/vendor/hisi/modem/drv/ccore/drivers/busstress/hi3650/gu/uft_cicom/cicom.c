...
UINT32 ck_addr_cicom1        = 0;
UINT32 lli_prm_cicom1        = 0;
...
{	
	...
	if(ck_addr_cicom1 == 0)
	{
		ck_addr_cicom1        = (UINT32)osl_cachedma_malloc(1024);
	}
	if(lli_prm_cicom1 == 0) 
	{
		lli_prm_cicom1        = (UINT32)osl_cachedma_malloc(1024);
	}
    pdu_len = num_8*8;
    
    setbit(SYSCTRL_SCMISCCTRL, Cicom1_sel ,0x1);//select W(0) / G(1)
    
    //cacheDisable(1);/*需要确认是否要实现，zuofenghua*/??
    
    // cfg the lli_param
    set32( (lli_prm_cicom1 + 0x00000000), ck_addr_cicom1               );//CK_ADDR              
    set32( (lli_prm_cicom1 + 0x00000004), cc1                          );//CC                   
    set32( (lli_prm_cicom1 + 0x00000008), pdu_len                      );//SRC_PDU_TOTAL_LEN    
    set32( (lli_prm_cicom1 + 0x0000000c), 0x00000002                   );//CUSTOM_CFG
    set32( (lli_prm_cicom1 + 0x00000010), src_addr                     );//LLI_SRC_PDU_SEG_ADDR 
    set32( (lli_prm_cicom1 + 0x00000014), des_addr                     );//LLI_DST_TTF_SEG_ADDR
    set32( (lli_prm_cicom1 + 0x00000018), 0x00000000                   );//LLI_DST_MBX_SEG_ADDR
    set32( (lli_prm_cicom1 + 0x0000001c), 0x00000000                   );//LLI_PRM_ADDR      
 
    // cfg the key_data
    set32( (ck_addr_cicom1 + 0x00000000), ck1_1);
    set32( (ck_addr_cicom1 + 0x00000004), ck1_2);
    set32( (ck_addr_cicom1 + 0x00000008), ck1_3);
    set32( (ck_addr_cicom1 + 0x0000000c), ck1_4);
    
    //set registers
    set32(asicCICOM1PRIOR_SELECT     , 0x00000000           );
    set32(asicCICOM1LLI_PARAM_ADDR_DL, lli_prm_cicom1              );
    setbits(asicCICOM1GLOBAL_CFG_DL  , 1, 0, alg_sel        );  
    setbits(asicCICOM1GLOBAL_CFG_DL  , 11, 8, 0x0           );  
    setbits(asicCICOM1GLOBAL_CFG_DL  , 20, 16, cb           );  
    set32(asicCICOM1INT_EN_DL        , 0x00000000           );//int enable
    //set32(asicCICOMEN_DL            , 0x00000001           );//start cicom
...
{
	set32(asicCICOM1EN_DL, 0x00000001           );//start cicom
}
...
int cicom1_v500_trans_check(UINT32 src_addr, UINT32 des_addr, UINT32 num_8)
{
    ...
    {
    	//cicom_state = read32(asicCICOMEN_UL);
    	cicom_state = read32(asicCICOM1EN_DL);
    	cicom_state = cicom_state & 0x01;
    }
...
//#endif
	...
	if ( 0 == cnt3 )
	{
		printf("CICOM1 Memory Compare OK!\n"); 
	}
	else
	{
		printf("CICOM1 Memory Compare FAILED!\n");
	}
#endif	
	if (cnt3 == 0)
	{
