...
int g_busstress_test_acc_task_priority = 48;
int g_busstress_test_cipher_task_priority = 50;
int g_busstress_test_kdf_task_priority = 50;
int g_busstress_test_mdm_a9_memcpy_task_priority = 50;
int g_busstress_test_get_a9_memcpy_bandwidth_task_priority = 50;
int g_busstress_test_ipf_task_priority = 50;
...
extern int bsp_cdsp_stress_test(void);
...
extern int ipf_dl_stress_test_start(int task_priority,int dtime);
...
extern s32 edma_test_lli_loop_tcm(enum edma_req_id req);
...
u32 ddr_mem_size = 0x200000;
/* 搬移1k */
BSP_S32 a9_mem_stress_test_start_ddr2ddr(BSP_S32 s32DelayValue)
{
    /* 申请不可cache内存，避免cache操作 */
	g_stress_src_addr = osl_cachedma_malloc(ddr_mem_size);//malloc(DDR_MEM_SIZE_FOR_A9);
    //g_stress_src_addr = osl_malloc(ddr_mem_size);
    if(NULL == g_stress_src_addr)
	{
		return ERROR;
	}
	/* 申请不可cache内存，避免cache操作 */
	g_stress_dst_addr = osl_cachedma_malloc(ddr_mem_size);//malloc(DDR_MEM_SIZE_FOR_A9);
	//g_stress_dst_addr = osl_malloc(ddr_mem_size);//malloc(DDR_MEM_SIZE_FOR_A9);
	...
	a9MemcpyTaskSpawnExt((BSP_U32)g_stress_src_addr, (BSP_U32)g_stress_dst_addr,ddr_mem_size,s32DelayValue);
	return OK;
}
...
const char *g_port_res_map_str[4] = {
	"PORT0:A9_M1\n",
	"PORT1:BBP_TCM/DDR, TL BBE16, C BBE16\n",
	"PORT2:CCPU_M0\n",
	"PORT3:EDMA, EDMA1, CIPHER, GU CICOM0/1,UPACC,HDLC\n",
};

/* PORT4保留不用，PORT5用于DDR，不走AXI总线，不关注 */
enum port_we_care_with_axi{
	PORT0_A9_M1,
	PORT1_BBP_TCM_DDR_TL_BBE16_C_BBE16,
	PORT2_CCPU_M0,
	PORT3_EDMA_EDMA1_CIPHER_GU_CICOM,
	PORT_BUTTOM
};
...
void busstress_test_amon_info_task(void *para)
{
	...
	int ret= 0;
	int i = 0;
	u64 port_rd_access[PORT_BUTTOM] = {0};
	u64 port_wr_access[PORT_BUTTOM] = {0};
	...
	for (;;)
	{
		...
		SRE_Printf("******************** 50MHz*64bit*0.8 in theory on porting\n");
		/* 打印本次的各资源的读写带宽及利用率 */
		...
		/* port1=res0+res1+res2 */
		port_rd_access[1] = (((u64)(busstress_test_amon_cnt.soc_read_count[0].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[0].low_count)) +
		                    (((u64)(busstress_test_amon_cnt.soc_read_count[1].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[1].low_count)) +
		                    (((u64)(busstress_test_amon_cnt.soc_read_count[2].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[2].low_count));
		port_wr_access[1] = (((u64)(busstress_test_amon_cnt.soc_write_count[0].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[0].low_count)) +
                            (((u64)(busstress_test_amon_cnt.soc_write_count[1].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[1].low_count)) +
                            (((u64)(busstress_test_amon_cnt.soc_write_count[2].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[2].low_count));

		/* port2=res3*/
		port_rd_access[2] = ((u64)(busstress_test_amon_cnt.soc_read_count[3].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[3].low_count);
		port_wr_access[2] = ((u64)(busstress_test_amon_cnt.soc_write_count[3].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[3].low_count);

		/* port3=res4+res5+res6+res7*/
		port_rd_access[3] = (((u64)(busstress_test_amon_cnt.soc_read_count[4].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[4].low_count)) +
		                    (((u64)(busstress_test_amon_cnt.soc_read_count[5].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[5].low_count)) +
		                    (((u64)(busstress_test_amon_cnt.soc_read_count[6].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[6].low_count)) +
		                    (((u64)(busstress_test_amon_cnt.soc_read_count[7].hig_count) << 32) | (busstress_test_amon_cnt.soc_read_count[7].low_count));
		port_wr_access[3] = (((u64)(busstress_test_amon_cnt.soc_write_count[4].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[4].low_count)) +
                            (((u64)(busstress_test_amon_cnt.soc_write_count[5].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[5].low_count)) +
                            (((u64)(busstress_test_amon_cnt.soc_write_count[6].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[6].low_count)) +
                            (((u64)(busstress_test_amon_cnt.soc_write_count[7].hig_count) << 32) | (busstress_test_amon_cnt.soc_write_count[7].low_count));


        for (i = PORT0_A9_M1; i < PORT_BUTTOM; i++)
    	{
...
int busstress_test_show_usage(void)
{
	...
	for (i = 0; i < g_amon_info_show_cnt % AMON_INFO_SHOW_MAX_CNT; i++)
	{
		SRE_Printf("%04d %.3f%%  %.3f%%  %.3f%%  %.3f%%  ", i,
							g_res_rd_usage_times[i][AMON_RES0_BBP_DDR],
...
#ifndef RES3_FOR_PERI_MST
//#define RES3_FOR_PERI_MST
#endif
...
int busstress_test_start_monitor(int amon_info_tsk_priority)
{
	...
	busstress_test_amon_cfg.soc_config[0].port = 1;
	busstress_test_amon_cfg.soc_config[0].master_id = 0x800F8020;
#else
    /*res0:监控GU UPACC*/
	busstress_test_amon_cfg.soc_config[0].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[0].port = 3;
	busstress_test_amon_cfg.soc_config[0].master_id = 0x800F8030;
#endif
	...
	busstress_test_amon_cfg.soc_config[1].master_id = 0x000F8020;
#else
    /* res1:监控TL BBE16*/
	busstress_test_amon_cfg.soc_config[1].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[1].port = 1;
	busstress_test_amon_cfg.soc_config[1].master_id = 0x800F8040;
#endif
    /* res2:监控C BBE16*/
    busstress_test_amon_cfg.soc_config[2].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
    busstress_test_amon_cfg.soc_config[2].port = 1;
    busstress_test_amon_cfg.soc_config[2].master_id = 0x800F8060;

    /* PORT2:CCPU_M0*/
	/* res3:监控CCPU_M0*/
    busstress_test_amon_cfg.soc_config[3].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
    busstress_test_amon_cfg.soc_config[3].port = 2;
    busstress_test_amon_cfg.soc_config[3].master_id = 0;

    /* PORT3:EDMA, EDMA1, CIPHER, GU CICOM */
    /* res4:监控EDMA*/
	busstress_test_amon_cfg.soc_config[4].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[4].port = 3;
	busstress_test_amon_cfg.soc_config[4].master_id = 0x80078000;

#ifdef AXIMON_1
    /* res5:监控EDMA1(EDMA_mdm)*/
	busstress_test_amon_cfg.soc_config[5].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[5].port = 3;
	busstress_test_amon_cfg.soc_config[5].master_id = 0x80078010;
#else
	/* res5:监控GU CICOM1*/
	busstress_test_amon_cfg.soc_config[5].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[5].port = 3;
	busstress_test_amon_cfg.soc_config[5].master_id = 0x80078060;
#endif

	/* res6:监控CIPHER */
	busstress_test_amon_cfg.soc_config[6].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[6].port = 3;
	busstress_test_amon_cfg.soc_config[6].master_id = 0x80078020;

#ifdef AXIMON_1
	/* res7:监控GU CICOM0*/
	busstress_test_amon_cfg.soc_config[7].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[7].port = 3;
	busstress_test_amon_cfg.soc_config[7].master_id = 0x80078040;
#else
	/* res7:监控GU HDLC*/
	busstress_test_amon_cfg.soc_config[7].opt_type = (AMON_OPT_READ | AMON_OPT_WRITE);
	busstress_test_amon_cfg.soc_config[7].port = 3;
	busstress_test_amon_cfg.soc_config[7].master_id = 0x80078050;
#endif
	...
	/* 起任务查看信息 */
	ret = osl_task_init((char*)"amon_info_tsk", amon_info_tsk_priority, 0x1000, (OSL_TASK_FUNC)busstress_test_amon_info_task,
...
struct bbp_busstress_test_task_para g_bbp_busstress_test_task_para = {0xE1010000,BBP_BUSSTRESS_TEST_BBP_TRANS_SIZE,8};
...
int bbp_busstress_test_spawn_task(int priority)
{
    int ret = 0;
#if 0
    g_bbp_busstress_test_task_para.bbp_addr = 0xE1010000;
...
enum busstress_module_no{
    ...
    BUSSTRESS_TEST_CBBE16,                      /* 1 */
	BUSSTRESS_TEST_ACC,                        /* 2 *//* ACC,CIPHER,KDF，只运行ACC即可 */
	BUSSTRESS_TEST_EDMA,                       /* 3 */
	BUSSTRESS_TEST_MDM_A9_AXIMEM2AXIMEM,       /* 4 */
	BUSSTRESS_TEST_MDM_A9_BBE16DTCM2BBE16DTCM, /* 5 */
	BUSSTRESS_TEST_MDM_A9_DDR2DDR,             /* 6 */
	BUSSTRESS_TEST_IPF,                        /* 7 */
	BUSSTRESS_TEST_SOCP,                       /* 8 */
	BUSSTRESS_TEST_GU,                         /* 9 */
	BUSSTRESS_TEST_BBP,                        /* 10 */
	BUSSTRESS_TEST_EDMA_TCM,
	BUSSTRESS_TEST_BUTTOM                      /* 11 */
};

unsigned int g_busstress_test_en_run[BUSSTRESS_TEST_BUTTOM] =
{
	0,	/* BUSSTRESS_TEST_BBE16 					 */
    0,  /* BUSSTRESS_TEST_CBBE16 */
	0,	/* BUSSTRESS_TEST_ACC				      */ /* ACC,CIPHER,KDF，只运行ACC即可 */
	0,	/* BUSSTRESS_TEST_EDMA					 */
	0,	/* BUSSTRESS_TEST_MDM_A9_AXIMEM2AXIMEM		 */
	0,	/* BUSSTRESS_TEST_MDM_A9_BBE16DTCM2BBE16DTCM  */
...
int g_get_a9_memcpy_bandwidth_delay = 36000;
...
void busstress_set_module(int module_id);
void busstress_free_module(int module_id);
...
BSP_S32 busstress_test_start(int busstress_task_priority)
{
	...
	if (1 == g_busstress_test_en_run[BUSSTRESS_TEST_BBE16])
	{
        ...
        (void)bsp_bbe_load_muti();

	    bsp_bbe_memcpy_stress_test_start(enum_bbe16_move_type);

        (void)bsp_bbe_run();

		printf("tlbbe16 %s busstress test start ++++++ok\n", g_bbe16_move_type_str);
	}
	else
	{
		printf("tlbbe16 busstress no run******\n");
	}
	/* CBBE16  */
	if (1 == g_busstress_test_en_run[BUSSTRESS_TEST_CBBE16])
	{
        /* cdsp */
        (void)bsp_bbe_load(BSP_DSP_CBBE);

        (void)bsp_cdsp_stress_test();

        (void)bsp_bbe_run_ex(BSP_DSP_CBBE);
        printf("cbbe16 %s busstress test start ++++++ok\n", g_bbe16_move_type_str);

	}
    else
	{
		printf("cbbe16 busstress no run******\n");
	}
	/* ACC 即为CIPHER*/
    ...
    /* EDMA循环搬移模式，从DDR到BBE16DTCM */
	if (1 == g_busstress_test_en_run[BUSSTRESS_TEST_EDMA_TCM])
	{
		(void)edma_test_lli_loop_tcm(0x25);
		printf("ccore edma stress test start ++++++ok\n");
	}
	else
	{
		printf("edma tcm busstress no run******\n");
	}
	...
	if (1 == g_busstress_test_en_run[BUSSTRESS_TEST_SOCP])
	{
		...
		/* bbe16dtcm->ddr */
		ret = SOCP_ST_ENCODE_STRESS_TCM(g_busstress_test_socp_bbe16dtcm_to_ddr_task_priority,
