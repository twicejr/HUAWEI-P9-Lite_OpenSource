...
unsigned int g_stSocp_busstress_test_succ = 0;
unsigned int g_stSocp_busstress_test_fail = 0;
...
do{\
    if(a != b)\
    {\
        printf("%s(%d): value error... 0x%x 0x%x\n", __FUNCTION__, __LINE__, a, b);\
        return BSP_ERROR;\
    }\
}while(0)
...
BSP_S32 SocpAllocEncChan1(BSP_U32 u32ChanId)
{
    ...
    if(BSP_NULL == p)
    {
        printf("alloc chan failed, no mem\n");
        return -1;
    }
    ...
    if(BSP_OK != ret)
    {
        printf("alloc chan failed, return error(0x%x)\n", ret);
...
BSP_S32 SOCP_ST_INIT_001()
{
    ...
    printf("SOCP TEST: INIT CASE 1 PASSED\n");
...
/* 重复初始化成功, 测试之前重启单板 */
BSP_S32 SOCP_ST_INIT_002()
{
    CHECK_RET(socp_init(), BSP_OK);
    CHECK_RET(socp_init(), BSP_OK);

    printf("SOCP TEST: INIT CASE 2 PASSED\n");
    return 0;
}

BSP_U32 SOCP_ST_RESET_001()
{
    CHECK_RET(SocpResetAll(), BSP_OK);

    g_strSocpStat.bInitFlag = BSP_FALSE;

    printf("SOCP TEST: RESET CASE 1 PASSED\n");

    return 0;
}

/* 申请编码源通道，空指针检查 */
BSP_S32 SOCP_ST_ENC_SRC_001()
{
    DEV_INIT();

    CHECK_RET(bsp_socp_coder_set_src_chan(4, BSP_NULL), BSP_ERR_SOCP_NULL);

    printf("SOCP TEST: ALLOC CODER SRC CHAN CASE 1 PASSED\n");
    return 0;
}
...
BSP_S32 SOCP_ST_ENC_SRC_002(void)
{
    ...
    printf("SOCP TEST: ALLOC CODER SRC CHAN CASE 2 PASSED\n");
    return 0;
}
...
BSP_S32 SOCP_ST_ENC_SRC_003(void)
{
    ...
    printf("SOCP TEST: ALLOC CODER SRC CHAN CASE 3 PASSED\n");
    return 0;
}


/* 申请编码源通道，申请全部通道，最后无空闲通道 */
BSP_S32 SOCP_ST_ENC_SRC_004(void)
{
    BSP_U32 u32ChanId;
    SOCP_CODER_SRC_CHAN_S EncSrcAttr;
    int i;
    ...
    u32ChanId = SOCP_CHAN_DEF(SOCP_CODER_SRC_CHAN, i);
    CHECK_RET(bsp_socp_coder_set_src_chan(u32ChanId, &EncSrcAttr), BSP_ERR_SOCP_INVALID_CHAN);
    printf("set enc src chan success\n");
    for(i=SOCP_ENCSRC_CHN_BASE; i<(SOCP_ENCSRC_CHN_BASE+SOCP_ENCSRC_CHN_NUM); i++)
    {
        u32ChanId = SOCP_CHAN_DEF(SOCP_CODER_SRC_CHAN, i);
        CHECK_RET(bsp_socp_free_channel(u32ChanId), BSP_OK);
    ...
    printf("SOCP TEST: ALLOC CODER SRC CHAN CASE 4 PASSED\n");
    return 0;
}


/* 释放通道，释放无效通道*/
BSP_S32 SOCP_ST_FREE_CHAN_001()
{
    BSP_U32 u32ChanId;
    ...
    printf("SOCP TEST: FREE CHANNEL CASE 1 PASSED\n");
    return 0;
}


/* 释放通道，释放未申请通道 */
BSP_S32 SOCP_ST_FREE_CHAN_002()
{
    BSP_U32 u32ChanId;
...
BSP_S32 SOCP_ST_FREE_CHAN_003()
{
    ...
    if(-1 == (s32)u32ChanId)
    {
        printf("SOCP TEST: FREE CHANNEL CASE 3 FAILED\n");
        return 0;
    }
    CHECK_RET(bsp_socp_free_channel(u32ChanId), BSP_OK);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);

    printf("SOCP TEST: FREE CHANNEL CASE 3 PASSED\n");

    return 0;
}

/* 释放通道，反复申请释放 */
BSP_S32 SOCP_ST_FREE_CHAN_006()
{
    BSP_U32 u32ChanId[8];
    BSP_S32 n=9;
    BSP_S32 i, j, ret;
    ...
    for(j=0; j<10; j++)
    {
        for(i=4; i<n; i++)
        {
            ...
            {
                printf("alloc channel error\n");
                printf("SOCP TEST: FREE CHANNEL CASE 6 FAILED\n");
                return 0;
            }
            //printf(" u32ChanId[%d] is %d\n", i, u32ChanId[i]);
        }
        ...
        for(i=n-1; i>=4; i--)
        {
            ...
            if(BSP_OK != ret)
            {
                printf("free channel error, ret=0x%x\n", ret);
                printf("SOCP TEST: FREE CHANNEL CASE 6 FAILED\n");
                return 0;
            }
            osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    ...
    printf("SOCP TEST: FREE CHANNEL CASE 6 PASSED\n");
    return 0;
}


/* 设置事件回调函数，设置无效通道的事件回调函数 */
BSP_S32 SOCP_ST_SET_EVENT_001()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: SET EVENT CB CASE 1 PASSED\n");
    return 0;
}


/* 设置事件回调函数，设置未申请通道的事件回调函数 */
BSP_S32 SOCP_ST_SET_EVENT_002()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: SET EVENT CB CASE 2 PASSED\n");
    return 0;
}

/* 设置事件回调函数，设置有效事件回调函数 */
BSP_S32 SOCP_ST_SET_EVENT_004()
{
    BSP_U32 u32ChanId1;
    DEV_INIT();
    ...
    if(-1 == (s32)u32ChanId1)
    {
        printf("SOCP TEST: SET EVENT CB CASE 4 FAILED\n");
        return 0;
    }
    CHECK_RET(bsp_socp_register_event_cb(u32ChanId1, SocpEventCB), BSP_OK);
    bsp_socp_free_channel(u32ChanId1);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: SET EVENT CB CASE 4 PASSED\n");
    return 0;
}


/* 设置事件回调函数，设置事件回调函数为空 */
BSP_S32 SOCP_ST_SET_EVENT_005()
{
    BSP_U32 u32ChanId1;
    DEV_INIT();
    ...
    if(-1 == (s32)u32ChanId1)
    {
        printf("SOCP TEST: SET EVENT CB CASE 5 FAILED\n");
        return 0;
    }
    CHECK_RET(bsp_socp_register_event_cb(u32ChanId1, BSP_NULL), BSP_OK);
    bsp_socp_free_channel(u32ChanId1);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: SET EVENT CB CASE 5 PASSED\n");
    return 0;
}

/* 设置RD回调函数，设置无效通道的RD回调函数 */
BSP_S32 SOCP_ST_SET_RD_001()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: SET RD CB CASE 1 PASSED\n");
    return 0;
}

/* 设置RD回调函数，设置未申请通道的RD回调函数 */
BSP_S32 SOCP_ST_SET_RD_002()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: SET RD CB CASE 2 PASSED\n");
    return 0;
}

/* 设置RD回调函数，设置非LIST方式的RD回调函数 */
BSP_S32 SOCP_ST_SET_RD_004()
{
    BSP_U32 u32ChanId;
    ...
    if(-1 == (s32)u32ChanId)
    {
        printf("SOCP TEST: SET RD CB CASE 4 FAILED\n");
        return 0;
    }
    ...
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: SET RD CB CASE 4 PASSED\n");
    return 0;
}


/* 设置RD回调函数，设置有效RD回调函数 */
BSP_S32 SOCP_ST_SET_RD_005()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    if(-1 == (s32)u32ChanId)
    {
        printf("SOCP TEST: SET RD CB CASE 5 FAILED\n");
        return 0;
    }
    ...
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: SET RD CB CASE 5 PASSED\n");
    return 0;
}


/* 设置RD回调函数，设置RD回调函数为空 */
BSP_S32 SOCP_ST_SET_RD_006()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    if(-1 == (s32)u32ChanId)
    {
        printf("SOCP TEST: SET RD CB CASE 6 FAILED\n");
        return 0;
    }
    ...
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: SET RD CB CASE 6 PASSED\n");
    return 0;
}


/* 通道控制，启动无效通道 */
BSP_S32 SOCP_ST_START_001()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: START CHANNEL CASE 1 PASSED\n");
    return 0;
}


/* 通道控制，启动未申请通道 */
BSP_S32 SOCP_ST_START_002()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: START CHANNEL CASE 2 PASSED\n");
    return 0;
}


/* 通道控制，启动时，目的通道未设置 */
BSP_S32 SOCP_ST_START_003()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    if(-1 == (s32)u32ChanId)
    {
        printf("alloc encoder src chan failed\n");
        printf("SOCP TEST: START CHANNEL CASE 3 FAILED\n");
        return 0;
    }
    ...
    bsp_socp_free_channel(u32ChanId);
    osl_free(g_pSocpBuf[--g_SocpBufCnt]);
    printf("SOCP TEST: START CHANNEL CASE 3 PASSED\n");
    return 0;
}


/* 通道控制，停止无效通道 */
BSP_S32 SOCP_ST_STOP_001()
{
    BSP_U32 u32ChanId;
    BSP_U32 u32Ret;
    DEV_INIT();
    ...
    printf(" the u32Ret is %d\n", u32Ret);
    CHECK_RET(u32Ret, BSP_ERR_SOCP_INVALID_CHAN);

    printf("SOCP TEST: STOP CHANNEL CASE 1 PASSED\n");
    return 0;
}


/* 通道控制，停止未申请通道 */
BSP_S32 SOCP_ST_STOP_002()
{
    BSP_U32 u32ChanId;
    DEV_INIT();
    ...
    printf("SOCP TEST: STOP CHANNEL CASE 2 PASSED\n");
    return 0;
}
...
BSP_S32 SocpStAllocEncSrc(SOCP_ST_CASE_S *pCase)
{
    ...
    if(0 == pCase->u32InputStart)
    {
        printf("BD buffer Need to ReAlloc!\n");
        p = (BSP_U8*)osl_cachedma_malloc(pCase->u32InputSize);
        if(BSP_NULL == p)
        {
            printf("%s[%d] ALLOC ENC SRC FAILED!\n", __FUNCTION__, __LINE__);
            return BSP_ERROR;
        }
    ...
    {
        printf("BD buffer have alloced!\n");
        p = (BSP_U8*)pCase->u32InputStart;
    }
    ...
    if(pCase->eMode == SOCP_ENCSRC_CHNMODE_LIST)
    {
        ...
        if(BSP_NULL == pRd)
        {
            printf("%s[%d] ALLOC ENC SRC RD BUF FAILED!\n", __FUNCTION__, __LINE__);
            osl_free(g_pSocpBuf[--g_SocpBufCnt]);
            return BSP_ERROR;
        }
    ...
    if(BSP_OK != bsp_socp_coder_set_src_chan(pCase->SrcCh, &EncSrcAttr))
    {
        ...
        printf("alloc enc src channel failed!\n");
...
BSP_S32 SocpStEventCB_1(BSP_U32 u32ChanID, SOCP_EVENT_ENUM_UIN32 u32Event, BSP_U32 u32Param)
{
    ...
    switch(u32Event)
    {
        ...
        {
            printf("chan type is [%d], id is [%d], event is packet header check error!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_OUTBUFFER_OVERFLOW:
        {
            printf("chan type is [%d], id is [%d], event is outbuffer overflow!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_HDLC_HEADER_ERROR:
        {
            printf("chan type is [%d], id is [%d], event is 0x7e error!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_CRC_ERROR:
        {
            printf("chan type is [%d], id is [%d], event is crc error!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_PKT_LENGTH_ERROR:
        {
            printf("chan type is [%d], id is [%d], event is pkt length error!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_DATA_TYPE_ERROR:
        {
            printf("chan type is [%d], id is [%d], event is data type error!\n", u32ChanType, u32ChanId);
            break;
        }
        case SOCP_EVENT_DECODER_UNDERFLOW:
        {
            printf("chan type is [%d], id is [%d], event is inbuf underflow error!\n", u32ChanType, u32ChanId);
            break;
        }
        default:
        {
            printf("the event:0x%x is valid!\n", u32Event);
            return BSP_ERROR;
        }
...
BSP_S32 SocpStReadCB_1(BSP_U32 u32ChanID)
{
    ...
    printf("func: SocpStReadCB_1: chan[%d] get data len=[%d]\n", u32ChanID&0xff, len);
    if(Buffer.pBuffer)
    {
        memcpy(g_socpRxdata, Buffer.pBuffer, Buffer.u32Size);
    }
    if(Buffer.pRbBuffer)
    {
        memcpy(g_socpRxdata+Buffer.u32Size, Buffer.pRbBuffer, Buffer.u32RbSize);
    }

    for(i=0; i<len; i++)
    {
        printf("0x%02x ", g_socpRxdata[i]);
        if(i && (0 == i%8))
            printf("\n");
    }
    printf(" the chan id is %d\n",u32ChanID);
...
BSP_S32 SocpStReadCB_2(BSP_U32 u32ChanID)
{
    ...
    len = Buffer.u32Size + Buffer.u32RbSize;
    g_socpRxTotal[c] += len;
    printf("func: SocpStReadCB_2: c[%d] get data R=%d T=%d\n", u32ChanID&0xff, len, g_socpRxTotal[c]);
    /*if(Buffer.pBuffer)
    {
        memcpy(g_socpRxdata, Buffer.pBuffer, Buffer.u32Size);
    }
...
BSP_S32 SocpStReadCB_3(BSP_U32 u32ChanID)
{
    ...
    printf("func: SocpStReadCB_3: chan[%d] get data R=%d T=%d\n", chan, len, g_socpRxTotal[chan]);
    if(Buffer.pBuffer)
    {
        memcpy(g_socpRxdata, Buffer.pBuffer, Buffer.u32Size);
    }
...
BSP_S32 SocpStReadCB_4(BSP_U32 u32ChanID)
{
    ...
    CHECK_RET(bsp_socp_get_read_buff(u32ChanID, &Buffer), BSP_OK);
    len = Buffer.u32Size + Buffer.u32RbSize;
    printf("func: SocpStReadCB_4: chan[%x] get data len=[%d]\n", u32ChanID, len);
#endif
    return 0;
}
...
BSP_S32 socp_encode_task001(BSP_VOID *param)
{
    ...
    if(!packet)
    {
        printf("%s(%d): malloc faile chan=0x%x len=0x%x\n", __FUNCTION__, __LINE__, pCase->SrcCh, len);
        return -1;
    }
    ...
    while(cnt)
    {
        ...
        if(WtBuff.u32Size + WtBuff.u32RbSize >= (u32)len)
        {
            ...
            {
                memcpy(WtBuff.pBuffer, packet, len);
            }
            else
            {
                memcpy(WtBuff.pBuffer, packet, WtBuff.u32Size);
                memcpy(WtBuff.pRbBuffer, packet+WtBuff.u32Size, len-WtBuff.u32Size);
            }
...
BSP_S32 socp_encode_task003(BSP_VOID *param)
{
    ...
    if(!packet)
    {
        printf("%s(%d): malloc failed chan=0x%x\n", __FUNCTION__, __LINE__, pCase->SrcCh);
        return -1;
    }
    g_pSocpBuf[g_SocpBufCnt++] = packet;
    ...
    while(cnt)
    {
        ...
        if(WtBuff.u32Size + WtBuff.u32RbSize >= 8)
        {
            if(WtBuff.u32Size >= 8)
            {
                memcpy(WtBuff.pBuffer, bd, 8);
            }
            else
            {
                memcpy(WtBuff.pBuffer, bd, WtBuff.u32Size);
                memcpy(WtBuff.pRbBuffer, bd+WtBuff.u32Size, 8-WtBuff.u32Size);
            }
    ...
    printf("cnt is %d, chan[%d] write done! totol=0x%x size=0x%x\n", cnt, pCase->SrcCh, g_socpTxTotal[c], g_socpTxTotal[c]*len);
...
int socp_encode_task005(void *param)
{
    ...
    if(!packet)
    {
        printf("%s(%d): malloc faile chan=0x%x\n", __FUNCTION__, __LINE__, pCase->SrcCh);
        return -1;
    }
    g_pSocpBuf[g_SocpBufCnt++] = packet;
...
BSP_S32 socp_encode_dcore_task_stress(BSP_VOID *param)
{
    ...
    while(1)
    {
        ...
        {
            //printf("encoder stress send cnt is %d\n",u32StrEncSendCnt);
        }
...
BSP_S32 SOCP_ST_ENCODE_006_START()
{
    ...
    sprintf(task_name,"socpTest%02d",6);
...
BSP_S32 SOCP_ST_ENCODE_007_START()
{
    char task_name[30] = {0};

    DEV_INIT();

    sprintf(task_name,"socpTest%02d",7);
...
BSP_S32 SOCP_ST_ENCODE_007_STOP()
{
    /*停止通道*/
...
/* 编码测试,目标通道接收数据唯一性测试*/
BSP_S32 SOCP_ST_ENCODE_012_START()
{
    char task_name[30] = {0};
    int i;
    BSP_U32 u32Ret;
    ...
    sprintf(task_name,"socpTest%02d",10);
...
BSP_S32 SOCP_ST_ENCODE_012_STOP()
{
    /*停止通道*/
    CHECK_RET(bsp_socp_stop(g_stCase[0].SrcCh), BSP_OK);
    CHECK_RET(bsp_socp_free_channel(g_stCase[0].SrcCh), BSP_OK);
...
/* 编码测试,测试数据包长为1的情况*/
BSP_S32 SOCP_ST_ENCODE_014_START()
{
    char task_name[30] = {0};
    SOCP_ST_CASE_S *pCase = &g_stCase[0];
...
BSP_S32 SOCP_ST_ENCODE_014_STOP()
{
    /*停止通道*/
    CHECK_RET(bsp_socp_stop(g_stCase[0].SrcCh), BSP_OK);
    CHECK_RET(bsp_socp_free_channel(g_stCase[0].SrcCh), BSP_OK);
...
/* 编码测试,测试包头错误 */
BSP_S32 SOCP_ST_ENCODE_016_START()
{
    char task_name[30] = {0};
...
BSP_S32 SOCP_ST_ENCODE_016_STOP()
{
    /*停止通道*/
    CHECK_RET(bsp_socp_stop(g_stCase[0].SrcCh), BSP_OK);
    CHECK_RET(bsp_socp_free_channel(g_stCase[0].SrcCh), BSP_OK);
...
/*双核编码测试，M核上发送，A核上接收*/
BSP_S32 SOCP_ST_ENCODE_DCORE_STRESS(int priority)
{
    char task_name[30] = {0};

    DEV_INIT();

    sprintf(task_name,"socpTestDdr2Ddr");
    if(BSP_OK != osl_task_init(task_name, priority, 0x1000, (OSL_TASK_FUNC)socp_encode_dcore_task_stress, &g_stCase[0], &(g_stCase[0].TaskId)))
    {
        return BSP_ERROR;
    }
...
BSP_S32 socp_encode_nonddr_task_stress(BSP_VOID *param)
{
	...
	printf("%s,%d malloc successed! len = %d\n",__FUNCTION__,__LINE__,len);
    SocpStGenPkt(packet, pCase->u32DataLen);
...
BSP_S32 SOCP_ST_ENCODE_STRESS_TCM(int priority, u32 addr,u32 ulDatalen)
{
    ...
    sprintf(task_name,"socpTestBbe16dtcmToDdr");
    if(BSP_OK != osl_task_init(task_name, priority, 0x1000, (OSL_TASK_FUNC)socp_encode_nonddr_task_stress, (void*)pCase, (OSL_TASK_ID *)(&pCase->TaskId)))
    {
        return BSP_ERROR;
    }
...
BSP_S32 SOCP_ST_ENCODE_STRESS_SRAM(int priority, u32 addr,u32 ulDatalen)
{
    ...
    sprintf(task_name,"socpTestAxiMemToDdr");
    if(BSP_OK != osl_task_init(task_name, priority, 0x1000, (OSL_TASK_FUNC)socp_encode_nonddr_task_stress, (void*)pCase, (OSL_TASK_ID *)(&pCase->TaskId)))
    {
        return BSP_ERROR;
    }
...
BSP_S32 socp_bus_stress(BSP_VOID *param)
{
    SOCP_ST_CASE_S *pCase = (SOCP_ST_CASE_S*)param;
    int len = pCase->u32InputSize;
    BSP_U8* packet;
    BSP_U8* buffer;
    SOCP_BUFFER_RW_STRU WtBuff;
    BSP_U8 bd[8];
    ...
    packet = (BSP_U8*)osl_cachedma_malloc(0x1000);
    if(!packet)
    {
        printf("%s(%d): malloc failed chan=0x%x\n", __FUNCTION__, __LINE__, pCase->SrcCh);
        return -1;
    }
    SocpStGenBD1((BSP_U32*)bd, packet, 0x1000);

    buffer = (BSP_U8*)osl_cachedma_malloc(len);
    if(!buffer)
    {
        printf("%s(%d): malloc failed chan=0x%x\n", __FUNCTION__, __LINE__, pCase->SrcCh);
        osl_cachedma_free(packet);
    ...
    for(i = 0;i<len/8 ;i++)
    {
        memcpy(buffer+i*8,bd,8);
    }


    while(1)
    {
        SOCP_REG_READ(SOCP_REG_ENCSRC_RDQWPTR(c), write);
        SOCP_REG_READ(SOCP_REG_ENCSRC_RDQRPTR(c), read);
        if(read != write)
        {
            SOCP_REG_WRITE(SOCP_REG_ENCSRC_RDQRPTR(c), write);
        }
        len = 0;
        ...
        if(WtBuff.u32Size + WtBuff.u32RbSize >= 8)
        {
            ...
            if(WtBuff.u32Size >= 8)
            {
                memcpy(SOCP_PHYS_TO_VIRT(WtBuff.pBuffer), buffer, WtBuff.u32Size);
                len = WtBuff.u32Size;
            }
            ...
            if(WtBuff.u32RbSize >= 8)
            {
                ...
                memcpy(SOCP_PHYS_TO_VIRT(WtBuff.pRbBuffer), buffer, WtBuff.u32RbSize);
            }
        ...
        else
        {
            ...
            g_stSocp_busstress_test_fail ++;
        }

    }

    osl_cachedma_free(packet);
    osl_cachedma_free(buffer);
    return 0;
}



BSP_S32 SOCP_ST_BUSSTRESS_TEST(int priority,u32 addr,u32 len)
{
    SOCP_ST_CASE_S *pCase = &g_stCase[3];
    char task_name[30] = {0};

    pCase->u32InputStart = addr;
    pCase->u32InputSize  = len;
    pCase->u32DataLen    = 0x1000;
    pCase->eMode = SOCP_ENCSRC_CHNMODE_LIST;
    pCase->SrcCh = SOCP_CODER_SRC_LDSP1;
    pCase->u32RDSize = 0xf000;

    CHECK_RET(SocpStAllocEncSrc(pCase), BSP_OK);

    CHECK_RET(bsp_socp_start(pCase->SrcCh),BSP_OK);
    sprintf(task_name,"socpBusTest");
    if(BSP_OK != osl_task_init(task_name, priority, 0x1000, (OSL_TASK_FUNC)socp_bus_stress, (void*)pCase, (OSL_TASK_ID *)(&pCase->TaskId)))
    {
        return BSP_ERROR;
    }
...
void socp_bustest_start(void)
{
    u8* buffer = NULL;
    u32 datalen = 0x20000;

    buffer = osl_cachedma_malloc(datalen);
    if(buffer)
    {
        printf("buffer malloc success ! 0x%x\n",datalen);
        (void)SOCP_ST_BUSSTRESS_TEST(50,(u32)buffer,datalen);
    }
}

void socp_bustest_show(void)
{
