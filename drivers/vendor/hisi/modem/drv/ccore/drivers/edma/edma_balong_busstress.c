...
#define MEM_FOR_EDMAC_BUSSTRESS SHM_MEM_MDMA9_PM_BOOT_ADDR
#endif
...
s32 edma_stress_test_routine()
{
    ...
    while(get_test_switch_stat(EDMAC))
    {
        ...
        if(count > 0)
        {
        	if(do_data_check%check_times == 0)
        	{
        ...
        if(do_data_check%check_times == 0)
        {
	        memset_s((void*)g_edma_busstress_info.stDMATaskHandle[cur_idx].u32SrcAddr,EDMA_TEST_LEN,cur_idx,EDMA_TEST_LEN);
	        memset_s((void*)g_edma_busstress_info.stDMATaskHandle[cur_idx].u32DstAddr,EDMA_TEST_LEN,0xff,EDMA_TEST_LEN);
        }
...
s32 edma_stress_test_start(s32 task_priority,s32 test_rate)
{
    ...
    g_edma_busstress_info.stDMATaskHandle[0].u32SrcAddr = 0;
    g_edma_busstress_info.stDMATaskHandle[0].u32DstAddr = 0;
    g_edma_busstress_info.stDMATaskHandle[0].ulLength   = EDMA_TEST_LEN;
    g_edma_busstress_info.stDMATaskHandle[0].ulChIdx    = chann_id;

    
    g_edma_busstress_info.stDMATaskHandle[1].u32SrcAddr = 0;
    g_edma_busstress_info.stDMATaskHandle[1].u32DstAddr = (unsigned int)EDMA_HANDLE_1_AXI_MEM_64_DST;
    g_edma_busstress_info.stDMATaskHandle[1].ulLength   = EDMA_TEST_LEN;
    g_edma_busstress_info.stDMATaskHandle[1].ulChIdx    = chann_id;

    
    g_edma_busstress_info.stDMATaskHandle[2].u32SrcAddr = (unsigned int)EDMA_HANDLE_2_AXI_MEM_64_SRC;
    g_edma_busstress_info.stDMATaskHandle[2].u32DstAddr = (unsigned int)EDMA_HANDLE_2_AXI_MEM_64_DST;
    g_edma_busstress_info.stDMATaskHandle[2].ulLength   = EDMA_TEST_LEN;
    g_edma_busstress_info.stDMATaskHandle[2].ulChIdx    = chann_id;

    
    g_edma_busstress_info.stDMATaskHandle[3].u32SrcAddr = (unsigned int)EDMA_HANDLE_3_AXI_MEM_64_SRC;
    g_edma_busstress_info.stDMATaskHandle[3].u32DstAddr = 0;
    g_edma_busstress_info.stDMATaskHandle[3].ulLength   = EDMA_TEST_LEN;
    g_edma_busstress_info.stDMATaskHandle[3].ulChIdx    = chann_id;
    ...
    g_edma_busstress_info.stDMATaskHandle[0].u32SrcAddr = (u32)osl_cachedma_malloc(EDMA_TEST_LEN);
    g_edma_busstress_info.stDMATaskHandle[0].u32DstAddr = (u32)osl_cachedma_malloc(EDMA_TEST_LEN);
    g_edma_busstress_info.stDMATaskHandle[1].u32SrcAddr = (u32)osl_cachedma_malloc(EDMA_TEST_LEN);
    g_edma_busstress_info.stDMATaskHandle[3].u32DstAddr = (u32)osl_cachedma_malloc(EDMA_TEST_LEN);
    /*创建发包测试任务，收包驱动会自行做处理*/
...
s32 edma2_stress_test_routine()
{
    ...
    while(get_test_switch_stat(EDMAC))
    {
        ...
        cur_idx = count%EDMA_HANDLE_NUM;
        memset_s((void*)(void*)g_edma2_busstress_info.stDMATaskHandle[cur_idx].u32SrcAddr,EDMA_TEST_LEN,cur_idx,EDMA_TEST_LEN);
        memset_s((void*)(void*)g_edma2_busstress_info.stDMATaskHandle[cur_idx].u32DstAddr,EDMA_TEST_LEN,0xff,EDMA_TEST_LEN);
...
s32 edma2_stress_test_start(s32 task_priority,s32 test_rate)
{
    ...
    g_edma2_busstress_info.stDMATaskHandle[1].u32DstAddr = (unsigned int)EDMA2_HANDLE_1_AXI_MEM_64_DST;
    g_edma2_busstress_info.stDMATaskHandle[1].ulLength   = EDMA_TEST_LEN;
    g_edma2_busstress_info.stDMATaskHandle[1].ulChIdx    = chann_id;

    
    g_edma2_busstress_info.stDMATaskHandle[2].u32SrcAddr = (unsigned int)EDMA2_HANDLE_2_AXI_MEM_64_SRC;
    g_edma2_busstress_info.stDMATaskHandle[2].u32DstAddr = (unsigned int)EDMA2_HANDLE_2_AXI_MEM_64_DST;
    g_edma2_busstress_info.stDMATaskHandle[2].ulLength   = EDMA_TEST_LEN;
    g_edma2_busstress_info.stDMATaskHandle[2].ulChIdx    = chann_id;

    
    g_edma2_busstress_info.stDMATaskHandle[3].u32SrcAddr = (unsigned int)EDMA2_HANDLE_3_AXI_MEM_64_SRC;
...
#define edma_ddr_num 2
u32 edma_lli_loop_src[edma_ddr_num][16000] ;
u32 edma_lli_loop_des[edma_ddr_num][16000] ;
...
u32 edma_ddr_index = 0;

s32 edma_test_lli_loop_ddr(enum edma_req_id req )
{
    ...
    psttemp = edma_lli_loop_cb;
        ...
        psttemp->src_addr = (unsigned int)edma_lli_loop_src[edma_ddr_index];
        psttemp->des_addr = (unsigned int)edma_lli_loop_des[edma_ddr_index];
		edma_ddr_index++;
		edma_ddr_index = edma_ddr_index%edma_ddr_num;
        psttemp->cnt0 = (64*1024-1);
        psttemp->bindx = 0;
        psttemp->cindx = 0;
        psttemp->cnt1  = 0;
...
s32 edma_test_lli_loop_sram(enum edma_req_id req )
{
    edma_addr_t  edma_addr    = 0;
    s32         ret_id       = 0;
    struct edma_cb *psttemp   = NULL;
    ...
    psttemp = edma_lli_loop_cb;
        ...
        psttemp->lli = EDMA_SET_LLI(edma_addr + 0, 0);
        psttemp->config = EDMA_SET_CONFIG(req, EDMA_M2M, EDMA_TRANS_WIDTH_64, EDMA_BUR_LEN_16);
        psttemp->src_addr = (unsigned int)MEM_FOR_EDMAC_BUSSTRESS;//edma_lli_loop_src;
        psttemp->des_addr = (unsigned int)MEM_FOR_EDMAC_BUSSTRESS;//edma_lli_loop_des;
        psttemp->cnt0 = AXI_MEM_64_SIZE_FOR_EDMAC;
        psttemp->bindx = 0;
        psttemp->cindx = 0;
        psttemp->cnt1  = 0;
