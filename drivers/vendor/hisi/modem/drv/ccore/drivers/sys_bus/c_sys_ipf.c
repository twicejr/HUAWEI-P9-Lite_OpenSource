...
void sys_ipf_report(struct master* master)
{
	IPF_PRINT(" ul_total_bd_num(BDs) :%d\n",ul_total_bd_num);
}

void sys_ipf_init(struct master* master)
{
	int s32Ret = 0;
    ...
    master->stop = NULL;
    master->record = NULL;
    master->report = sys_ipf_report;

    s32Ret = of_property_read_u32_index(master->np, "ipf_addr_lim", 0, &g_ipf_addr_lim);
    if(s32Ret)
    {
        sys_bus_trace("read ipf_addr_lim err\n");
        return;
    }

    writel(0x10, g_ipf_addr_lim);
    ...
    /* task_priority = 10; */
	osl_task_init("ipf ul stress task", 10, 0x2000, (OSL_TASK_FUNC)ipf_ul_stress_test_routine, 0,&ipf_ul_stress_test_task_id);
