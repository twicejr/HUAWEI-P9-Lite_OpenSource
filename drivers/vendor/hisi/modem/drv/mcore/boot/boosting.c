...
const u16 hpm_uhvt_25[15] = {1400, 1390, 1370, 1360, 1340, 1320, 1300, 1290,
	                         1280, 1270, 1260, 1250, 1250, 1250, 1250};
const u16 hpm_hvt_25[15]  = {1090, 1090, 1080, 1080, 1070, 1060, 1050, 1050,
	                         1040, 1040, 1030, 1030, 1030, 1030, 1030};
const u16 hpm_svt_25[15]  = {900,  900,  900,  880,  880,  870,  860,  850,
	                         840,  840,  830,  830,  830,  830,  830};

const u16 hpm_uhvt_50[15] = {1500, 1480, 1470, 1460, 1420, 1410, 1390, 1370,
	                         1360, 1340, 1330, 1320, 1320, 1310, 1310};
const u16 hpm_hvt_50[15]  = {1160, 1150, 1140, 1130, 1120, 1120, 1110, 1100,
	                         1090, 1080, 1070, 1070, 1070, 1070, 1070};
const u16 hpm_svt_50[15]  = {920,  920,  910,  910,  900,  900,  890,  880,
	                         880,  880,  870,  870,  870,  870,  870};
...
s32 boosting(void)
{
	...
	for(i = 0; i<5 ; i++)
	{
		/* waiting 1.5 * 4ms;规避tsensor芯片bug，延时增大至4倍 */
		loop_count = 400000;
		while(loop_count)
		{
			loop_count--;
    ...
    if (hpm_svt_opc < hpm_uhvt_opc || hpm_hvt_opc < hpm_uhvt_opc)
	{
		return ; /* hpm value overflow */
	}

    /* 24 boosting according to Tsensor & hpm_opc*/
	n = (temperature % 10) ? (temperature + 30)/10 : (temperature + 20)/10;
	n = (temperature < -20) ? 0 : n;
	n = (temperature > 120) ? 14: n;

#if defined(CONFIG_PMIC_HI6551)
	if(((1000*26*10000/333)/hpm_hvt_opc > hpm_hvt_50[n]) || ((1000*26*10000/333)/hpm_uhvt_opc > hpm_uhvt_50[n])
		|| ((1000*26*10000/333)/hpm_svt_opc > hpm_svt_50[n]))
	{
		/*buck3&ldo3 volt up 50mV*/
		hpm_temp_print_ptr->up_volt = 50;
		bsp_pmu_volt_set(PMIC_HI6551_BUCK3,950);
		bsp_pmu_volt_set(PMIC_HI6551_LDO03,950);
	}
	else if(((1000*26*10000/333)/hpm_hvt_opc > hpm_hvt_25[n]) || ((1000*26*10000/333)/hpm_uhvt_opc > hpm_uhvt_25[n])
		||((1000*26*10000/333)/hpm_svt_opc > hpm_svt_25[n]))
	{
		/*buck3 volt up 25mV,ldo3 volt up 50mV*/
		hpm_temp_print_ptr->up_volt = 25;
		bsp_pmu_volt_set(PMIC_HI6551_BUCK3,925);
		bsp_pmu_volt_set(PMIC_HI6551_LDO03,950);
	}
#endif
            ...
            || ((1000*26*10000/333)/hpm_svt_opc > hpm_svt_50[n]))
        {
            /*buck3&ldo3 volt up 50mV*/
            hpm_temp_print_ptr->up_volt = 50;
            bsp_pmu_volt_set(PMIC_HI6559_BUCK3,950);
            ...
            ||((1000*26*10000/333)/hpm_svt_opc > hpm_svt_25[n]))
        {
            /*buck3 volt up 25mV,ldo3 volt up 50mV*/
            hpm_temp_print_ptr->up_volt = 25;
            bsp_pmu_volt_set(PMIC_HI6559_BUCK3,925);
            bsp_pmu_volt_set(PMIC_HI6559_LDO03,950);
        }
#endif
