...
#define APPA9_WSRC_0_USB_PME   (0x1UL<<23)
...
#define PD_CRG_SSRSTEN1 0x0360
#define PD_CRG_SSRSTDIS1 0x0364
...
#define SYS_REG_BAK_NUM 12
#define SYS_REG_SET_NUM 14
...
#define SYS_BAK_RESTORE_CONFIG(config,index,start_addr,end_addr) do{\
            config->bak[index].start = start_addr;\
            config->bak[index].end   = end_addr;\
            index++;\
        }while(0)
#define SYS_BAK_SET_CONFIG(config,index,from_addr) do{\
            config->set[index].from = from_addr;\
            index++;\
        }while(0)
