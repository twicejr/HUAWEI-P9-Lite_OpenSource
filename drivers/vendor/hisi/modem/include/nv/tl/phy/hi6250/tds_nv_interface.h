...
#define MAX_TDS_VRAMP_PA_BAND_NUM    (2)

typedef struct
{
    UINT16 usPAOnTime;    /*PA Vramp提前配置时间，单位为1/24 Chip*/
    UINT16 usPAOffTime;   /*PA Vramp延迟配置时间，单位为1/24 Chip*/
    UINT16 usPaAgcOnTime; /*TX增益事件中AUXDAC SSI指令发送提前量，TDS实际不使用*/
    UINT16 usReserved;
}TDS_VRAMP_PA_ONOFFTIME_STRU;
...
typedef struct
{
    ...
    UINT16 usReserved;
}TDS_NV_VRAMP_BAND_PARA_STRU;

typedef struct
{
    UINT16 usVrampBandNum;
    UINT16 usReserved;
    TDS_VRAMP_PA_ONOFFTIME_STRU stPaOnOffTime;
    TDS_NV_VRAMP_BAND_PARA_STRU astVrampBandStru[MAX_TDS_VRAMP_PA_BAND_NUM];
