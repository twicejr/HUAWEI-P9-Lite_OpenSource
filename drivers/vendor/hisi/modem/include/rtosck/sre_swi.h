...
/**
 * @ingroup SRE_swi
 * 软中断优先级，任务裁减时保留给IDLE软中断使用。
 */
#define OS_SWI_PRIORITY_LOWEST                              15
...
/**
 * @ingroup SRE_swi
 * 软中断错误码：软中断配置的最大支持数不合适，最大支持数不能大于125且不能为0。
 *
 * 值: 0x02000907
 *
 * 解决方案:在软中断配置项中减少最大支持数，使之小于等于125，大于0.
 */
 ...
 * 值: 0x02000913
 *
 * 解决方案:任务模块被裁剪后，需激活IDLE软中断，查看系统栈是否配置错误。
 */
#define OS_ERRNO_SWI_ACTIVE_FAILED                           SRE_ERRNO_OS_ERROR(OS_MID_SWI, 0x13)
...
/**
* @ingroup  SRE_swi
* @brief 软中断调用处理函数钩子函数类型定义。
*
* @par 描述:
* 用户通过软中断调用钩子处理函数类型定义软中断调用处理函数钩子，在软中断调用处理函数时，调用该钩子。
* @attention
* 该钩子可通过SRE_HookAdd和SRE_HookDel进行添加和删除
*
* @param  uwSwiPid    [IN] 类型为#UINT32，软中断PID。
*
* @retval 无。
* @par 依赖:
* <ul><li>sre_swi.h：该接口声明所在的头文件。</li></ul>
* @since RTOSck V100R002C00
* @see SRE_HookAdd | SRE_HookDel
*/
typedef VOID (* SWI_ENTRY_HOOK)(UINT32 uwSwiPid);

/**
* @ingroup  SRE_swi
* @brief 软中断退出处理函数钩子函数类型定义。
*
* @par 描述:
* 用户通过软中断退出钩子处理函数类型定义软中断退出处理函数钩子，在软中断退出处理函数时，调用该钩子。
* @attention
* 该钩子可通过SRE_HookAdd和SRE_HookDel进行添加和删除
*
* @param  uwSwiPid    [IN] 类型为#UINT32，软中断PID。
...
* @see SRE_HookAdd | SRE_HookDel
*/
typedef VOID (* SWI_EXIT_HOOK)(UINT32 uwSwiPid);
...
typedef struct tagSWI_INIT_PARA
{
    ...
    CHAR *pcSwiName;                    /**< 类型为CHAR*，软中断名称，最大长度为OS_SWI_NAME_LEN  。 */
}SWI_INIT_PARA_S;
 ...
 * @attention
 * <ul>
 * <li>优先级取值范围[0, 15]，最高优先级0，最低优先级15。 </li>
 ...
 *
 * @par 描述:
 * 根据掩码值uwMask清除软中断实时值的对应位，当实时值清为0时会激活软中断。
 *
 * @attention
 * <ul>
 * <li>参数为创建成功时返回的PID号，也可以通过SRE_HuntByName获取软中断的PID。</li>
 ...
 * @attention
 * <ul>
 * <li>该操作适用于SD6108/SD6181平台的DSP核，SD6182/SD6157/SD6183平台的所有核。</li>
 * </ul>
 *
 * @param  无
 *
 ...
 * @attention
 * <ul>
 * <li>该操作适用于SD6108/SD6181平台的DSP核，SD6182/SD6157平台的所有核。</li>
 * </ul>
 *
 * @param  无
 *
...
#define SRE_SwiCpSaveCfg(uwSwiID, pstParam)  SRE_OK
#define SRE_SwiCpEntry(VOID)
#define SRE_SwiCpExit(VOID)
