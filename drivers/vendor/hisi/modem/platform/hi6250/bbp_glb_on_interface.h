...
typedef union
{
    unsigned int      mode_sel_reg;
    struct
    {
        unsigned int  cpu_mode_sel : 2;  /* bit[0-1] : BBP工作模式指示。0，W；1，TDS；2，1x；3，EV-DO。 */
...
typedef union
{
    unsigned int      rfic2_rstn_reg;
    struct
    {
        unsigned int  rfic2_rstn : 1;  /* bit[0]   : 软件配置的RFIC2复位信号，低有效。0，复位；1，解复位。
                                                     上电初始化配置为0，根据RFIC手册要求，维持一段时间后配置为1。
                                                     该寄存器仅供K3V5使用。 */
...
typedef union
{
    unsigned int      rfic0_rstn_reg;
    struct
    {
        unsigned int  rfic0_rstn : 1;  /* bit[0]   : 软件配置的RFIC0复位信号，低有效。0，复位；1，解复位。
                                                     上电初始化配置为0，根据RFIC手册要求，维持一段时间后配置为1。
                                                     该寄存器从CTU接口0x002C移植过来。 */
        unsigned int  reserved   : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rfic1_rstn_reg;
    struct
    {
        unsigned int  rfic1_rstn : 1;  /* bit[0]   : 软件配置的RFIC1复位信号，低有效。0，复位；1，解复位。
                                                     上电初始化配置为0，根据RFIC手册要求，维持一段时间后配置为1。
                                                     该寄存器从CTU接口0x032C移植过来。 */
        unsigned int  reserved   : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_timer_update_en_reg;
    struct
    {
        unsigned int  cpu_adj_period_update_en : 1;  /* bit[0]   : 32k公共定时器的校准参数更新使能信号，脉冲信号，需要调整校准周期或校准方向时，应先配置cpu_adjust_direction和cpu_adjust_period，再配置此信号 */
        unsigned int  reserved                 : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_timer_update_para_reg;
    struct
    {
        unsigned int  cpu_adjust_period    : 24; /* bit[0-23] : 32k公共定时器的时钟校准周期，表示每cpu_adjust_period个时钟周期，加速或者减速1个周期 */
        unsigned int  reserved             : 6;  /* bit[24-29]: 保留 */
        unsigned int  cpu_adjust_en        : 1;  /* bit[30]   : 32k公共定时器的校准使能，该信号为1时，校准功能生效 */
        unsigned int  cpu_adjust_direction : 1;  /* bit[31]   : 32k公共定时器的时钟校准方向，1表示每cpu_adjust_period个时钟周期，加速1个周期
                                                                0 表示每cpu_adjust_period个时钟周期，减速1个周期 */
    } reg;
...
typedef union
{
    unsigned int      comm_timer_en_reg;
    struct
    {
        unsigned int  cpu_comm_timer_en : 1;  /* bit[0]   : 32k公共定时器的使能信号，需要使用公共定时器时，需配置为1 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_timer_rpt2_reg;
    struct
    {
        unsigned int  adjust_timer : 24; /* bit[0-23] : 32K校准定时器上报，测试用 */
...
typedef union
{
    unsigned int      dsds_sim_flag_reg;
    struct
    {
        unsigned int  cpu_sim1_en : 1;  /* bit[0]   : SIM1占用标志，软件用于读写，BBP不使用 */
        unsigned int  cpu_sim2_en : 1;  /* bit[1]   : SIM2占用标志，软件用于读写，BBP不使用 */
        unsigned int  reserved    : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_32k_int_reg;
    struct
    {
        unsigned int  comm_32k_int_sta : 1;  /* bit[0]   : 32K公共定时器的定时中断 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_32k_int_mask_reg;
    struct
    {
        unsigned int  comm_32k_int_mask : 1;  /* bit[0]   : 32K公共定时器的定时中断屏蔽寄存器，1表示屏蔽，0表示放开中断 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_32k_int_en_reg;
    struct
    {
        unsigned int  comm_32k_int_en : 1;  /* bit[0]   : 32k公共定时器中断使能寄存器。1，表示使能中断；0，表示不使能中断。 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_32k_int3_clr_reg;
    struct
    {
        unsigned int  comm_32k_int_clr : 1;  /* bit[0]   : 32k公共定时器中断清除寄存器。1，表示清除相应比特位置中断；0，表示不清除相应比特位置的中断。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      comm_32k_int_period_reg;
    struct
    {
        unsigned int  cpu_int_period : 24; /* bit[0-23] : 32K时钟域的自定义中断周期，配置范围是0~512s。单位是32k时钟的周期个数。每隔cpu_int_period个32k时钟合周期，产生1次中断。注意：如果期望的调整周期是N，则应配置N-1 */
        unsigned int  reserved       : 8;  /* bit[24-31]: 保留 */
    } reg;
