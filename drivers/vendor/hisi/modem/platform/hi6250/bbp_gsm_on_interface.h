...
typedef union
{
    unsigned int      drx_cpu_int_sta_reg;
    struct
    {
        unsigned int  reserved_0          : 24; /* bit[0-23] : 保留 */
        unsigned int  drx_int_state_cpu24 : 1;  /* bit[24]   : 唤醒中断输出，32KHz时钟域的脉冲信号，用于DRX睡眠结束之后唤醒ARM。
                                                               产生条件：定时唤醒。
                                                               注：CPU立即唤醒时需要软件屏蔽唤醒中断位，否则有可能收到无用的唤醒中断。 */
        unsigned int  drx_int_state_cpu25 : 1;  /* bit[25]   : GBBP到达定时休眠开始时刻产生的中断信号，一个104MHz脉冲信号。用于指示ARM切换到32KHz时钟。 */
        unsigned int  reserved_1          : 1;  /* bit[26]   : 保留 */
        unsigned int  drx_int_state_cpu27 : 1;  /* bit[27]   : 切换到104M中断输出，104M时钟域的脉冲信号，用于指示DRX睡眠结束之后唤醒ARM。
                                                               产生条件：CPU立即唤醒、定时唤醒。 */
...
typedef union
{
    unsigned int      drx_cpu_int_mask_reg;
    struct
    {
        unsigned int  reserved_0         : 24; /* bit[0-23] : 保留 */
        unsigned int  cpu_drx_int_mask24 : 1;  /* bit[24]   : 唤醒中断屏蔽寄存器，1表示屏蔽，0表示打开屏蔽。
                                                              注：CPU立即唤醒时需要软件屏蔽唤醒中断位，否则有可能收到无用的唤醒中断。 */
        unsigned int  cpu_drx_int_mask25 : 1;  /* bit[25]   : ARM指定的定时休眠开始之后发出的定时中断屏蔽寄存器 */
        unsigned int  reserved_1         : 1;  /* bit[26]   : 保留 */
        unsigned int  cpu_drx_int_mask27 : 1;  /* bit[27]   : 时钟切换到104M中断输出屏蔽寄存器 */
        unsigned int  reserved_2         : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drx_cpu_int_clr_reg;
    struct
    {
        unsigned int  reserved_0        : 24; /* bit[0-23] : 保留 */
        unsigned int  cpu_drx_int_clr24 : 1;  /* bit[24]   : 唤醒中断清零寄存器 */
        unsigned int  cpu_drx_int_clr25 : 1;  /* bit[25]   : ARM指定的定时休眠开始之后发出的定时中断清零寄存器 */
        unsigned int  reserved_1        : 1;  /* bit[26]   : 保留 */
        unsigned int  cpu_drx_int_clr27 : 1;  /* bit[27]   : 时钟切换到104M中断输出清零寄存器 */
        unsigned int  reserved_2        : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drx_dsp_int_sta_reg;
    struct
    {
        unsigned int  reserved_0          : 24; /* bit[0-23] : 保留 */
        unsigned int  drx_int_state_dsp24 : 1;  /* bit[24]   : 唤醒中断输出，32KHz时钟域的脉冲信号，用于DRX睡眠结束之后唤醒ARM。
                                                               产生条件：定时唤醒。
                                                               注：CPU立即唤醒时需要软件屏蔽唤醒中断位，否则有可能收到无用的唤醒中断。 */
        unsigned int  drx_int_state_dsp25 : 1;  /* bit[25]   : GBBP到达定时休眠开始时刻产生的中断信号，一个104MHz脉冲信号。用于指示ARM切换到32KHz时钟。 */
        unsigned int  reserved_1          : 1;  /* bit[26]   : 保留 */
        unsigned int  drx_int_state_dsp27 : 1;  /* bit[27]   : 切换到104M中断输出，104M时钟域的脉冲信号，用于指示DRX睡眠结束之后唤醒ARM。
                                                               产生条件：CPU立即唤醒、定时唤醒。 */
        unsigned int  reserved_2          : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drx_dsp_int_mask_reg;
    struct
    {
        unsigned int  reserved_0         : 24; /* bit[0-23] : 保留 */
        unsigned int  dsp_drx_int_mask24 : 1;  /* bit[24]   : 唤醒中断屏蔽寄存器，1表示屏蔽，0表示打开屏蔽。
                                                              注：CPU立即唤醒时需要软件屏蔽唤醒中断位，否则有可能收到无用的唤醒中断。 */
        unsigned int  dsp_drx_int_mask25 : 1;  /* bit[25]   : ARM指定的定时休眠开始之后发出的定时中断屏蔽寄存器 */
        unsigned int  reserved_1         : 1;  /* bit[26]   : 保留 */
        unsigned int  dsp_drx_int_mask27 : 1;  /* bit[27]   : 时钟切换到104M中断输出屏蔽寄存器 */
        unsigned int  reserved_2         : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drx_dsp_int_clr_reg;
    struct
    {
        unsigned int  reserved_0        : 24; /* bit[0-23] : 保留 */
        unsigned int  dsp_drx_int_clr24 : 1;  /* bit[24]   : 唤醒中断清零寄存器 */
        unsigned int  dsp_drx_int_clr25 : 1;  /* bit[25]   : ARM指定的定时休眠开始之后发出的定时中断清零寄存器 */
        unsigned int  reserved_1        : 1;  /* bit[26]   : 保留 */
        unsigned int  dsp_drx_int_clr27 : 1;  /* bit[27]   : 时钟切换到104M中断输出清零寄存器 */
        unsigned int  reserved_2        : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      arm_int_delay_reg_pd_reg;
    struct
    {
        unsigned int  cpu_int_dly_clk : 16; /* bit[0-15] : ARM 104M中断延迟上报间隔时间参数，单位为imi_clk */
...
typedef union
{
    unsigned int      dsp_int_delay_reg_pd_reg;
    struct
    {
        unsigned int  dsp_int_dly_clk : 16; /* bit[0-15] : PHY 104M中断延迟上报间隔时间参数，单位为imi_clk */
        unsigned int  reserved        : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  gdrx_sleep_en    : 1;  /* bit[0]   : bit[0] DRX睡眠启动，写1开启，1个104MHz。写0无意义。 */
        unsigned int  gdrx_sleep_mode  : 1;  /* bit[1]   : bit[1] 0：立即睡眠（默认），1：等到sleep_point时开始 */
        unsigned int  gdrx_wake_up_imi : 1;  /* bit[2]   : bit[2] CPU立即唤醒GDRX使能，逻辑自清零。该信号配置前应先确保GBBP 104M系统时钟已经打开并且稳定。 */
...
typedef union
{
    unsigned int      wake_cnf_reg;
    struct
    {
        unsigned int  gdrx_switch_len : 11; /* bit[0-10] : bit[10:0] 上报唤醒中断到时钟切换时间内32.768k计数个数。 */
...
typedef union
{
    unsigned int      wakeup_len_reg;
    struct
    {
        unsigned int  gdrx_wakeup_len : 26; /* bit[0-25] : 睡眠到上报唤醒中断这段时间内32.768k计数个数，该配置值必需大于1。 */
...
typedef union
{
    unsigned int      msr_cnf_reg;
    struct
    {
        unsigned int  gdrx_msr_frac     : 11; /* bit[0-10] : 睡眠计数小数部分，该计数器的进位表示1个104M时钟。 */
        unsigned int  reserved_0        : 5;  /* bit[11-15]: 保留 */
        unsigned int  gdrx_msr_integer  : 7;  /* bit[16-22]: 睡眠计数整数部分，少于96的104MHz时钟个数。
                                                             和gdrx_msr_timebase一起表示了睡眠计数的全部整数部分。 */
        unsigned int  reserved_1        : 1;  /* bit[23]   : 保留 */
        unsigned int  gdrx_msr_timebase : 6;  /* bit[24-29]: bit[29:24]睡眠计数96倍部分（指示1个32K有多少个96个104M钟的倍数）。
                                                             注：参数gdrx_msr_timebase、gdrx_msr_integer和gdrx_msr_frac表示了一个32K时钟包含的104M时钟的个数，分别以x、y和z表示这三个参数的取值。
                                                             经过时钟校准测量后可以得到一个32K时钟相当于m.n个104M时钟，m、n分别为整数和小数部分。它们之间有如下关系：m=x*96+y、n=z/2048。 */
...
typedef union
{
    unsigned int      cpu_drx_depth_cfg_reg;
    struct
    {
        unsigned int  cpu_drx_depth : 27; /* bit[0-26] : AMR判断GBBP睡眠时间深度的门限值，当GDRX内部32K计数值大于该门限时，内部连线信号为1，通知ARM。 */
...
typedef union
{
    unsigned int      wake_gtc_qb_frac_rpt_reg;
    struct
    {
        unsigned int  gdrx_gtc_wake_frac : 7;  /* bit[0-6]  : DRX唤醒切换时加载到GTC的FRAC计数值，测试用 */
        unsigned int  reserved_0         : 9;  /* bit[7-15] : 保留 */
        unsigned int  gdrx_gtc_wake_chip : 13; /* bit[16-28]: DRX唤醒切换时加载到GTC的gtc_timebase计数值，测试用 */
...
typedef union
{
    unsigned int      wake_gtc_t1_t2_t3_rpt_reg;
    struct
    {
        unsigned int  gdrx_gtc_t1_cnt : 11; /* bit[0-10] : DRX唤醒切换时加载到GTC的T1计数值，测试用 */
        unsigned int  reserved_0      : 5;  /* bit[11-15]: 保留 */
        unsigned int  gdrx_gtc_t2_cnt : 5;  /* bit[16-20]: DRX唤醒切换时加载到GTC的T2计数值，测试用 */
        unsigned int  reserved_1      : 3;  /* bit[21-23]: 保留 */
        unsigned int  gdrx_gtc_t3_cnt : 6;  /* bit[24-29]: DRX唤醒切换时加载到GTC的T3计数值，测试用 */
        unsigned int  reserved_2      : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drx_wake_gtc_fn_rpt_reg;
    struct
    {
        unsigned int  gdrx_gtc_fn_low  : 11; /* bit[0-10] : DRX唤醒切换时加载到GTC的FN_LOW计数值，测试用 */
        unsigned int  reserved_0       : 5;  /* bit[11-15]: 保留 */
        unsigned int  gdrx_gtc_fn_high : 11; /* bit[16-26]: DRX唤醒切换时加载到GTC的FN_HIGH计数值，测试用 */
...
typedef union
{
    unsigned int      gdrx_int_mask_state_rpt_reg;
    struct
    {
        unsigned int  drx_int_state_cpu    : 1;  /* bit[0]    : DRX模块中断状态上报ARM
                                                                中断状态上报即是0x01200寄存器bit24上报值； 测试用 */
        unsigned int  reserved_0           : 3;  /* bit[1-3]  : 保留 */
        unsigned int  drx_int_state_dsp    : 1;  /* bit[4]    : DRX模块中断状态上报PHY
                                                                中断状态上报即是0x01200寄存器bit24上报值；，测试用 */
        unsigned int  reserved_1           : 11; /* bit[5-15] : 保留 */
        unsigned int  cpu_drx_int_mask_32k : 1;  /* bit[16]   : DRX模块中断屏蔽ARM配置值上报，DRX模块的中断屏蔽即是0xd1204寄存器bit24配置值，测试用 */
        unsigned int  reserved_2           : 3;  /* bit[17-19]: 保留 */
        unsigned int  dsp_drx_int_mask_32k : 1;  /* bit[20]   : DRX模块中断屏蔽PHY配置值上报，DRX模块的中断屏蔽即是0xD1214寄存器bit24配置值，测试用 */
...
typedef union
{
    unsigned int      drx_test_pin_sel_reg;
    struct
    {
        unsigned int  gdrx_test_pin_sel      : 4;  /* bit[0-3] : GDRX模块测试引脚选择信号 */
        unsigned int  reserved               : 27; /* bit[4-30]: 保留 */
        unsigned int  gdrx_test_pin_sel_flag : 1;  /* bit[31]  : GBBP模块输出的testpin选择信号，该信号为1时，选择非掉电区的GDRX的testpin，否则选择掉电区输出的test_pin */
    } reg;
