...
/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      DLFE RX blanking使能，配置1时启动RX blanking功能，对数据做填0处理。
   UNION结构:  WBBP_RXBLANK_EN_UNION */
#define WBBP_RXBLANK_EN_ADDR                          (SOC_BBP_WCDMA_BASE_ADDR + 0x91694)

/* 寄存器说明：
 bit[31:29]  保留
 bit[28:0]   RX blanking信号起始位置的C模base_cnt上报，当对应的cpu_rxblank_start_pos_valid有效时该值有效。
   UNION结构:  WBBP_RXBLANK_START_POS_UNION */
#define WBBP_RXBLANK_START_POS_ADDR                   (SOC_BBP_WCDMA_BASE_ADDR + 0x91698)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      RX blanking信号起始位置的C模base_cnt上报有效标志，BBP在rxblank_ind的上升沿锁存base_cnt值同时拉高valid，软件读取后配置cpu_rxblank_start_pos_clr信号清除标志。
   UNION结构:  WBBP_RXBLANK_START_POS_VALID_UNION */
#define WBBP_RXBLANK_START_POS_VALID_ADDR             (SOC_BBP_WCDMA_BASE_ADDR + 0x9169C)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      cpu_rxblank_start_pos_valid清除标志，脉冲信号，软件读取cpu_rxblank_start_pos后配置该信号，该信号需要保证在下一次的rxbland_ind到来前配置，否则会造成错误。
   UNION结构:  WBBP_RXBLANK_START_POS_CLR_UNION */
#define WBBP_RXBLANK_START_POS_CLR_ADDR               (SOC_BBP_WCDMA_BASE_ADDR + 0x916A0)

/* 寄存器说明：
 bit[31:29]  保留
 bit[28:0]   RX blanking信号起始位置的C模base_cnt上报，当对应的cpu_rxblank_end_pos_valid有效时该值有效
   UNION结构:  WBBP_RXBLANK_END_POS_UNION */
#define WBBP_RXBLANK_END_POS_ADDR                     (SOC_BBP_WCDMA_BASE_ADDR + 0x916A4)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      RX blanking信号起始位置的C模base_cnt上报有效标志，BBP在rxblank_ind的下降沿锁存base_cnt值同时拉高valid，软件读取后配置cpu_rxblank_end_pos_clr信号清除标志。
   UNION结构:  WBBP_RXBLANK_END_POS_VALID_UNION */
#define WBBP_RXBLANK_END_POS_VALID_ADDR               (SOC_BBP_WCDMA_BASE_ADDR + 0x916A8)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      cpu_rxblank_end_pos_valid清除标志，脉冲信号，软件读取cpu_rxblank_end_pos后配置该信号，该信号需要保证在下一次的rxbland_ind到来前配置，否则会造成错误。
   UNION结构:  WBBP_RXBLANK_END_POS_CLR_UNION */
#define WBBP_RXBLANK_END_POS_CLR_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x916AC)



/***======================================================================***
                     (2/9) register_define_w_dlfe_dcsc
 ***======================================================================***/
/* 寄存器说明：
 bit[31:16]  天线2一帧内DAGC IQ数据饱和次数。
 bit[15:0]   天线1一帧内DAGC IQ数据饱和次数。
...
/* 寄存器说明：ZSP1中断标志寄存器
 bit[31:7]   保留
 bit[6]      发送完成中断标志：0此中断未产生；1 此中断产生。
 bit[5]      发送存储器全空中断标志：0此中断未产生；1 此中断产生。
 bit[4]      发送存储器半空中断标志：0此中断未产生；1 此中断产生。
...
/* 寄存器说明：
 bit[31:10]  保留
 bit[9:8]    ABB数据由14bit转换12bit的控制。
             0表示四舍五入2bit，限幅至12bit；
             1表示四舍五入1bit，限幅至12bit；
             2表示四舍五入0bit，限幅至12bit。
 bit[7:5]    保留
 bit[4]      下行采样数据格式标志
             1'b0:二进制补码
             1'b1:自然码
...
/* 寄存器说明：
 bit[31:17]  保留
 bit[16:12]  为了调整定时偏差，使RX通道A与通道B的IQ数据对齐，该配置为指定通道
             延迟采样占个数与该值实际数值相同，只取0~26的值，为0即不延迟。tds 专用。
 bit[11:9]   保留
 bit[8]      定时偏差调整功能打开使能，高电平有效。tds 专用。
 bit[7:5]    保留
 bit[4]      定时偏差调整功能通道选择，0表示A通道，1表示B通道。tds 专用。
 bit[3:1]    保留
 bit[0]      接收通道个数：
             1'b1：双天线（天线0和天线1）；
             1'b0：单天线（天线0）。tds 专用。
...
/* 寄存器说明：
 bit[31:7]   保留
 bit[6:4]    ABB环回模式选择信号。
             3'd0：ABB下行A通道FIFO前环回至上行；
             3'd1：ABB下行B通道FIFO前环回至上行；
             3'd2：ABB下行主集通道FIFO后环回至上行；
             3'd3：ABB下行分集通道FIFO后环回至上行；
             3'd4：ABB上行环回至下行A通道。
 bit[3:1]    保留
 bit[0]      ABB环回模式使能信号，高电平有效
   UNION结构:  WBBP_ABB_TRX_LOOP_UNION */
#define WBBP_ABB_TRX_LOOP_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x96038)

/* 寄存器说明：
 bit[31:13]  保留
 bit[12]     RX 分集FIFO曾经满状态清零信号。
 bit[11:9]   保留
 bit[8]      RX 主集FIFO曾经满状态清零信号。
 bit[7:5]    保留
 bit[4]      RX 分集FIFO曾经满状态锁存上报。可被dsp_abb_rx_fifo1_state_clr清零。
 bit[3:1]    保留
 bit[0]      RX 主集FIFO曾经满状态锁存上报。可被dsp_abb_rx_fifo0_state_clr清零。
   UNION结构:  WBBP_ABB_FIFO_DBG_UNION */
#define WBBP_ABB_FIFO_DBG_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x9603C)

/* 寄存器说明：
 bit[31:16]  U&TDS融合模块RAM控制低32比特。
 bit[15:0]   
...
/* 寄存器说明：
 bit[31:24]  保留
 bit[23:16]  U&TDS融合模块ROM控制比特。
 bit[15:0]   U&TDS融合模块RAM控制高16比特。
   UNION结构:  WBBP_SHARE_ROM_CTRL_UNION */
#define WBBP_SHARE_ROM_CTRL_ADDR                      (SOC_BBP_WCDMA_BASE_ADDR + 0x96064)

/* 寄存器说明：
 bit[31:14]  保留
 bit[13:12]  IMI ADDR选择使能。高比特对应上行前端，低比特对应下行前端。
 bit[11:10]  保留
 bit[9:8]    ULFE模块IMI时钟域RST控制。高比特对应上行前端，低比特对应下行前端。在对模块进行复位时，将该bit置1，之后再将该bit复位为0。
 bit[7:6]    保留
 bit[5:4]    RST控制。高比特对应上行前端，低比特对应下行前端。在对模块进行复位时，将该bit置1，之后再将该bit复位为0。
             bit[4]下行软复位和天线使能配置约束：复位前先关闭wt_dlfe模块0x91400寄存器bit[1:0]天线使能，然后进行软复位，撤销复位后再打开wt_dlfe模块0x91400寄存器bit[1:0]天线使能。并且天线使能关闭时间要大于2个符号。
 bit[3]      保留
 bit[2:0]    CLK控制。bit[2]对应上行前端射频控制，bit[1]对应上行前端数据通路，bit[0]对应下行前端。在对前端模块进行时钟控制时，将该bit置1表示打开模块的时钟；反之，将该bit置0表示关闭ULFE模块的时钟。
   UNION结构:  WBBP_SYS_RST_UNION */
#define WBBP_SYS_RST_ADDR                             (SOC_BBP_WCDMA_BASE_ADDR + 0x96068)

/* 寄存器说明：
   详      述：MCTRL冗余寄存器1，EC留用
   UNION结构 ：无 */
#define WBBP_MCTR_REDUND1_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x9606C)

/* 寄存器说明：
   详      述：MCTRL冗余寄存器2，EC留用
   UNION结构 ：无 */
#define WBBP_MCTR_REDUND2_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x96070)

/* 寄存器说明：
   详      述：MCTRL冗余寄存器3，EC留用
   UNION结构 ：无 */
...
/* 寄存器说明：
 ...
 bit[6]      FIFO_1空信号上报
 bit[5]      FIFO_1满信号上报
 bit[4]      fifo_1复位信号，在FIFO状态异常时，配置为1，进行清零
 bit[3]      保留
 bit[2]      FIFO_0空信号上报
 bit[1]      FIFO_0满信号上报
 bit[0]      fifo_0复位信号，在FIFO_0状态异常时，配置为1，进行清零
...
/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      W软基带使能。
   UNION结构:  WBBP_CS_W_SDR_EN_UNION */
#define WBBP_CS_W_SDR_EN_ADDR                         (SOC_BBP_WCDMA_BASE_ADDR + 0x9607C)
...
/* 寄存器说明：
 ...
 bit[0]      sdr mem模式选择信号
             0：时钟122M
             1：时钟245M
   UNION结构:  WBBP_SHARE_MEM_SDR_SEL_UNION */
#define WBBP_SHARE_MEM_SDR_SEL_ADDR                   (SOC_BBP_WCDMA_BASE_ADDR + 0x96080)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      w_tds_mem_share模块的时钟门控bypass信号
   UNION结构:  WBBP_SHARE_MEM_CLK_BYPASS_UNION */
#define WBBP_SHARE_MEM_CLK_BYPASS_ADDR                (SOC_BBP_WCDMA_BASE_ADDR + 0x96084)



/***======================================================================***
                     (4/9) register_define_w_ulfe_mem
 ***======================================================================***/
...
/* 寄存器说明：
 bit[31]     DSP强制衰减使能信号。
 bit[30:23]  保留
 bit[22:16]  DSP强制DBB衰减补偿增益。取值范围上下门限软件可控，默认为[20,120]，超过120按120处理。精度0.1dB。
             20表示数字功率增加1dB，21表示增加0.9dB，以此类推，取值每增加1，功率增加值减少0.1dB，30表示增加0dB，31表示减少0.1dB，……，120表示减少9dB。
...
/* 寄存器说明：
 ...
 bit[22:16]  RFCMOS数字回退功率上报，单位0.1dB.取值范围上下门限软件可控，默认为[20,120],对应回退功率范围[增加1dB,减少9dB]
...
/* 寄存器说明：
 bit[31:21]  保留
 bit[20:19]  驻波检测时隙前后向功率检测次数，2bits无符号数。默认值为0时，表示检测次数为4次；其他值暂时不用，用于扩展。
 bit[18:16]  保留
 bit[15]     驻波检测有效清零信号。
 bit[14:5]   前反向通道切换等待延时，单位为chip，默认值为80。
...
/* 寄存器说明：
 bit[31:10]  保留
 bit[9:0]    上变频提前上行时隙头生效时刻。单位1/4chip。
...
/* 寄存器说明：
 bit[31:10]  保留
 bit[9:0]    ET Gain1/Gain2参数提前上行时隙头生效时刻。单位1/4chip。
...
/* 寄存器说明：
 bit[31:7]   保留
 bit[6:0]    DPD power shift db值
   UNION结构:  WBBP_PWR_SFT_RPT_UNION */
#define WBBP_PWR_SFT_RPT_ADDR                         (SOC_BBP_WCDMA_BASE_ADDR + 0x9D4DC)

/* 寄存器说明：
   详      述：hkadc处理超时计数上报，可维可测使用，每次时隙头到来时状态机不在IDLE态时计数加1。PD或上行关闭时清零。
   UNION结构 ：无 */
#define WBBP_HKADC_ABNORMAL_CNT_RPT_ADDR              (SOC_BBP_WCDMA_BASE_ADDR + 0x9D4E0)



/***======================================================================***
                     (6/9) register_define_w_pd_mem
 ***======================================================================***/
/* 寄存器说明：PD功率表
 bit[31:10]  保留
...
/***======================================================================***
                     (7/9) register_define_ulfe_wt_share
 ***======================================================================***/
/* 寄存器说明：门控时钟
 bit[31:19]  保留
...
/* 寄存器说明：ULFE内部模块功能开关寄存器
 bit[31:21]  保留
 bit[20]     DCUPANCO工作使能。1：使能NCO功能；0：不使能NCO功能。
 bit[19]     DPD工作使能。1：使能DPD功能；0：不使能DPD功能。
...
/* 寄存器说明：配置特殊时隙UpPTS参数标识
 bit[31:1]   保留
 bit[0]      DSP在配置特殊时隙UpPTS参数时一定要配置该寄存器标识：
             0：非特殊时隙UpPTS参数；
             1：特殊时隙UpPTS参数；
             该参数在配置完其它上行参数后最后配置。
...
/* 寄存器说明：CPU接口配置的单音频率选择信号
 bit[31:1]   保留
 bit[0]      CPU接口配置的单音频率选择信号：
             0:0.96M单音
             1:0.48M单音，默认为0.96M单音，前向兼容原有版本。
...
/* 寄存器说明：主辅载波上变频选择
 bit[31:5]   保留
 bit[4]      DSP强制配置NCO频点搬移、主辅载波相加是否随dcupa_active_flag指示生效。为1表示频点受dcupa_active_flag控制；为0表示不受dcupa_active_flag控制，默认为1。
 bit[3:1]    保留
 bit[0]      DSP是否强制配置NCO频点搬移、主辅载波相加：1表示强制NCO频点搬移、主辅载波相加；0表示该信号不起作用
...
/* 寄存器说明：DPD bypass
 bit[31:1]   保留
 bit[0]      DPD补偿通路bypass功能，当DPD使能打开并且配置了bypass功能时处理延时与DPD相同，但是不做数据通路补偿。
...
/* 寄存器说明：C模DPD相关配置1
 bit[31:23]  保留
 bit[22:12]  C模上行最大理论发射功率修正值，有符号数
 bit[11]     保留
 bit[10:0]   C模上行理论发射功率，有符号数
   UNION结构:  WBBP_C_DPD_CONFIG1_UNION */
#define WBBP_C_DPD_CONFIG1_ADDR                       (SOC_BBP_WCDMA_BASE_ADDR + 0x9EAA0)

/* 寄存器说明：C模DPD相关配置2
 bit[31:24]  保留
 bit[23:12]  C模DPD power_shift，无符号数
 bit[11:9]   保留
 bit[8:0]    C模RF回退索引值，无符号数
   UNION结构:  WBBP_C_DPD_CONFIG2_UNION */
#define WBBP_C_DPD_CONFIG2_ADDR                       (SOC_BBP_WCDMA_BASE_ADDR + 0x9EAA4)



/***======================================================================***
                     (8/9) register_define_wc_dump
 ***======================================================================***/
/* 寄存器说明：
 ...
 bit[11:10]  保留
 bit[9:8]    下行SDR IF模块工作模式。0表示W模；1表示无意义；2表示1X；3表示EVDO。
 bit[7:5]    保留
 bit[4]      下行SDR IF模块搬运启停模式。0，主分集都用主集启停信号；1，主分集用各自启停信号。
 bit[3:1]    保留
 bit[0]      下行SDR IF模块使能，高电平有效。
   UNION结构:  WBBP_DLFE_SDR_EN_UNION */
#define WBBP_DLFE_SDR_EN_ADDR                         (SOC_BBP_WCDMA_BASE_ADDR + 0x98000)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      下行搬数时钟门控bypass信号，高电平有效，表示时钟常开。
   UNION结构:  WBBP_DLFE_SDR_CKG_BYS_UNION */
#define WBBP_DLFE_SDR_CKG_BYS_ADDR                    (SOC_BBP_WCDMA_BASE_ADDR + 0x98004)
...
/* 寄存器说明：
 ...
 bit[3:1]    保留
 bit[0]      下行主集搬运启动脉冲。
   UNION结构:  WBBP_DLFE_SDR_START_STOP_UNION */
#define WBBP_DLFE_SDR_START_STOP_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x98008)
...
/* 寄存器说明：
 ...
 bit[11:0]   下行Buffer深度，单位32word，用于计算Buffer首地址的时间戳。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_UNION */
#define WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_ADDR          (SOC_BBP_WCDMA_BASE_ADDR + 0x9800C)
...
/* 寄存器说明：
 ...
 bit[21:0]   下行搬运启动时刻。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_DLFE_SDR_TIME_CTRL_UNION */
#define WBBP_DLFE_SDR_TIME_CTRL_ADDR                  (SOC_BBP_WCDMA_BASE_ADDR + 0x98010)

/* 寄存器说明：
 bit[31:12]  保留
 bit[11:0]   下行搬运数据量，单位32word。
             该参数仅在cpu_dlfe_sdr_stop_mode为1时有效。
   UNION结构:  WBBP_DLFE_SDR_TIME_CTRL2_UNION */
#define WBBP_DLFE_SDR_TIME_CTRL2_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x98014)
...
/* 寄存器说明：
 ...
 bit[15:0]   下行Buffer深度，单位word。
             ...
             建议在G模软化时使用。
             G模软化含义无效。
   UNION结构:  WBBP_DLFE_SDR_WORD_DEPTH_UNION */
#define WBBP_DLFE_SDR_WORD_DEPTH_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x98018)

/* 寄存器说明：
 bit[31:6]   保留
 bit[5:0]    下行天线1当前搬运word数。
   UNION结构:  WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98040)

/* 寄存器说明：
 bit[31:12]  下行天线1当前搬运页数。根据cpu_dlfe_sdr_buffer_depth统计。
 bit[11:0]   下行天线1当前搬运块数。根据cpu_dlfe_sdr_buffer_depth统计。
   UNION结构:  WBBP_DLFE_SDR_AT1_RD_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_RD_RPT_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x98044)

/* 寄存器说明：
 bit[31:6]   保留
 bit[5:0]    下行天线2当前搬运word数。
   UNION结构:  WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98048)

/* 寄存器说明：
 bit[31:12]  下行天线2当前搬运页数。根据cpu_dlfe_sdr_buffer_depth统计。
 bit[11:0]   下行天线2当前搬运块数。根据cpu_dlfe_sdr_buffer_depth统计。
   UNION结构:  WBBP_DLFE_SDR_AT2_RD_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_RD_RPT_ADDR                 (SOC_BBP_WCDMA_BASE_ADDR + 0x9804C)
...
/* 寄存器说明：
 ...
 bit[27:0]   下行天线1启动搬运后第一个数据对应时间戳。
             ...
             bit[27:16]：cfn。
             G模软化含义无效。
   UNION结构:  WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_ADDR          (SOC_BBP_WCDMA_BASE_ADDR + 0x98050)

/* 寄存器说明：
 bit[31:28]  保留
 bit[27:0]   下行天线2启动搬运后第一个数据对应base_cnt时间戳，单位chip。
             时间戳含义同上。
   UNION结构:  WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_ADDR          (SOC_BBP_WCDMA_BASE_ADDR + 0x98054)
...
/* 寄存器说明：
 ...
 bit[27:0]   下行天线1启动搬运后Buffer首地址数据对应时间戳，经过时隙头锁存。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_ADDR       (SOC_BBP_WCDMA_BASE_ADDR + 0x98058)
...
/* 寄存器说明：
 ...
 bit[27:0]   下行天线2启动搬运后Buffer首地址数据对应时间戳，经过时隙头锁存。
             时间戳含义同上。
             时隙头含义同上。
   UNION结构:  WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_ADDR       (SOC_BBP_WCDMA_BASE_ADDR + 0x9805C)
...
/* 寄存器说明：
   详      述：下行天线1接收数据。
               ...
               G模软化含义无效。
   UNION结构 ：无 */
#define WBBP_DLFE_SDR_AT1_DATA_ADDR                   (SOC_BBP_WCDMA_BASE_ADDR + 0x98060)
...
/* 寄存器说明：
   详      述：下行天线2接收数据。
               ...
               G模软化含义无效。
   UNION结构 ：无 */
#define WBBP_DLFE_SDR_AT2_DATA_ADDR                   (SOC_BBP_WCDMA_BASE_ADDR + 0x98068)
...
/* 寄存器说明：
 ...
 bit[3:1]    保留
 bit[0]      下行天线1搬运状态。
   UNION结构:  WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98070)
...
/* 寄存器说明：
 ...
 bit[3:1]    保留
 bit[0]      下行天线2搬运状态。
   UNION结构:  WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98074)
...
/* 寄存器说明：
 ...
 bit[15:0]   下行天线1当前搬运字数。根据cpu_dlfe_sdr_word_depth统计。
             建议在G模软化时使用。
   UNION结构:  WBBP_DLFE_SDR_AT1_RD_WORD_RPT_UNION */
#define WBBP_DLFE_SDR_AT1_RD_WORD_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98078)
...
/* 寄存器说明：
 ...
 bit[15:0]   下行天线2当前搬运字数。根据cpu_dlfe_sdr_word_depth统计。
             建议在G模软化时使用。
   UNION结构:  WBBP_DLFE_SDR_AT2_RD_WORD_RPT_UNION */
#define WBBP_DLFE_SDR_AT2_RD_WORD_RPT_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x9807C)
...
/* 寄存器说明：
 ...
 bit[0]      上行SDR IF模块使能，高电平有效。
   UNION结构:  WBBP_ULFE_SDR_EN_UNION */
#define WBBP_ULFE_SDR_EN_ADDR                         (SOC_BBP_WCDMA_BASE_ADDR + 0x98100)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      上行搬数时钟门控bypass信号，高电平有效，表示时钟常开。
   UNION结构:  WBBP_ULFE_SDR_CKG_UNION */
#define WBBP_ULFE_SDR_CKG_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x98104)
...
/* 寄存器说明：
 ...
 bit[3:0]    保留
   UNION结构:  WBBP_ULFE_SDR_START_UNION */
#define WBBP_ULFE_SDR_START_ADDR                      (SOC_BBP_WCDMA_BASE_ADDR + 0x98108)
...
/* 寄存器说明：
 ...
 bit[12:0]   C模时，表示上行时隙头相对sys_cnt时隙边界提前量，单位1/4chip。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_ULFE_SDR_OFFSET_UNION */
#define WBBP_ULFE_SDR_OFFSET_ADDR                     (SOC_BBP_WCDMA_BASE_ADDR + 0x9810C)
...
/* 寄存器说明：
 ...
 bit[12:0]   C模时，表示上行dbb控制字提前时隙头生效位置，单位1/4chip。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_ULFE_SDR_ACTIVE_TIME_UNION */
#define WBBP_ULFE_SDR_ACTIVE_TIME_ADDR                (SOC_BBP_WCDMA_BASE_ADDR + 0x98110)

/* 寄存器说明：
 bit[31:13]  保留
 bit[12:0]   C模时，表示上行dpd功率提前时隙头生效位置，单位1/4chip。
   UNION结构:  WBBP_ULFE_SDR_DPD_ACTIVE_TIME_UNION */
#define WBBP_ULFE_SDR_DPD_ACTIVE_TIME_ADDR            (SOC_BBP_WCDMA_BASE_ADDR + 0x98114)
...
/* 寄存器说明：
 ...
 bit[3:1]    保留
 bit[0]      上行搬运状态。
   UNION结构:  WBBP_ULFE_SDR_DUMP_EN_RPT_UNION */
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ADDR                (SOC_BBP_WCDMA_BASE_ADDR + 0x98140)

/* 寄存器说明：
   详      述：WC前端AGC采数长度，即需要采集的32bit数据个数。全0表示无限长。
   UNION结构 ：无 */
#define WBBP_DUMP_DATA_LENGTH_W_ADDR                  (SOC_BBP_WCDMA_BASE_ADDR + 0x98200)

/* 寄存器说明：
 bit[31:20]  WC前端AGC采数采数起始位置。全1表示立即启动。
             W模，CFN位置。
             C模，bit[28:0]表示c_base_cnt上的位置，bit[31:29]配置为0。
 bit[19:16]  WC前端AGC采数采数起始位置。全1表示立即启动。
             W模，帧内slot位置。
 bit[15:4]   WC前端AGC采数采数起始位置。全1表示立即启动。
             W模，slot内chip位置。
 bit[3:0]    WC前端AGC采数采数起始位置。全1表示立即启动。
             W模，chip内clk位置。
   UNION结构:  WBBP_DUMP_START_TIME_W_UNION */
#define WBBP_DUMP_START_TIME_W_ADDR                   (SOC_BBP_WCDMA_BASE_ADDR + 0x98204)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      WC前端AGC采数启动脉冲。
   UNION结构:  WBBP_DUMP_START_W_UNION */
#define WBBP_DUMP_START_W_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x98208)

/* 寄存器说明：
 bit[31:1]   保留
 bit[0]      WC前端AGC采数停止脉冲。
   UNION结构:  WBBP_DUMP_STOP_W_UNION */
#define WBBP_DUMP_STOP_W_ADDR                         (SOC_BBP_WCDMA_BASE_ADDR + 0x9820C)

/* 寄存器说明：
   详      述：WC前端AGC采数接收数据。
   UNION结构 ：无 */
#define WBBP_DUMP_DMA_RDATA_W_ADDR                    (SOC_BBP_WCDMA_BASE_ADDR + 0x98240)

/* 寄存器说明：
   详      述：WC前端AGC采数起始时刻上报。
   UNION结构 ：无 */
#define WBBP_DUMP_STAMP_W_PART1_CPU_ADDR              (SOC_BBP_WCDMA_BASE_ADDR + 0x98250)

/* 寄存器说明：
   详      述：WC前端AGC采数起始时刻上报。
   UNION结构 ：无 */
#define WBBP_DUMP_STAMP_W_PART2_CPU_ADDR              (SOC_BBP_WCDMA_BASE_ADDR + 0x98254)



/***======================================================================***
                     (9/9) register_define_wc_dump_mem
 ***======================================================================***/
/* 寄存器说明：
 bit[31:28]  保留
 ...
 bit[11:0]   C模时，表示上行前端数据I路，12比特有符号数，软件每1/4 PCG或slot写一遍。
             ...
             G模软化含义无效。
   UNION结构:  WBBP_ULFE_SDR_RAM_UNION */
#define WBBP_ULFE_SDR_RAM_ADDR                        (SOC_BBP_WCDMA_BASE_ADDR + 0x98800)
#define WBBP_ULFE_SDR_RAM_MEMDEPTH  (512)
...
typedef union
{
    unsigned int      aagc_rssi_5s_inc_th_reg;
    struct
    {
        unsigned int  s1_2s2_5s_th : 8;  /* bit[0-7]  : 五档AAGC状态跳转S1->S2的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S1->S2->S3->S4->S5的顺序，天线口的RSSI递增，AAGC的增益递减。
                                                        C模不配置。 */
        unsigned int  s2_2s3_5s_th : 8;  /* bit[8-15] : 五档AAGC状态跳转S2->S3的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s3_2s4_5s_th : 8;  /* bit[16-23]: 五档AAGC状态跳转S3->S4的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s4_2s5_5s_th : 8;  /* bit[24-31]: 五档AAGC状态跳转S4->S5的RSSI门限。
                                                        C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_5s_dec_th_reg;
    struct
    {
        unsigned int  s2_2s1_5s_th : 8;  /* bit[0-7]  : 五档AAGC状态跳转S2->S1的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S5->S4->S3->S2->S1的顺序，天线口的RSSI递减，AAGC的增益递增。
                                                        C模不配置。 */
        unsigned int  s3_2s2_5s_th : 8;  /* bit[8-15] : 五档AAGC状态跳转S3->S2的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s4_2s3_5s_th : 8;  /* bit[16-23]: 五档AAGC状态跳转S4->S3的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s5_2s4_5s_th : 8;  /* bit[24-31]: 五档AAGC状态跳转S5->S4的RSSI门限。
                                                        C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      s1_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s1_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S1（RF RSSI为-102~-90dBm）状态时主集RF控制字。
                                                             C模不配置。 */
...
typedef union
{
    unsigned int      s2_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s2_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S2（RF RSSI为-87~-69dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s3_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s3_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S3（RF RSSI为-66~-54dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s4_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s4_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S4（RF RSSI为-51~-39dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s5_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s5_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S5（RF RSSI为-36~-21dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s1_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s1_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S1（RF RSSI为-102~-90dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s2_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s2_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S2（RF RSSI为-87~-69dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s3_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s3_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S3（RF RSSI为-66~-54dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s4_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s4_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S4（RF RSSI为-51~-39dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s5_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s5_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S5（RF RSSI为-36~-21dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_offset_at1_reg;
    struct
    {
        unsigned int  dsp_dc_offset_i_at1   : 12; /* bit[0-11] : 天线1接收I路直流偏置值。 */
        unsigned int  reserved_0            : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_dc_offset_q_at1   : 12; /* bit[16-27]: 天线1接收Q路直流偏置值。 */
        unsigned int  dsp_dc_offset_sel_at1 : 1;  /* bit[28]   : DSP配置的天线1直流偏置消除模块反馈值选择信号。1，选择dc_offset；0，选择计算获得的反馈值。 */
        unsigned int  dsp_dcr_en_at1        : 1;  /* bit[29]   : DSP控制天线1 DCR启停使能。1，DSP打开DCR；0，DSP关闭DCR，IQ数据直通。 */
...
typedef union
{
    unsigned int      dc_offset_at2_reg;
    struct
    {
        unsigned int  dsp_dc_offset_i_at2   : 12; /* bit[0-11] : 天线2接收I路直流偏置值。 */
        unsigned int  reserved_0            : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_dc_offset_q_at2   : 12; /* bit[16-27]: 天线2接收Q路直流偏置值。 */
        unsigned int  dsp_dc_offset_sel_at2 : 1;  /* bit[28]   : DSP配置的天线2直流偏置消除模块反馈值选择信号。1，选择dc_offset；0，选择计算获得的反馈值。 */
        unsigned int  dsp_dcr_en_at2        : 1;  /* bit[29]   : DSP控制天线2 DCR启停使能。1，DSP打开DCR；0，DSP关闭DCR，IQ数据直通。 */
        unsigned int  reserved_1            : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_offset_shift_at1_reg;
    struct
    {
        unsigned int  dsp_dcr_acc_sel_1st_intra_at1 : 2;  /* bit[0-1]  : DSP配置天线1本频去直流模块直流偏置值的计算，用于配置增益调整后第一阶段累加值的右移位数。0，右移12位；1，右移11位；2，右移10位；3，右移8位。 */
        unsigned int  dsp_dcr_acc_sel_2nd_intra_at1 : 2;  /* bit[2-3]  : DSP配置天线1本频去直流模块直流偏置值的计算，用于配置增益调整后第二阶段累加值的右移位数。0，右移16位；1，右移14位。 */
        unsigned int  dsp_dcr_dly_sel_at1           : 2;  /* bit[4-5]  : 增益调整后，不进行DCR计算的时间选择。
                                                                         W下：0，表示16chip；1，表示24chip；2，表示32chip；3，表示8chip。
                                                                         1X下：0，表示0chip；1，表示8chip。
                                                                         1X默认为8chip；EVDO默认24chip。 */
        unsigned int  reserved_0                    : 2;  /* bit[6-7]  : 保留 */
        unsigned int  dsp_dcr_period_1st_at1        : 4;  /* bit[8-11] : 增益调整后第一阶段的时间，单位256chip。
                                                                         1X下为1536chip，故需配置0x6；EVDO下为2048chip，故需配置为0x8。 */
        unsigned int  reserved_1                    : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_dcr_alg_sel_at1           : 2;  /* bit[16-17]: DCR算法选择。
                                                                         00，表示使用与V2相同的算法；
                                                                         01，表示使用V3R1算法；
                                                                         11，表示使用V3R2算法。
                                                                         1X仅支持V3R2算法；EVDO默认配置为V3R2算法，支持配置为V3R1算法。 */
        unsigned int  reserved_2                    : 6;  /* bit[18-23]: 保留 */
        unsigned int  dsp_dcr_acc_sel_1st_inter_at1 : 2;  /* bit[24-25]: DSP配置天线1异频去直流模块直流偏置值的计算，用于配置增益调整后第一阶段累加值的右移位数。0，右移12位；1，右移11位；2，右移10位；3，右移8位。 */
        unsigned int  dsp_dcr_acc_sel_2nd_inter_at1 : 2;  /* bit[26-27]: DSP配置天线1异频去直流模块直流偏置值的计算，用于配置增益调整后第二阶段累加值的右移位数。0，右移16位；1，右移14位。 */
        unsigned int  dsp_dcr_inherit_at1           : 1;  /* bit[28]   : 0，档位调整时，DC从0开始重新收敛；1，档位调整时，DC继承先前计算值。 */
...
typedef union
{
    unsigned int      dc_offset_shift_at2_reg;
    struct
    {
        unsigned int  dsp_dcr_acc_sel_1st_intra_at2 : 2;  /* bit[0-1]  : DSP配置天线2本频去直流模块直流偏置值的计算，用于配置增益调整后第一阶段累加值的右移位数。0，右移12位；1，右移11位；2，右移10位；3，右移8位。 */
        unsigned int  dsp_dcr_acc_sel_2nd_intra_at2 : 2;  /* bit[2-3]  : DSP配置天线2本频去直流模块直流偏置值的计算，用于配置增益调整后第二阶段累加值的右移位数。0，右移16位；1，右移14位。 */
        unsigned int  dsp_dcr_dly_sel_at2           : 2;  /* bit[4-5]  : 增益调整后，不进行DCR计算的时间选择。
                                                                         W下：0，表示16chip；1，表示24chip；2，表示32chip；3，表示8chip。
                                                                         1X下：0，表示0chip；1，表示8chip。
                                                                         1X默认为8chip；EVDO默认24chip。 */
        unsigned int  reserved_0                    : 2;  /* bit[6-7]  : 保留 */
        unsigned int  dsp_dcr_period_1st_at2        : 4;  /* bit[8-11] : 增益调整后第一阶段的时间，单位256chip。
                                                                         1X下为1536chip，故需配置0x6；EVDO下为2048chip，故需配置为0x8。 */
        unsigned int  reserved_1                    : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_dcr_alg_sel_at2           : 2;  /* bit[16-17]: DCR算法选择。
                                                                         00，表示使用与V2相同的算法；
                                                                         01，表示使用V3R1算法；
                                                                         11，表示使用V3R2算法。
                                                                         1X仅支持V3R2算法；EVDO默认配置为V3R2算法，支持配置为V3R1算法。 */
        unsigned int  reserved_2                    : 6;  /* bit[18-23]: 保留 */
        unsigned int  dsp_dcr_acc_sel_1st_inter_at2 : 2;  /* bit[24-25]: DSP配置天线2异频去直流模块直流偏置值的计算，用于配置增益调整后第一阶段累加值的右移位数。0，右移12位；1，右移11位；2，右移10位；3，右移8位。 */
        unsigned int  dsp_dcr_acc_sel_2nd_inter_at2 : 2;  /* bit[26-27]: DSP配置天线2异频去直流模块直流偏置值的计算，用于配置增益调整后第二阶段累加值的右移位数。0，右移16位；1，右移14位。 */
        unsigned int  dsp_dcr_inherit_at2           : 1;  /* bit[28]   : 0，档位调整时，DC从0开始重新收敛；1，档位调整时，DC继承先前计算值。 */
        unsigned int  reserved_3                    : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_filter_cfg_reg;
    struct
    {
        unsigned int  dagc_filter_coe : 3;  /* bit[0-2] : DAGC统计能量滤波系数。0，表示1；1，表示1/2；2，表示1/4；3，表示1/8；4，表示1/16；5，表示1/32；6，表示1/64。
                                                          C模不配置。 */
        unsigned int  reserved_0      : 1;  /* bit[3]   : 保留 */
        unsigned int  aagc_filter_coe : 3;  /* bit[4-6] : AAGC统计能量滤波系数。0，表示1；1，表示1/2；2，表示1/4；3，表示1/8；4，表示1/16；5，表示1/32；6，表示1/64。
                                                          C模不配置。 */
...
typedef union
{
    unsigned int      wagc_sat_num_reg;
    struct
    {
        unsigned int  dagc_sat_num_1f_at1 : 16; /* bit[0-15] : 天线1一帧内DAGC IQ数据饱和次数。
                                                               1X和EVDO为150个256chip的时间。 */
        unsigned int  dagc_sat_num_1f_at2 : 16; /* bit[16-31]: 天线2一帧内DAGC IQ数据饱和次数。
                                                               1X和EVDO为150个256chip的时间。 */
    } reg;
...
typedef union
{
    unsigned int      wagc_mode_reg;
    struct
    {
        unsigned int  wagc_mode_aagc_intra : 2;  /* bit[0-1]  : 本频AAGC增益调整时间选择。0，表示一个符号调整一次增益；1，表示一个时隙调整一次增益；2，表示2个符号调整一次增益；3，表示4个符号调整一次增益。
                                                                1X和EVDO是固定值，无需配置。 */
        unsigned int  wagc_mode_dagc_intra : 2;  /* bit[2-3]  : 本频DAGC增益调整时间选择。0，表示一个符号调整一次增益；1，表示一个时隙调整一次增益；2，表示2个符号调整一次增益；3，表示4个符号调整一次增益。
                                                                1X和EVDO是固定值，无需配置。 */
        unsigned int  wagc_mode_aagc_inter : 2;  /* bit[4-5]  : 异频AAGC增益调整时间选择。0，表示一个符号调整一次增益；1，表示一个时隙调整一次增益；2，表示2个符号调整一次增益；3，表示4个符号调整一次增益。
                                                                1X和EVDO是固定值，无需配置。 */
        unsigned int  wagc_mode_dagc_inter : 2;  /* bit[6-7]  : 异频DAGC增益调整时间选择。0，表示一个符号调整一次增益；1，表示一个时隙调整一次增益；2，表示2个符号调整一次增益；3，表示4个符号调整一次增益。
                                                                1X和EVDO是固定值，无需配置。 */
        unsigned int  reserved_0           : 4;  /* bit[8-11] : 保留 */
        unsigned int  wagc_adjust_mode     : 2;  /* bit[12-13]: W模AGC增益调整射频反馈预留时间选择。0，预留32chip；1，预留48chip；2，预留64chip。
                                                                1X和EVDO是固定值，无需配置。 */
...
typedef union
{
    unsigned int      dagc_4bit_filter_cfg_reg;
    struct
    {
        unsigned int  dagc_4bit_filter_coe : 3;  /* bit[0-2] : 纠偏后4bitDAGC统计能量滤波系数。0，表示1，不滤波；1，表示1/2；2，表示1/4；3，表示1/8；4，表示1/16；5，表示1/32。
                                                               C模不配置。 */
...
typedef union
{
    unsigned int      wagc_dcr_rpt_at1_reg;
    struct
    {
        unsigned int  dc_offset_i_cpu_at1 : 12; /* bit[0-11] : 天线1当前I路输入直流结果值上报，测试用。 */
        unsigned int  reserved_0          : 4;  /* bit[12-15]: 保留 */
        unsigned int  dc_offset_q_cpu_at1 : 12; /* bit[16-27]: 天线1当前Q路输入直流结果值上报，测试用。 */
        unsigned int  dcr_1st_sel_at1     : 1;  /* bit[28]   : 新DCR算法天线1第一阶段指示。
                                                               1，第一阶段；0，第二阶段。 */
...
typedef union
{
    unsigned int      wagc_dcr_rpt_at2_reg;
    struct
    {
        unsigned int  dc_offset_i_cpu_at2 : 12; /* bit[0-11] : 天线2当前I路输入直流结果值上报，测试用。 */
        unsigned int  reserved_0          : 4;  /* bit[12-15]: 保留 */
        unsigned int  dc_offset_q_cpu_at2 : 12; /* bit[16-27]: 天线2当前Q路输入直流结果值上报，测试用。 */
        unsigned int  dcr_1st_sel_at2     : 1;  /* bit[28]   : 新DCR算法天线2第一阶段指示。
                                                               1，第一阶段；0，第二阶段。 */
        unsigned int  reserved_1          : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iq_ak_pk_sel_ant1_reg;
    struct
    {
        unsigned int  iq_mis_sel_ant1   : 1;  /* bit[0]   : 天线1 IQ Mismatch纠正系数选择。0，表示使用IQ Mismatch自己产生；1，表示使用DSP配置值。 */
        unsigned int  reserved_0        : 7;  /* bit[1-7] : 保留 */
        unsigned int  dsp_iq_mis_en_at1 : 1;  /* bit[8]   : 天线1 IQ Mismatch使能，高电平有效。 */
...
typedef union
{
    unsigned int      iq_ak_pk_ant1_reg;
    struct
    {
        unsigned int  dsp_iq_ak_ant1 : 10; /* bit[0-9]  : DSP配置的天线1 IQ Mismatch纠正系数Ak。 */
        unsigned int  reserved_0     : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_iq_pk_ant1 : 11; /* bit[16-26]: DSP配置的天线1 IQ Mismatch纠正系数Pk。 */
...
typedef union
{
    unsigned int      iq_ak_pk_sel_ant2_reg;
    struct
    {
        unsigned int  iq_mis_sel_ant2   : 1;  /* bit[0]   : 天线2 IQ Mismatch纠正系数选择。0，表示使用IQ Mismatch自己产生；1，表示使用DSP配置值。 */
        unsigned int  reserved_0        : 7;  /* bit[1-7] : 保留 */
        unsigned int  dsp_iq_mis_en_at2 : 1;  /* bit[8]   : 天线2 IQ Mismatch使能，高电平有效。 */
        unsigned int  reserved_1        : 23; /* bit[9-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iq_ak_pk_ant2_reg;
    struct
    {
        unsigned int  dsp_iq_ak_ant2 : 10; /* bit[0-9]  : DSP配置的天线2 IQ Mismatch纠正系数Ak。 */
        unsigned int  reserved_0     : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_iq_pk_ant2 : 11; /* bit[16-26]: DSP配置的天线2 IQ Mismatch纠正系数Pk。 */
        unsigned int  reserved_1     : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      agc_rpt_at1_reg;
    struct
    {
        unsigned int  agc_state_rpt_at1 : 30; /* bit[0-29] : 天线1档位上报。连续存储10个DRSSI统计周期的档位值，每个档位占3比特，最新的档位放在低3比特。 */
...
typedef union
{
    unsigned int      agc_rpt_at2_reg;
    struct
    {
        unsigned int  agc_state_rpt_at2 : 30; /* bit[0-29] : 天线2档位上报。连续存储10个DRSSI统计周期的档位值，每个档位占3比特，最新的档位放在低3比特。 */
        unsigned int  reserved          : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iq_mis_adjust_mode_reg;
    struct
    {
        unsigned int  iq_mis_adjust_mode : 2;  /* bit[0-1] : IQ Mismatch模块预留给RF增益的调整时间。
                                                             W下：0，预留24chip，对应AAGC的32chip；1，预留40chip，对应AAGC的48chip；2，预留56chip，对应AAGC的64chip。
                                                             1X下：0，预留8chip；1，预留16chip；2，预留24chip。
                                                             EVDO下：固定，无需配置。 */
...
typedef union
{
    unsigned int      iq_mis_mode_reg;
    struct
    {
        unsigned int  iq_mis_mode : 3;  /* bit[0-2] : IQ Mismatch计算周期，需要与AAGC的调整周期一致。0，表示一个单位；1，表示W的一个时隙，或者1X的一个PCG；2，表示2个单位；3，表示4个单位。4，表示6个单位。
                                                      1X：默认值为1。
                                                      EVDO：上述选项无效，为一个时隙的固定值，不需要配置。
                                                      注：单位为256chip。 */
        unsigned int  reserved    : 29; /* bit[3-31]: 保留 */
...
typedef union
{
    unsigned int      wagc_switch_reg;
    struct
    {
        unsigned int  dlfe_at1_en               : 1;  /* bit[0]    : 天线1前端开关。1，表示打开；0，表示关闭。W、TDS和C共用。 */
        unsigned int  dlfe_at2_en               : 1;  /* bit[1]    : 天线2前端开关。1，表示打开；0，表示关闭。W、TDS和C共用。 */
        unsigned int  dsp_wagc_switch_sync_mode : 2;  /* bit[2-3]  : 0，主辅天线打开按符号同步；1，主辅天线打开按1/4chip同步；2，主辅天线打开按时隙同步。
                                                                     C模不配置。 */
        unsigned int  dsp_dpa_mode_abb_blk      : 1;  /* bit[4]    : 0表示BLOCK检测和ABB BLOCK控制模块跟AGC内部定时器对齐；1表示BLOCK检测和ABB BLOCK控制模块跟DPA时隙头对齐。
                                                                     由于不支持DPA模式，请固定使用默认值，不能改配。
                                                                     C模不配置。 */
        unsigned int  dsp_dpa_mode_rssi         : 1;  /* bit[5]    : 0表示RSSI计算和AGC档位调整跟AGC内部定时器对齐；1表示RSSI计算和AGC档位调整跟DPA时隙头对齐。
                                                                     由于不支持DPA模式，请固定使用默认值，不能改配。
                                                                     C模不配置。 */
        unsigned int  dsp_dpa_mode_iqmis        : 1;  /* bit[6]    : 0表示IQ Mismatch计算跟AGC内部定时器对齐；1表示IQ Mismatch计算跟DPA时隙头对齐。
                                                                     由于不支持DPA模式，请固定使用默认值，不能改配。
                                                                     C模不配置。 */
        unsigned int  reserved_0                : 1;  /* bit[7]    : 保留 */
        unsigned int  dsp_srst_at1              : 1;  /* bit[8]    : 主集软复位。1表示进入复位状态；0表示退出软复位状态。
                                                                     软复位和天线使能配置约束：复位前先关闭本寄存器bit[0]天线使能，然后进行软复位，撤销复位后再打开本寄存器bit[0]天线使能。并且天线使能关闭时间要大于2个符号。 */
        unsigned int  dsp_srst_at2              : 1;  /* bit[9]    : 分集软复位。1表示进入复位状态；0表示退出软复位状态。
                                                                     软复位和天线使能配置约束：复位前先关闭本寄存器bit[1]天线使能，然后进行软复位，撤销复位后再打开本寄存器bit[1]天线使能。并且天线使能关闭时间要大于2个符号。 */
        unsigned int  reserved_1                : 2;  /* bit[10-11]: 保留 */
        unsigned int  dlfe_mode_sel             : 2;  /* bit[12-13]: 模式选择。0表示W；1表示TDS；2表示cdma 1X；3表示cdma EVDO。 */
...
typedef union
{
    unsigned int      init_agc_cfg_at1_reg;
    struct
    {
        unsigned int  dsp_init_drssi_25db_at1   : 9;  /* bit[0-8]  : 刚刚打开AGC时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  dsp_init_rssi_rf_at1      : 11; /* bit[9-19] : 刚刚打开AGC时，BBP使用此值进行能量测量，单位0.125dB。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  dsp_init_5s_agc_state_at1 : 3;  /* bit[20-22]: 刚刚打开AGC时，BBP使用此值进行自研RF五档增益控制。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  reserved_0                : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_init_8s_agc_state_at1 : 3;  /* bit[24-26]: 刚刚打开AGC时，BBP使用此值进行自研RF八档增益控制。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  reserved_1                : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_cfg_at2_reg;
    struct
    {
        unsigned int  dsp_init_drssi_25db_at2   : 9;  /* bit[0-8]  : 刚刚打开AGC时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  dsp_init_rssi_rf_at2      : 11; /* bit[9-19] : 刚刚打开AGC时，BBP使用此值进行能量测量，单位0.125dB。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  dsp_init_5s_agc_state_at2 : 3;  /* bit[20-22]: 刚刚打开AGC时，BBP使用此值进行自研RF五档增益控制。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  reserved_0                : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_init_8s_agc_state_at2 : 3;  /* bit[24-26]: 刚刚打开AGC时，BBP使用此值进行自研RF八档增益控制。
                                                                     说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。
                                                                     C模不配置。 */
        unsigned int  reserved_1                : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      inter_agc_cfg_at1_reg;
    struct
    {
        unsigned int  dsp_inter_drssi_25db_at1   : 9;  /* bit[0-8]  : 刚刚进入异频GAP时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                      C模不配置。 */
        unsigned int  dsp_inter_rssi_rf_at1      : 11; /* bit[9-19] : 刚刚进入异频GAP时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。
                                                                      C模不配置。 */
        unsigned int  dsp_inter_5s_agc_state_at1 : 3;  /* bit[20-22]: 刚刚进入异频GAP时，BBP使用此值进行自研RF五档增益控制。
                                                                      C模不配置。 */
        unsigned int  reserved_0                 : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_inter_8s_agc_state_at1 : 3;  /* bit[24-26]: 刚刚进入异频GAP时，BBP使用此值进行自研RF八档增益控制。
                                                                      C模不配置。 */
        unsigned int  reserved_1                 : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      inter_agc_cfg_at2_reg;
    struct
    {
        unsigned int  dsp_inter_drssi_25db_at2   : 9;  /* bit[0-8]  : 刚刚进入异频GAP时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                      C模不配置。 */
        unsigned int  dsp_inter_rssi_rf_at2      : 11; /* bit[9-19] : 刚刚进入异频GAP时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。
                                                                      C模不配置。 */
        unsigned int  dsp_inter_5s_agc_state_at2 : 3;  /* bit[20-22]: 刚刚进入异频GAP时，BBP使用此值进行自研RF五档增益控制。
                                                                      C模不配置。 */
        unsigned int  reserved_0                 : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_inter_8s_agc_state_at2 : 3;  /* bit[24-26]: 刚刚进入异频GAP时，BBP使用此值进行自研RF八档增益控制。
                                                                      C模不配置。 */
        unsigned int  reserved_1                 : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_rpt_at1_reg;
    struct
    {
        unsigned int  dsp_rpt_drssi_25db_at1 : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（双载波之和），BBP使用此值查表DAGC ROM。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_rssi_rf_at1    : 11; /* bit[9-19] : BBP上报天线口RSSI值（双载波之和），0.125dB精度。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_agc_state_at1  : 3;  /* bit[20-22]: BBP上报用于控制RF增益的状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5；5，表示S6；6，表示S7；7，表示S8。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
...
typedef union
{
    unsigned int      init_agc_rpt_at2_reg;
    struct
    {
        unsigned int  dsp_rpt_drssi_25db_at2 : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（双载波之和），BBP使用此值查表DAGC ROM。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_rssi_rf_at2    : 11; /* bit[9-19] : BBP上报天线口RSSI值（双载波之和），0.125dB精度。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_agc_state_at2  : 3;  /* bit[20-22]: BBP上报用于控制RF增益的状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5；5，表示S6；6，表示S7；7，表示S8。
                                                                  说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  reserved               : 9;  /* bit[23-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_8s_inc_th_reg;
    struct
    {
        unsigned int  s1_2s2_8s_th : 8;  /* bit[0-7]  : 八档AAGC状态跳转S1->S2的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S1->S2->S3->S4->S5的顺序，天线口的RSSI递增，AAGC的增益递减。
                                                        C模不配置。 */
        unsigned int  s2_2s3_8s_th : 8;  /* bit[8-15] : 八档AAGC状态跳转S2->S3的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s3_2s4_8s_th : 8;  /* bit[16-23]: 八档AAGC状态跳转S3->S4的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s4_2s5_8s_th : 8;  /* bit[24-31]: 八档AAGC状态跳转S4->S5的RSSI门限。
                                                        C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_8s_dec_th_reg;
    struct
    {
        unsigned int  s2_2s1_8s_th : 8;  /* bit[0-7]  : 八档AAGC状态跳转S2->S1的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S5->S4->S3->S2->S1的顺序，天线口的RSSI递减，AAGC的增益递增。
                                                        C模不配置。 */
        unsigned int  s3_2s2_8s_th : 8;  /* bit[8-15] : 八档AAGC状态跳转S3->S2的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s4_2s3_8s_th : 8;  /* bit[16-23]: 八档AAGC状态跳转S4->S3的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s5_2s4_8s_th : 8;  /* bit[24-31]: 八档AAGC状态跳转S5->S4的RSSI门限。
                                                        C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s12_intra_at1_reg;
    struct
    {
        unsigned int  s1_8s_rf_gain_intra_at1 : 12; /* bit[0-11] : 八档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s2_8s_rf_gain_intra_at1 : 12; /* bit[16-27]: 八档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
...
typedef union
{
    unsigned int      rf_8s_gain_s34_intra_at1_reg;
    struct
    {
        unsigned int  s3_8s_rf_gain_intra_at1 : 12; /* bit[0-11] : 八档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s4_8s_rf_gain_intra_at1 : 12; /* bit[16-27]: 八档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s1_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s1_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S1（RF RSSI为-102~-90dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s2_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s2_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S2（RF RSSI为-87~-69dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s3_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s3_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S3（RF RSSI为-66~-54dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s4_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s4_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S4（RF RSSI为-51~-39dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s5_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s5_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S5（RF RSSI为-36~-21dBm）状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      channel_fix_dig_gain_reg;
    struct
    {
        unsigned int  dsp_channel_gain_at1       : 7;  /* bit[0-6]  : 天线1 Gainfix与Gaindig增益之和，即前端通道中比较固定部分的增益，单位dB。
                                                                      AGC在RF_SC/RF_DC可以分时共用。
                                                                      C模不配置。 */
        unsigned int  reserved_0                 : 1;  /* bit[7]    : 保留 */
        unsigned int  dsp_channel_gain_at2       : 7;  /* bit[8-14] : 天线2 Gainfix与Gaindig增益之和，即前端通道中比较固定部分的增益，单位dB。
                                                                      AGC在RF_SC/RF_DC可以分时共用。
                                                                      C模不配置。 */
        unsigned int  reserved_1                 : 5;  /* bit[15-19]: 保留 */
        unsigned int  dsp_ch_switch_rfgain_intra : 1;  /* bit[20]   : rfgain本频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
        unsigned int  dsp_ch_switch_rfgain_inter : 1;  /* bit[21]   : rfgain异频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
        unsigned int  dsp_ch_switch_rfctrl_intra : 1;  /* bit[22]   : rfctrl本频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
        unsigned int  dsp_ch_switch_rfctrl_inter : 1;  /* bit[23]   : rfctrl异频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
        unsigned int  dsp_ch_switch_dc_intra     : 1;  /* bit[24]   : 静态DC本频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
        unsigned int  dsp_ch_switch_dc_inter     : 1;  /* bit[25]   : 静态DC异频通道选择。1表示天线1和天线2交换。
                                                                      C模不配置。 */
...
typedef union
{
    unsigned int      aagc_ctrl_test_reg;
    struct
    {
        unsigned int  dsp_ctrl_aagc_en    : 1;  /* bit[0]   : DSP控制AAGC增益使能。0，表示BBP控制AAGC增益；1，表示DSP控制AAGC增益。
                                                              C模不配置。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3] : 保留 */
        unsigned int  dsp_ctrl_aagc_state : 3;  /* bit[4-6] : DSP控制AAGC状态机。0，表示S1，即AAGC增益为s1_rf_gain；1，表示S2，即AAGC增益为s2_rf_gain；2，表示S3，即AAGC增益为s3_rf_gain；3，表示S4，即AAGC增益为s4_rf_gain；4，表示S5，即AAGC增益为s5_rf_gain；5，表示S6，即AAGC增益为s6_rf_gain；6，表示S7，即AAGC增益为s7_rf_gain；7，表示S8，即AAGC增益为s8_rf_gain。
                                                              C模不配置。 */
        unsigned int  reserved_1          : 25; /* bit[7-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_gain_delay_reg;
    struct
    {
        unsigned int  aagc_delay_slot_cnt_intra : 6;  /* bit[0-5]  : 自研RF AGC增益档的本频迟滞时间。时隙调整时，单位为时隙；符号调整时，单位为符号。
                                                                     C模不配置。 */
        unsigned int  reserved_0                : 2;  /* bit[6-7]  : 保留 */
        unsigned int  aagc_delay_slot_cnt_inter : 6;  /* bit[8-13] : 自研RF AGC增益档的异频频迟滞时间。时隙调整时，单位为时隙；符号调整时，单位为符号。
                                                                     C模不配置。 */
        unsigned int  reserved_1                : 2;  /* bit[14-15]: 保留 */
        unsigned int  aagc_delay_db             : 6;  /* bit[16-21]: Block检测检测到干扰时，自研RF -60dBm增益档的RSSI加上此值后，确定增益档位。有符号数，单位dB。
                                                                     C模不配置。 */
...
typedef union
{
    unsigned int      s1_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s1_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S1（RF RSSI为-102~-90dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s2_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s2_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S2（RF RSSI为-87~-69dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s3_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s3_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S3（RF RSSI为-66~-54dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s4_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s4_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S4（RF RSSI为-51~-39dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s5_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s5_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S5（RF RSSI为-36~-21dBm）状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s51_intra_at1_at2_reg;
    struct
    {
        unsigned int  s5_8s_rf_gain_intra_at1 : 12; /* bit[0-11] : 八档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s1_8s_rf_gain_intra_at2 : 12; /* bit[16-27]: 八档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s23_intra_at2_reg;
    struct
    {
        unsigned int  s2_8s_rf_gain_intra_at2 : 12; /* bit[0-11] : 八档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s3_8s_rf_gain_intra_at2 : 12; /* bit[16-27]: 八档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s45_intra_at2_reg;
    struct
    {
        unsigned int  s4_8s_rf_gain_intra_at2 : 12; /* bit[0-11] : 八档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s5_8s_rf_gain_intra_at2 : 12; /* bit[16-27]: 八档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s12_inter_at1_reg;
    struct
    {
        unsigned int  s1_8s_rf_gain_inter_at1 : 12; /* bit[0-11] : 八档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s2_8s_rf_gain_inter_at1 : 12; /* bit[16-27]: 八档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s34_inter_at1_reg;
    struct
    {
        unsigned int  s3_8s_rf_gain_inter_at1 : 12; /* bit[0-11] : 八档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s4_8s_rf_gain_inter_at1 : 12; /* bit[16-27]: 八档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s51_inter_at1_at2_reg;
    struct
    {
        unsigned int  s5_8s_rf_gain_inter_at1 : 12; /* bit[0-11] : 八档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s1_8s_rf_gain_inter_at2 : 12; /* bit[16-27]: 八档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      block_detect_en_reg;
    struct
    {
        unsigned int  dsp_block_detect_en     : 1;  /* bit[0]   : 干扰检测使能信号。0，表示不打开干扰检测功能；1，表示打开干扰检测功能。
                                                                  说明：此功能仅在D/F态下使用。
                                                                  C模不配置。 */
        unsigned int  dsp_abb_blk_en          : 1;  /* bit[1]   : ABB block检测使能信号。1，BBP会给ABB开窗；0，BBP不给ABB开窗。 */
        unsigned int  reserved_0              : 2;  /* bit[2-3] : 保留 */
        unsigned int  dsp_cpc_force_dcr1st_en : 1;  /* bit[4]   : 退出cpc_drx状态时，RSSI和AGC档位的来源，1表示使用DSP配置的init值，0表示使用进入cpc_drx状态时的保存值。
                                                                  C模不配置。 */
        unsigned int  reserved_1              : 3;  /* bit[5-7] : 保留 */
        unsigned int  dsp_cpc_force_agc_en    : 1;  /* bit[8]   : 退出cpc_drx状态时，是否启动第一阶段DCR算法，1表示启动，0表示不启动。
                                                                  C模不配置。 */
...
typedef union
{
    unsigned int      block_lpf_sel_reg;
    struct
    {
        unsigned int  reserved           : 30; /* bit[0-29]: 保留 */
        unsigned int  block_default_1sel : 1;  /* bit[30]  : 打开Block检测时Block检测默认配置。1，表示使用4阶LPF；0，表示使用2阶LPF。
                                                             C模不配置。 */
        unsigned int  block_default_sel  : 1;  /* bit[31]  : 关闭Block检测时Block检测默认配置。1，表示使用4阶LPF；0，表示使用2阶LPF。
                                                             C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      block_hys_slot_reg;
    struct
    {
        unsigned int  block_2stage_slot : 6;  /* bit[0-5]  : 在2阶低通滤波器停留的最短时间，单位slot。
                                                             C模不配置。 */
        unsigned int  reserved_0        : 2;  /* bit[6-7]  : 保留 */
        unsigned int  block_hys_th      : 6;  /* bit[8-13] : Block条件满足次数达到block_hys_th次时，才切换Block控制状态。
                                                             C模不配置。 */
        unsigned int  reserved_1        : 2;  /* bit[14-15]: 保留 */
        unsigned int  block_4stage_slot : 6;  /* bit[16-21]: 在4阶低通滤波器停留的最短时间，单位slot。
                                                             C模不配置。 */
        unsigned int  reserved_2        : 10; /* bit[22-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      block_gen_th_reg;
    struct
    {
        unsigned int  block_gen_rrc_in_th  : 6;  /* bit[0-5]  : 干扰产生第一种条件中，RRC滤波器输入端RSSI判断门限，单位dB。
                                                                说明：干扰产生的第一种条件，RRC滤波器之前的RSSI值>=block_gen_rrc_in_th，且RRC前后的RSSI差值>=block_gen_diff_th。
                                                                C模不配置。 */
        unsigned int  reserved_0           : 2;  /* bit[6-7]  : 保留 */
        unsigned int  block_gen_rrc_in_2th : 6;  /* bit[8-13] : 干扰产生第二种条件中，RRC滤波器输入端RSSI判断门限，单位dB。
                                                                说明：干扰产生的第二种条件，RRC滤波器之前的RSSI值>=block_gen_rrc_in_2th。
                                                                C模不配置。 */
        unsigned int  reserved_1           : 2;  /* bit[14-15]: 保留 */
        unsigned int  block_gen_diff_th    : 6;  /* bit[16-21]: 干扰产生第一种条件中，RRC滤波器前后RSSI差值判断门限，单位dB。
                                                                C模不配置。 */
        unsigned int  reserved_2           : 10; /* bit[22-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      block_dis_lth_reg;
    struct
    {
        unsigned int  block_dis_rrc_out_lth : 6;  /* bit[0-5]  : 干扰消失第一种条件中，RRC滤波器输出端RSSI判断门限，单位dB。
                                                                 说明：干扰消失的第一种条件，RRC滤波器之后的RSSI<=block_dis_rrc_out_lth，且RRC滤波器之前的RSSI<=block_dis_rrc_in_th。
                                                                 C模不配置。 */
        unsigned int  reserved_0            : 10; /* bit[6-15] : 保留 */
        unsigned int  block_dis_rrc_in_th   : 6;  /* bit[16-21]: 干扰消失第一种条件中，RRC滤波器输入端RSSI判断门限，单位dB。
                                                                 C模不配置。 */
...
typedef union
{
    unsigned int      block_dis_hth_reg;
    struct
    {
        unsigned int  block_dis_rrc_out_hth : 6;  /* bit[0-5]  : 干扰消失第二种条件中，RRC滤波器输出端RSSI判断门限，单位dB。
                                                                 说明：干扰消失的第二种条件，RRC滤波器之后的RSSI>block_dis_rrc_out_hth，且RRC滤波器前后的RSSI差值<=block_dis_diff_th。
                                                                 C模不配置。 */
        unsigned int  reserved_0            : 10; /* bit[6-15] : 保留 */
        unsigned int  block_dis_diff_th     : 6;  /* bit[16-21]: 干扰消失第二种条件中，RRC滤波器前后RSSI差值判断门限，单位dB。
                                                                 C模不配置。 */
        unsigned int  reserved_1            : 10; /* bit[22-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_8s_inc_th_a20_reg;
    struct
    {
        unsigned int  s5_2s6_8s_th : 8;  /* bit[0-7]  : 八档AAGC状态跳转S5->S6的RSSI门限，单位dBm，配置二进制补码值。
                                                        可参考AAGC_RSSI_INC_TH寄存器的说明。
                                                        C模不配置。 */
        unsigned int  s6_2s7_8s_th : 8;  /* bit[8-15] : 八档AAGC状态跳转S6->S7的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s7_2s8_8s_th : 8;  /* bit[16-23]: 八档AAGC状态跳转S7->S8的RSSI门限。
                                                        C模不配置。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_8s_dec_th_a20_reg;
    struct
    {
        unsigned int  s6_2s5_8s_th : 8;  /* bit[0-7]  : 八档AAGC状态跳转S6->S5的RSSI门限，单位dBm，配置二进制补码值。
                                                        可参考AAGC_RSSI_DEC_TH寄存器的说明。
                                                        C模不配置。 */
        unsigned int  s7_2s6_8s_th : 8;  /* bit[8-15] : 八档AAGC状态跳转S7->S6的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s8_2s7_8s_th : 8;  /* bit[16-23]: 八档AAGC状态跳转S8->S7的RSSI门限。
                                                        C模不配置。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s6_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s6_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S6状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s7_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s7_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S7状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s8_8s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s8_8s_rf_ctrl_at1 : 24; /* bit[0-23] : 八档AAGC S8状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s6_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s6_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S6状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s7_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s7_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S7状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s8_8s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s8_8s_rf_ctrl_at2 : 24; /* bit[0-23] : 八档AAGC S8状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s67_inter_at1_a20_reg;
    struct
    {
        unsigned int  s6_8s_rf_gain_inter_at1 : 12; /* bit[0-11] : 八档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_8s_rf_gain_inter_at1 : 12; /* bit[16-27]: 八档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s8_inter_at1_a20_reg;
    struct
    {
        unsigned int  s8_8s_rf_gain_inter_at1 : 12; /* bit[0-11] : 八档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
...
typedef union
{
    unsigned int      rf_8s_gain_s67_inter_at2_a20_reg;
    struct
    {
        unsigned int  s6_8s_rf_gain_inter_at2 : 12; /* bit[0-11] : 八档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_8s_rf_gain_inter_at2 : 12; /* bit[16-27]: 八档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s8_inter_at2_a20_reg;
    struct
    {
        unsigned int  s8_8s_rf_gain_inter_at2 : 12; /* bit[0-11] : 八档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved                : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s67_intra_at1_a20_reg;
    struct
    {
        unsigned int  s6_8s_rf_gain_intra_at1 : 12; /* bit[0-11] : 八档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_8s_rf_gain_intra_at1 : 12; /* bit[16-27]: 八档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      block_rpt_at1_reg;
    struct
    {
        unsigned int  drssi_125db_rrc_in_at1  : 10; /* bit[0-9]  : RRC滤波器之前统计的DRSSI值，单位0.125dB。 */
        unsigned int  reserved_0              : 6;  /* bit[10-15]: 保留 */
        unsigned int  drssi_125db_rrc_out_at1 : 10; /* bit[16-25]: RRC滤波器之后统计的DRSSI值，单位0.125dB。 */
        unsigned int  reserved_1              : 2;  /* bit[26-27]: 保留 */
        unsigned int  bbp_hw_blk_ind_at1      : 1;  /* bit[28]   : BBP上报硬件Block检测结果。0，不处于Block；1，处于Block。 */
...
typedef union
{
    unsigned int      block_rpt_at2_reg;
    struct
    {
        unsigned int  drssi_125db_rrc_in_at2  : 10; /* bit[0-9]  : RRC滤波器之前统计的DRSSI值，单位0.125dB。 */
        unsigned int  reserved_0              : 6;  /* bit[10-15]: 保留 */
        unsigned int  drssi_125db_rrc_out_at2 : 10; /* bit[16-25]: RRC滤波器之后统计的DRSSI值，单位0.125dB。 */
        unsigned int  reserved_1              : 2;  /* bit[26-27]: 保留 */
        unsigned int  bbp_hw_blk_ind_at2      : 1;  /* bit[28]   : BBP上报硬件Block检测结果。0，不处于Block；1，处于Block。 */
        unsigned int  reserved_2              : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s23_inter_at2_reg;
    struct
    {
        unsigned int  s2_8s_rf_gain_inter_at2 : 12; /* bit[0-11] : 八档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s3_8s_rf_gain_inter_at2 : 12; /* bit[16-27]: 八档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s45_inter_at2_reg;
    struct
    {
        unsigned int  s4_8s_rf_gain_inter_at2 : 12; /* bit[0-11] : 八档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s5_8s_rf_gain_inter_at2 : 12; /* bit[16-27]: 八档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s8_intra_at1_a20_reg;
    struct
    {
        unsigned int  s8_8s_rf_gain_intra_at1 : 12; /* bit[0-11] : 八档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved                : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s67_intra_at2_a20_reg;
    struct
    {
        unsigned int  s6_8s_rf_gain_intra_at2 : 12; /* bit[0-11] : 八档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_8s_rf_gain_intra_at2 : 12; /* bit[16-27]: 八档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_gain_s8_intra_at2_a20_reg;
    struct
    {
        unsigned int  s8_8s_rf_gain_intra_at2 : 12; /* bit[0-11] : 八档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved                : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_gain_cfg_freq_reg;
    struct
    {
        unsigned int  rf_cfg_slot_num : 16; /* bit[0-15] : 周期配置RF增益控制寄存器的周期，单位slot。
                                                           C模不配置。 */
        unsigned int  reserved        : 15; /* bit[16-30]: 保留 */
        unsigned int  rf_cfg_slot_en  : 1;  /* bit[31]   : 周期配置RF增益控制寄存器的使能。1，表示周期配置RF增益控制寄存器；0，表示只在增益状态跳转时配置RF增益控制寄存器。
                                                           C模不配置。 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s1_vbias_at1_reg;
    struct
    {
        unsigned int  s1_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S1对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s2_vbias_at1_reg;
    struct
    {
        unsigned int  s2_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S2对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s3_vbias_at1_reg;
    struct
    {
        unsigned int  s3_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S3对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s4_vbias_at1_reg;
    struct
    {
        unsigned int  s4_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S4对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s5_vbias_at1_reg;
    struct
    {
        unsigned int  s5_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S5对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s6_vbias_at1_reg;
    struct
    {
        unsigned int  s6_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S6对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s7_vbias_at1_reg;
    struct
    {
        unsigned int  s7_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S7对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s8_vbias_at1_reg;
    struct
    {
        unsigned int  s8_5s_vbias_at1 : 24; /* bit[0-23] : 主天线BLOCK下S8对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s1_vbias_at2_reg;
    struct
    {
        unsigned int  s1_5s_vbias_at2 : 24; /* bit[0-23] : 主天线BLOCK下S1对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s2_vbias_at2_reg;
    struct
    {
        unsigned int  s2_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S2对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s3_vbias_at2_reg;
    struct
    {
        unsigned int  s3_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S3对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s4_vbias_at2_reg;
    struct
    {
        unsigned int  s4_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S4对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s5_vbias_at2_reg;
    struct
    {
        unsigned int  s5_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S5对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s6_vbias_at2_reg;
    struct
    {
        unsigned int  s6_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S6对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s7_vbias_at2_reg;
    struct
    {
        unsigned int  s7_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S7对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_s8_vbias_at2_reg;
    struct
    {
        unsigned int  s8_5s_vbias_at2 : 24; /* bit[0-23] : 辅天线BLOCK下S8对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s1_vbias_at1_reg;
    struct
    {
        unsigned int  s1_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S1对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s2_vbias_at1_reg;
    struct
    {
        unsigned int  s2_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S2对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s3_vbias_at1_reg;
    struct
    {
        unsigned int  s3_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S3对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s4_vbias_at1_reg;
    struct
    {
        unsigned int  s4_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S4对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s5_vbias_at1_reg;
    struct
    {
        unsigned int  s5_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S5对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s6_vbias_at1_reg;
    struct
    {
        unsigned int  s6_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S6对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s7_vbias_at1_reg;
    struct
    {
        unsigned int  s7_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S7对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s8_vbias_at1_reg;
    struct
    {
        unsigned int  s8_8s_vbias_at1 : 24; /* bit[0-23] : 主天线非BLOCK下S8对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s1_vbias_at2_reg;
    struct
    {
        unsigned int  s1_8s_vbias_at2 : 24; /* bit[0-23] : 主天线非BLOCK下S1对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s2_vbias_at2_reg;
    struct
    {
        unsigned int  s2_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S2对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s3_vbias_at2_reg;
    struct
    {
        unsigned int  s3_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S3对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s4_vbias_at2_reg;
    struct
    {
        unsigned int  s4_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S4对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s5_vbias_at2_reg;
    struct
    {
        unsigned int  s5_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S5对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s6_vbias_at2_reg;
    struct
    {
        unsigned int  s6_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S6对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s7_vbias_at2_reg;
    struct
    {
        unsigned int  s7_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S7对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_8s_s8_vbias_at2_reg;
    struct
    {
        unsigned int  s8_8s_vbias_at2 : 24; /* bit[0-23] : 辅天线非BLOCK下S8对应VBIAS控制字。
                                                           C模不配置。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s12_intra_at1_reg;
    struct
    {
        unsigned int  s1_5s_rf_gain_intra_at1 : 12; /* bit[0-11] : 五档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s2_5s_rf_gain_intra_at1 : 12; /* bit[16-27]: 五档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s34_intra_at1_reg;
    struct
    {
        unsigned int  s3_5s_rf_gain_intra_at1 : 12; /* bit[0-11] : 五档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s4_5s_rf_gain_intra_at1 : 12; /* bit[16-27]: 五档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s51_intra_at1_at2_reg;
    struct
    {
        unsigned int  s5_5s_rf_gain_intra_at1 : 12; /* bit[0-11] : 五档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s1_5s_rf_gain_intra_at2 : 12; /* bit[16-27]: 五档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s23_intra_at2_reg;
    struct
    {
        unsigned int  s2_5s_rf_gain_intra_at2 : 12; /* bit[0-11] : 五档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s3_5s_rf_gain_intra_at2 : 12; /* bit[16-27]: 五档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s45_intra_at2_reg;
    struct
    {
        unsigned int  s4_5s_rf_gain_intra_at2 : 12; /* bit[0-11] : 五档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s5_5s_rf_gain_intra_at2 : 12; /* bit[16-27]: 五档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s12_inter_at1_reg;
    struct
    {
        unsigned int  s1_5s_rf_gain_inter_at1 : 12; /* bit[0-11] : 五档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s2_5s_rf_gain_inter_at1 : 12; /* bit[16-27]: 五档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s34_inter_at1_reg;
    struct
    {
        unsigned int  s3_5s_rf_gain_inter_at1 : 12; /* bit[0-11] : 五档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s4_5s_rf_gain_inter_at1 : 12; /* bit[16-27]: 五档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s51_inter_at1_at2_reg;
    struct
    {
        unsigned int  s5_5s_rf_gain_inter_at1 : 12; /* bit[0-11] : 五档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s1_5s_rf_gain_inter_at2 : 12; /* bit[16-27]: 五档AAGC S1（RF RSSI为-102~-90dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s23_inter_at2_reg;
    struct
    {
        unsigned int  s2_5s_rf_gain_inter_at2 : 12; /* bit[0-11] : 五档AAGC S2（RF RSSI为-87~-69dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s3_5s_rf_gain_inter_at2 : 12; /* bit[16-27]: 五档AAGC S3（RF RSSI为-66~-54dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s45_inter_at2_reg;
    struct
    {
        unsigned int  s4_5s_rf_gain_inter_at2 : 12; /* bit[0-11] : 五档AAGC S4（RF RSSI为-51~-39dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s5_5s_rf_gain_inter_at2 : 12; /* bit[16-27]: 五档AAGC S5（RF RSSI为-36~-21dBm）状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      drssi_over_ctrl_at1_reg;
    struct
    {
        unsigned int  dsp_drssi_over_win_length_at1 : 8;  /* bit[0-7]  : 八档AAGC DRSSI门限判断周期，单位为DRSSI统计周期。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
        unsigned int  dsp_drssi_over_num_at1        : 8;  /* bit[8-15] : 八档AAGC DRSSI门限迟滞次数。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
        unsigned int  dsp_drssi_over_threshold_at1  : 10; /* bit[16-25]: 八档AAGC DRSSI迟滞门限。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
...
typedef union
{
    unsigned int      drssi_over_ctrl_at2_reg;
    struct
    {
        unsigned int  dsp_drssi_over_win_length_at2 : 8;  /* bit[0-7]  : 八档AAGC DRSSI门限判断周期，单位为DRSSI统计周期。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
        unsigned int  dsp_drssi_over_num_at2        : 8;  /* bit[8-15] : 八档AAGC DRSSI门限迟滞次数。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
        unsigned int  dsp_drssi_over_threshold_at2  : 10; /* bit[16-25]: 八档AAGC DRSSI迟滞门限。
                                                                         AGC在RF_SC/RF_DC可以分时共用。
                                                                         C模不配置。 */
        unsigned int  reserved                      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_state_sel_reg;
    struct
    {
        unsigned int  dsp_aagc_state_sel : 2;  /* bit[0-1] : AAGC状态机选择。0，4阶LPF使用五档状态机，2阶LPF使用八档状态机；1，固定使用五档状态机；2，固定使用八档状态机。
                                                             C模不配置。 */
        unsigned int  reserved           : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      spur_remove_en_reg;
    struct
    {
        unsigned int  dsp_spur_remove_at1_en   : 1;  /* bit[0]   : 主集谐波干扰消除使能，高电平有效。 */
        unsigned int  dsp_remove_allrun_at1_en : 1;  /* bit[1]   : 在软件使能dsp_spur_remove_at1_en有效时，主集始终进行谐波干扰消除的使能信号。此时与档位、RSSI值、本频标志等其他条件无关。1，表示始终进行谐波干扰消除；0，表示由其他条件决定。主要用于调试。 */
        unsigned int  reserved_0               : 2;  /* bit[2-3] : 保留 */
        unsigned int  dsp_spur_remove_at2_en   : 1;  /* bit[4]   : 分集谐波干扰消除使能，高电平有效。 */
        unsigned int  dsp_remove_allrun_at2_en : 1;  /* bit[5]   : 在软件使能dsp_spur_remove_at1_en有效时，分集始终进行谐波干扰消除的使能信号。此时与档位、RSSI值、本频标志等其他条件无关。1，表示始终进行谐波干扰消除；0，表示由其他条件决定。主要用于调试。 */
...
typedef union
{
    unsigned int      spur_cfg_reg;
    struct
    {
        unsigned int  dsp_spur_freq           : 9;  /* bit[0-8]  : 主集和分集共用的谐波干扰信号相对于20kHz的频率倍数，有符号数补码表示。取值范围-118~-1、1~118。默认配置1。 */
        unsigned int  reserved_0              : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_rssi_threshold_spur : 11; /* bit[16-26]: 主集和分集共用的RSSI门限值。当RSSI值小于该门限值时，进行谐波干扰消除。 */
        unsigned int  reserved_1              : 5;  /* bit[27-31]: 保留 */
    } reg;
} WBBP_SPUR_CFG_UNION;
...
typedef union
{
    unsigned int      at1_spur_removed_dsp_reg;
    struct
    {
        unsigned int  at1_removed_i_dsp : 9;  /* bit[0-8]  : 主集上报谐波干扰消除值实部，用于调试。 */
        unsigned int  reserved_0        : 7;  /* bit[9-15] : 保留 */
        unsigned int  at1_removed_q_dsp : 9;  /* bit[16-24]: 主集上报谐波干扰消除值虚部，用于调试。 */
...
typedef union
{
    unsigned int      at2_spur_removed_dsp_reg;
    struct
    {
        unsigned int  at2_removed_i_dsp : 9;  /* bit[0-8]  : 分集上报谐波干扰消除值实部，用于调试。 */
        unsigned int  reserved_0        : 7;  /* bit[9-15] : 保留 */
        unsigned int  at2_removed_q_dsp : 9;  /* bit[16-24]: 分集上报谐波干扰消除值虚部，用于调试。 */
        unsigned int  reserved_1        : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s8_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s8_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S8状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iq_ak_pk_init_cfg_at1_reg;
    struct
    {
        unsigned int  dsp_init_iq_ak_at1 : 10; /* bit[0-9]  : 刚刚打开AGC时，BBP使用此值进行IQ Mismatch校正。
                                                              说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved_0         : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_init_iq_pk_at1 : 11; /* bit[16-26]: 刚刚打开AGC时，BBP使用此值进行IQ Mismatch校正。
                                                              说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iq_ak_pk_init_cfg_at2_reg;
    struct
    {
        unsigned int  dsp_init_iq_ak_at2 : 10; /* bit[0-9]  : 刚刚打开AGC时，BBP使用此值进行IQ Mismatch校正。
                                                              说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved_0         : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_init_iq_pk_at2 : 11; /* bit[16-26]: 刚刚打开AGC时，BBP使用此值进行IQ Mismatch校正。
                                                              说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      hsdpa_dc_cfg_reg;
    struct
    {
        unsigned int  dsp_wagc_cell_sel : 1;  /* bit[0]    : 下变频模块输出主辅载波信号位置选择，使主载波落在cell1上。0，主载波在低频点；1，主载波在高频点。
                                                             C模不配置。 */
        unsigned int  reserved_0        : 3;  /* bit[1-3]  : 保留 */
        unsigned int  dsp_wagc_nco_en   : 1;  /* bit[4]    : NCO模块使能，高电平有效。
                                                             C模不配置。 */
        unsigned int  reserved_1        : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_ph_acc_clr_en : 1;  /* bit[8]    : NCO相位清零脉冲。
                                                             C模不配置。 */
        unsigned int  reserved_2        : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_dl_dc_en      : 1;  /* bit[12]   : AGC所处RF_SC/RF_DC工作状态。0，RF_SC；1，RF_DC。
                                                             C模不配置。 */
...
typedef union
{
    unsigned int      wagc_iq_dly_ctrl_reg;
    struct
    {
        unsigned int  dsp_sc_nblk_dly_sel : 2;  /* bit[0-1]  : RF_SC非Block下射频通道数字域1/4chip延时。2'd0，延时0/4chip；2'd1，延时1/4chip；2'd2，延时2/4chip；2'd3，延时3/4chip。
                                                               C模不配置。 */
        unsigned int  reserved_0          : 2;  /* bit[2-3]  : 保留 */
        unsigned int  dsp_sc_blk_dly_sel  : 2;  /* bit[4-5]  : RF_SC Block下射频通道数字域1/4chip延时。2'd0，延时0/4chip；2'd1，延时1/4chip；2'd2，延时2/4chip；2'd3，延时3/4chip。
                                                               C模不配置。 */
        unsigned int  reserved_1          : 2;  /* bit[6-7]  : 保留 */
        unsigned int  dsp_dc_nblk_dly_sel : 2;  /* bit[8-9]  : RF_DC非Block下射频通道数字域1/4chip延时。2'd0，延时0/4chip；2'd1，延时1/4chip；2'd2，延时2/4chip；2'd3，延时3/4chip。
                                                               C模不配置。 */
        unsigned int  reserved_2          : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_dc_blk_dly_sel  : 2;  /* bit[12-13]: RF_DC Block下射频通道数字域1/4chip延时。2'd0，延时0/4chip；2'd1，延时1/4chip；2'd2，延时2/4chip；2'd3，延时3/4chip。
                                                               C模不配置。 */
...
typedef union
{
    unsigned int      init_scdc_agc_cfg0_at1_reg;
    struct
    {
        unsigned int  dsp_scdc_rssi_rf_at1  : 11; /* bit[0-10] : RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。
                                                                 C模不配置。 */
        unsigned int  reserved_0            : 13; /* bit[11-23]: 保留 */
        unsigned int  dsp_scdc_5s_state_at1 : 3;  /* bit[24-26]: RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行自研RF五档增益控制。
                                                                 C模不配置。 */
        unsigned int  reserved_1            : 1;  /* bit[27]   : 保留 */
        unsigned int  dsp_scdc_8s_state_at1 : 3;  /* bit[28-30]: RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行自研RF八档增益控制。
                                                                 C模不配置。 */
...
typedef union
{
    unsigned int      init_scdc_agc_cfg1_at1_reg;
    struct
    {
        unsigned int  dsp_scdc_drssi_at1            : 9;  /* bit[0-8]  : RF_SC/RF_DC切换刚刚完成时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                         C模不配置。 */
        unsigned int  reserved_0                    : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_init_scdc_rssi_en_at1     : 1;  /* bit[16]   : RF_SC/RF_DC切换刚刚完成时，DSP配置天线口RSSI初始值使能。1，DSP配置天线口RSSI初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  reserved_1                    : 3;  /* bit[17-19]: 保留 */
        unsigned int  dsp_init_scdc_5s_state_en_at1 : 1;  /* bit[20]   : RF_SC/RF_DC切换刚刚完成时，DSP配置五档状态机初始值使能。1，DSP配置五档状态机初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  dsp_init_scdc_8s_state_en_at1 : 1;  /* bit[21]   : RF_SC/RF_DC切换刚刚完成时，DSP配置八档状态机初始值使能。1，DSP配置八档状态机初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  reserved_2                    : 2;  /* bit[22-23]: 保留 */
        unsigned int  dsp_init_scdc_drssi_en_at1    : 1;  /* bit[24]   : 刚刚进入RF_SC时，DSP配置0.25dB精度DRSSI初始值使能。1，DSP配置0.25dB精度DRSSI初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
...
typedef union
{
    unsigned int      init_scdc_agc_cfg0_at2_reg;
    struct
    {
        unsigned int  dsp_scdc_rssi_rf_at2  : 11; /* bit[0-10] : RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。
                                                                 C模不配置。 */
        unsigned int  reserved_0            : 13; /* bit[11-23]: 保留 */
        unsigned int  dsp_scdc_5s_state_at2 : 3;  /* bit[24-26]: RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行自研RF五档增益控制。
                                                                 C模不配置。 */
        unsigned int  reserved_1            : 1;  /* bit[27]   : 保留 */
        unsigned int  dsp_scdc_8s_state_at2 : 3;  /* bit[28-30]: RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行自研RF八档增益控制。
                                                                 C模不配置。 */
        unsigned int  reserved_2            : 1;  /* bit[31]   : 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_scdc_agc_cfg1_at2_reg;
    struct
    {
        unsigned int  dsp_scdc_drssi_at2            : 9;  /* bit[0-8]  : RF_SC/RF_DC切换刚刚完成时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                         C模不配置。 */
        unsigned int  reserved_0                    : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_init_scdc_rssi_en_at2     : 1;  /* bit[16]   : RF_SC/RF_DC切换刚刚完成时，DSP配置天线口RSSI初始值使能。1，DSP配置天线口RSSI初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  reserved_1                    : 3;  /* bit[17-19]: 保留 */
        unsigned int  dsp_init_scdc_5s_state_en_at2 : 1;  /* bit[20]   : RF_SC/RF_DC切换刚刚完成时，DSP配置五档状态机初始值使能。1，DSP配置五档状态机初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  dsp_init_scdc_8s_state_en_at2 : 1;  /* bit[21]   : RF_SC/RF_DC切换刚刚完成时，DSP配置八档状态机初始值使能。1，DSP配置八档状态机初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  reserved_2                    : 2;  /* bit[22-23]: 保留 */
        unsigned int  dsp_init_scdc_drssi_en_at2    : 1;  /* bit[24]   : 刚刚进入RF_SC时，DSP配置0.25dB精度DRSSI初始值使能。1，DSP配置0.25dB精度DRSSI初始值；0，BBP继承以前的值。
                                                                         C模不配置。 */
        unsigned int  reserved_3                    : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_rpt_cell_at1_reg;
    struct
    {
        unsigned int  dsp_rpt_cell_drssi_25db_at1 : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（单载波），BBP使用此值查表DAGC ROM。
                                                                       说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_cell_rssi_rf_at1    : 11; /* bit[9-19] : BBP上报天线口RSSI值（单载波），0.125dB精度。
                                                                       说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
...
typedef union
{
    unsigned int      init_agc_rpt_cell_at2_reg;
    struct
    {
        unsigned int  dsp_rpt_cell_drssi_25db_at2 : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（单载波），BBP使用此值查表DAGC ROM。
                                                                       说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_cell_rssi_rf_at2    : 11; /* bit[9-19] : BBP上报天线口RSSI值（单载波），0.125dB精度。
                                                                       说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  reserved                    : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      soft_blk_cfg_reg;
    struct
    {
        unsigned int  dsp_blk_start_cnt : 16; /* bit[0-15] : DSP通过配置该时间值，选择在哪一个定时中断读取ABB和BBP上报的用于软件Block判决的信号能量。
                                                             C模不配置。 */
...
typedef union
{
    unsigned int      abb_blk_rpt_at1_reg;
    struct
    {
        unsigned int  abb_drssi_valid_at1 : 1;  /* bit[0]    : DSP读取的ABB和BBP上报的用于软件Block判决的信号能量有效指示。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  wagc_blk_drssi_at1  : 9;  /* bit[4-12] : DSP读取的BBP上报的用于软件Block判决的信号能量。 */
...
typedef union
{
    unsigned int      blk_output_reg;
    struct
    {
        unsigned int  dsp_blk_spi_data    : 24; /* bit[0-23] : RF Block状态SPI控制字。
                                                               C模不配置。 */
        unsigned int  dsp_blk_spi_ind     : 1;  /* bit[24]   : RF Block状态SPI控制字触发脉冲。
                                                               C模不配置。 */
        unsigned int  reserved_0          : 3;  /* bit[25-27]: 保留 */
        unsigned int  dsp_blk_spi_ch_info : 2;  /* bit[28-29]: RF Block状态SPI控制字通道选择。
                                                               C模不配置。
                                                               00：选择ch0；
                                                               01：选择ch1；
                                                               10：选择ch2；
...
typedef union
{
    unsigned int      abb_blk_rpt_at2_reg;
    struct
    {
        unsigned int  abb_drssi_valid_at2 : 1;  /* bit[0]    : DSP读取的ABB和BBP上报的用于软件Block判决的信号能量有效指示。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  wagc_blk_drssi_at2  : 9;  /* bit[4-12] : DSP读取的BBP上报的用于软件Block判决的信号能量。 */
        unsigned int  reserved_1          : 19; /* bit[13-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_ckg_bypass_reg;
    struct
    {
        unsigned int  dsp_dlfe_ckg_bypass : 29; /* bit[0-28] : 时钟门控旁路寄存器。0，时钟门控使能；1，时钟门控旁路。
                                                               [0] 主天线spur remove模块，
                                                               [1] 主天线rxiq模块
                                                               [2] 主天线下变频模块
                                                               [3] 主天线scfilter模块
...
typedef union
{
    unsigned int      dc_sta_cal_rpt_at1_reg;
    struct
    {
        unsigned int  sta_dc_cal_finish_cpu_at1 : 1;  /* bit[0]   : 天线1 RX静态DC校准相关测量模块运算完成标志。 */
...
typedef union
{
    unsigned int      dc_sta_cal_rpt_at2_reg;
    struct
    {
        unsigned int  sta_dc_cal_finish_cpu_at2 : 1;  /* bit[0]   : 天线2 RX静态DC校准相关测量模块运算完成标志。 */
        unsigned int  reserved                  : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_ctrl0_at1_reg;
    struct
    {
        unsigned int  dsp_sta_dc_cal_en_at1 : 1;  /* bit[0]   : 天线1配置DCR RX静态DC校准模式。
                                                                1，DCR处于RX静态DC校准模式；0，DCR处于正常工作模式。 */
        unsigned int  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_ctrl0_at2_reg;
    struct
    {
        unsigned int  dsp_sta_dc_cal_en_at2 : 1;  /* bit[0]   : 天线2配置DCR RX静态DC校准模式。
                                                                1，DCR处于RX静态DC校准模式；0，DCR处于正常工作模式。 */
        unsigned int  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_ctrl1_at1_reg;
    struct
    {
        unsigned int  dsp_sta_dc_cal_clr_at1 : 1;  /* bit[0]   : 天线1 RX静态DC校准相关测量模块完成标志清零脉冲。 */
        unsigned int  reserved               : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_ctrl1_at2_reg;
    struct
    {
        unsigned int  dsp_sta_dc_cal_clr_at2 : 1;  /* bit[0]   : 天线2 RX静态DC校准相关测量模块完成标志清零脉冲。 */
        unsigned int  reserved               : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s1_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s1_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态1时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s2_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s2_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态2时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s3_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s3_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态3时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s4_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s4_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态4时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s5_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s5_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态5时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s6_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s6_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态6时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s7_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s7_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态7时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s8_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s8_8s_dc_corr_intra_at1 : 24; /* bit[0-23] : 8档AGC状态8时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s1_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s1_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态1时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s2_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s2_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态2时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s3_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s3_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态3时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s4_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s4_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态4时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s5_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s5_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态5时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s1_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s1_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态1时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s2_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s2_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态2时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s3_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s3_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态3时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s4_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s4_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态4时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s5_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s5_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态5时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s6_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s6_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态6时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s7_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s7_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态7时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s8_8s_at1_reg;
    struct
    {
        unsigned int  dsp_s8_8s_dc_corr_inter_at1 : 24; /* bit[0-23] : 8档AGC状态8时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s1_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s1_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态1时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s2_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s2_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态2时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s3_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s3_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态3时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s4_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s4_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态4时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s5_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s5_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态5时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s1_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s1_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态1时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s2_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s2_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态2时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s3_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s3_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态3时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s4_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s4_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态4时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s5_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s5_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态5时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s6_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s6_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态6时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s7_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s7_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态7时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s8_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s8_8s_dc_corr_intra_at2 : 24; /* bit[0-23] : 8档AGC状态8时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s1_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s1_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态1时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s2_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s2_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态2时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s3_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s3_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态3时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s4_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s4_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态4时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s5_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s5_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态5时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s1_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s1_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态1时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s2_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s2_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态2时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s3_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s3_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态3时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s4_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s4_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态4时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s5_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s5_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态5时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s6_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s6_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态6时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s7_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s7_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态7时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s8_8s_at2_reg;
    struct
    {
        unsigned int  dsp_s8_8s_dc_corr_inter_at2 : 24; /* bit[0-23] : 8档AGC状态8时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s1_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s1_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态1时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s2_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s2_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态2时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s3_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s3_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态3时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s4_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s4_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态4时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s5_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s5_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态5时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_spi_en_reg;
    struct
    {
        unsigned int  dsp_sta_dc_spi_en : 1;  /* bit[0]   : 静态DC校准值SPI发送使能，高电平有效。
                                                            C模不配置。 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcr_2nd_ctrl_at1_reg;
    struct
    {
        unsigned int  small_dc_time_at1 : 8;  /* bit[0-7]  : DCR的小残余DC判决，当ΔDC值小于small_dc_th,small_dc_time次时，小残余标志有效。 */
        unsigned int  small_dc_th_at1   : 8;  /* bit[8-15] : DCR的小残余DC判决，当ΔDC值小于small_dc_th,small_dc_time次时，小残余标志有效。 */
        unsigned int  reserved          : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcr_2nd_ctrl_at2_reg;
    struct
    {
        unsigned int  small_dc_time_at2 : 8;  /* bit[0-7]  : DCR的小残余DC判决，当ΔDC值小于small_dc_th,small_dc_time次时，小残余标志有效。 */
        unsigned int  small_dc_th_at2   : 8;  /* bit[8-15] : DCR的小残余DC判决，当ΔDC值小于small_dc_th,small_dc_time次时，小残余标志有效。 */
        unsigned int  reserved          : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_state_rpt_at1_reg;
    struct
    {
        unsigned int  reserved_0               : 20; /* bit[0-19] : 保留 */
        unsigned int  dsp_rpt_agc_5s_state_at1 : 3;  /* bit[20-22]: BBP上报用于控制RF增益的5档状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5； */
        unsigned int  reserved_1               : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_rpt_agc_8s_state_at1 : 3;  /* bit[24-26]: BBP上报用于控制RF增益的8档状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5；5，表示S6；6，表示S7；7，表示S8。 */
...
typedef union
{
    unsigned int      wagc_state_rpt_at2_reg;
    struct
    {
        unsigned int  reserved_0               : 20; /* bit[0-19] : 保留 */
        unsigned int  dsp_rpt_agc_5s_state_at2 : 3;  /* bit[20-22]: BBP上报用于控制RF增益的5档状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5； */
        unsigned int  reserved_1               : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_rpt_agc_8s_state_at2 : 3;  /* bit[24-26]: BBP上报用于控制RF增益的8档状态机。0，表示S1；1，表示S2；2，表示S3；3，表示S4；4，表示S5；5，表示S6；6，表示S7；7，表示S8。 */
        unsigned int  reserved_2               : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_cfg_bbp_blk_at1_reg;
    struct
    {
        unsigned int  dsp_soft_blk_ind_at1_intra : 1;  /* bit[0]   : 软件配置天线1 BBP本频Block状态。
                                                                     1，处于Block状态；0，不处于Block状态。
                                                                     C模不配置。 */
        unsigned int  dsp_soft_blk_ind_at1_inter : 1;  /* bit[1]   : 软件配置天线1 BBP异频Block状态。
                                                                     1，处于Block状态；0，不处于Block状态。
                                                                     C模不配置。 */
        unsigned int  reserved                   : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_cfg_bbp_blk_at2_reg;
    struct
    {
        unsigned int  dsp_soft_blk_ind_at2_intra : 1;  /* bit[0]   : 软件配置天线2 BBP本频Block状态
                                                                     1，处于Block状态；0，不处于Block状态。
                                                                     C模不配置。 */
        unsigned int  dsp_soft_blk_ind_at2_inter : 1;  /* bit[1]   : 软件配置天线2 BBP异频Block状态。
                                                                     1，处于Block状态；0，不处于Block状态。
                                                                     C模不配置。 */
        unsigned int  reserved                   : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_iq_mis_rpt_at1_reg;
    struct
    {
        unsigned int  iq_mis_ak_cpu_at1 : 10; /* bit[0-9]  : W和TDS上报IQ Mismatch计算得到的滤波前ak值。 */
        unsigned int  reserved_0        : 6;  /* bit[10-15]: 保留 */
        unsigned int  iq_mis_pk_cpu_at1 : 11; /* bit[16-26]: W和TDS上报IQ Mismatch计算得到的滤波前pk值。 */
        unsigned int  reserved_1        : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_iq_mis_rpt_at2_reg;
    struct
    {
        unsigned int  iq_mis_ak_cpu_at2 : 10; /* bit[0-9]  : W和TDS上报IQ Mismatch计算得到的滤波前ak值。 */
        unsigned int  reserved_0        : 6;  /* bit[10-15]: 保留 */
        unsigned int  iq_mis_pk_cpu_at2 : 11; /* bit[16-26]: W和TDS上报IQ Mismatch计算得到的滤波前pk值。 */
        unsigned int  reserved_1        : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      spur_alpha_coe_reg;
    struct
    {
        unsigned int  dsp_spur_alpha_para : 3;  /* bit[0-2] : 谐波干扰消除，alpha滤波系数选择。
                                                              3'd0，不进行alpha滤波；
                                                              3'd1，alpha滤波系数为1/2；
                                                              3'd2，alpha滤波系数为1/4；
                                                              3'd3，alpha滤波系数为1/8；
...
typedef union
{
    unsigned int      wagc_iq_mis_post_rpt_at1_reg;
    struct
    {
        unsigned int  rxiq_est_a_post_at1 : 10; /* bit[0-9]  : W、C和TDS上报IQ Mismatch计算得到的滤波后ak值。 */
        unsigned int  reserved_0          : 6;  /* bit[10-15]: 保留 */
        unsigned int  rxiq_est_p_post_at1 : 11; /* bit[16-26]: W、C和TDS上报IQ Mismatch计算得到的滤波后pk值。 */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wagc_iq_mis_post_rpt_at2_reg;
    struct
    {
        unsigned int  rxiq_est_a_post_at2 : 10; /* bit[0-9]  : W、C和TDS上报IQ Mismatch计算得到的滤波后ak值。 */
        unsigned int  reserved_0          : 6;  /* bit[10-15]: 保留 */
        unsigned int  rxiq_est_p_post_at2 : 11; /* bit[16-26]: W、C和TDS上报IQ Mismatch计算得到的滤波后pk值。 */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_5s_inc_th_add_reg;
    struct
    {
        unsigned int  s5_2s6_5s_th : 8;  /* bit[0-7]  : 五档AAGC状态跳转S5->S6的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S1->S2->S3->S4->S5->S6->S7->S8的顺序，天线口的RSSI递增，AAGC的增益递减。
                                                        C模不配置。 */
        unsigned int  s6_2s7_5s_th : 8;  /* bit[8-15] : 五档AAGC状态跳转S6->S7的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s7_2s8_5s_th : 8;  /* bit[16-23]: 五档AAGC状态跳转S7->S8的RSSI门限。
                                                        C模不配置。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      aagc_rssi_5s_dec_th_add_reg;
    struct
    {
        unsigned int  s6_2s5_5s_th : 8;  /* bit[0-7]  : 五档AAGC状态跳转S6->S5的RSSI门限，单位dBm，配置二进制补码值。
                                                        说明：按照S8->S7->S6->S5->S4->S3->S2->S1的顺序，天线口的RSSI递减，AAGC的增益递增。
                                                        C模不配置。 */
        unsigned int  s7_2s6_5s_th : 8;  /* bit[8-15] : 五档AAGC状态跳转S7->S6的RSSI门限。
                                                        C模不配置。 */
        unsigned int  s8_2s7_5s_th : 8;  /* bit[16-23]: 五档AAGC状态跳转S8->S7的RSSI门限。
                                                        C模不配置。 */
        unsigned int  reserved     : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s67_intra_at1_reg;
    struct
    {
        unsigned int  s6_5s_rf_gain_intra_at1 : 12; /* bit[0-11] : 五档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_5s_rf_gain_intra_at1 : 12; /* bit[16-27]: 五档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s8_intra_at12_reg;
    struct
    {
        unsigned int  s8_5s_rf_gain_intra_at1 : 12; /* bit[0-11] : 五档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s8_5s_rf_gain_intra_at2 : 12; /* bit[16-27]: 五档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s67_intra_at2_reg;
    struct
    {
        unsigned int  s6_5s_rf_gain_intra_at2 : 12; /* bit[0-11] : 五档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_5s_rf_gain_intra_at2 : 12; /* bit[16-27]: 五档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s67_inter_at1_reg;
    struct
    {
        unsigned int  s6_5s_rf_gain_inter_at1 : 12; /* bit[0-11] : 五档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_5s_rf_gain_inter_at1 : 12; /* bit[16-27]: 五档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s8_inter_at12_reg;
    struct
    {
        unsigned int  s8_5s_rf_gain_inter_at1 : 12; /* bit[0-11] : 五档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s8_5s_rf_gain_inter_at2 : 12; /* bit[16-27]: 五档AAGC S8状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_5s_gain_s67_inter_at2_reg;
    struct
    {
        unsigned int  s6_5s_rf_gain_inter_at2 : 12; /* bit[0-11] : 五档AAGC S6状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_0              : 4;  /* bit[12-15]: 保留 */
        unsigned int  s7_5s_rf_gain_inter_at2 : 12; /* bit[16-27]: 五档AAGC S7状态时RF的增益，即LNA、LPF和VGA2增益和。单位0.125dB，配置二进制补码值。
                                                                   C模不配置。 */
        unsigned int  reserved_1              : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s6_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s6_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态6时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s7_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s7_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态7时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s8_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s8_5s_dc_corr_intra_at1 : 24; /* bit[0-23] : 5档AGC状态8时，本频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s6_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s6_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态6时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s7_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s7_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态7时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s8_5s_at1_reg;
    struct
    {
        unsigned int  dsp_s8_5s_dc_corr_inter_at1 : 24; /* bit[0-23] : 5档AGC状态8时，异频下天线1静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s6_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s6_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态6时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s7_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s7_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态7时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_intra_s8_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s8_5s_dc_corr_intra_at2 : 24; /* bit[0-23] : 5档AGC状态8时，本频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s6_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s6_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态6时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s7_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s7_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态7时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dc_sta_crro_cfg_inter_s8_5s_at2_reg;
    struct
    {
        unsigned int  dsp_s8_5s_dc_corr_inter_at2 : 24; /* bit[0-23] : 5档AGC状态8时，异频下天线2静态DC补偿值。 */
        unsigned int  reserved                    : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s6_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s6_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S6状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s7_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s7_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S7状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s8_5s_rf_ctrl_at1_reg;
    struct
    {
        unsigned int  s8_5s_rf_ctrl_at1 : 24; /* bit[0-23] : 五档AAGC S8状态时主集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s6_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s6_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S6状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      s7_5s_rf_ctrl_at2_reg;
    struct
    {
        unsigned int  s7_5s_rf_ctrl_at2 : 24; /* bit[0-23] : 五档AAGC S7状态时分集RF控制字。
                                                             C模不配置。 */
        unsigned int  reserved          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      cordic_phase_vld_reg;
    struct
    {
        unsigned int  dsp_pha_new_ind : 1;  /* bit[0]   : 保留 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tds_sc_offset_reg;
    struct
    {
        unsigned int  tds_sc_offset : 9;  /* bit[0-8] : 软件配置TDS采样纠偏值，有符号数。
                                                        C模不配置。 */
...
typedef union
{
    unsigned int      tds_fpu_func_en_reg;
    struct
    {
        unsigned int  spmv_en          : 1;  /* bit[0]    : tds spmv使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  dcoc_comp_en     : 1;  /* bit[1]    : tds dcoc补偿dc_used使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  dcoc_calc_en     : 1;  /* bit[2]    : tds dcoc计算dc_est使能信号。
                                                            1'b0，不使能；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  dagct1_comp_en   : 1;  /* bit[3]    : tds tds dagct1补偿使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  reserved_0       : 1;  /* bit[4]    : 保留 */
        unsigned int  rxiq_comp_en     : 1;  /* bit[5]    : tds rxiq校准因子补偿使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  rxiq_calc_en     : 1;  /* bit[6]    : tds rxiq校准因子计算使能信号。
                                                            1'b0，不使能；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  drssi_wb_calc_en : 1;  /* bit[7]    : tds drssi_wb计算使能信号。
                                                            1'b0，不使能；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  reserved_1       : 3;  /* bit[8-10] : 保留 */
        unsigned int  drssi_nb_calc_en : 1;  /* bit[11]   : tds drssi_nb计算使能信号。
                                                            1'b0，不使能；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  dagct2_comp_en   : 1;  /* bit[12]   : tds dagct2补偿使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  reserved_2       : 5;  /* bit[13-17]: 保留 */
        unsigned int  codc_comp_en     : 1;  /* bit[18]   : tds cordic模块处理使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  sinc_comp_en     : 1;  /* bit[19]   : tds sinc重采样滤波模块处理使能信号。
                                                            1'b0，不使能，旁路；
                                                            1'b1，使能。
                                                            C模不配置。 */
        unsigned int  reserved_3       : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fir_sel_reg;
    struct
    {
        unsigned int  fir_sel  : 2;  /* bit[0-1] : 下采样模块的抽点选择信号。
                                                   上电之后只配置一次，配置范围0~2。
                                                   0，选择下采样滤波器输出的第0、3、6…个数据；
                                                   1，选择下采样滤波器输出的第1、4、7…个数据；
                                                   2，选择下采样滤波器输出的第2、5、8…个数据；
...
typedef union
{
    unsigned int      data_sel_reg;
    struct
    {
        unsigned int  rrc_sel  : 2;  /* bit[0-1] : 接收滤波后数据选择输出。
                                                   2'b00，选择抽取第1路信号；
                                                   2'b01，选择抽取第2路信号；
                                                   2'b10，选择抽取第3路信号；
                                                   2'b11，选择抽取第4路信号。 */
...
typedef union
{
    unsigned int      dcoc_update_reg;
    struct
    {
        unsigned int  dcoc_update : 1;  /* bit[0]   : TD dcoc是否按样点更新dc_used。
                                                      1'b0，按样点更新；
                                                      1'b1，按时隙更新。
                                                      注意：本寄存器上电配置一次，不支持动态切换的情况。
                                                      C模不配置。 */
...
typedef union
{
    unsigned int      dcoc_sftbit1_reg;
    struct
    {
        unsigned int  dcoc_sftbit_i1 : 5;  /* bit[0-4]  : TD软件配置的天线1的I路数据移位bit数，配置范围10~19。
                                                          C模不配置。 */
        unsigned int  reserved_0     : 11; /* bit[5-15] : 保留 */
        unsigned int  dcoc_sftbit_q1 : 5;  /* bit[16-20]: TD软件配置的天线1的Q路数据移位bit数，配置范围10~19。
                                                          C模不配置。 */
...
typedef union
{
    unsigned int      dcoc_sftbit2_reg;
    struct
    {
        unsigned int  dcoc_sftbit_i2 : 5;  /* bit[0-4]  : TD软件配置的天线2的I路数据移位bit数，配置范围10~19。
                                                          C模不配置。 */
        unsigned int  reserved_0     : 11; /* bit[5-15] : 保留 */
        unsigned int  dcoc_sftbit_q2 : 5;  /* bit[16-20]: TD软件配置的天线2的Q路数据移位bit数，配置范围10~19。
                                                          C模不配置。 */
        unsigned int  reserved_1     : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcoc_cfg_vld_reg;
    struct
    {
        unsigned int  dcoc_adc_vld : 1;  /* bit[0]   : TD软件配置的dcoc的DC值是否生效标志。
                                                       1'b0，不生效；
                                                       1'b1，生效。
                                                       C模不配置。 */
        unsigned int  reserved     : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      fir_sft_reg;
    struct
    {
        unsigned int  fir_rx_sftbit : 4;  /* bit[0-3] : 下采样滤波器内部累加器移位bit数。
                                                        上电之后只配置一次，配置范围10~13。
                                                        C模不配置。 */
...
typedef union
{
    unsigned int      rx_cnt_reg;
    struct
    {
        unsigned int  para_force  : 1;  /* bit[0]    : 参数强制生效使能。
                                                       C模不配置。 */
        unsigned int  reserved_0  : 2;  /* bit[1-2]  : 保留 */
        unsigned int  dft_func_en : 1;  /* bit[3]    : rx_cnt计数使能，高电平有效，低电平时rx_cnt清零。
                                                       C模不配置。 */
        unsigned int  rx_cnt      : 16; /* bit[4-19] : ad_on包络个数，可维可测。 */
...
typedef union
{
    unsigned int      dacgt1_gain1_reg;
    struct
    {
        unsigned int  dagct1_gain1 : 10; /* bit[0-9]  : TD软件配置的天线1的pre_dagct增益，有符号数，有效范围[-336,336]。
                                                        C模不配置。 */
...
typedef union
{
    unsigned int      dacgt1_gain2_reg;
    struct
    {
        unsigned int  dagct1_gain2 : 10; /* bit[0-9]  : TD软件配置的天线2的pre_dagct增益，有符号数，有效范围[-336,336]。
                                                        C模不配置。 */
        unsigned int  reserved     : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dacgt2_gain1_reg;
    struct
    {
        unsigned int  dagct2_gain1 : 10; /* bit[0-9]  : TD软件配置的天线1的post_dagct增益，有符号数，有效范围[-336,336]。
                                                        C模不配置。 */
        unsigned int  reserved     : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dacgt2_gain2_reg;
    struct
    {
        unsigned int  dagct2_gain2 : 10; /* bit[0-9]  : TD软件配置的天线2的post_dagct增益，有符号数，有效范围[-336,336]。
                                                        C模不配置。 */
        unsigned int  reserved     : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rxiq_thres_reg;
    struct
    {
        unsigned int  rxiq_thres : 6;  /* bit[0-5] : W和TD rxiq校准因子更新阈值。取值范围[11,63]。W典型配置0xb。 */
...
typedef union
{
    unsigned int      rxiq_sftbit0_reg;
    struct
    {
        unsigned int  rxiq_sftbit_a1 : 4;  /* bit[0-3]  : 天线1的rxiq移位的bit数ShiftA。取值范围[10,15]。 */
        unsigned int  reserved_0     : 4;  /* bit[4-7]  : 保留 */
        unsigned int  rxiq_sftbit_b1 : 4;  /* bit[8-11] : 天线1的rxiq移位的bit数ShiftB。取值范围[9,15]。 */
        unsigned int  reserved_1     : 4;  /* bit[12-15]: 保留 */
        unsigned int  rxiq_sftbit_c1 : 2;  /* bit[16-17]: 天线1的rxiq移位的bit数ShiftC。取值范围[0,3]。 */
...
typedef union
{
    unsigned int      rxiq_sftbit1_reg;
    struct
    {
        unsigned int  rxiq_sftbit_a2 : 4;  /* bit[0-3]  : 天线2的rxiq移位的bit数ShiftA。取值范围[10,15]。 */
        unsigned int  reserved_0     : 4;  /* bit[4-7]  : 保留 */
        unsigned int  rxiq_sftbit_b2 : 4;  /* bit[8-11] : 天线2的rxiq移位的bit数ShiftB。取值范围[9,15]。 */
        unsigned int  reserved_1     : 4;  /* bit[12-15]: 保留 */
        unsigned int  rxiq_sftbit_c2 : 2;  /* bit[16-17]: 天线2的rxiq移位的bit数ShiftC。取值范围[0,3]。 */
        unsigned int  reserved_2     : 14; /* bit[18-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rxiq_limit_reg;
    struct
    {
        unsigned int  dsp_rxiq_a_limit : 9;  /* bit[0-8]  : ak值幅度限制，无符号数。 */
        unsigned int  reserved_0       : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_rxiq_p_limit : 10; /* bit[16-25]: pk值幅度限制，无符号数。 */
...
typedef union
{
    unsigned int      rxiq_weight_reg;
    struct
    {
        unsigned int  dsp_rxiq_ap_weight : 3;  /* bit[0-2] : AP权重，0~4对应链路16,8,4,2,1。 */
        unsigned int  reserved_0         : 1;  /* bit[3]   : 保留 */
        unsigned int  dsp_rxiq_vld       : 1;  /* bit[4]   : TD软件配置的rxiq值是否生效标志。
                                                             1'b0，不生效；
                                                             1'b1，生效。 */
...
typedef union
{
    unsigned int      rssi_x_sftbit_reg;
    struct
    {
        unsigned int  rssi_xsbit_nb : 5;  /* bit[0-4]  : rssi移位X的bit数。取值范围[3,10]。
                                                         C模不配置。 */
        unsigned int  rssi_xsbit_wb : 5;  /* bit[5-9]  : rssi移位X的bit数。取值范围[3,10]。
                                                         C模不配置。 */
        unsigned int  reserved      : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rssi_m_sftbit_reg;
    struct
    {
        unsigned int  rssi_msbit_128  : 5;  /* bit[0-4]  : rssi移位M的bit数。取值范围[6,17]。
                                                           C模不配置。 */
        unsigned int  reserved_0      : 3;  /* bit[5-7]  : 保留 */
        unsigned int  rssi_msbit_512  : 5;  /* bit[8-12] : rssi移位M的bit数。取值范围[6,17]。
                                                           C模不配置。 */
        unsigned int  reserved_1      : 3;  /* bit[13-15]: 保留 */
        unsigned int  rssi_msbit_1536 : 5;  /* bit[16-20]: rssi移位M的bit数。取值范围[6,17]。
                                                           C模不配置。 */
...
typedef union
{
    unsigned int      rssi1_data0_reg;
    struct
    {
        unsigned int  rssi1_data1 : 25; /* bit[0-24] : 逻辑上报的rssi1天线1的统计值。 */
...
typedef union
{
    unsigned int      rssi1_data1_reg;
    struct
    {
        unsigned int  rssi1_data2 : 25; /* bit[0-24] : 逻辑上报的rssi1天线2的统计值。 */
        unsigned int  reserved    : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rssi2_data0_reg;
    struct
    {
        unsigned int  rssi2_data1 : 25; /* bit[0-24] : 逻辑上报的rssi2天线1的统计值。 */
        unsigned int  reserved    : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rssi2_data1_reg;
    struct
    {
        unsigned int  rssi2_data2 : 25; /* bit[0-24] : 逻辑上报的rssi2天线2的统计值。 */
        unsigned int  reserved    : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dbg_rpt_at1_reg;
    struct
    {
        unsigned int  at1_dbg_data : 29; /* bit[0-28] : AT1 dbg采数上报。
                                                        [11:0] Q路数据
                                                        [23:12] I路数据
                                                        [24] tds_data_vld
                                                        [25] tds_ad_on1
...
typedef union
{
    unsigned int      dbg_rpt_at2_reg;
    struct
    {
        unsigned int  at2_dbg_data : 29; /* bit[0-28] : AT2 dbg采数上报。
                                                        [11:0] Q路数据
                                                        [23:12] I路数据
                                                        [24] tds_data_vld
                                                        [25] tds_ad_on1
...
typedef union
{
    unsigned int      c_work_mode_reg;
    struct
    {
        unsigned int  c_work_mode      : 1;  /* bit[0]   : 下行C模模式选择。0，初始模式；1，正常模式。 */
        unsigned int  dl_c_scfilter_en : 1;  /* bit[1]   : 下行W模和C模sc filter使能信号。0：关闭；1：打开。W模式下dl_c_scfilter_en或者多径的sc_filter使能都能够打开sc filter。 */
        unsigned int  reserved         : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_aagc_state_change_at1_reg;
    struct
    {
        unsigned int  c_aagc_state_change_at1 : 1;  /* bit[0]   : C模下天线1档位切换指示脉冲。 */
        unsigned int  reserved                : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_aagc_state_change_at2_reg;
    struct
    {
        unsigned int  c_aagc_state_change_at2 : 1;  /* bit[0]   : C模下天线2档位切换指示脉冲。 */
        unsigned int  reserved                : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_drssi_25db_at1_reg;
    struct
    {
        unsigned int  c_drssi_25db_at1 : 8;  /* bit[0-7] : DRSSI 0.25dB值，是DAGC8bit用于查找表求量化步阶MulFactor的ROM地址。AGC刚工作的时候，以此值作为初值。 */
...
typedef union
{
    unsigned int      c_drssi_25db_at2_reg;
    struct
    {
        unsigned int  c_drssi_25db_at2 : 8;  /* bit[0-7] : DRSSI 0.25dB值，是DAGC8bit用于查找表求量化步阶MulFactor的ROM地址。AGC刚工作的时候，以此值作为初值。 */
        unsigned int  reserved         : 24; /* bit[8-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_freq_switch_cfg_reg;
    struct
    {
        unsigned int  c_freq_switch_time   : 12; /* bit[0-11] : C模本频指示切换在base_cnt上的时刻，单位为chip。
                                                                注：base_cnt计数从0chip开始记起。 */
        unsigned int  reserved_0           : 4;  /* bit[12-15]: 保留 */
        unsigned int  c_freq_switch_type   : 1;  /* bit[16]   : 本异频切换类型。0，本频切换到异频；1，异频切换到本频。 */
        unsigned int  c_freq_switch_update : 1;  /* bit[17]   : C模本频指示切换控制脉冲。 */
...
typedef union
{
    unsigned int      dsp_rxiq_ahead_time_reg;
    struct
    {
        unsigned int  dsp_rxiq_ahead_time : 14; /* bit[0-13] : DSP配置RXIQ提前启动时间，用于和后续模块DRSSI在时间上对齐，配置精度为样点的整数倍。W模式要求配置值为17'd32。（V9为固定256个clk） */
...
typedef union
{
    unsigned int      c_drssi_time_reg;
    struct
    {
        unsigned int  c_drssi_start_time : 11; /* bit[0-10] : C模DRSSI统计在PCG或时隙内的起始时刻，单位chip。
                                                              1X和EVDO建议配置0。 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  c_drssi_stop_time  : 11; /* bit[16-26]: C模DRSSI统计在PCG或时隙内的结束时刻，单位chip。
                                                              1X配置小于1536(十进制)。
                                                              EVDO建议配置1648(十进制)。 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_drssi_linear_chip_num_at1_reg;
    struct
    {
        unsigned int  drssi_linear_chip_num_at1 : 12; /* bit[0-11] : 天线1 DRSSI上报线性值统计长度，单位chip。 */
        unsigned int  reserved                  : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_drssi_linear_chip_num_at2_reg;
    struct
    {
        unsigned int  drssi_linear_chip_num_at2 : 12; /* bit[0-11] : 天线2 DRSSI上报线性值统计长度，单位chip。 */
        unsigned int  reserved                  : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_ted_step_en_reg;
    struct
    {
        unsigned int  dsp_ted_step_en : 1;  /* bit[0]   : 采样纠偏检测步长强配使能，C模和强配功能下使用。 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_ted_step_update_reg;
    struct
    {
        unsigned int  dsp_ted_step_update : 1;  /* bit[0]   : 采样纠偏检测步长强配更新指示。 */
        unsigned int  reserved            : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_ted_step_reg;
    struct
    {
        unsigned int  dsp_ted_step_rx1_mc1 : 8;  /* bit[0-7]  : 天线1主载波采样纠偏检测步长。8bit有符号数。 */
        unsigned int  dsp_ted_step_rx1_mc2 : 8;  /* bit[8-15] : 天线1辅载波采样纠偏检测步长。8bit有符号数。 */
        unsigned int  dsp_ted_step_rx2_mc1 : 8;  /* bit[16-23]: 天线2主载波采样纠偏检测步长。8bit有符号数。 */
        unsigned int  dsp_ted_step_rx2_mc2 : 8;  /* bit[24-31]: 天线2辅载波采样纠偏检测步长。8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_dsp_cfg_en_reg;
    struct
    {
        unsigned int  dsp_sc_offset_en : 1;  /* bit[0]   : DSP强配采样偏差值使能。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_dsp_cfg_update_reg;
    struct
    {
        unsigned int  dsp_sc_offset_update : 1;  /* bit[0]   : DSP配置采样偏差值更新脉冲。 */
        unsigned int  reserved             : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_offset_at1_reg;
    struct
    {
        unsigned int  dsp_sc_offset_rx1_mc1 : 11; /* bit[0-10] : 天线1主载波DSP配置采样偏差值。 */
        unsigned int  reserved_0            : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_sc_offset_rx1_mc2 : 11; /* bit[16-26]: 天线1辅载波DSP配置采样偏差值。 */
        unsigned int  reserved_1            : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_offset_at2_reg;
    struct
    {
        unsigned int  dsp_sc_offset_rx2_mc1 : 11; /* bit[0-10] : 天线2主载波DSP配置采样偏差值。 */
        unsigned int  reserved_0            : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_sc_offset_rx2_mc2 : 11; /* bit[16-26]: 天线2辅载波DSP配置采样偏差值。 */
        unsigned int  reserved_1            : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_dlfe_single_tcxo_cfg_reg;
    struct
    {
        unsigned int  dsp_dlfe_base_cnt_sel        : 1;  /* bit[0]   : 下行前端纠偏fifo前的base_cnt来源：
                                                                       0，选择base_cnt；
                                                                       1，选择base_cnt_abb。
                                                                       W和C配置1，TDS下必须配置为0。 */
        unsigned int  dsp_dlfe_del_add_en          : 1;  /* bit[1]   : 删加点功能打开标志：0，关闭删加点功能；1，为开启删加点功能，此时sc_cal模块的删加点计算功能打开。 */
        unsigned int  dsp_dlfe_sc_cal_en           : 1;  /* bit[2]   : 采样偏差换算工作使能。 */
        unsigned int  dsp_fw_cfg_en                : 1;  /* bit[3]   : DSP配置AFC频偏值使能。 */
        unsigned int  dsp_freq_error_nco_fw_cfg_en : 1;  /* bit[4]   : 强行配置频偏校正所用NCO频率值使能。 */
        unsigned int  dsp_dlfe_nco_en              : 1;  /* bit[5]   : 频率校正NCO工作使能，TDS模式和非单TCXO模式下需要配置为0。 */
        unsigned int  dsp_dlfe_freq_error_en       : 1;  /* bit[6]   : 频偏校正使能。 */
        unsigned int  dsp_dlfe_fifo_en             : 1;  /* bit[7]   : 下行单TCXO异步fifo使能。 */
        unsigned int  dsp_dl_sc_cal_para_en        : 1;  /* bit[8]   : sc_filter模块的参数来源：
                                                                       1'b1：参数来源于sc_cal；
                                                                       1'b0：W参数来源于多径或强配，TDS和C模来源于强配。 */
        unsigned int  reserved                     : 23; /* bit[9-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_dlfe_single_tcxo_update_reg;
    struct
    {
        unsigned int  dsp_fw_update : 1;  /* bit[0]   : DSP配置AFC频偏值更新脉冲，当DSP配置AFC频偏值之后再配置更新脉冲，使频偏配置生效。 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_dlfe_single_tcxo_clr_reg;
    struct
    {
        unsigned int  dsp_sc_cal_clr            : 1;  /* bit[0]   : 采样偏差换算积分器清零信号，可维可测使用。 */
        unsigned int  dsp_freq_error_nco_ph_clr : 1;  /* bit[1]   : 频率校正NCO相位清零脉冲，可维可测使用。 */
        unsigned int  reserved                  : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_cal_para1_reg;
    struct
    {
        unsigned int  dsp_fw_cfg : 16; /* bit[0-15] : DSP配置AFC频偏值，16bit有符号数。 */
        unsigned int  reserved   : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_sc_cal_para2_reg;
    struct
    {
        unsigned int  dsp_inter_fc_ratio : 15; /* bit[0-14] : DSP配置下行前端配置异频相对本频频率比Fc,inter/Fc（其中Fc,inter为异频频率），15bit无符号数。 */
...
typedef union
{
    unsigned int      dsp_tcxo_vol_reg;
    struct
    {
        unsigned int  dsp_tcxo_vol : 16; /* bit[0-15] : DSP配置TCXO电压固定值，16bit无符号数。 */
        unsigned int  reserved     : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_freq_error_nco_fw_cfg_en_reg;
    struct
    {
        unsigned int  dsp_nco_para : 13; /* bit[0-12] : 该系数用于计算输入给NCO的输入频率。信号位宽为13U。
                                                        W模式下配置为1118，C模式下配置为3459（W和C下的配置值由k=4计算得出，K改变的话该值需要改变，具体计算公式为k/fs*2^32）。 */
...
typedef union
{
    unsigned int      dsp_sc_cal_para3_reg;
    struct
    {
        unsigned int  dsp_cfg_fc : 18; /* bit[0-17] : DSP配置下行前端配置本频1/Fc值，18bit无符号数。软件配置公式为：2^29/Fc*K，其中Fc为下行载波频率，K为每bit表示的Hz数（可以是2、4或8）。 */
...
typedef union
{
    unsigned int      dl_tds_sc_dlt_fsmp_reg;
    struct
    {
        unsigned int  dl_tds_sc_dlt_fsmp : 17; /* bit[0-16] : tds模式下采样调整步长;17bit */
...
typedef union
{
    unsigned int      dl_tds_sc_dlt_vld_reg;
    struct
    {
        unsigned int  dl_tds_sc_dlt_vld : 1;  /* bit[0]   : dsp配置采样调整步长有效指示,为脉冲信号; */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_tds_sc_ini_vld_reg;
    struct
    {
        unsigned int  dl_tds_sc_ini_vld : 1;  /* bit[0]   : dsp配置采样初始偏差值有效指示,为脉冲信号; */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_tds_sc_fsmp_mod_reg;
    struct
    {
        unsigned int  dl_tds_sc_fsmp_mod : 1;  /* bit[0]   : dsp配置参数模式:
                                                             1:直接根据vld信号锁存参数;
                                                             0:需要根据ad_on的上升沿与vld信号共同锁存参数;
                                                             默认值为1; */
        unsigned int  reserved           : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      dl_en_reg;
    struct
    {
        unsigned int  dl_sc_abn_del_en : 1;  /* bit[0]   : sc_cal模块在fifo几乎空，几乎满的异常删加点使能：
                                                           0：关闭异常删加点功能；
                                                           1：打开异常删加点功能。
                                                           默认打开。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      dl_rpt_ch_sel_reg;
    struct
    {
        unsigned int  rpt_ch_sel : 2;  /* bit[0-1] : SC_CAL可维可测上报通道选择：
                                                     2'b00：通道0（主天线主载波）
                                                     2'b01：通道1（主天线辅载波）
                                                     2'b10：通道2（辅天线主载波）
                                                     2'b11：通道3（辅天线辅载波） */
...
typedef union
{
    unsigned int      dl_sc_cal_num_rpt_reg;
    struct
    {
        unsigned int  dl_sc_add_num_rpt : 16; /* bit[0-15] : sc_cal模块计算加点个数统计。 */
        unsigned int  dl_sc_del_num_rpt : 16; /* bit[16-31]: sc_cal模块计算删点个数统计。 */
    } reg;
...
typedef union
{
    unsigned int      dl_sc_cal_num_rpt_clr_reg;
    struct
    {
        unsigned int  dl_sc_add_num_clr : 1;  /* bit[0]   : sc_cal模块计算加点个数统计清零。 */
        unsigned int  dl_sc_del_num_clr : 1;  /* bit[1]   : sc_cal模块计算删点个数统计清零。 */
        unsigned int  reserved          : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo0_empty_full_rpt_reg;
    struct
    {
        unsigned int  fifo0_empty_abn_dsp : 1;  /* bit[0]    : 通道0（主天线主载波） 异常空标志:
                                                               1'b0:FIFO不异常空；
                                                               1'b1:FIFO异常空。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  fifo0_full_dsp      : 1;  /* bit[4]    : 通道0 （主天线主载波）FIFO 满标志:
                                                               1'b0:FIFO不满；
                                                               1'b1:FIFO满。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7]  : 保留 */
        unsigned int  fifo0_cnt_a         : 5;  /* bit[8-12] : 通道0（主天线主载波） FIFO 写侧剩余数据个数 */
        unsigned int  reserved_2          : 3;  /* bit[13-15]: 保留 */
        unsigned int  fifo0_cnt_b         : 5;  /* bit[16-20]: 通道0（主天线主载波） FIFO 读侧剩余数据个数 */
...
typedef union
{
    unsigned int      dl_fifo0_empty_full_num_reg;
    struct
    {
        unsigned int  fifo0_alemp_num : 8;  /* bit[0-7]  : 通道0（主天线主载波） FIFO将空次数上报 */
        unsigned int  fifo0_alful_num : 8;  /* bit[8-15] : 通道0（主天线主载波） FIFO将满次数上报 */
        unsigned int  fifo0_emp_num   : 8;  /* bit[16-23]: 通道0（主天线主载波） FIFO空次数上报 */
        unsigned int  fifo0_ful_num   : 8;  /* bit[24-31]: 通道0（主天线主载波） FIFO满次数上报 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo1_empty_full_rpt_reg;
    struct
    {
        unsigned int  fifo1_empty_abn_dsp : 1;  /* bit[0]    : 通道1（主天线辅载波） 异常空标志:
                                                               1'b0:FIFO不异常空；
                                                               1'b1:FIFO异常空。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  fifo1_full_dsp      : 1;  /* bit[4]    : 通道1 （主天线辅载波）FIFO 满标志:
                                                               1'b0:FIFO不满；
                                                               1'b1:FIFO满。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7]  : 保留 */
        unsigned int  fifo1_cnt_a         : 5;  /* bit[8-12] : 通道0（主天线辅载波） FIFO 写侧剩余数据个数 */
        unsigned int  reserved_2          : 3;  /* bit[13-15]: 保留 */
        unsigned int  fifo1_cnt_b         : 5;  /* bit[16-20]: 通道1（主天线辅载波） FIFO 读侧剩余数据个数 */
        unsigned int  reserved_3          : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo1_empty_full_num_reg;
    struct
    {
        unsigned int  fifo1_alemp_num : 8;  /* bit[0-7]  : 通道1（主天线辅载波） FIFO将空次数上报 */
        unsigned int  fifo1_alful_num : 8;  /* bit[8-15] : 通道1（主天线辅载波） FIFO将满次数上报 */
        unsigned int  fifo1_emp_num   : 8;  /* bit[16-23]: 通道1（主天线辅载波） FIFO空次数上报 */
        unsigned int  fifo1_ful_num   : 8;  /* bit[24-31]: 通道1（主天线辅载波） FIFO满次数上报 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo2_empty_full_rpt_reg;
    struct
    {
        unsigned int  fifo2_empty_abn_dsp : 1;  /* bit[0]    : 通道2（辅天线主载波） 异常空标志:
                                                               1'b0:FIFO不异常空；
                                                               1'b1:FIFO异常空。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  fifo2_full_dsp      : 1;  /* bit[4]    : 通道2 （辅天线主载波）FIFO 满标志:
                                                               1'b0:FIFO不满；
                                                               1'b1:FIFO满。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7]  : 保留 */
        unsigned int  fifo2_cnt_a         : 5;  /* bit[8-12] : 通道2（辅天线主载波） FIFO 写侧剩余数据个数 */
        unsigned int  reserved_2          : 3;  /* bit[13-15]: 保留 */
        unsigned int  fifo2_cnt_b         : 5;  /* bit[16-20]: 通道2（辅天线主载波） FIFO 读侧剩余数据个数 */
        unsigned int  reserved_3          : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo2_empty_full_num_reg;
    struct
    {
        unsigned int  fifo2_alemp_num : 8;  /* bit[0-7]  : 通道2（辅天线主载波） FIFO将空次数上报 */
        unsigned int  fifo2_alful_num : 8;  /* bit[8-15] : 通道2（辅天线主载波） FIFO将满次数上报 */
        unsigned int  fifo2_emp_num   : 8;  /* bit[16-23]: 通道2（辅天线主载波） FIFO空次数上报 */
        unsigned int  fifo2_ful_num   : 8;  /* bit[24-31]: 通道2（辅天线主载波） FIFO满次数上报 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo3_empty_full_rpt_reg;
    struct
    {
        unsigned int  fifo3_empty_abn_dsp : 1;  /* bit[0]    : 通道3（辅天线辅载波） 异常空标志:
                                                               1'b0:FIFO不异常空；
                                                               1'b1:FIFO异常空。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  fifo3_full_dsp      : 1;  /* bit[4]    : 通道3（辅天线辅载波）FIFO 满标志:
                                                               1'b0:FIFO不满；
                                                               1'b1:FIFO满。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7]  : 保留 */
        unsigned int  fifo3_cnt_a         : 5;  /* bit[8-12] : 通道3（辅天线辅载波） FIFO 写侧剩余数据个数 */
        unsigned int  reserved_2          : 3;  /* bit[13-15]: 保留 */
        unsigned int  fifo3_cnt_b         : 5;  /* bit[16-20]: 通道3（辅天线辅载波） FIFO 读侧剩余数据个数 */
        unsigned int  reserved_3          : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo3_empty_full_num_reg;
    struct
    {
        unsigned int  fifo3_alemp_num : 8;  /* bit[0-7]  : 通道3（辅天线辅载波） FIFO将空次数上报 */
        unsigned int  fifo3_alful_num : 8;  /* bit[8-15] : 通道3（辅天线辅载波） FIFO将满次数上报 */
        unsigned int  fifo3_emp_num   : 8;  /* bit[16-23]: 通道3（辅天线辅载波） FIFO空次数上报 */
        unsigned int  fifo3_ful_num   : 8;  /* bit[24-31]: 通道3（辅天线辅载波） FIFO满次数上报 */
    } reg;
...
typedef union
{
    unsigned int      dl_fifo_clr_reg;
    struct
    {
        unsigned int  dsp_dl_fifo_clr_en     : 1;  /* bit[0]   : FIFO清零电平信号:
                                                                 1'b0:FIFO不清零
                                                                 1'b1:FIFO清零 */
        unsigned int  dsp_dl_fifo_rpt_clr_en : 1;  /* bit[1]   : FIFO异常空满上报状态清零信号:
                                                                 1'b0:FIFO异常空满不清零
                                                                 1'b1:FIFO异常空满清零 */
        unsigned int  reserved               : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_base_cnt_rpt_reg;
    struct
    {
        unsigned int  w_base_cnt_rpt : 17; /* bit[0-16] : 下行前端W_BASE_CNT上报，每个时隙边界后的第二个样点上报。 */
        unsigned int  reserved       : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx1_mc1_dagc_ctrl_rpt_reg;
    struct
    {
        unsigned int  w_rx1_mc1_dagc_ctrl_rpt      : 16; /* bit[0-15] : W主天线主载波dagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  w_rx1_mc1_dagc_ctrl_4bit_rpt : 12; /* bit[16-27]: W主天线主载波dagc_ctrl_4bit上报，每个符号边界后的第二个chip上报。 */
...
typedef union
{
    unsigned int      w_rx1_mc2_dagc_ctrl_rpt_reg;
    struct
    {
        unsigned int  w_rx1_mc2_dagc_ctrl_rpt      : 16; /* bit[0-15] : W主天线辅载波dagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  w_rx1_mc2_dagc_ctrl_4bit_rpt : 12; /* bit[16-27]: W主天线辅载波dagc_ctrl_4bit上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved                     : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx2_mc1_dagc_ctrl_rpt_reg;
    struct
    {
        unsigned int  w_rx2_mc1_dagc_ctrl_rpt      : 16; /* bit[0-15] : W辅天线主载波dagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  w_rx2_mc1_dagc_ctrl_4bit_rpt : 12; /* bit[16-27]: W辅天线主载波dagc_ctrl_4bit上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved                     : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx2_mc2_dagc_ctrl_rpt_reg;
    struct
    {
        unsigned int  w_rx2_mc2_dagc_ctrl_rpt      : 16; /* bit[0-15] : W辅天线辅载波dagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  w_rx2_mc2_dagc_ctrl_4bit_rpt : 12; /* bit[16-27]: W辅天线辅载波dagc_ctrl_4bit上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved                     : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_aagc_ctrl_rpt_reg;
    struct
    {
        unsigned int  w_rx1_aagc_ctrl_rpt : 12; /* bit[0-11] : W主天线aagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved_0          : 4;  /* bit[12-15]: 保留 */
        unsigned int  w_rx2_aagc_ctrl_rpt : 12; /* bit[16-27]: W辅天线aagc_ctrl上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved_1          : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx1_mc1_rssi_rpt_reg;
    struct
    {
        unsigned int  w_rx1_mc1_rssi_rpt : 11; /* bit[0-10] : W主天线主载波rssi上报，每个符号边界后的第二个chip上报。 */
...
typedef union
{
    unsigned int      w_rx1_mc2_rssi_rpt_reg;
    struct
    {
        unsigned int  w_rx1_mc2_rssi_rpt : 11; /* bit[0-10] : W主天线辅载波rssi上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved           : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx2_mc1_rssi_rpt_reg;
    struct
    {
        unsigned int  w_rx2_mc1_rssi_rpt : 11; /* bit[0-10] : W辅天线主载波rssi上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved           : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rx2_mc2_rssi_rpt_reg;
    struct
    {
        unsigned int  w_rx2_mc2_rssi_rpt : 11; /* bit[0-10] : W辅天线辅载波rssi上报，每个符号边界后的第二个chip上报。 */
        unsigned int  reserved           : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wc_nco_rpt_reg;
    struct
    {
        unsigned int  nco_fw_cfg_rpt : 16; /* bit[0-15] : WC NCO输入的频偏值上报（该频偏值来自SC CAL模块） */
        unsigned int  reserved       : 16; /* bit[16-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_ssi_ch_sel_reg;
    struct
    {
        unsigned int  wagc0_intra_ssi_ch_sel : 2;  /* bit[0-1]  : W下行主天线本频SSI控制字通道选择。
                                                                  C模不配置。
                                                                  00：选择ch0；
                                                                  01：选择ch1；
                                                                  10：选择ch2；
                                                                  11：选择ch3； */
        unsigned int  reserved_0             : 2;  /* bit[2-3]  : 保留 */
        unsigned int  wagc0_inter_ssi_ch_sel : 2;  /* bit[4-5]  : W下行主天线异频SSI控制字通道选择。
                                                                  C模不配置。
                                                                  00：选择ch0；
                                                                  01：选择ch1；
                                                                  10：选择ch2；
                                                                  11：选择ch3； */
        unsigned int  reserved_1             : 2;  /* bit[6-7]  : 保留 */
        unsigned int  wagc1_intra_ssi_ch_sel : 2;  /* bit[8-9]  : W下行辅天线本频SSI控制字通道选择。
                                                                  C模不配置。
                                                                  00：选择ch0；
                                                                  01：选择ch1；
                                                                  10：选择ch2；
                                                                  11：选择ch3； */
        unsigned int  reserved_2             : 2;  /* bit[10-11]: 保留 */
        unsigned int  wagc1_inter_ssi_ch_sel : 2;  /* bit[12-13]: W下行辅天线异频SSI控制字通道选择。
                                                                  C模不配置。
                                                                  00：选择ch0；
                                                                  01：选择ch1；
                                                                  10：选择ch2；
...
typedef union
{
    unsigned int      wc_dc_reg_at1_0_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_8s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_8s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S2的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S2的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_1_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_8s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_8s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S4的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S4的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_2_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_8s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_8s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S6的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S6的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_3_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_8s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_8s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S8的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S8的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_4_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_5s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_5s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S2的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S2的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_5_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_5s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_5s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S4的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S4的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_6_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_5s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_5s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S6的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S6的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_7_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_5s_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_5s_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_i_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S8的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_q_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S8的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_0_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_8s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_8s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S2的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S2的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_1_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_8s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_8s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S4的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S4的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_2_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_8s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_8s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S6的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S6的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_3_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_8s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_8s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S8的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S8的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_4_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_5s_i_at2 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_5s_q_at2 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S1的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_i_at2 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S2的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_q_at2 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S2的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_5_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_5s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_5s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S3的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S4的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S4的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_6_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_5s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_5s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S5的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S6的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S6的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_7_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_5s_i_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_5s_q_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S7的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S8的DC配置值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S8的DC配置值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_0_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_8s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_8s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S2的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S2的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_1_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_8s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_8s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S4的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S4的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_2_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_8s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_8s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S6的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S6的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_3_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_8s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，8档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_8s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，8档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，8档AAGC S8的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，8档AAGC S8的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_4_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_5s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_5s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S2的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S2的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_5_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_5s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_5s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S4的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S4的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_6_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_5s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_5s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S6的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S6的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at1_rpt_7_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_5s_i_rpt_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路，5档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_5s_q_rpt_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路，5档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_i_rpt_at1 : 8;  /* bit[16-23]: WC模 天线1 I路，5档AAGC S8的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_q_rpt_at1 : 8;  /* bit[24-31]: WC模 天线1 Q路，5档AAGC S8的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_0_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_8s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_8s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S2的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_8s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S2的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_1_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_8s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_8s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S4的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_8s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S4的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_2_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_8s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_8s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S6的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_8s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，8档AAGC S6的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_3_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_8s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，8档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_8s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，8档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，8档AAGC S8的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_8s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 I路，8档AAGC S8的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_4_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s1_5s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s1_5s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S1的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S2的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s2_5s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S2的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_5_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s3_5s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s3_5s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S3的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S4的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s4_5s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S4的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_6_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s5_5s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s5_5s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S5的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S6的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s6_5s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S6的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_reg_at2_rpt_7_reg;
    struct
    {
        unsigned int  dsp_dc_reg_s7_5s_i_rpt_at2 : 8;  /* bit[0-7]  : WC模 天线2 I路，5档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s7_5s_q_rpt_at2 : 8;  /* bit[8-15] : WC模 天线2 Q路，5档AAGC S7的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_i_rpt_at2 : 8;  /* bit[16-23]: WC模 天线2 I路，5档AAGC S8的DC上报值，8bit有符号数。 */
        unsigned int  dsp_dc_reg_s8_5s_q_rpt_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路，5档AAGC S8的DC上报值，8bit有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      wc_fast_dc_en_reg;
    struct
    {
        unsigned int  dsp_dcoc_fast_en      : 1;  /* bit[0]   : WC模DC快速收敛功能开关。
                                                                0：关闭DC快速收敛功能；
                                                                1：打开DC快速收敛功能，在去直流一阶段重新启动时，DC直流初值为软件配置的DC初始值，或者硬件根据档位选出的DC初始值。
                                                                W模
                                                                TDS模不支持该功能，需要关闭。 */
        unsigned int  reserved_0            : 3;  /* bit[1-3] : 保留 */
        unsigned int  dsp_dc_reg_rpt_sel    : 2;  /* bit[4-5] : 硬件上报各档位DC直流值频率选择。
                                                                00：按样点更新。
                                                                01：按chip更新。
                                                                10：按符号更新。
                                                                11：reserved。
                                                                W模使用，X模，TDS模不涉及。 */
        unsigned int  reserved_1            : 2;  /* bit[6-7] : 保留 */
        unsigned int  dsp_dc_fast_manual_en : 1;  /* bit[8]   : WC模软件DC直流初值配置使能。
                                                                0：不使能。DC初值选择硬件根据档位选出的DC值。
                                                                1：使能。DC初值选择软件接口配置的DC值。
                                                                要dsp_dcoc_fast_en有效时，该初值才能生效。
                                                                W模配置为0，C模配置为1，TDS模不涉及。 */
...
typedef union
{
    unsigned int      dc_reg_load_bp_reg;
    struct
    {
        unsigned int  dsp_dc_reg_load_vld : 1;  /* bit[0]   : 硬件各档位DC_REG加载软件值使能脉冲。
                                                              W模使用，X模，TDS模不涉及。 */
        unsigned int  reserved            : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wc_dc_fast_manual_reg;
    struct
    {
        unsigned int  dsp_dc_fast_manual_i_at1 : 8;  /* bit[0-7]  : WC模 天线1 I路DC配置值，8bit有符号数。
                                                                    配置值 = DC直流值/4，并限幅到
                                                                    [2^7-1,(-2^7+1)]范围内。 */
        unsigned int  dsp_dc_fast_manual_q_at1 : 8;  /* bit[8-15] : WC模 天线1 Q路DC配置值，8bit有符号数。
                                                                    配置值 = DC直流值/4，并限幅到
                                                                    [2^7-1,(-2^7+1)]范围内。 */
        unsigned int  dsp_dc_fast_manual_i_at2 : 8;  /* bit[16-23]: WC模 天线2 I路DC配置值，8bit有符号数。
                                                                    配置值 = DC直流值/4，并限幅到
                                                                    [2^7-1,(-2^7+1)]范围内。 */
        unsigned int  dsp_dc_fast_manual_q_at2 : 8;  /* bit[24-31]: WC模 天线2 Q路DC配置值，8bit有符号数。
                                                                    配置值 = DC直流值/4，并限幅到
                                                                    [2^7-1,(-2^7+1)]范围内。 */
    } reg;
...
typedef union
{
    unsigned int      wc_fast_agc_en_reg;
    struct
    {
        unsigned int  dsp_fast_agc_en : 1;  /* bit[0]   : 快速AGC切档使能。
                                                          0：关闭快速AGC切档；
                                                          1：打开快速AGC切档。
                                                          W模使用。C/TDS不涉及。 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      wdrx_agc_open_en_reg;
    struct
    {
        unsigned int  dsp_wdrx_agc_open_en : 1;  /* bit[0]   : WDRX唤醒解寻呼模式下AGC自动打开使能。
                                                               0：AGC只通过软件开关打开。
                                                               1：WDRX唤醒解寻呼模式下，AGC在软件配置开关打开后，延迟等待到BBP自动打开脉冲才启动。
                                                               W模使用。C/TDS不涉及。 */
        unsigned int  reserved             : 31; /* bit[1-31]: 保留 */
...
/*****************************************************************************
 结构名    : WBBP_RXBLANK_EN_UNION
 结构说明  : RXBLANK_EN 寄存器结构定义。地址偏移量:0x694，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_en_reg;
    struct
    {
        unsigned int  cpu_rxblank_en : 1;  /* bit[0]   : DLFE RX blanking使能，配置1时启动RX blanking功能，对数据做填0处理。 */
        unsigned int  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_RXBLANK_EN_UNION;
#define WBBP_RXBLANK_EN_cpu_rxblank_en_START  (0)
#define WBBP_RXBLANK_EN_cpu_rxblank_en_END    (0)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_START_POS_UNION
 结构说明  : RXBLANK_START_POS 寄存器结构定义。地址偏移量:0x698，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_start_pos_reg;
    struct
    {
        unsigned int  cpu_rxblank_start_pos : 29; /* bit[0-28] : RX blanking信号起始位置的C模base_cnt上报，当对应的cpu_rxblank_start_pos_valid有效时该值有效。 */
        unsigned int  reserved              : 3;  /* bit[29-31]: 保留 */
    } reg;
} WBBP_RXBLANK_START_POS_UNION;
#define WBBP_RXBLANK_START_POS_cpu_rxblank_start_pos_START  (0)
#define WBBP_RXBLANK_START_POS_cpu_rxblank_start_pos_END    (28)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_START_POS_VALID_UNION
 结构说明  : RXBLANK_START_POS_VALID 寄存器结构定义。地址偏移量:0x69C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_start_pos_valid_reg;
    struct
    {
        unsigned int  cpu_rxblank_start_pos_valid : 1;  /* bit[0]   : RX blanking信号起始位置的C模base_cnt上报有效标志，BBP在rxblank_ind的上升沿锁存base_cnt值同时拉高valid，软件读取后配置cpu_rxblank_start_pos_clr信号清除标志。 */
        unsigned int  reserved                    : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_RXBLANK_START_POS_VALID_UNION;
#define WBBP_RXBLANK_START_POS_VALID_cpu_rxblank_start_pos_valid_START  (0)
#define WBBP_RXBLANK_START_POS_VALID_cpu_rxblank_start_pos_valid_END    (0)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_START_POS_CLR_UNION
 结构说明  : RXBLANK_START_POS_CLR 寄存器结构定义。地址偏移量:0x6A0，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_start_pos_clr_reg;
    struct
    {
        unsigned int  cpu_rxblank_start_pos_clr : 1;  /* bit[0]   : cpu_rxblank_start_pos_valid清除标志，脉冲信号，软件读取cpu_rxblank_start_pos后配置该信号，该信号需要保证在下一次的rxbland_ind到来前配置，否则会造成错误。 */
        unsigned int  reserved                  : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_RXBLANK_START_POS_CLR_UNION;
#define WBBP_RXBLANK_START_POS_CLR_cpu_rxblank_start_pos_clr_START  (0)
#define WBBP_RXBLANK_START_POS_CLR_cpu_rxblank_start_pos_clr_END    (0)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_END_POS_UNION
 结构说明  : RXBLANK_END_POS 寄存器结构定义。地址偏移量:0x6A4，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_end_pos_reg;
    struct
    {
        unsigned int  cpu_rxblank_end_pos : 29; /* bit[0-28] : RX blanking信号起始位置的C模base_cnt上报，当对应的cpu_rxblank_end_pos_valid有效时该值有效 */
        unsigned int  reserved            : 3;  /* bit[29-31]: 保留 */
    } reg;
} WBBP_RXBLANK_END_POS_UNION;
#define WBBP_RXBLANK_END_POS_cpu_rxblank_end_pos_START  (0)
#define WBBP_RXBLANK_END_POS_cpu_rxblank_end_pos_END    (28)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_END_POS_VALID_UNION
 结构说明  : RXBLANK_END_POS_VALID 寄存器结构定义。地址偏移量:0x6A8，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_end_pos_valid_reg;
    struct
    {
        unsigned int  cpu_rxblank_end_pos_valid : 1;  /* bit[0]   : RX blanking信号起始位置的C模base_cnt上报有效标志，BBP在rxblank_ind的下降沿锁存base_cnt值同时拉高valid，软件读取后配置cpu_rxblank_end_pos_clr信号清除标志。 */
        unsigned int  reserved                  : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_RXBLANK_END_POS_VALID_UNION;
#define WBBP_RXBLANK_END_POS_VALID_cpu_rxblank_end_pos_valid_START  (0)
#define WBBP_RXBLANK_END_POS_VALID_cpu_rxblank_end_pos_valid_END    (0)


/*****************************************************************************
 结构名    : WBBP_RXBLANK_END_POS_CLR_UNION
 结构说明  : RXBLANK_END_POS_CLR 寄存器结构定义。地址偏移量:0x6AC，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      rxblank_end_pos_clr_reg;
    struct
    {
        unsigned int  cpu_rxblank_end_pos_clr : 1;  /* bit[0]   : cpu_rxblank_end_pos_valid清除标志，脉冲信号，软件读取cpu_rxblank_end_pos后配置该信号，该信号需要保证在下一次的rxbland_ind到来前配置，否则会造成错误。 */
        unsigned int  reserved                : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_RXBLANK_END_POS_CLR_UNION;
#define WBBP_RXBLANK_END_POS_CLR_cpu_rxblank_end_pos_clr_START  (0)
#define WBBP_RXBLANK_END_POS_CLR_cpu_rxblank_end_pos_clr_END    (0)
...
typedef union
{
    unsigned int      wagc_sat_num_dcsc_reg;
    struct
    {
        unsigned int  dagc_sat_num_1f_at1_dcsc : 16; /* bit[0-15] : 天线1一帧内DAGC IQ数据饱和次数。 */
        unsigned int  dagc_sat_num_1f_at2_dcsc : 16; /* bit[16-31]: 天线2一帧内DAGC IQ数据饱和次数。 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_cfg_at1_dcsc_reg;
    struct
    {
        unsigned int  dsp_init_drssi_25db_at1_dcsc : 9;  /* bit[0-8]  : 刚刚打开AGC时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                        说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  dsp_init_rssi_rf_at1_dcsc    : 11; /* bit[9-19] : 刚刚打开AGC时，BBP使用此值进行能量测量，单位0.125dB。
                                                                        说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved                     : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_cfg_at2_dcsc_reg;
    struct
    {
        unsigned int  dsp_init_drssi_25db_at2_dcsc : 9;  /* bit[0-8]  : 刚刚打开AGC时，BBP使用此值查表DAGC ROM，单位0.25dB。
                                                                        说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  dsp_init_rssi_rf_at2_dcsc    : 11; /* bit[9-19] : 刚刚打开AGC时，BBP使用此值进行能量测量，单位0.125dB。
                                                                        说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。DRX唤醒时，打开AGC之前，软件将之前读取的值配置到此寄存器。 */
        unsigned int  reserved                     : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      inter_agc_cfg_at1_dcsc_reg;
    struct
    {
        unsigned int  dsp_inter_drssi_25db_at1_dcsc : 9;  /* bit[0-8]  : 刚刚进入异频GAP时，BBP使用此值查表DAGC ROM，单位0.25dB。 */
        unsigned int  dsp_inter_rssi_rf_at1_dcsc    : 11; /* bit[9-19] : 刚刚进入异频GAP时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。 */
        unsigned int  reserved                      : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      inter_agc_cfg_at2_dcsc_reg;
    struct
    {
        unsigned int  dsp_inter_drssi_25db_at2_dcsc : 9;  /* bit[0-8]  : 刚刚进入异频GAP时，BBP使用此值查表DAGC ROM，单位0.25dB。 */
        unsigned int  dsp_inter_rssi_rf_at2_dcsc    : 11; /* bit[9-19] : 刚刚进入异频GAP时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。 */
        unsigned int  reserved                      : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_scdc_agc_cfg0_at1_dcsc_reg;
    struct
    {
        unsigned int  dsp_scdc_rssi_rf_at1_dcsc : 11; /* bit[0-10] : RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。 */
        unsigned int  reserved                  : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_scdc_agc_cfg1_at1_dcsc_reg;
    struct
    {
        unsigned int  dsp_scdc_drssi_at1_dcsc         : 9;  /* bit[0-8]  : RF_SC/RF_DC切换刚刚完成时，BBP使用此值查表DAGC ROM，单位0.25dB。 */
        unsigned int  reserved_0                      : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_init_scdc_rssi_en_at1_dcsc  : 1;  /* bit[16]   : RF_SC/RF_DC切换刚刚完成时，DSP配置天线口RSSI初始值使能。1，DSP配置天线口RSSI初始值；0，BBP继承以前的值。 */
        unsigned int  reserved_1                      : 7;  /* bit[17-23]: 保留 */
        unsigned int  dsp_init_scdc_drssi_en_at1_dcsc : 1;  /* bit[24]   : 刚刚进入RF_SC时，DSP配置0.25dB精度DRSSI初始值使能。1，DSP配置0.25dB精度DRSSI初始值；0，BBP继承以前的值。 */
...
typedef union
{
    unsigned int      init_scdc_agc_cfg0_at2_dcsc_reg;
    struct
    {
        unsigned int  dsp_scdc_rssi_rf_at2_dcsc : 11; /* bit[0-10] : RF_SC/RF_DC切换刚刚完成时，BBP使用此值进行能量测量，需配置二进制补码值，单位0.125dB。 */
        unsigned int  reserved                  : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_scdc_agc_cfg1_at2_dcsc_reg;
    struct
    {
        unsigned int  dsp_scdc_drssi_at2_dcsc         : 9;  /* bit[0-8]  : RF_SC/RF_DC切换刚刚完成时，BBP使用此值查表DAGC ROM，单位0.25dB。 */
        unsigned int  reserved_0                      : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_init_scdc_rssi_en_at2_dcsc  : 1;  /* bit[16]   : RF_SC/RF_DC切换刚刚完成时，DSP配置天线口RSSI初始值使能。1，DSP配置天线口RSSI初始值；0，BBP继承以前的值。 */
        unsigned int  reserved_1                      : 7;  /* bit[17-23]: 保留 */
        unsigned int  dsp_init_scdc_drssi_en_at2_dcsc : 1;  /* bit[24]   : 刚刚进入RF_SC时，DSP配置0.25dB精度DRSSI初始值使能。1，DSP配置0.25dB精度DRSSI初始值；0，BBP继承以前的值。 */
        unsigned int  reserved_2                      : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_rpt_cell_at1_dcsc_reg;
    struct
    {
        unsigned int  dsp_rpt_cell_drssi_25db_at1_dcsc : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（单载波），BBP使用此值查表DAGC ROM。
                                                                            说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_cell_rssi_rf_at1_dcsc    : 11; /* bit[9-19] : BBP上报天线口RSSI值（单载波），0.125dB精度。
                                                                            说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  reserved                         : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      init_agc_rpt_cell_at2_dcsc_reg;
    struct
    {
        unsigned int  dsp_rpt_cell_drssi_25db_at2_dcsc : 9;  /* bit[0-8]  : BBP上报0.25dB精度的DRSSI值（单载波），BBP使用此值查表DAGC ROM。
                                                                            说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  dsp_rpt_cell_rssi_rf_at2_dcsc    : 11; /* bit[9-19] : BBP上报天线口RSSI值（单载波），0.125dB精度。
                                                                            说明：主要用于DRX流程。进入DRX睡眠时，软件从BBP读取寄存器获取。 */
        unsigned int  reserved                         : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wt_dbg_en_reg;
    struct
    {
        unsigned int  wt_dbg_en           : 1;  /* bit[0]   : W模式与TDS模式数据采集使能：
                                                              0：不使能；
                                                              1：使能； */
        unsigned int  w_udl_sel           : 1;  /* bit[1]   : W模式上、下行数据选择：
                                                              0：选择下行数据；
                                                              1：选择上行数据； */
        unsigned int  reserved_0          : 2;  /* bit[2-3] : 保留 */
        unsigned int  dsp2mux_dbg_clk_sel : 1;  /* bit[4]   : dbg时钟选择 */
        unsigned int  reserved_1          : 27; /* bit[5-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      zsp_int_flag_reg;
    struct
    {
        unsigned int  rx_half    : 1;  /* bit[0]   : 接收数据存储器半满中断标志：0此中断未产生；1 此中断产生。 */
        unsigned int  rx_full    : 1;  /* bit[1]   : 接收数据存储器全满中断标志：0此中断未产生；1 此中断产生。 */
        unsigned int  rx_finish  : 1;  /* bit[2]   : 接收数据存储器完成中断标志，同时是其它模块上报参数有效中断标志：0此中断未产生；1 此中断产生。 */
        unsigned int  ad_end_flg : 1;  /* bit[3]   : AD_STOP中断时，接收数据保存位置：0 前半个FIFO；1 后半个FIFO。 */
        unsigned int  tx_half    : 1;  /* bit[4]   : 发送存储器半空中断标志：0此中断未产生；1 此中断产生。 */
        unsigned int  tx_full    : 1;  /* bit[5]   : 发送存储器全空中断标志：0此中断未产生；1 此中断产生。 */
        unsigned int  tx_finish  : 1;  /* bit[6]   : 发送完成中断标志：0此中断未产生；1 此中断产生。 */
...
#define WBBP_ZSP_INT_FLAG_tx_half_START     (4)
#define WBBP_ZSP_INT_FLAG_tx_half_END       (4)
#define WBBP_ZSP_INT_FLAG_tx_full_START     (5)
#define WBBP_ZSP_INT_FLAG_tx_full_END       (5)
#define WBBP_ZSP_INT_FLAG_tx_finish_START   (6)
#define WBBP_ZSP_INT_FLAG_tx_finish_END     (6)
...
typedef union
{
    unsigned int      rx_addr_reg;
    struct
    {
        unsigned int  ad_end_addr : 16; /* bit[0-15] : 接收数据存储器地址；当AD接收结束时，指示接收存储器的当前地址。 */
        unsigned int  reserved    : 16; /* bit[16-31]: 保留 */
    } reg;
} WBBP_RX_ADDR_UNION;
...
typedef union
{
    unsigned int      zsp_int_mask_reg;
    struct
    {
        unsigned int  rx_half_mask   : 1;  /* bit[0]   : 接收存储器半满中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  rx_full_mask   : 1;  /* bit[1]   : 接收存储器全满中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  rx_finish_mask : 1;  /* bit[2]   : 接收完成中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  reserved_0     : 1;  /* bit[3]   : 保留 */
        unsigned int  tx_half_mask   : 1;  /* bit[4]   : 发送存储器半满中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  tx_full_mask   : 1;  /* bit[5]   : 发送存储器全满中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  tx_finish_mask : 1;  /* bit[6]   : 发送完成中断屏蔽位：1不屏蔽此中断；0屏蔽此中断，相应的中断状态位仍被置位。 */
        unsigned int  reserved_1     : 25; /* bit[7-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      int_clear_reg;
    struct
    {
        unsigned int  rx_half_clear   : 1;  /* bit[0]   : 接收存储器半满中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  rx_full_clear   : 1;  /* bit[1]   : 接收存储器全满中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  rx_finish_clear : 1;  /* bit[2]   : 接收完成中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  reserved_0      : 1;  /* bit[3]   : 保留 */
        unsigned int  tx_half_clear   : 1;  /* bit[4]   : 发送存储器半满中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  tx_full_clear   : 1;  /* bit[5]   : 发送存储器全满中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  tx_finish_clear : 1;  /* bit[6]   : 发送完成中断清除位：0不清除此中断；1清除此中断，相应的中断状态位仍被置位。 */
        unsigned int  reserved_1      : 25; /* bit[7-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      int_alarm_reg;
    struct
    {
        unsigned int  rx_half_alarm   : 1;  /* bit[0]   : 接收存储器半满中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  rx_full_alarm   : 1;  /* bit[1]   : 接收存储器全满中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  rx_finish_alarm : 1;  /* bit[2]   : 接收完成中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  reserved_0      : 1;  /* bit[3]   : 保留 */
        unsigned int  tx_half_alarm   : 1;  /* bit[4]   : 发送存储器半满中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  tx_full_alarm   : 1;  /* bit[5]   : 发送存储器全满中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  tx_finish_alarm : 1;  /* bit[6]   : 发送完成中断告警位：
                                                          0不告警此中断；
                                                          1告警此中断。 */
        unsigned int  reserved_1      : 25; /* bit[7-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mem_rrc_reg;
    struct
    {
        unsigned int  mem_in_dl  : 16; /* bit[0-15] : [15]为下行W与TDS模式选择信号，0为W，1为TDS;
                                                      [14:12]为dlfe输出的ad_on1/ad_on/dvld；
                                                      [11:0]为dlfe输出I路数据的高12bit； */
        unsigned int  mem_out_ul : 16; /* bit[16-31]: [31]为上行W与TDS模式选择信号，0为W，1为TDS;
                                                      [30]为tds输入的da_on信号；
                                                      [29:28]为memory输出的da_on/dvld；
                                                      [27:16]为memory的输出I路数据高12bit； */
    } reg;
...
typedef union
{
    unsigned int      abb_code_reg;
    struct
    {
        unsigned int  dsp_abb_rx_code_sel   : 1;  /* bit[0]    : 上行采样数据格式标志
                                                                 1'b0:二进制补码
                                                                 1'b1:自然码 */
        unsigned int  reserved_0            : 3;  /* bit[1-3]  : 保留 */
        unsigned int  dsp_abb_tx_code_sel   : 1;  /* bit[4]    : 下行采样数据格式标志
                                                                 1'b0:二进制补码
                                                                 1'b1:自然码 */
        unsigned int  reserved_1            : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_abb_shift_num_sel : 2;  /* bit[8-9]  : ABB数据由14bit转换12bit的控制。
                                                                 0表示四舍五入2bit，限幅至12bit；
                                                                 1表示四舍五入1bit，限幅至12bit；
                                                                 2表示四舍五入0bit，限幅至12bit。 */
...
typedef union
{
    unsigned int      abb_dly_reg;
    struct
    {
        unsigned int  dsp_rx_ant_num      : 1;  /* bit[0]    : 接收通道个数：
                                                               1'b1：双天线（天线0和天线1）；
                                                               1'b0：单天线（天线0）。tds 专用。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3]  : 保留 */
        unsigned int  dsp_rx_ant_dly_sel  : 1;  /* bit[4]    : 定时偏差调整功能通道选择，0表示A通道，1表示B通道。tds 专用。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_rx_ant_dly_en   : 1;  /* bit[8]    : 定时偏差调整功能打开使能，高电平有效。tds 专用。 */
        unsigned int  reserved_2          : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_rx_ant_data_dly : 5;  /* bit[12-16]: 为了调整定时偏差，使RX通道A与通道B的IQ数据对齐，该配置为指定通道
                                                               延迟采样占个数与该值实际数值相同，只取0~26的值，为0即不延迟。tds 专用。 */
...
typedef union
{
    unsigned int      abb_trx_loop_reg;
    struct
    {
        unsigned int  dsp_abb_trx_loop_en       : 1;  /* bit[0]   : ABB环回模式使能信号，高电平有效 */
        unsigned int  reserved_0                : 3;  /* bit[1-3] : 保留 */
        unsigned int  dsp_abb_trx_loop_mode_sel : 3;  /* bit[4-6] : ABB环回模式选择信号。
                                                                    3'd0：ABB下行A通道FIFO前环回至上行；
                                                                    3'd1：ABB下行B通道FIFO前环回至上行；
                                                                    3'd2：ABB下行主集通道FIFO后环回至上行；
                                                                    3'd3：ABB下行分集通道FIFO后环回至上行；
...
typedef union
{
    unsigned int      abb_fifo_dbg_reg;
    struct
    {
        unsigned int  dsp_abb_rx_fifo0_state     : 1;  /* bit[0]    : RX 主集FIFO曾经满状态锁存上报。可被dsp_abb_rx_fifo0_state_clr清零。 */
        unsigned int  reserved_0                 : 3;  /* bit[1-3]  : 保留 */
        unsigned int  dsp_abb_rx_fifo1_state     : 1;  /* bit[4]    : RX 分集FIFO曾经满状态锁存上报。可被dsp_abb_rx_fifo1_state_clr清零。 */
        unsigned int  reserved_1                 : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_abb_rx_fifo0_state_clr : 1;  /* bit[8]    : RX 主集FIFO曾经满状态清零信号。 */
        unsigned int  reserved_2                 : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_abb_rx_fifo1_state_clr : 1;  /* bit[12]   : RX 分集FIFO曾经满状态清零信号。 */
        unsigned int  reserved_3                 : 19; /* bit[13-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      share_mem_ctrl_reg;
    struct
    {
        unsigned int  mem_ctrl_s    : 16; /* bit[0-15] :  */
        unsigned int  mem_ctrl_1w2r : 16; /* bit[16-31]: U&TDS融合模块RAM控制低32比特。 */
    } reg;
...
typedef union
{
    unsigned int      share_rom_ctrl_reg;
    struct
    {
        unsigned int  mem_ctrl_1wr2wr : 16; /* bit[0-15] : U&TDS融合模块RAM控制高16比特。 */
        unsigned int  rom_ctrl        : 8;  /* bit[16-23]: U&TDS融合模块ROM控制比特。 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      sys_rst_reg;
    struct
    {
        unsigned int  clk_rst_ctrl  : 3;  /* bit[0-2]  : CLK控制。bit[2]对应上行前端射频控制，bit[1]对应上行前端数据通路，bit[0]对应下行前端。在对前端模块进行时钟控制时，将该bit置1表示打开模块的时钟；反之，将该bit置0表示关闭ULFE模块的时钟。 */
        unsigned int  reserved_0    : 1;  /* bit[3]    : 保留 */
        unsigned int  sys_rst_ctrl  : 2;  /* bit[4-5]  : RST控制。高比特对应上行前端，低比特对应下行前端。在对模块进行复位时，将该bit置1，之后再将该bit复位为0。
                                                         bit[4]下行软复位和天线使能配置约束：复位前先关闭wt_dlfe模块0x91400寄存器bit[1:0]天线使能，然后进行软复位，撤销复位后再打开wt_dlfe模块0x91400寄存器bit[1:0]天线使能。并且天线使能关闭时间要大于2个符号。 */
        unsigned int  reserved_1    : 2;  /* bit[6-7]  : 保留 */
        unsigned int  imi_rst_ctrl  : 2;  /* bit[8-9]  : ULFE模块IMI时钟域RST控制。高比特对应上行前端，低比特对应下行前端。在对模块进行复位时，将该bit置1，之后再将该bit复位为0。 */
        unsigned int  reserved_2    : 2;  /* bit[10-11]: 保留 */
        unsigned int  imi_addr_ctrl : 2;  /* bit[12-13]: IMI ADDR选择使能。高比特对应上行前端，低比特对应下行前端。 */
        unsigned int  reserved_3    : 18; /* bit[14-31]: 保留 */
    } reg;
} WBBP_SYS_RST_UNION;
...
typedef union
{
    ...
    struct
    {
        unsigned int  data_fifo0_rst       : 1;  /* bit[0]   : fifo_0复位信号，在FIFO_0状态异常时，配置为1，进行清零 */
        unsigned int  data_fifo0_full_rpt  : 1;  /* bit[1]   : FIFO_0满信号上报 */
        unsigned int  data_fifo0_empty_rpt : 1;  /* bit[2]   : FIFO_0空信号上报 */
        unsigned int  reserved_0           : 1;  /* bit[3]   : 保留 */
        unsigned int  data_fifo1_rst       : 1;  /* bit[4]   : fifo_1复位信号，在FIFO状态异常时，配置为1，进行清零 */
        unsigned int  data_fifo1_full_rpt  : 1;  /* bit[5]   : FIFO_1满信号上报 */
        unsigned int  data_fifo1_empty_rpt : 1;  /* bit[6]   : FIFO_1空信号上报 */
        unsigned int  reserved_1           : 25; /* bit[7-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : WBBP_CS_W_SDR_EN_UNION
 结构说明  : CS_W_SDR_EN 寄存器结构定义。地址偏移量:0x07C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      cs_w_sdr_en_reg;
    struct
    {
        unsigned int  cs_w_sdr_en : 1;  /* bit[0]   : W软基带使能。 */
        unsigned int  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_CS_W_SDR_EN_UNION;
#define WBBP_CS_W_SDR_EN_cs_w_sdr_en_START  (0)
#define WBBP_CS_W_SDR_EN_cs_w_sdr_en_END    (0)


/*****************************************************************************
 结构名    : WBBP_SHARE_MEM_SDR_SEL_UNION
 结构说明  : SHARE_MEM_SDR_SEL 寄存器结构定义。地址偏移量:0x080，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      share_mem_sdr_sel_reg;
    struct
    {
        unsigned int  share_mem_sdr_sel : 1;  /* bit[0]   : sdr mem模式选择信号
                                                            0：时钟122M
                                                            1：时钟245M */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_SHARE_MEM_SDR_SEL_UNION;
#define WBBP_SHARE_MEM_SDR_SEL_share_mem_sdr_sel_START  (0)
#define WBBP_SHARE_MEM_SDR_SEL_share_mem_sdr_sel_END    (0)


/*****************************************************************************
 结构名    : WBBP_SHARE_MEM_CLK_BYPASS_UNION
 结构说明  : SHARE_MEM_CLK_BYPASS 寄存器结构定义。地址偏移量:0x084，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      share_mem_clk_bypass_reg;
    struct
    {
        unsigned int  share_mem_clk_bypass : 1;  /* bit[0]   : w_tds_mem_share模块的时钟门控bypass信号 */
        unsigned int  reserved             : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_SHARE_MEM_CLK_BYPASS_UNION;
#define WBBP_SHARE_MEM_CLK_BYPASS_share_mem_clk_bypass_START  (0)
#define WBBP_SHARE_MEM_CLK_BYPASS_share_mem_clk_bypass_END    (0)
...
typedef union
{
    unsigned int      apt_comp_h_ram_reg;
    struct
    {
        unsigned int  apt_h_comp_ram_data_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
        unsigned int  reserved_0            : 8;  /* bit[8-15] : 保留 */
        unsigned int  apt_h_comp_ram_data_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
...
typedef union
{
    unsigned int      apt_comp_m_ram_reg;
    struct
    {
        unsigned int  apt_m_comp_ram_data_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
        unsigned int  reserved_0            : 8;  /* bit[8-15] : 保留 */
        unsigned int  apt_m_comp_ram_data_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
        unsigned int  reserved_1            : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_comp_l_ram_reg;
    struct
    {
        unsigned int  apt_l_comp_ram_data_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
        unsigned int  reserved_0            : 8;  /* bit[8-15] : 保留 */
        unsigned int  apt_l_comp_ram_data_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个功率补偿值，高段存comp1[7:0],低段存comp0[7:0]，以此类推。 */
        unsigned int  reserved_1            : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_h_ram_reg;
    struct
    {
        unsigned int  apt_h_word_ram_data_0 : 9;  /* bit[0-8]  : 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_0            : 7;  /* bit[9-15] : 保留 */
        unsigned int  apt_h_word_ram_data_1 : 9;  /* bit[16-24]: 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_1            : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_m_ram_reg;
    struct
    {
        unsigned int  apt_m_word_ram_data_0 : 9;  /* bit[0-8]  : 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_0            : 7;  /* bit[9-15] : 保留 */
        unsigned int  apt_m_word_ram_data_1 : 9;  /* bit[16-24]: 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_1            : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_l_ram_reg;
    struct
    {
        unsigned int  apt_l_word_ram_data_0 : 9;  /* bit[0-8]  : 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_0            : 7;  /* bit[9-15] : 保留 */
        unsigned int  apt_l_word_ram_data_1 : 9;  /* bit[16-24]: 存放APT控制字，每个地址位宽16比特，用于存放两个APT控制字，高段存apt_word1[7:0],低段存apt_word0[7:0]，以此类推（控制字在以前版本7比特基础上高比特加0，扩展成8比特）。 */
        unsigned int  reserved_1            : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias1_h_ram_reg;
    struct
    {
        unsigned int  wpavbias1_h_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias1_h_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias1_m_ram_reg;
    struct
    {
        unsigned int  wpavbias1_m_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias1_m_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias1_l_ram_reg;
    struct
    {
        unsigned int  wpavbias1_l_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias1_l_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。(MMMB(用低7bit)和EAGLE PA共用) */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias2_h_ram_reg;
    struct
    {
        unsigned int  wpavbias2_h_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias2_h_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias2_m_ram_reg;
    struct
    {
        unsigned int  wpavbias2_m_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias2_m_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias2_l_ram_reg;
    struct
    {
        unsigned int  wpavbias2_l_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias2_l_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias3_h_ram_reg;
    struct
    {
        unsigned int  wpavbias3_h_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias3_h_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias3_m_ram_reg;
    struct
    {
        unsigned int  wpavbias3_m_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias3_m_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pavbias3_l_ram_reg;
    struct
    {
        unsigned int  wpavbias3_l_word_0 : 9;  /* bit[0-8]  : 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_0         : 7;  /* bit[9-15] : 保留 */
        unsigned int  wpavbias3_l_word_1 : 9;  /* bit[16-24]: 每个地址位宽18比特，用于存放两个PA VBIAS值。 */
        unsigned int  reserved_1         : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      papowcomp_h_ram_reg;
    struct
    {
        unsigned int  pa_power_comp_h_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_0        : 8;  /* bit[8-15] : 保留 */
        unsigned int  pa_power_comp_h_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_1        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      papowcomp_m_ram_reg;
    struct
    {
        unsigned int  pa_power_comp_m_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_0        : 8;  /* bit[8-15] : 保留 */
        unsigned int  pa_power_comp_m_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_1        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      papowcomp_l_ram_reg;
    struct
    {
        unsigned int  pa_power_comp_l_0 : 8;  /* bit[0-7]  : 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_0        : 8;  /* bit[8-15] : 保留 */
        unsigned int  pa_power_comp_l_1 : 8;  /* bit[16-23]: 每个地址位宽16比特，用于存放两个有符号功率值。 */
        unsigned int  reserved_1        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_atten_h_ram_reg;
    struct
    {
        unsigned int  dbb_atten_h_ram_data : 8;  /* bit[0-7]  : 对应PA高增益,8比特数字功率回退值 */
        unsigned int  rf_atten_h_ram_data  : 16; /* bit[8-23] : 对应PA高增益,16比特RF控制字 */
        unsigned int  temp_comp_index_h    : 4;  /* bit[24-27]: 对应PA中增益,1控制字温补功率索引值(0~13) */
        unsigned int  reserved             : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_atten_m_ram_reg;
    struct
    {
        unsigned int  dbb_atten_m_ram_data : 8;  /* bit[0-7]  : 对应PA中增益,8比特数字功率回退值 */
        unsigned int  rf_atten_m_ram_data  : 16; /* bit[8-23] : 对应PA中增益,16比特RF控制字 */
        unsigned int  temp_comp_index_m    : 4;  /* bit[24-27]: 对应PA中增益,1控制字温补功率索引值(0~13) */
        unsigned int  reserved             : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_atten_l_ram_reg;
    struct
    {
        unsigned int  dbb_atten_l_ram_data : 8;  /* bit[0-7]  : 对应PA低增益,8比特数字功率回退值 */
        unsigned int  rf_atten_l_ram_data  : 16; /* bit[8-23] : 对应PA低增益,16比特RF控制字 */
        unsigned int  temp_comp_index_l    : 4;  /* bit[24-27]: 对应PA低增益,控制字温补功率索引值(0~13) */
        unsigned int  reserved             : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_ram1_reg;
    struct
    {
        unsigned int  dpd_comp_i : 12; /* bit[0-11] : DPD补偿I路系数 */
        unsigned int  dpd_comp_q : 12; /* bit[12-23]: DPD补偿Q路系数 */
        unsigned int  reserved   : 8;  /* bit[24-31]: 保留 */
    } reg;
} WBBP_DPD_RAM1_UNION;
...
typedef union
{
    unsigned int      dsp_pa_model_reg;
    struct
    {
        unsigned int  dsp_pa_mode_ctrl : 2;  /* bit[0-1] : 2'b01，DSP强制PA增益为高；2'b10，DSP强制PA增益为中；2'b11，DSP强制PA增益为低；其他，由BBP自行控制。 */
        unsigned int  reserved         : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_ctrl_word_reg;
    struct
    {
        unsigned int  dsp_pa_high_ctrl : 2;  /* bit[0-1]  : PA高增益控制字，根据射频器件配置。Infineon高增益为2'b00。 */
        unsigned int  reserved_0       : 2;  /* bit[2-3]  : 保留 */
        unsigned int  dsp_pa_mid_ctrl  : 2;  /* bit[4-5]  : PA中增益控制字，根据射频器件配置。Infineon中增益为2'b01。 */
        unsigned int  reserved_1       : 2;  /* bit[6-7]  : 保留 */
        unsigned int  dsp_pa_low_ctrl  : 2;  /* bit[8-9]  : PA低增益控制字，根据射频器件配置。Infineon低增益为2'b11。 */
        unsigned int  reserved_2       : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_pa_init_ctrl : 2;  /* bit[12-13]: PA初始增益控制字，根据射频器件配置。 */
        unsigned int  reserved_3       : 18; /* bit[14-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_pa_gain_reg;
    struct
    {
        unsigned int  dsp_w_pa_high_gain : 9;  /* bit[0-8]  : 当PA处于高增益模式下PA的增益大小，单位0.1dB，无符号数。 */
        unsigned int  dsp_w_pa_mid_gain  : 9;  /* bit[9-17] : 当PA处于中增益模式下PA的增益大小，单位0.1dB，无符号数。 */
        unsigned int  dsp_w_pa_low_gain  : 9;  /* bit[18-26]: 当PA处于低增益模式下PA的增益大小，单位0.1dB，无符号数。 */
...
typedef union
{
    unsigned int      pa_hm_threshold_reg;
    struct
    {
        unsigned int  dsp_pa_h2m_threshold : 11; /* bit[0-10] : PA从高增益到中增益的切换门限，二进制补码。 */
        unsigned int  reserved_0           : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_pa_m2h_threshold : 11; /* bit[16-26]: PA从中增益到高增益的切换门限，二进制补码。 */
        unsigned int  reserved_1           : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_ml_threshold_reg;
    struct
    {
        unsigned int  dsp_pa_m2l_threshold : 11; /* bit[0-10] : PA从中增益到低增益的切换门限，二进制补码。支持PA高、中两档模式时配置成0x400。 */
        unsigned int  reserved_0           : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_pa_l2m_threshold : 11; /* bit[16-26]: PA从低增益到中增益的切换门限，二进制补码。支持PA高、中两档模式时配置成0x400。 */
        unsigned int  reserved_1           : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_hm_mode_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h_mode : 11; /* bit[0-10] : PA高增益表最小地址对应的功率取负，二进制补码。 */
        unsigned int  reserved_0       : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_m_mode : 11; /* bit[16-26]: PA中增益表最小地址对应的功率取负，二进制补码。 */
        unsigned int  reserved_1       : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_l_mode_reg;
    struct
    {
        unsigned int  dsp_nv_pa_l_mode : 11; /* bit[0-10] : PA低增益表最小地址对应的功率取负，二进制补码。 */
        unsigned int  reserved         : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pc_delta_pow_freq_reg;
    struct
    {
        unsigned int  dsp_delta_pow_freq : 10; /* bit[0-9]  : 单板功率频率补偿值，单位0.1dBm。由于单板功率的偏差造成发射功率的偏差。DSP根据当前的单板的频率偏差信息，选择偏差值，配置给BBP；BBP中，与发射功率相加补偿后，查APC表。此值需在对单板进行测试后确定。 */
        unsigned int  reserved           : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_mode_sel_reg;
    struct
    {
        unsigned int  dsp_pa_pwm_sel      : 1;  /* bit[0]   : 0，选择PDM控制输出；1，选择PWM控制输出。 */
        unsigned int  reserved_0          : 3;  /* bit[1-3] : 保留 */
        unsigned int  dsp_apt_ctrl_mode   : 1;  /* bit[4]   : 为了提供满足不同PA的APT控制要求，PA电压控制模式可进行选择。0，PDM配置；1，MIPI口配置。 */
        unsigned int  reserved_1          : 3;  /* bit[5-7] : 保留 */
        unsigned int  dsp_apt_renew_pulse : 1;  /* bit[8]   : apt强制更新脉冲，仅用于强配APT控制字。每次写1产生一个单时钟脉冲，自清零 */
        unsigned int  reserved_2          : 23; /* bit[9-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_apt_active_en_reg;
    struct
    {
        unsigned int  reserved_0        : 16; /* bit[0-15] : 保留 */
        unsigned int  dsp_apt_active_en : 1;  /* bit[16]   : PA APT功能生效开关，高电平有效，该开关有效时，PA APT功能可以进行正常工作，否则PDM脉冲一直为0。 */
...
typedef union
{
    unsigned int      dsp_apt_config_reg;
    struct
    {
        unsigned int  dsp_apt_config_value : 7;  /* bit[0-6] : PA供电电压强制配置值，默认值为0。无符号数。 */
        unsigned int  reserved_0           : 1;  /* bit[7]   : 保留 */
        unsigned int  dsp_apt_config_en    : 1;  /* bit[8]   : PA供电电压强制配置使能，电平信号，该功能有效时，PA电压控制值由dsp_apt_config_value决定。 */
        unsigned int  reserved_1           : 23; /* bit[9-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pacomp_offset_hm_reg;
    struct
    {
        unsigned int  dsp_pacomp_offset_m : 11; /* bit[0-10] : PA为中档位时，用于查PA非线性补偿表的offset值，单位0.1dBm，有符号数。 */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_pacomp_offset_h : 11; /* bit[16-26]: PA为高档位时，用于查PA非线性补偿表的offset值，单位0.1dBm，有符号数。 */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pacomp_offset_l_reg;
    struct
    {
        unsigned int  dsp_pacomp_offset_l : 11; /* bit[0-10] : PA为低档位时，用于查PA非线性补偿表的offset值，单位0.1dBm，有符号数。 */
        unsigned int  reserved            : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_h_comp_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h_comp : 11; /* bit[0-10] : PA工作在高增益模式时的增益补偿值，有符号补码数，单位0.1dB。
                                                            DSP根据当前的频段信息和温度信息，选择偏差值，配置给BBP；BBP将此值与发射功率相加后，查APC表。此值需在不同的工作频段下进行单板测试后确定。 */
        unsigned int  reserved         : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_m_comp_reg;
    struct
    {
        unsigned int  dsp_nv_pa_m_comp : 11; /* bit[0-10] : PA工作在中增益模式时的增益补偿值，有符号补码数，单位0.1dB。
                                                            DSP根据当前的频段信息和温度信息，选择偏差值，配置给BBP；BBP将此值与发射功率相加后，查APC表。此值需在不同的工作频段下进行单板测试后确定。 */
        unsigned int  reserved         : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_l_comp_reg;
    struct
    {
        unsigned int  dsp_nv_pa_l_comp : 11; /* bit[0-10] : PA工作在低增益模式时的增益补偿值，有符号补码数，单位0.1dB。
                                                            DSP根据当前的频段信息和温度信息，选择偏差值，配置给BBP；BBP将此值与发射功率相加后，查APC表。此值需在不同的工作频段下进行单板测试后确定。 */
        unsigned int  reserved         : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot1a_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h2m_comp1 : 11; /* bit[0-10] : PA从高增益到中增益的功率补偿，二进制补码。 */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_m2h_comp1 : 11; /* bit[16-26]: PA从中增益到高增益的功率补偿，二进制补码。 */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot1b_reg;
    struct
    {
        unsigned int  dsp_nv_pa_m2l_comp1 : 11; /* bit[0-10] : PA从中增益到低增益的功率补偿，二进制补码。 */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_l2m_comp1 : 11; /* bit[16-26]: PA从低增益到中增益的功率补偿，二进制补码。 */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot2a_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h2m_comp2 : 11; /* bit[0-10] : PA高切中后第二时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_m2h_comp2 : 11; /* bit[16-26]: PA中切高后第二时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot2b_reg;
    struct
    {
        unsigned int  dsp_nv_pa_m2l_comp2 : 11; /* bit[0-10] : PA中切低后第二时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_l2m_comp2 : 11; /* bit[16-26]: PA低切中后第二时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot3a_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h2m_comp3 : 11; /* bit[0-10] : PA高切中后第三时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_m2h_comp3 : 11; /* bit[16-26]: PA中切高后第三时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot3b_reg;
    struct
    {
        unsigned int  dsp_nv_pa_m2l_comp3 : 11; /* bit[0-10] : PA中切低后第三时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_l2m_comp3 : 11; /* bit[16-26]: PA低切中后第三时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot4a_reg;
    struct
    {
        unsigned int  dsp_nv_pa_h2m_comp4 : 11; /* bit[0-10] : PA高切中后第四时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_m2h_comp4 : 11; /* bit[16-26]: PA中切高后第四时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_comp_slot4b_reg;
    struct
    {
        unsigned int  dsp_nv_pa_m2l_comp4 : 11; /* bit[0-10] : PA中切低后第四时隙功率补偿  */
        unsigned int  reserved_0          : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_nv_pa_l2m_comp4 : 11; /* bit[16-26]: PA低切中后第四时隙功率补偿  */
        unsigned int  reserved_1          : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_w_tx_apc_atten_reg;
    struct
    {
        unsigned int  dsp_w_tx_apc_rf_atten  : 16; /* bit[0-15] : DSP强制射频增益衰减控制字。无符号数。 */
        unsigned int  dsp_w_tx_apc_dbb_atten : 7;  /* bit[16-22]: DSP强制DBB衰减补偿增益。取值范围上下门限软件可控，默认为[20,120]，超过120按120处理。精度0.1dB。
                                                                  20表示数字功率增加1dB，21表示增加0.9dB，以此类推，取值每增加1，功率增加值减少0.1dB，30表示增加0dB，31表示减少0.1dB，……，120表示减少9dB。 */
        unsigned int  reserved               : 8;  /* bit[23-30]: 保留 */
        unsigned int  dsp_w_tx_apc_atten_en  : 1;  /* bit[31]   : DSP强制衰减使能信号。 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_rf_atten_addr_reg;
    struct
    {
        unsigned int  dsp_w_tx_ssi_addr : 8;  /* bit[0-7] : RFCMOS方案射频增益控制字对应SSI地址。 */
        unsigned int  reserved          : 24; /* bit[8-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_dc_offset_en_reg;
    struct
    {
        unsigned int  reserved_0         : 4;  /* bit[0-3] : 保留 */
        unsigned int  dsp_temp_update_en : 1;  /* bit[4]   : 温度补偿更新指示,当软件刷新PA温补,控制字温补,PD温补寄存器时,先将本比特置1,刷新完成后将本比特置0. */
        unsigned int  reserved_1         : 27; /* bit[5-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_rf_report_reg;
    struct
    {
        unsigned int  rf_ctrl_word  : 16; /* bit[0-15] : RF控制字上报，无符号数 */
        unsigned int  dbb_atten_pow : 7;  /* bit[16-22]: RFCMOS数字回退功率上报，单位0.1dB.取值范围上下门限软件可控，默认为[20,120],对应回退功率范围[增加1dB,减少9dB] */
        unsigned int  w_pa_mode     : 2;  /* bit[23-24]: PA控制字上报，无符号数 */
        unsigned int  reserved      : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pa_vbias_config_reg;
    struct
    {
        unsigned int  dsp_wpavbias_config_value  : 8;  /* bit[0-7]  : PA VBIAS控制字强配值 */
        unsigned int  dsp_wpavbias_config_en     : 1;  /* bit[8]    : PA VBIAS控制字强配使能 */
        unsigned int  reserved_0                 : 7;  /* bit[9-15] : 保留 */
        unsigned int  dsp_temp_comp_config_value : 7;  /* bit[16-22]: ATTEN控制字温度补偿功率强配值,有符号数,单位0.1dB */
        unsigned int  reserved_1                 : 1;  /* bit[23]   : 保留 */
        unsigned int  dsp_temp_comp_config_en    : 1;  /* bit[24]   : ATTEN控制字温度补偿功率强配使能. */
        unsigned int  reserved_2                 : 7;  /* bit[25-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_pa_report_reg;
    struct
    {
        unsigned int  wpavbias_word   : 8;  /* bit[0-7]  : PA VBIAS控制字上报,无符号数. */
        unsigned int  temp_comp_power : 7;  /* bit[8-14] : RF控制字温度补偿上报,有符号数,单位0.1dB */
        unsigned int  reserved        : 17; /* bit[15-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      temp_comp0_3_reg;
    struct
    {
        unsigned int  temp_comp_0power : 7;  /* bit[0-6]  : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_0       : 1;  /* bit[7]    : 保留 */
        unsigned int  temp_comp_1power : 7;  /* bit[8-14] : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_1       : 1;  /* bit[15]   : 保留 */
        unsigned int  temp_comp_2power : 7;  /* bit[16-22]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_2       : 1;  /* bit[23]   : 保留 */
        unsigned int  temp_comp_3power : 7;  /* bit[24-30]: RF控制字温度补偿功率,有符号数,0.1dB */
...
typedef union
{
    unsigned int      temp_comp4_7_reg;
    struct
    {
        unsigned int  temp_comp_4power : 7;  /* bit[0-6]  : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_0       : 1;  /* bit[7]    : 保留 */
        unsigned int  temp_comp_5power : 7;  /* bit[8-14] : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_1       : 1;  /* bit[15]   : 保留 */
        unsigned int  temp_comp_6power : 7;  /* bit[16-22]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_2       : 1;  /* bit[23]   : 保留 */
        unsigned int  temp_comp_7power : 7;  /* bit[24-30]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_3       : 1;  /* bit[31]   : 保留 */
    } reg;
...
typedef union
{
    unsigned int      temp_comp8_11_reg;
    struct
    {
        unsigned int  temp_comp_8power  : 7;  /* bit[0-6]  : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_0        : 1;  /* bit[7]    : 保留 */
        unsigned int  temp_comp_9power  : 7;  /* bit[8-14] : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_1        : 1;  /* bit[15]   : 保留 */
        unsigned int  temp_comp_10power : 7;  /* bit[16-22]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_2        : 1;  /* bit[23]   : 保留 */
        unsigned int  temp_comp_11power : 7;  /* bit[24-30]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_3        : 1;  /* bit[31]   : 保留 */
    } reg;
...
typedef union
{
    unsigned int      temp_comp12_15_reg;
    struct
    {
        unsigned int  temp_comp_12power : 7;  /* bit[0-6]  : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_0        : 1;  /* bit[7]    : 保留 */
        unsigned int  temp_comp_13power : 7;  /* bit[8-14] : RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_1        : 1;  /* bit[15]   : 保留 */
        unsigned int  temp_comp_14power : 7;  /* bit[16-22]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_2        : 1;  /* bit[23]   : 保留 */
        unsigned int  temp_comp_15power : 7;  /* bit[24-30]: RF控制字温度补偿功率,有符号数,0.1dB */
        unsigned int  reserved_3        : 1;  /* bit[31]   : 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_dmpr_config_reg;
    struct
    {
        unsigned int  dsp_dmpr_config_value : 6;  /* bit[0-5] : 软件强配dmpr功率值,有符号数,单位 0.1dB. */
        unsigned int  reserved_0            : 2;  /* bit[6-7] : 保留 */
        unsigned int  dsp_dmpr_config_en    : 1;  /* bit[8]   : 软件强配dmpr功率使能 */
        unsigned int  reserved_1            : 23; /* bit[9-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_shdn_ctrl_reg;
    struct
    {
        unsigned int  tx_shdn_ctrl : 2;  /* bit[0-1] : 上行通路开关控制。2'b10，表示DSP强制上行通路关闭；2'b11，表示DSP强制上行通路打开；其他，表示上行通路不受DSP控制，而由上行调制模块控制。
                                                       此寄存器同时控制上行PA、RF等的开关。也即如果配置为2'b10，表示上行RF、PA等同时关闭。 */
        unsigned int  reserved     : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_rf_shdn_ctrl_reg;
    struct
    {
        unsigned int  rf_tx_shdn_ctrl : 2;  /* bit[0-1] : 上行RF开关控制。2'b10，表示DSP强制上行RF关闭；2'b11，表示DSP强制上行RF打开；其他，表示上行RF不受DSP控制，而由上行调制模块控制。
                                                          此寄存器仅控制上行RF的开关。同时，仅在TX_SHDN_CTRL被配置为非DSP强制控制的情况下上行RF才受此寄存器的控制。 */
        unsigned int  reserved        : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_pa_shdn_ctrl_reg;
    struct
    {
        unsigned int  pa_shdn_ctrl : 2;  /* bit[0-1] : 上行PA开关控制。2'b10，表示DSP强制上行PA关闭；2'b11，表示DSP强制上行PA打开；其他，表示上行PA不受DSP控制，而由上行调制模块控制。
                                                       此寄存器仅控制上行PA的开关。同时，仅在TX_SHDN_CTRL被配置为非DSP强制控制的情况下上行PA才受此寄存器的控制。 */
        unsigned int  reserved     : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      abb_shdn_ctrl_reg;
    struct
    {
        unsigned int  abb_tx_shdn_ctrl : 2;  /* bit[0-1] : 上行ABB开关控制。2'b10，表示DSP强制上行ABB关闭；2'b11，表示DSP强制上行ABB打开；其他，表示上行ABB不受DSP控制，而由上行调制模块控制。
                                                           此寄存器仅控制上行ABB的开关。同时，仅在TX_SHDN_CTRL被配置为非DSP强制控制的情况下上行ABB才受此寄存器的控制。 */
        unsigned int  reserved         : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_et_timing_reg;
    struct
    {
        unsigned int  dsp_apt_et_active_time : 10; /* bit[0-9]  : D态下，APT_ET使能信号生效提前时隙头时间量，单位chip。 */
        unsigned int  reserved_0             : 2;  /* bit[10-11]: 保留 */
        unsigned int  et_shdn_ctrl           : 2;  /* bit[12-13]: 2'd10:ET强关，2'd11:ET强开，其它:ET由其它控制。 */
        unsigned int  reserved_1             : 2;  /* bit[14-15]: 保留 */
        unsigned int  dsp_tx_pow_time        : 10; /* bit[16-25]: APT_ET切换点提前时隙头时间量，单位chip。 */
        unsigned int  reserved_2             : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_rf_time_ctrl_reg;
    struct
    {
        unsigned int  tx_rf_open_time : 12; /* bit[0-11] : 提前打开上行RF的chip数。取值范围0~1023。典型值800us等于3072chip。 */
        unsigned int  reserved_0      : 4;  /* bit[12-15]: 保留 */
        unsigned int  tx_rf_idle_time : 12; /* bit[16-27]: 提前打开上行RF IDLE状态的chip数。取值范围0~1023。典型值100us等于384chip。 */
        unsigned int  reserved_1      : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_rf_close_time_ctrl_reg;
    struct
    {
        unsigned int  tx_rf_close_time      : 10; /* bit[0-9]  : 延后关闭上行RF的chip数。 */
        unsigned int  reserved_0            : 6;  /* bit[10-15]: 保留 */
        unsigned int  tx_rf_idle_close_time : 10; /* bit[16-25]: 延后关闭上行RF IDLE状态的chip数。 */
        unsigned int  reserved_1            : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_pa_time_ctrl_reg;
    struct
    {
        unsigned int  tx_pa_open_time  : 11; /* bit[0-10] : 提前打开上行PA的chip数。取值范围0~1023。典型值40us等于154chip。 */
        unsigned int  reserved_0       : 5;  /* bit[11-15]: 保留 */
        unsigned int  tx_pa_close_time : 10; /* bit[16-25]: 延后关闭上行PA的chip数。 */
        unsigned int  reserved_1       : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_pdm_time_ctrl_reg;
    struct
    {
        unsigned int  tx_pdm_open_time  : 11; /* bit[0-10] : 提前打开上行RF增益的chip数。取值范围0~1023。 */
        unsigned int  reserved_0        : 5;  /* bit[11-15]: 保留 */
        unsigned int  tx_pdm_close_time : 10; /* bit[16-25]: 延后关闭上行RF增益的chip数。 */
        unsigned int  reserved_1        : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_abb_time_ctrl_reg;
    struct
    {
        unsigned int  tx_abb_open_time  : 12; /* bit[0-11] : 提前打开上行ABB的chip数。取值范围0~1023。 */
        unsigned int  reserved_0        : 4;  /* bit[12-15]: 保留 */
        unsigned int  tx_abb_close_time : 10; /* bit[16-25]: 延后关闭上行ABB的chip数。 */
        unsigned int  reserved_1        : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ulpower_active_time_reg;
    struct
    {
        unsigned int  ulpower_active_time : 10; /* bit[0-9]  : 上行发射功率生效时间，单位chip。表示上行发射功率需提前上行时隙头多久时间送至射频器件。由于射频器件有延时，为使上行发射功率在上行时隙头处生效，需提前将上行发射功率计算结果送至射频器件。 */
        unsigned int  reserved            : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_phase_comp_time_reg;
    struct
    {
        unsigned int  dsp_w_tx_phase_comp_time : 10; /* bit[0-9]  : 相位补偿生效时刻相对于时隙边界提前量，单位1/4chip。 */
        unsigned int  reserved                 : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcoffset_active_time_reg;
    struct
    {
        unsigned int  dsp_w_dcoffset_active_time : 10; /* bit[0-9]  : 直流补偿生效时刻提前量  */
        unsigned int  reserved                   : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_vbias_active_time_reg;
    struct
    {
        unsigned int  dsp_vbias_active_time : 10; /* bit[0-9]  : APT电压生效时刻，默认值为时隙边界提前40chip。 */
        unsigned int  reserved              : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_active_time_hm_reg;
    struct
    {
        unsigned int  cpu_pa_h2m_active_time : 10; /* bit[0-9]  : PA从高增益向中增益切换时提前上行时隙头的时间，单位chip。 */
        unsigned int  reserved_0             : 6;  /* bit[10-15]: 保留 */
        unsigned int  cpu_pa_m2h_active_time : 10; /* bit[16-25]: PA从中增益向高增益切换时提前上行时隙头的时间，单位chip。 */
        unsigned int  reserved_1             : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_active_time_ml_reg;
    struct
    {
        unsigned int  cpu_pa_m2l_active_time : 10; /* bit[0-9]  : PA从中增益向低增益切换时提前上行时隙头的时间，单位chip。 */
        unsigned int  reserved_0             : 6;  /* bit[10-15]: 保留 */
        unsigned int  cpu_pa_l2m_active_time : 10; /* bit[16-25]: PA从低增益向中增益切换时提前上行时隙头的时间，单位chip。 */
        unsigned int  reserved_1             : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_mh_vbias_active_time_reg;
    struct
    {
        unsigned int  pa_h2m_vbias_active_time : 10; /* bit[0-9]  : PA在高切中档位时，新VBIAS控制字生效时间相对于时隙头的提前量。
                                                                    该参数无效，统一用wpavbias_active_time。 */
        unsigned int  reserved_0               : 6;  /* bit[10-15]: 保留 */
        unsigned int  pa_m2h_vbias_active_time : 10; /* bit[16-25]: PA在中切高档位时，新VBIAS控制字生效时间相对于时隙头的提前量。
                                                                    该参数无效，统一用wpavbias_active_time。 */
        unsigned int  reserved_1               : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_lm_vbias_active_time_reg;
    struct
    {
        unsigned int  pa_m2l_vbias_active_time : 10; /* bit[0-9]  : PA在中切低档位时，新VBIAS控制字生效时间相对于时隙头的提前量。
                                                                    该参数无效，统一用wpavbias_active_time。 */
        unsigned int  reserved_0               : 6;  /* bit[10-15]: 保留 */
        unsigned int  pa_l2m_vbias_active_time : 10; /* bit[16-25]: PA在低切中档位时，新VBIAS控制字生效时间相对于时隙头的提前量。
                                                                    该参数无效，统一用wpavbias_active_time。 */
        unsigned int  reserved_1               : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fe_vbias_active_time_reg;
    struct
    {
        unsigned int  wrfvbias_active_time : 10; /* bit[0-9]  : RFVBIAS提前上行时隙头生效时刻，单位为chip。(删除) */
        unsigned int  reserved_0           : 6;  /* bit[10-15]: 保留 */
        unsigned int  wpavbias_active_time : 10; /* bit[16-25]: PAVBIAS提前上行时隙头生效时刻，单位为chip。在MMMB PA和EAGLE PA(第一个控制字)都可使用。 */
        unsigned int  reserved_1           : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rrc_delay_chip_num_reg;
    struct
    {
        unsigned int  rrc_delay_chip_num : 6;  /* bit[0-5] : 上行调制RRC延迟时间，单位chip。 */
        unsigned int  reserved           : 26; /* bit[6-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      cpu_tx_swc_reg;
    struct
    {
        unsigned int  cpu_tx_swc_active_time : 10; /* bit[0-9]  : 天线调谐配置提前帧头配置量，当上行打开后，WBBP每次在帧头提前cpu_tx_sw_active_time进行配置，单位为chip，默认值为0。 */
        unsigned int  reserved_0             : 6;  /* bit[10-15]: 保留 */
        unsigned int  cpu_tx_swc_cfg_flag    : 1;  /* bit[16]   : 天线调谐配置更新标志，脉冲信号。 */
        unsigned int  reserved_1             : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wpa_vbias_active_time2_3_reg;
    struct
    {
        unsigned int  wpavbias2_active_time : 10; /* bit[0-9]  : EAGLE PA 第二个PAVBIAS控制字提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_0            : 6;  /* bit[10-15]: 保留 */
        unsigned int  wpavbias3_active_time : 10; /* bit[16-25]: EAGLE PA 第三个PAVBIAS控制字提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_1            : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      wpa_vbias_active_time4_reg;
    struct
    {
        unsigned int  wpavbias4_active_time       : 10; /* bit[0-9]  : EAGLE PA 第四个PAVBIAS控制字(trigger拉高)提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_0                  : 6;  /* bit[10-15]: 保留 */
        unsigned int  wpavbias4_active_again_time : 10; /* bit[16-25]: EAGLE PA 第四个PAVBIAS控制字(trigger生效)提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_1                  : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dbb_atten_active_time_reg;
    struct
    {
        unsigned int  dbb_active_time          : 10; /* bit[0-9]  : 数字增益回退提前上行时隙头生效时刻。单位1/4chip。 */
        unsigned int  reserved_0               : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_dbb_powcomp_flag_en  : 1;  /* bit[12]   : 上下时隙中间HSDPA功率补偿使能，高电平表示在HSDPCCH时隙头位置dbb做补偿。 */
        unsigned int  reserved_1               : 3;  /* bit[13-15]: 保留 */
        unsigned int  dbb_halfslot_active_time : 14; /* bit[16-29]: hsdpa信道数字增益回退在上行时隙头后生效时刻。单位1/4chip。 */
...
typedef union
{
    unsigned int      tx_lowpow_active_time1_2_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow1_active_time : 10; /* bit[0-9]  : 上行低功耗控制字1提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_0                 : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_tx_lowpow2_active_time : 10; /* bit[16-25]: 上行低功耗控制字2提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_1                 : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_lowpow_active_time3_4_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow3_active_time : 10; /* bit[0-9]  : 上行低功耗控制字3提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_0                 : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_tx_lowpow4_active_time : 10; /* bit[16-25]: 上行低功耗控制字4提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_1                 : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_spi_2abb_reg;
    struct
    {
        unsigned int  dsp_spi_2abb_word : 16; /* bit[0-15] : DSP直接使用SPI口对HKADC进行读写时的指令寄存器值。高8位为指令部分，低8位为数据部分，BBP从最高位开始发送。 */
        unsigned int  reserved          : 14; /* bit[16-29]: 保留 */
        unsigned int  dsp_spi_2abb_ren  : 1;  /* bit[30]   : 读写标志。0，表示写；1，表示读。 */
        unsigned int  dsp_spi_2abb_en   : 1;  /* bit[31]   : DSP直接使用SPI口的标志信号，高有效。 */
    } reg;
...
typedef union
{
    unsigned int      dsp_pow_det_reg;
    struct
    {
        unsigned int  dsp_pow_det : 1;  /* bit[0]   : 强制功率检测使能。1，表示一次功率检测；0，无效。BBP在功率检测结果输出后进行自清零，软件通过查询此bit可确定功率检测结果是否有效。 */
        unsigned int  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_start_2abb_reg;
    struct
    {
        unsigned int  pow_det_start_2abb          : 16; /* bit[0-15] : 1）当使用RFIC-PD方案时，表示：控制海思RF的功率检测器件和AD采样的打开门限。当发射功率超过此门限时，打开RF的功率检测器件与AD。
                                                                       说明：V3只使用bit[9:0]；此门限必须小于pow_det_open_threshold，且为正数；
                                                                       2）当使用HKADC-PD方案时，表示：通知ABB进行HKADC采样的指令寄存器值，高8位为指令部分，低8位为数据部分，BBP从最高位开始发送。 */
        unsigned int  dsp_pd_bp_to_start_interval : 10; /* bit[16-25]: 配置ABB从bypass为低到启动start （ABB开始转换）的时间间隔，单位为chip。 */
        unsigned int  reserved_0                  : 2;  /* bit[26-27]: 保留 */
        unsigned int  dsp_hkadc_ssi_clk_cfg       : 3;  /* bit[28-30]: HKADC SSI Master释放单线控制权到Master开始检查Slave返回的同步位的时间配置值。 
                                                                       延迟=1.5+0.5*Step_num(clk).
                                                                       Step_num=0~7 */
...
typedef union
{
    unsigned int      pow_det_read1_2abb_reg;
    struct
    {
        unsigned int  pow_det_read1_2abb       : 16; /* bit[0-15] : 1）当使用RFIC-PD方案时，表示：控制海思RF的功率检测器件和AD采样的的关闭门限。当发射功率小于此门限时，关闭RF的功率检测器件与AD。
                                                                    说明：V3只使用bit[10:0]；此门限必须小于pow_det_close_threshold；
                                                                    2）当使用HKADC-PD方案时，表示：读取ABB中HKADC采样结果第一个寄存器的指令寄存器值，高8位为指令部分，低8位为数据部分，BBP从最高位开始发送。 */
        unsigned int  dsp_abb_pd_bypass_l_word : 16; /* bit[16-31]: 配置打开HKADC PD通道的指令寄存器值。BBP从最高位开始发。 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_read2_2abb_reg;
    struct
    {
        unsigned int  pow_det_read2_2abb       : 16; /* bit[0-15] : 读取ABB中HKADC采样结果第二个寄存器的指令寄存器值。高8位为指令部分，低8位为数据部分，BBP从最高位开始发送。
                                                                    软件根据实际需要配置，BBP将此信息通过SPI口配置给ABB后，ABB通过此SPI口将结果送给BBP。
                                                                    说明：Hi6411V110，此寄存器值需配置16'hce00。 */
        unsigned int  dsp_abb_pd_bypass_h_word : 16; /* bit[16-31]: 配置关闭HKADC PD通道的指令寄存器值。BBP从最高位开始发。 */
    } reg;
...
typedef union
{
    unsigned int      sample_time_abb_reg;
    struct
    {
        unsigned int  sample_time_abb         : 10; /* bit[0-9]  : HKADC采样信号处理时间，单位chip。具体表示意义请参考《Hi6750V100 芯片WBBP子系统与CPU接口说明书.doc》之16.2.1。 */
        unsigned int  reserved_0              : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_pd_sam_num          : 2;  /* bit[12-13]: 配置W上行每个时隙进行PD检测的次数。
                                                                   2'd0：4次；
                                                                   2'd1：8次；
                                                                   2'd2：16次；
                                                                   2'd3：2次 */
        unsigned int  dsp_pd_dcoffset_sam_num : 2;  /* bit[14-15]: 配置WHKADC上行每个时隙进行dc offset检测的次数。
                                                                   2'd0：4次；
                                                                   2'd1：8次；
                                                                   2'd2：16次；
                                                                   2'd3：2次 */
        unsigned int  dsp_hkadc_chsel_time    : 4;  /* bit[16-19]: 授权有效到发送HKADC通道控制字时刻间隔,单位chip */
        unsigned int  reserved_1              : 10; /* bit[20-29]: 保留 */
        unsigned int  dsp_hkadc_sm_clr_en     : 1;  /* bit[30]   : hkadc状态机复位信号，电平信号。
                                                                   1：表示复位hkadc状态机；0表示正常工作。 */
        unsigned int  reserved_2              : 1;  /* bit[31]   : 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_result_reg;
    struct
    {
        unsigned int  pow_det_result_2dsp : 10; /* bit[0-9]  : 功率检测结果。BBP将每次功率检测结果上报软件。所有从ABB读出的结果，通过此寄存器上报。发射功率越高，上报结果越大，呈线性规律。 */
        unsigned int  reserved_0          : 2;  /* bit[10-11]: 保留 */
        unsigned int  pd_ssi_rpt_cpu      : 12; /* bit[12-23]: 上报HKADC_PD进行SSI传输的次数。定位用。 */
        unsigned int  reserved_1          : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pmax_limit_en_reg;
    struct
    {
        unsigned int  pmax_limit_en  : 1;  /* bit[0]    : 最大功率限制使能。1，表示有效；0，表示无效。 */
        unsigned int  reserved_0     : 3;  /* bit[1-3]  : 保留 */
        unsigned int  ad_width_sel   : 1;  /* bit[4]    : AD位宽选择。1，表示8bit；0，表示10bit。 */
        unsigned int  reserved_1     : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_rfic_pd_en : 2;  /* bit[8-9]  : 00:无效值。
                                                          01:6360 RFIC模式，不仲裁，不去直流。
                                                          10:MMMB PA模式，仲裁，不去直流。
                                                          11:6361 RFIC模式，仲裁，去直流。 */
        unsigned int  reserved_2     : 22; /* bit[10-31]: 保留 */
...
typedef union
{
    unsigned int      rf_pa_stable_time_reg;
    struct
    {
        unsigned int  rf_pa_stable_time : 12; /* bit[0-11] : 发射功率变化后，RF与PA的稳定时间，单位chip。具体表示意义请参考《Hi6750V100 芯片WBBP子系统与CPU接口说明书.doc》之16.2.1。 */
        unsigned int  reserved          : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_freq_comp_reg;
    struct
    {
        unsigned int  pow_det_freq_comp : 11; /* bit[0-10] : 功率检测时的频率补偿值，单位0.1dBm。软件根据当前的频点将相应的NV项配置到BBP。 */
        unsigned int  reserved          : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_temp_comp_reg;
    struct
    {
        unsigned int  pow_det_temp_comp : 11; /* bit[0-10] : 功率检测时的温度补偿值，单位0.1dBm。软件根据当前的温度将相应的NV项配置到BBP。 */
        unsigned int  reserved          : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_interval_reg;
    struct
    {
        unsigned int  pow_det_interval : 10; /* bit[0-9]  : 两次功率检测之间的时间间隔，单位chip。 */
        unsigned int  reserved         : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pow_det_threshold_reg;
    struct
    {
        unsigned int  pow_det_open_threshold  : 10; /* bit[0-9]  : 功率检测打开门限。当上行理论发射功率超过此门限时，BBP打开功率检测功能。范围0~1023，单位0.1dBm。
                                                                   说明：根据功率检测总体方案，功率检测范围为10~27dBm，即此寄存器默认值为100。 */
        unsigned int  reserved_0              : 6;  /* bit[10-15]: 保留 */
        unsigned int  pow_det_close_threshold : 10; /* bit[16-25]: 功率检测关闭门限。当上行理论发射功率低于此门限时，BBP关闭功率检测功能。范围0~1023，单位0.1dBm。
                                                                   注意：pow_det_close_threshold必须小于pow_det_open_threshold，否则，功率检测功能无法打开。 */
        unsigned int  reserved_1              : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      act_nom_error_reg;
    struct
    {
        unsigned int  act_nom_error      : 11; /* bit[0-10] : 实际发射功率-理论发射功率，单位0.1dBm。 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  act_nom_error_aver : 11; /* bit[16-26]: act_nom_error一帧内的平均值。 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcoffset_pow_line_value_reg;
    struct
    {
        unsigned int  pd_dcoffset_result : 10; /* bit[0-9]  : HKADC每时隙直流测量平均结果上报。查表前的线性值，无符号数。 */
        unsigned int  reserved_0         : 6;  /* bit[10-15]: 保留 */
        unsigned int  pow_mea_result     : 10; /* bit[16-25]: 每时隙功率测量平均结果上报。查表前的线性值，无符号数。 */
        unsigned int  reserved_1         : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_config_pd_dcoffset_reg;
    struct
    {
        unsigned int  dsp_pd_dcoffset_config_value : 10; /* bit[0-9]  : 软件强配HKADC直流,无符号数 */
        unsigned int  dsp_pd_dcoffset_config_en    : 1;  /* bit[10]   : 软件强配HKADC直流使能,高电平有效 */
        unsigned int  pow_det_error_hold_clr_sel   : 1;  /* bit[11]   : PD关闭时,PD检测误差,1:保持   0:清零 */
        unsigned int  dsp_hkadc_powdet_mode        : 1;  /* bit[12]   : HKADC非信令下,软件读取PD模式:
                                                                        0 表示采样一次dcoffset上报.
                                                                        1 表示一个时隙内先采样平均得到dcoffset然后采样平均得到功率检测线性值,最后上报值=功率检测线性值-dcoffset; */
        unsigned int  reserved                     : 19; /* bit[13-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pd_test_ctrl_reg;
    struct
    {
        unsigned int  pd_test_mode               : 1;  /* bit[0]   : PD测试模式选择。1，表示测试模式；0，表示正常模式。 */
        unsigned int  reserved_0                 : 3;  /* bit[1-3] : 保留 */
        unsigned int  pd_test_pattern_r5_negtive : 1;  /* bit[4]   : 测试模式下，存在R5信道的上行R99时隙是否进行功率检测。
                                                                     bit[4]：1，表示存在R5信道的上行R99时隙不进行功率检测；0，表示存在R5信道的上行R99时隙进行功率检测。（根据总体方案，只有R5信道b值不变的时隙才进行功率检测。） */
        unsigned int  pd_test_pattern_r6         : 1;  /* bit[5]   : 测试模式下，存在R6信道的上行R99时隙是否进行功率检测。bit[5]：1，表示存在R6信道的上行R99时隙不进行功率检测；0，表示存在R6信道的上行R99时隙进行功率检测。 */
        unsigned int  pd_test_pattern_r5         : 1;  /* bit[6]   : 测试模式下，存在R5信道的上行R99时隙是否进行功率检测。bit[6]：1，表示只有存在R5信道的上行R99时隙进行功率检测。 */
        unsigned int  reserved_1                 : 25; /* bit[7-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pmax_hold_en_pd_connect_ctrl_word_reg;
    struct
    {
        unsigned int  pmax_hold_en         : 1;  /* bit[0]    : 功率检测保持使能信号，用于在功率检测期间需要温度检测的情况。在需要进行温度检测时，软件将此寄存器配置为1，之后，将HKADC通道配置为温度检测，然后软件完成温度检测。温度检测完成后，软件将HKADC通道配置为功率检测后，再将此寄存器配置为0。 */
        unsigned int  reserved_0           : 2;  /* bit[1-2]  : 保留 */
        unsigned int  dsp_hkadc_pdclr_en   : 1;  /* bit[3]    : 0:表示不输出PD清零控制字给PD芯片(hi6361方案).
                                                                1:表示输出PD清零控制字给PD芯片(hi6360方案). */
        unsigned int  pd_connect_ctrl_word : 24; /* bit[4-27] : HKADC powerdetct连接上行通道的控制字. */
        unsigned int  reserved_1           : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pd_disconnect_ctrl_word_reg;
    struct
    {
        unsigned int  pd_disconnect_ctrl_word : 24; /* bit[0-23] : HKADC powerdetct不连接上行通道的控制字. */
        unsigned int  reserved                : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_tx_swd_cfg_reg;
    struct
    {
        unsigned int  dsp_tx_swd_en          : 1;  /* bit[0]    : 驻波检测启动使能，高电平有效，该信号有效时，驻波检测功能打开。 */
        unsigned int  dsp_tx_swd_slot        : 4;  /* bit[1-4]  : 驻波检测启动时隙号，默认值为6。 */
        unsigned int  dsp_tx_swd_stable_time : 10; /* bit[5-14] : 前反向通道切换等待延时，单位为chip，默认值为80。 */
        unsigned int  dsp_swd_valid_clr      : 1;  /* bit[15]   : 驻波检测有效清零信号。 */
        unsigned int  reserved_0             : 3;  /* bit[16-18]: 保留 */
        unsigned int  dsp_rv_pd_sam_num      : 2;  /* bit[19-20]: 驻波检测时隙前后向功率检测次数，2bits无符号数。默认值为0时，表示检测次数为4次；其他值暂时不用，用于扩展。 */
        unsigned int  reserved_1             : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_tx_swd_fw_ssi_cfg_reg;
    struct
    {
        unsigned int  dsp_tx_swd_fw_ssi_cfg : 24; /* bit[0-23] : 前向ssi控制字 */
        unsigned int  reserved              : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_tx_swd_rv_ssi_cfg_reg;
    struct
    {
        unsigned int  dsp_tx_swd_rv_ssi_cfg : 24; /* bit[0-23] : 反向ssi控制字 */
        unsigned int  reserved              : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_tx_swd_rpt_reg;
    struct
    {
        unsigned int  dsp_fw_vlt_value   : 10; /* bit[0-9]  : 前向电压上报值，10bits，无符号数 */
        unsigned int  reserved_0         : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_rv_vlt_value   : 10; /* bit[12-21]: 反向电压上报值，10bits，无符号数 */
        unsigned int  reserved_1         : 2;  /* bit[22-23]: 保留 */
        unsigned int  dsp_pd_comp_value  : 6;  /* bit[24-29]: PD检测功率补偿值，有符号数 */
        unsigned int  dsp_swd_valid_flag : 2;  /* bit[30-31]: 反向功率检测结果有效标志，电平信号。低比特为测试位，无需关注。2’b1x表示反向检测功率有效；2’b0x表示反向检测功率无效。dsp_swd_valid_clr为高脉冲时， dsp_swd_valid_flag清零 */
    } reg;
...
typedef union
{
    unsigned int      mipi_apt_addr_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_apt_addr      : 16; /* bit[0-15] : mipi apt控制字地址，高3比特位指令属性(默认为写)，低13比特位命令帧(地址)。 */
        unsigned int  dsp_tx_mipi_pavbias_addr1 : 16; /* bit[16-31]: mipi pavbias控制字1地址，高3比特位指令属性(默认为写)，低13比特位命令帧(地址)。 */
    } reg;
...
typedef union
{
    unsigned int      mipi_pavbias_addr_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias_addr2 : 16; /* bit[0-15] : mipi pavbias控制字2地址，高3比特位指令属性(默认为写)，低13比特位命令帧(地址)。 */
        unsigned int  dsp_tx_mipi_pavbias_addr3 : 16; /* bit[16-31]: mipi pavbias控制字3地址，高3比特位指令属性(默认为写)，低13比特位命令帧(地址)。 */
    } reg;
...
typedef union
{
    unsigned int      mipi_pavbias_trigger1_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias_trig_word1 : 25; /* bit[0-24] : mipi pavbias trigger控制字1，高3比特位指令属性(默认为写)，中间13比特位命令帧(地址)，低9比特为数据。 */
        unsigned int  reserved_0                     : 3;  /* bit[25-27]: 保留 */
        unsigned int  dsp_tx_mipi_pavbias_trig_en1   : 1;  /* bit[28]   : mipi pavbias trigger控制字1发送使能，高电平有效。 */
        unsigned int  reserved_1                     : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mipi_pavbias_trigger2_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias_trig_word2 : 25; /* bit[0-24] : mipi pavbias trigger控制字2，高3比特位指令属性(默认为写)，中间13比特位命令帧(地址)，低9比特为数据。 */
        unsigned int  reserved_0                     : 3;  /* bit[25-27]: 保留 */
        unsigned int  dsp_tx_mipi_pavbias_trig_en2   : 1;  /* bit[28]   : mipi pavbias trigger控制字2发送使能，高电平有效。 */
        unsigned int  reserved_1                     : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word1_h_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word1_h : 24; /* bit[0-23] : 上行低功耗控制字1(高增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word1_m_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word1_m : 24; /* bit[0-23] : 上行低功耗控制字1(中增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word1_l_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word1_l : 24; /* bit[0-23] : 上行低功耗控制字1(低增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word2_h_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word2_h : 24; /* bit[0-23] : 上行低功耗控制字2(高增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word2_m_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word2_m : 24; /* bit[0-23] : 上行低功耗控制字2(中增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word2_l_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word2_l : 24; /* bit[0-23] : 上行低功耗控制字2(低增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word3_h_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word3_h : 24; /* bit[0-23] : 上行低功耗控制字3(高增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word3_m_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word3_m : 24; /* bit[0-23] : 上行低功耗控制字3(中增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word3_l_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word3_l : 24; /* bit[0-23] : 上行低功耗控制字3(低增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word4_h_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word4_h : 24; /* bit[0-23] : 上行低功耗控制字4(高增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word4_m_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word4_m : 24; /* bit[0-23] : 上行低功耗控制字4(中增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      lowpow_ssi_word4_l_reg;
    struct
    {
        unsigned int  dsp_tx_lowpow_ssi_word4_l : 24; /* bit[0-23] : 上行低功耗控制字4(低增益)。 */
        unsigned int  reserved                  : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mipi_pavbias1_2_cfg_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias1_config_value : 9;  /* bit[0-8]  : EAGLE PA第一个VBIAS控制字强配值。 */
        unsigned int  reserved_0                        : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_tx_mipi_pavbias1_config_en    : 1;  /* bit[12]   : EAGLE PA第一个VBIAS控制字强配使能。 */
        unsigned int  reserved_1                        : 3;  /* bit[13-15]: 保留 */
        unsigned int  dsp_tx_mipi_pavbias2_config_value : 9;  /* bit[16-24]: EAGLE PA第二个VBIAS控制字强配值。 */
        unsigned int  reserved_2                        : 3;  /* bit[25-27]: 保留 */
        unsigned int  dsp_tx_mipi_pavbias2_config_en    : 1;  /* bit[28]   : EAGLE PA第一个VBIAS控制字强配使能。 */
        unsigned int  reserved_3                        : 3;  /* bit[29-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mipi_pavbias3_cfg_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias3_config_value : 9;  /* bit[0-8]  : EAGLE PA第三个VBIAS控制字强配值。 */
        unsigned int  reserved_0                        : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_tx_mipi_pavbias3_config_en    : 1;  /* bit[12]   : EAGLE PA第一个VBIAS控制字强配使能。 */
        unsigned int  reserved_1                        : 3;  /* bit[13-15]: 保留 */
        unsigned int  dsp_pavbias1_mode_sel             : 1;  /* bit[16]   : PA VBIAS通道选择。
                                                                             0：表示MMMB PA，VBIAS通过SSI送给AUXDAC。
                                                                             1：表示EAGLE PA，第一个VBIAS控制字走MIPI。 */
...
typedef union
{
    unsigned int      papd_redund1_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_apt_config_value : 9;  /* bit[0-8] : EAGLE PA APT控制字强配值。 */
        unsigned int  wpapd_redund1                : 23; /* bit[9-31]: PAPD冗余寄存器1 */
    } reg;
...
typedef union
{
    unsigned int      papd_redund2_reg;
    struct
    {
        unsigned int  wpapd_redund2   : 25; /* bit[0-24] : PAPD冗余寄存器2 */
        unsigned int  vbias_ctrl_word : 7;  /* bit[25-31]: 普通PA APT控制字上报，无符号数。 */
    } reg;
...
typedef union
{
    unsigned int      apt_step_mod_sel_reg;
    struct
    {
        unsigned int  dsp_apt_step_mode_sel : 1;  /* bit[0]   : apt步长模式选择
                                                                1'b0:0.8step
                                                                1'b1:0.4step */
        unsigned int  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mipi_ch_exchange_reg;
    struct
    {
        unsigned int  dsp_tx_mipi_pavbias_trigger2_ch_exchange : 3;  /* bit[0-2]  : PA VBIAS TRIGGER2 MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_0                               : 1;  /* bit[3]    : 保留 */
        unsigned int  dsp_tx_mipi_pavbias_trigger1_ch_exchange : 3;  /* bit[4-6]  : PA VBIAS TRIGGER1 MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_1                               : 1;  /* bit[7]    : 保留 */
        unsigned int  dsp_tx_mipi_pavbias3_ch_exchange         : 3;  /* bit[8-10] : PA VBIAS3 MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_2                               : 1;  /* bit[11]   : 保留 */
        unsigned int  dsp_tx_mipi_pavbias2_ch_exchange         : 3;  /* bit[12-14]: PA VBIAS2 MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_3                               : 1;  /* bit[15]   : 保留 */
        unsigned int  dsp_tx_mipi_pavbias1_ch_exchange         : 3;  /* bit[16-18]: PA VBIAS1 MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_4                               : 1;  /* bit[19]   : 保留 */
        unsigned int  dsp_tx_mipi_apt_ch_exchange              : 3;  /* bit[20-22]: APT MIPI控制字发送通道指示，该参数配置值即为mipi的发送通道编号。 */
        unsigned int  reserved_5                               : 9;  /* bit[23-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com2et_ctrl1_reg;
    struct
    {
        unsigned int  dsp_post_gain            : 10; /* bit[0-9]  : 包络适配使用的增益 */
        unsigned int  reserved_0               : 2;  /* bit[10-11]: 保留 */
        unsigned int  dsp_post_offset          : 10; /* bit[12-21]: 包络适配使用的偏移 */
        unsigned int  reserved_1               : 2;  /* bit[22-23]: 保留 */
        unsigned int  dsp_post_gain_offset_ind : 1;  /* bit[24]   : dsp_post_gain和dsp_post_offset的有效性指示脉冲信号，高有效。 */
        unsigned int  dsp_post_cfg_en          : 1;  /* bit[25]   : 第三方ET包络增益和偏移强配使能信号。dsp_post_cfg_en==1'b0，选择根据APC Setting选择后的gain；dsp_post_cfg_en==1'b1，选择dsp_post_offset，dsp_post_gain */
        unsigned int  reserved_2               : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com2et_ctrl2_reg;
    struct
    {
        unsigned int  dsp_apt_et_mode_time : 11; /* bit[0-10] : 上行ET和APT模式切换提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_0           : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_et_tx_pwr_time   : 11; /* bit[16-26]: 上行ET理论发射功率提前上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved_1           : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com2et_ctrl3_reg;
    struct
    {
        unsigned int  dsp_et_mipi_apt_time : 11; /* bit[0-10] : 上行ET APT电压控制字上行时隙头生效时刻，单位为chip。 */
        unsigned int  reserved             : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_active_time_reg;
    struct
    {
        unsigned int  dsp_tx_dpd_active_time : 10; /* bit[0-9]  : DPD提前上行时隙头生效时刻。单位1/4chip。 */
        unsigned int  reserved               : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_delay_active_time_reg;
    struct
    {
        unsigned int  dsp_et_delay_active_time : 11; /* bit[0-10] : ET整数延时和分数延时提前上行时隙头生效时刻。单位1/4chip。 */
        unsigned int  reserved                 : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_etif_sel_reg;
    struct
    {
        unsigned int  dsp_tx_etif_sel : 2;  /* bit[0-1] : ET数据接口选择信号，扩展位2bit。2'b0:选择CFR之后DPD之前的数据抽点到ET，与V8R1实现一致；2'b1:选择DPD后数据抽点到ET；2'b1X,选择DPC后数据抽点到ET */
        unsigned int  reserved        : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_rf_ssi_ch_sel_reg;
    struct
    {
        unsigned int  dsp_tx_rf_ssi_ch_sel : 2;  /* bit[0-1] : SSI发送通道指示 */
        unsigned int  reserved             : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_pwr_mod_sel_reg;
    struct
    {
        unsigned int  dsp_apt_pwr_mod_sel : 1;  /* bit[0]   : APT功率模式选择：1'b0,理论发射功率；1'b1,实际发射功率 */
        unsigned int  reserved            : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      freq_cnt_active_time_reg;
    struct
    {
        unsigned int  dsp_freq_cvt_active_time : 10; /* bit[0-9]  : 上变频提前上行时隙头生效时刻。单位1/4chip。 */
        unsigned int  reserved                 : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_mode_reg;
    struct
    {
        unsigned int  dsp_et_mod : 1;  /* bit[0]   : ET模式选择，1’b0:第三方ET；1’b1:自研ET。默认为0 */
        unsigned int  reserved   : 31; /* bit[1-31]: 保留 */
    } reg;
} WBBP_ET_MODE_UNION;
...
typedef union
{
    unsigned int      gain_compensate_reg;
    struct
    {
        unsigned int  dsp_gain1_com : 12; /* bit[0-11] : GAIN1对应温补、频补的线性增益值，无符号数 */
        unsigned int  reserved_0    : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_gain2_com : 14; /* bit[16-29]: GAIN2对应温补、频补的线性增益值，无符号数 */
        unsigned int  reserved_1    : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain1_calibration0_1_reg;
    struct
    {
        unsigned int  dsp_gain1_cali0 : 10; /* bit[0-9]  : GAIN1对应的子表0线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain1_cali1 : 10; /* bit[16-25]: GAIN1对应的子表1线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain1_calibration2_3_reg;
    struct
    {
        unsigned int  dsp_gain1_cali2 : 10; /* bit[0-9]  : GAIN1对应的子表2线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain1_cali3 : 10; /* bit[16-25]: GAIN1对应的子表3线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain1_calibration4_5_reg;
    struct
    {
        unsigned int  dsp_gain1_cali4 : 10; /* bit[0-9]  : GAIN1对应的子表4线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain1_cali5 : 10; /* bit[16-25]: GAIN1对应的子表5线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain1_calibration6_7_reg;
    struct
    {
        unsigned int  dsp_gain1_cali6 : 10; /* bit[0-9]  : GAIN1对应的子表6线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain1_cali7 : 10; /* bit[16-25]: GAIN1对应的子表7线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain2_calibration0_1_reg;
    struct
    {
        unsigned int  dsp_gain2_cali0 : 10; /* bit[0-9]  : GAIN2对应的子表0线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain2_cali1 : 10; /* bit[16-25]: GAIN2对应的子表1线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain2_calibration2_3_reg;
    struct
    {
        unsigned int  dsp_gain2_cali2 : 10; /* bit[0-9]  : GAIN2对应的子表2线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain2_cali3 : 10; /* bit[16-25]: GAIN2对应的子表3线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain2_calibration4_5_reg;
    struct
    {
        unsigned int  dsp_gain2_cali4 : 10; /* bit[0-9]  : GAIN2对应的子表4线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain2_cali5 : 10; /* bit[16-25]: GAIN2对应的子表5线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      gain2_calibration6_7_reg;
    struct
    {
        unsigned int  dsp_gain2_cali6 : 10; /* bit[0-9]  : GAIN2对应的子表6线性增益值，无符号数 */
        unsigned int  reserved_0      : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_gain2_cali7 : 10; /* bit[16-25]: GAIN2对应的子表7线性增益值，无符号数 */
        unsigned int  reserved_1      : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_temp_comp_reg;
    struct
    {
        unsigned int  dsp_tem_comp : 11; /* bit[0-10] : 第三方ET温补和频补，功率的db值，有符号数 */
        unsigned int  reserved     : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_gain_round_sel_reg;
    struct
    {
        unsigned int  dsp_gain1_round_sel : 2;  /* bit[0-1] : GAIN1四舍五入右移bit选择：2'b0:2bit, 2'b1:3bit, 2'b1X:4bit */
        unsigned int  reserved_0          : 2;  /* bit[2-3] : 保留 */
        unsigned int  dsp_gain2_round_sel : 2;  /* bit[4-5] : GAIN2四舍五入右移bit选择：2'b0:2bit, 2'b1:3bit, 2'b1X:4bit */
        unsigned int  reserved_1          : 26; /* bit[6-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      apt_et_mode_force_reg;
    struct
    {
        unsigned int  dsp_apt_et_mod_sel : 2;  /* bit[0-1] : 2'b0，由bbp根据PA档位自动控制；2’b1,软件强制为APT模式；2'b1X,软件强制为ET模式 */
        unsigned int  reserved           : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_apt_word_en_reg;
    struct
    {
        unsigned int  dsp_et_apt_word_en : 1;  /* bit[0]   : 1'b0，bbp根据当前是否处于apt模式，确认发送apt控制字；1'b1，代表无论apt或et模式，apt控制字都发送，兼容v8 */
        unsigned int  reserved           : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      pa_threshold_update_ready_reg;
    struct
    {
        unsigned int  dsp_pa_ud_ready : 1;  /* bit[0]   : PA GAIN切换门限更新指示，接口中先完成PA GAIN切换门限的配置，配置全部完成后配置dsp_pa_ud_ready，通知BBP配置结束，逻辑自清零信号 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      db2l_rom_offset_reg;
    struct
    {
        unsigned int  dsp_dpc_offset : 7;  /* bit[0-6]  : DPC db转线性值rom表地址偏移值，无符号数 */
        unsigned int  reserved_0     : 1;  /* bit[7]    : 保留 */
        unsigned int  dsp_dpd_offset : 7;  /* bit[8-14] : DPD db转线性值rom表地址偏移值，无符号数 */
...
typedef union
{
    unsigned int      et_gain_active_time_reg;
    struct
    {
        unsigned int  dsp_gain_active_time : 10; /* bit[0-9]  : ET Gain1/Gain2参数提前上行时隙头生效时刻。单位1/4chip。 */
        unsigned int  reserved             : 22; /* bit[10-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dbb_db2l_rom_threshold_reg;
    struct
    {
        unsigned int  dsp_dbb_rom_up_thr  : 7;  /* bit[0-6]  : DPC db转线性值rom表地址上限，范围0~127，默认为120，兼容v8无符号数 */
        unsigned int  reserved_0          : 1;  /* bit[7]    : 保留 */
        unsigned int  dsp_dbb_rom_low_thr : 7;  /* bit[8-14] : DPC db转线性值rom表地址下限，范围0~127，默认为20，兼容v8，无符号数 */
        unsigned int  reserved_1          : 17; /* bit[15-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      et_gain_cfg_en_reg;
    struct
    {
        unsigned int  dsp_et_gain_cfg_en : 1;  /* bit[0]   : ET Gain1/Gain2强配使能 */
        unsigned int  reserved           : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dbb_atten_value_rpt_reg;
    struct
    {
        unsigned int  dbb_table_value_rpt   : 8;  /* bit[0-7]  : 8比特数字功率回退值上报 */
        unsigned int  div_residue_latch_rpt : 4;  /* bit[8-11] : 除10后余数部分数字功率回退值上报 */
        unsigned int  d_mpr_out_sav_rpt     : 6;  /* bit[12-17]: 数字 MPR回退值上报 */
        unsigned int  reserved              : 14; /* bit[18-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      mpr_out_rpt_reg;
    struct
    {
        unsigned int  mpr_out_pd_rpt  : 6;  /* bit[0-5]  : PD MPR回退值上报 */
        unsigned int  reserved_0      : 2;  /* bit[6-7]  : 保留 */
        unsigned int  apt_mpr_out_rpt : 6;  /* bit[8-13] : APT MPR回退值上报 */
        unsigned int  reserved_1      : 18; /* bit[14-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com2et_ctrl_rpt1_reg;
    struct
    {
        unsigned int  com2et_apt_pa_vcc_rpt    : 10; /* bit[0-9]  : 输出给ET模块的的APT控制字 */
        unsigned int  reserved_0               : 2;  /* bit[10-11]: 保留 */
        unsigned int  com2et_w_pa_mode_rpt     : 2;  /* bit[12-13]: 输出给ET模块的的PA档位控制字 */
        unsigned int  reserved_1               : 2;  /* bit[14-15]: 保留 */
        unsigned int  com2et_tx_power_db_rpt   : 11; /* bit[16-26]: 输出给ET模块的的绝对发射功率 */
        unsigned int  reserved_2               : 1;  /* bit[27]   : 保留 */
        unsigned int  com2et_apt_et_mode_rpt   : 1;  /* bit[28]   : 输出给ET模块的的APT、ET模式选择信号 */
        unsigned int  com2et_apt_et_act_en_rpt : 1;  /* bit[29]   : 输出给ET模块的ET线控信号 */
...
#define WBBP_COM2ET_CTRL_RPT1_com2et_apt_et_act_en_rpt_START  (29)
#define WBBP_COM2ET_CTRL_RPT1_com2et_apt_et_act_en_rpt_END    (29)
...
typedef union
{
    unsigned int      com2et_ctrl_rpt2_reg;
    struct
    {
        unsigned int  com2et_post_offset_rpt : 10; /* bit[0-9]  : 输出给ET模块的包络适配使用的偏移 */
        unsigned int  reserved_0             : 2;  /* bit[10-11]: 保留 */
        unsigned int  com2et_post_gain_rpt   : 10; /* bit[12-21]: 输出给ET模块的包络适配使用的增益 */
        unsigned int  reserved_1             : 10; /* bit[22-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : WBBP_PWR_SFT_RPT_UNION
 结构说明  : PWR_SFT_RPT 寄存器结构定义。地址偏移量:0x4DC，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      pwr_sft_rpt_reg;
    struct
    {
        unsigned int  pwr_sft_rpt : 7;  /* bit[0-6] : DPD power shift db值 */
        unsigned int  reserved    : 25; /* bit[7-31]: 保留 */
    } reg;
} WBBP_PWR_SFT_RPT_UNION;
#define WBBP_PWR_SFT_RPT_pwr_sft_rpt_START  (0)
#define WBBP_PWR_SFT_RPT_pwr_sft_rpt_END    (6)
...
typedef union
{
    unsigned int      pd_ram_reg;
    struct
    {
        unsigned int  pd_ram_data : 10; /* bit[0-9]  : 存储180个功率对应的电压线性值，10比特无符号数，地址单位0.1dBm. */
        unsigned int  reserved    : 22; /* bit[10-31]: 保留 */
    } reg;
} WBBP_PD_RAM_UNION;
...
typedef union
{
    unsigned int      ulfe_ckg_en_reg;
    struct
    {
        unsigned int  ulfe_ckg_en : 19; /* bit[0-18] : ULFE自动门控功能使能；1表示对应子模块自动门控功能打开，0表示对应子模块自动门控功能关闭
                                                       bit[0]: 主载波rrc模块自动门控功能使能
                                                       bit[1]: W、C 削波延迟子模块自动门控功能使能
                                                       bit[2]: W、C和TDS FIR/第一级削波子模块自动门控使能
                                                       bit[3]: W和C 第二级削波子模块自动门控使能
...
typedef union
{
    unsigned int      ulfe_u_tds_mod_sel_reg;
    struct
    {
        unsigned int  ulfe_mode_sel : 2;  /* bit[0-1] : 2'b0：选择W
                                                        2'b1：选择TDS
                                                        2'b10:1X
                                                        2'b11：EVDO */
        unsigned int  reserved      : 30; /* bit[2-31]: 保留 */
...
typedef union
{
    unsigned int      ulfe_func_en_reg;
    struct
    {
        unsigned int  dsp_wt_tx_phase_comp_en : 1;  /* bit[0]    : tx phase comp使能信号
                                                                   1'b0:W、TDS和C不使能；
                                                                   1'b1:W、TDS和C使能。 */
        unsigned int  reserved_0              : 3;  /* bit[1-3]  : 保留 */
        unsigned int  dsp_power_comp_en       : 1;  /* bit[4]    : power comp使能信号
                                                                   1'b0:W、TDS和C不使能，旁路
                                                                   1'b1:W、TDS和C使能 */
        unsigned int  reserved_1              : 3;  /* bit[5-7]  : 保留 */
        unsigned int  dsp_txiq_preabe_en      : 1;  /* bit[8]    : txiq 预畸变使能信号
                                                                   1'b0:W、TDS和C不使能，旁路
                                                                   1'b1:W、TDS和C使能 */
        unsigned int  reserved_2              : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_tds_dbg_en          : 1;  /* bit[12]   : tds_dbg使能信号
                                                                   1'b0:W、C不使能，TDS使能；
                                                                   1'b1:W、C使能，TDS不使能。 */
        unsigned int  reserved_3              : 2;  /* bit[13-14]: 保留 */
        unsigned int  dsp_freq_error_en       : 1;  /* bit[15]   : fe_freq_error模块使能信号
                                                                   1'b0：W、TDS和C不使能
                                                                   1'b1：W、TDS和C使能 */
        unsigned int  dsp_sc_cal_en           : 1;  /* bit[16]   : fe_sc_cal模块使能信号
                                                                   1'b0：W、TDS和C不使能
                                                                   1'b1：W、TDS和C使能 */
        unsigned int  dsp_nco_en              : 1;  /* bit[17]   : fe_nco模块使能信号
                                                                   1'b0：W、TDS和C不使能
                                                                   1'b1：W、TDS和C使能
                                                                   TDS为0 */
        unsigned int  dsp_base_cnt_abb_en     : 1;  /* bit[18]   : base_cnt_abb模块使能信号
                                                                   1'b0：W、TDS和C不使能
                                                                   1'b1：W、TDS和C使能 */
        unsigned int  dsp_w_dpd_en            : 1;  /* bit[19]   : DPD工作使能。1：使能DPD功能；0：不使能DPD功能。 */
        unsigned int  dsp_dc_nco_en           : 1;  /* bit[20]   : DCUPANCO工作使能。1：使能NCO功能；0：不使能NCO功能。 */
...
typedef union
{
    unsigned int      tx_mode_reg_reg;
    struct
    {
        unsigned int  wt_pc_tx_mode : 2;  /* bit[0-1] : W、C和TDS上行TX模式控制：
                                                        00：正常工作
                                                        01：测试波形模式，连续循环发送测试波形数据
                                                        1x：保留 */
        unsigned int  reserved      : 30; /* bit[2-31]: 保留 */
...
typedef union
{
    unsigned int      tx_send_num_reg_reg;
    struct
    {
        unsigned int  wt_send_num   : 6;  /* bit[0-5]  : W、C和TDS测试波形模式下的波形信号，发送样点数据数目，典型值为16，取值范围1~32 */
        unsigned int  reserved_0    : 2;  /* bit[6-7]  : 保留 */
        unsigned int  wt_delt_index : 2;  /* bit[8-9]  : W、C和TDS测试波形模式下发送以下样点数据的间隔：
                                                         0：间隔为1（默认值 ）；
                                                         1：间隔为2；
                                                         2：间隔为3；
                                                         3：间隔为4； */
...
typedef union
{
    unsigned int      tx_send_data0_reg;
    struct
    {
        unsigned int  wt_send_data_i0 : 12; /* bit[0-11] : W、C和TDS测试波形模式下，第0个数据实部 */
        unsigned int  wt_send_data_q0 : 12; /* bit[12-23]: W、C和TDS测试波形模式下，第0个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data1_reg;
    struct
    {
        unsigned int  wt_send_data_i1 : 12; /* bit[0-11] : 测试波形模式下，第1个数据实部 */
        unsigned int  wt_send_data_q1 : 12; /* bit[12-23]: 测试波形模式下，第1个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data2_reg;
    struct
    {
        unsigned int  wt_send_data_i2 : 12; /* bit[0-11] : 测试波形模式下，第2个数据实部 */
        unsigned int  wt_send_data_q2 : 12; /* bit[12-23]: 测试波形模式下，第2个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data3_reg;
    struct
    {
        unsigned int  wt_send_data_i3 : 12; /* bit[0-11] : 测试波形模式下，第3个数据实部 */
        unsigned int  wt_send_data_q3 : 12; /* bit[12-23]: 测试波形模式下，第3个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data4_reg;
    struct
    {
        unsigned int  wt_send_data_i4 : 12; /* bit[0-11] : 测试波形模式下，第4个数据实部 */
        unsigned int  wt_send_data_q4 : 12; /* bit[12-23]: 测试波形模式下，第4个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data5_reg;
    struct
    {
        unsigned int  wt_send_data_i5 : 12; /* bit[0-11] : 测试波形模式下，第5个数据实部 */
        unsigned int  wt_send_data_q5 : 12; /* bit[12-23]: 测试波形模式下，第5个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data6_reg;
    struct
    {
        unsigned int  wt_send_data_i6 : 12; /* bit[0-11] : 测试波形模式下，第6个数据实部 */
        unsigned int  wt_send_data_q6 : 12; /* bit[12-23]: 测试波形模式下，第6个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data7_reg;
    struct
    {
        unsigned int  wt_send_data_i7 : 12; /* bit[0-11] : 测试波形模式下，第7个数据实部 */
        unsigned int  wt_send_data_q7 : 12; /* bit[12-23]: 测试波形模式下，第7个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data8_reg;
    struct
    {
        unsigned int  wt_send_data_i8 : 12; /* bit[0-11] : 测试波形模式下，第8个数据实部 */
        unsigned int  wt_send_data_q8 : 12; /* bit[12-23]: 测试波形模式下，第8个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data9_reg;
    struct
    {
        unsigned int  wt_send_data_i9 : 12; /* bit[0-11] : 测试波形模式下，第9个数据实部 */
        unsigned int  wt_send_data_q9 : 12; /* bit[12-23]: 测试波形模式下，第9个数据虚部 */
        unsigned int  reserved        : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data10_reg;
    struct
    {
        unsigned int  wt_send_data_i10 : 12; /* bit[0-11] : 测试波形模式下，第10个数据实部 */
        unsigned int  wt_send_data_q10 : 12; /* bit[12-23]: 测试波形模式下，第10个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data11_reg;
    struct
    {
        unsigned int  wt_send_data_i11 : 12; /* bit[0-11] : 测试波形模式下，第11个数据实部 */
        unsigned int  wt_send_data_q11 : 12; /* bit[12-23]: 测试波形模式下，第11个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data12_reg;
    struct
    {
        unsigned int  wt_send_data_i12 : 12; /* bit[0-11] : 测试波形模式下，第12个数据实部 */
        unsigned int  wt_send_data_q12 : 12; /* bit[12-23]: 测试波形模式下，第12个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data13_reg;
    struct
    {
        unsigned int  wt_send_data_i13 : 12; /* bit[0-11] : 测试波形模式下，第13个数据实部 */
        unsigned int  wt_send_data_q13 : 12; /* bit[12-23]: 测试波形模式下，第13个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data14_reg;
    struct
    {
        unsigned int  wt_send_data_i14 : 12; /* bit[0-11] : 测试波形模式下，第14个数据实部 */
        unsigned int  wt_send_data_q14 : 12; /* bit[12-23]: 测试波形模式下，第14个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data15_reg;
    struct
    {
        unsigned int  wt_send_data_i15 : 12; /* bit[0-11] : 测试波形模式下，第15个数据实部 */
        unsigned int  wt_send_data_q15 : 12; /* bit[12-23]: 测试波形模式下，第15个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data16_reg;
    struct
    {
        unsigned int  wt_send_data_i16 : 12; /* bit[0-11] : 测试波形模式下，第16个数据实部 */
        unsigned int  wt_send_data_q16 : 12; /* bit[12-23]: 测试波形模式下，第16个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data17_reg;
    struct
    {
        unsigned int  wt_send_data_i17 : 12; /* bit[0-11] : 测试波形模式下，第17个数据实部 */
        unsigned int  wt_send_data_q17 : 12; /* bit[12-23]: 测试波形模式下，第17个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data18_reg;
    struct
    {
        unsigned int  wt_send_data_i18 : 12; /* bit[0-11] : 测试波形模式下，第18个数据实部 */
        unsigned int  wt_send_data_q18 : 12; /* bit[12-23]: 测试波形模式下，第18个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data19_reg;
    struct
    {
        unsigned int  wt_send_data_i19 : 12; /* bit[0-11] : 测试波形模式下，第19个数据实部 */
        unsigned int  wt_send_data_q19 : 12; /* bit[12-23]: 测试波形模式下，第19个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data20_reg;
    struct
    {
        unsigned int  wt_send_data_i20 : 12; /* bit[0-11] : 测试波形模式下，第20个数据实部 */
        unsigned int  wt_send_data_q20 : 12; /* bit[12-23]: 测试波形模式下，第20个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data21_reg;
    struct
    {
        unsigned int  wt_send_data_i21 : 12; /* bit[0-11] : 测试波形模式下，第21个数据实部 */
        unsigned int  wt_send_data_q21 : 12; /* bit[12-23]: 测试波形模式下，第21个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data22_reg;
    struct
    {
        unsigned int  wt_send_data_i22 : 12; /* bit[0-11] : 测试波形模式下，第22个数据实部 */
        unsigned int  wt_send_data_q22 : 12; /* bit[12-23]: 测试波形模式下，第22个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data23_reg;
    struct
    {
        unsigned int  wt_send_data_i23 : 12; /* bit[0-11] : 测试波形模式下，第23个数据实部 */
        unsigned int  wt_send_data_q23 : 12; /* bit[12-23]: 测试波形模式下，第23个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data24_reg;
    struct
    {
        unsigned int  wt_send_data_i24 : 12; /* bit[0-11] : 测试波形模式下，第24个数据实部 */
        unsigned int  wt_send_data_q24 : 12; /* bit[12-23]: 测试波形模式下，第24个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data25_reg;
    struct
    {
        unsigned int  wt_send_data_i25 : 12; /* bit[0-11] : 测试波形模式下，第25个数据实部 */
        unsigned int  wt_send_data_q25 : 12; /* bit[12-23]: 测试波形模式下，第25个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data26_reg;
    struct
    {
        unsigned int  wt_send_data_i26 : 12; /* bit[0-11] : 测试波形模式下，第26个数据实部 */
        unsigned int  wt_send_data_q26 : 12; /* bit[12-23]: 测试波形模式下，第26个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data27_reg;
    struct
    {
        unsigned int  wt_send_data_i27 : 12; /* bit[0-11] : 测试波形模式下，第27个数据实部 */
        unsigned int  wt_send_data_q27 : 12; /* bit[12-23]: 测试波形模式下，第27个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data28_reg;
    struct
    {
        unsigned int  wt_send_data_i28 : 12; /* bit[0-11] : 测试波形模式下，第28个数据实部 */
        unsigned int  wt_send_data_q28 : 12; /* bit[12-23]: 测试波形模式下，第28个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data29_reg;
    struct
    {
        unsigned int  wt_send_data_i29 : 12; /* bit[0-11] : 测试波形模式下，第29个数据实部 */
        unsigned int  wt_send_data_q29 : 12; /* bit[12-23]: 测试波形模式下，第29个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data30_reg;
    struct
    {
        unsigned int  wt_send_data_i30 : 12; /* bit[0-11] : 测试波形模式下，第30个数据实部 */
        unsigned int  wt_send_data_q30 : 12; /* bit[12-23]: 测试波形模式下，第30个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_send_data31_reg;
    struct
    {
        unsigned int  wt_send_data_i31 : 12; /* bit[0-11] : 测试波形模式下，第31个数据实部 */
        unsigned int  wt_send_data_q31 : 12; /* bit[12-23]: 测试波形模式下，第31个数据虚部 */
        unsigned int  reserved         : 8;  /* bit[24-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_iq_cal_en_reg;
    struct
    {
        unsigned int  dsp_wt_tx_mismatch_iqsel : 1;  /* bit[0]    : W、C和TDS 
                                                                    0:Q路
                                                                    1:I路 */
        unsigned int  reserved_0               : 15; /* bit[1-15] : 保留 */
        unsigned int  dsp_wt_tx_mismatch_en    : 1;  /* bit[16]   : W、C和TDS TXIQMISMATCH校准使能信号（dsp_wt_tx_ip2_corr_en=0时才有效），0：上行通路为正常工作模式；1：上行产生0.96MHz正交信号。 */
        unsigned int  reserved_1               : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_ip2_cor_en_reg;
    struct
    {
        unsigned int  reserved_0            : 16; /* bit[0-15] : 保留 */
        unsigned int  dsp_wt_tx_ip2_corr_en : 1;  /* bit[16]   : W&TDS IP2校准使能信号0：上行通路为正常工作模式；1：上行产生0.96MHz同相信号。 */
        unsigned int  reserved_1            : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_iq_mismatch_comp_reg;
    struct
    {
        unsigned int  dsp_wt_tx_amp_comp   : 10; /* bit[0-9]  : W&TDS TXIQMISMATCH幅度校正因子，有符号数。 */
        unsigned int  reserved_0           : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_wt_tx_phase_comp : 10; /* bit[16-25]: W&TDS TXIQMISMATCH相位校正因子，有符号数。 */
        unsigned int  reserved_1           : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_iq_dc_offset_comp_reg;
    struct
    {
        unsigned int  dsp_wt_tx_dc_offset_i : 10; /* bit[0-9]  : W&TDS TXIQMISMATCH I路直流偏置补偿值，有符号数。 */
        unsigned int  reserved_0            : 6;  /* bit[10-15]: 保留 */
        unsigned int  dsp_wt_tx_dc_offset_q : 10; /* bit[16-25]: W&TDS TXIQMISMATCH Q路直流偏置补偿值，有符号数。 */
        unsigned int  reserved_1            : 6;  /* bit[26-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      iqmismatch_corr_len_reg;
    struct
    {
        unsigned int  dsp_wt_corr_len_sel : 2;  /* bit[0-1] : W&TDS IQMISMATCH相关长度选择：0:256,1:1024,2:2048,3:4096 */
        unsigned int  reserved            : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_iq_cor_en_reg;
    struct
    {
        unsigned int  dsp_wt_tx_iq_corr_clr   : 1;  /* bit[0]    : W&TDS TX Iqmismatch校准相关运算结束标志清零信号，自清零。 */
        unsigned int  reserved_0              : 15; /* bit[1-15] : 保留 */
        unsigned int  dsp_wt_tx_iq_corr_start : 1;  /* bit[16]   : W&TDS TX IQmismatch校准相关模块启动信号，自清零，该信号有效后启动一次相关运算。 */
        unsigned int  reserved_1              : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_iq_cal_rpt_reg;
    struct
    {
        unsigned int  wt_tx_iq_corr_dc     : 13; /* bit[0-12] : W&TDS TX Iqmismatch载漏相关值上报。无符号数。 */
        unsigned int  reserved_0           : 3;  /* bit[13-15]: 保留 */
        unsigned int  wt_tx_iq_corr_iq     : 13; /* bit[16-28]: W&TDS TX Iqmismatch校准IQ相关值上报。无符号数。 */
        unsigned int  reserved_1           : 2;  /* bit[29-30]: 保留 */
        unsigned int  wt_tx_iq_corr_finish : 1;  /* bit[31]   : W&TDS TX Iqmismatch校准相关运算完成标志，高电平表示相关计算完成。由dsp_tx_iq_corr_clr清零。 */
    } reg;
...
typedef union
{
    unsigned int      ul_dc_offset_en_reg;
    struct
    {
        unsigned int  dsp_w_ul_dc_offset_en : 1;  /* bit[0]   : 上行发射I/Q路直流偏置补偿值有效使能，高电平有效。有效时对上行I/Q路分别进行直流偏置值补偿。(SiGe 有效，CMOS 无效) */
        unsigned int  reserved              : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_dc_offset_reg;
    struct
    {
        unsigned int  dsp_w_ul_dc_offset_i : 12; /* bit[0-11] : 上行发射I路直流偏置补偿值。 */
        unsigned int  reserved_0           : 4;  /* bit[12-15]: 保留 */
        unsigned int  dsp_w_ul_dc_offset_q : 12; /* bit[16-27]: 上行发射Q路直流偏置补偿值。 */
        unsigned int  reserved_1           : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      w_tx_lmh_phase_comp_reg;
    struct
    {
        unsigned int  dsp_w_tx_m2h_phase_comp : 8;  /* bit[0-7]  : 中切高增益时相位补偿值，有符号数。 */
        unsigned int  dsp_w_tx_h2m_phase_comp : 8;  /* bit[8-15] : 高切中增益时相位补偿值，有符号数。 */
        unsigned int  dsp_w_tx_m2l_phase_comp : 8;  /* bit[16-23]: 中切低增益时相位补偿值，有符号数。 */
        unsigned int  dsp_w_tx_l2m_phase_comp : 8;  /* bit[24-31]: 低切中增益时相位补偿值，有符号数。 */
    } reg;
...
typedef union
{
    unsigned int      ul_rf_gate0_reg;
    struct
    {
        unsigned int  dsp_w_cfr_gate0   : 13; /* bit[0-12] : 上行削波第一级生效门限。当削波使能打开时，对上行RRC滤波后数据进行削波处理。可配范围64~8191。 */
        unsigned int  reserved_0        : 3;  /* bit[13-15]: 保留 */
        unsigned int  dsp_w_cfr_gain0   : 12; /* bit[16-27]: 上行削波第一级功率调整增益。当削波使能打开时，对上行RRC滤波后数据进行削波处理。 */
        unsigned int  reserved_1        : 2;  /* bit[28-29]: 保留 */
        unsigned int  dsp_w_cfr_bypass2 : 1;  /* bit[30]   : 上行削波bypass标志，高电平有效。复位值为高电平。
                                                             当bypass1为高电平，削波使能关闭，数据无延时直通输出；当bypass1为低电平，bypass2为高电平，削波使能关闭，数据延时16.75chip时间后直接输出；当bypass1和bypass2均为低电平，削波使能打开，削波后数据比原数据延时16.75chip时间输出。 */
        unsigned int  dsp_w_cfr_bypass1 : 1;  /* bit[31]   : 上行削波bypass标志，高电平有效。复位值为高电平。
                                                             当bypass1为高电平，削波使能关闭，数据无延时直通输出；当bypass1为低电平，bypass2为高电平，削波使能关闭，数据延时16.75chip时间后直接输出；当bypass1和bypass2均为低电平，削波使能打开，削波后数据比原数据延时17.5chip时间输出。 */
    } reg;
...
typedef union
{
    unsigned int      ul_rf_gate1_reg;
    struct
    {
        unsigned int  dsp_w_cfr_gate1 : 13; /* bit[0-12] : 上行削波第二级生效门限。当削波使能打开时，对上行RRC滤波后数据进行削波处理。可配范围64~8191。 */
        unsigned int  reserved_0      : 3;  /* bit[13-15]: 保留 */
        unsigned int  dsp_w_cfr_gain1 : 12; /* bit[16-27]: 上行削波第二级功率调整增益。当削波使能打开时，对上行RRC滤波后数据进行削波处理。 */
        unsigned int  reserved_1      : 4;  /* bit[28-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      abb_data_width_reg;
    struct
    {
        unsigned int  dsp_w_ul_data_12bit : 1;  /* bit[0]   : 上行输出到ABB数据有效位宽指示。0表示12bit输出；1表示10bit输出，在bit[1:0]上固定填0。测试用，默认配置0。 */
        unsigned int  reserved            : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rf_send_data_reg;
    struct
    {
        unsigned int  dsp_w_rf_send_data_en : 1;  /* bit[0]    : 测试模式下发射数据使能，高电平有效。此时射频发送数据为CPU直接配置的值，未经过调制及RRC滤波。 */
        unsigned int  reserved_0            : 7;  /* bit[1-7]  : 保留 */
        unsigned int  dsp_w_rf_send_data    : 12; /* bit[8-19] : 测试模式下射频发射数据。 */
        unsigned int  reserved_1            : 12; /* bit[20-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fir_sftbit_reg;
    struct
    {
        unsigned int  dsp_tds_fir_sftbit : 4;  /* bit[0-3] : 上下采样滤波器内部累加器移位bit数
                                                             上电之后只配置一次，配置范围10~13 */
        unsigned int  reserved           : 28; /* bit[4-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      uppts_flg_reg;
    struct
    {
        unsigned int  tds_uppts_flg : 1;  /* bit[0]   : DSP在配置特殊时隙UpPTS参数时一定要配置该寄存器标识：
                                                        0：非特殊时隙UpPTS参数；
                                                        1：特殊时隙UpPTS参数；
                                                        该参数在配置完其它上行参数后最后配置。 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      pc_gene_reg;
    struct
    {
        unsigned int  dsp_tds_pc_gene   : 17; /* bit[0-16] : 上行功控模块的衰减因子值 */
        unsigned int  reserved_0        : 7;  /* bit[17-23]: 保留 */
        unsigned int  dsp_tds_pc_sftbit : 5;  /* bit[24-28]: 上行功控模块乘以衰减因子后的移位比特数，配置范围13~16 */
        unsigned int  reserved_1        : 3;  /* bit[29-31]: 保留 */
    } reg;
} WBBP_PC_GENE_UNION;
...
typedef union
{
    unsigned int      abb_fmt_reg;
    struct
    {
        unsigned int  reserved           : 16; /* bit[0-15] : 保留 */
        unsigned int  tds_flush_zero_num : 16; /* bit[16-31]: 正常数据包络发送完成后，输出给DA的冗余0的个数，该功能为保留功能 */
    } reg;
} WBBP_ABB_FMT_UNION;
...
typedef union
{
    unsigned int      tds_pa_change_mode_reg;
    struct
    {
        unsigned int  tds_pa_change_mode : 3;  /* bit[0-2] : TDS PA档位 */
        unsigned int  reserved           : 29; /* bit[3-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dft_func_reg;
    struct
    {
        unsigned int  tds_dft_func_en : 1;  /* bit[0]   : da_on包络数据统计有效 */
        unsigned int  tds_para_force  : 1;  /* bit[1]   : 参数强制锁定 */
        unsigned int  reserved        : 30; /* bit[2-31]: 保留 */
    } reg;
} WBBP_DFT_FUNC_UNION;
...
typedef union
{
    unsigned int      tx_rx_cnt_reg;
    struct
    {
        unsigned int  reserved : 16; /* bit[0-15] : 保留 */
        unsigned int  tx_cnt   : 16; /* bit[16-31]: da_on包络个数 */
    } reg;
...
typedef union
{
    unsigned int      rrc_reg;
    struct
    {
        unsigned int  tds_rrc_out_ul : 14; /* bit[0-13] : [13:12]为rrc输出的da_on/dvld；
                                                          [11:0]为rrc的输出的上行数据； */
        unsigned int  reserved       : 18; /* bit[14-31]: 保留 */
    } reg;
} WBBP_RRC_UNION;
...
typedef union
{
    unsigned int      fir_txiq_reg;
    struct
    {
        unsigned int  tds_pc_out_ul  : 14; /* bit[0-13] : [13:12]为pc输出的da_on/dvld；
                                                          [11:0]为pc的输出上行数据； */
        unsigned int  reserved_0     : 2;  /* bit[14-15]: 保留 */
        unsigned int  tds_fir_out_ul : 14; /* bit[16-29]: [13:12]为fir输出的da_on/dvld；
                                                          [11:0]为fir的输出的上行数据； */
        unsigned int  reserved_1     : 2;  /* bit[30-31]: 保留 */
    } reg;
} WBBP_FIR_TXIQ_UNION;
...
typedef union
{
    unsigned int      sinc_daiu_reg;
    struct
    {
        unsigned int  reserved_0      : 16; /* bit[0-15] : 保留 */
        unsigned int  tds_txiq_out_ul : 14; /* bit[16-29]: [29:28]为txiq输出的da_on/dvld；
                                                           [27:16]为txiq的输出上行数据； */
        unsigned int  reserved_1      : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned int  tds_fe_sync_fifo_out_ul : 14; /* bit[0-13] : W、TDS和C:
                                                                   [13:12]为FIFO输出的da_on/dvld；
                                                                   [11:0]为FIFO输出的上行数据； */
        unsigned int  reserved                : 18; /* bit[14-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      sc_freq_reg;
    struct
    {
        unsigned int  tds_freq_error_out_ul : 14; /* bit[0-13] : W、TDS和C:
                                                                 [13:12]为频偏校正输出的da_on/dvld；
                                                                 [11:0]为频偏校正输出上行数据； */
        unsigned int  reserved_0            : 2;  /* bit[14-15]: 保留 */
        unsigned int  tds_sc_filter_out_ul  : 14; /* bit[16-29]: W、TDS和C:
                                                                 [29:28]为采样纠偏滤波器输出的da_on/dvld；
                                                                 [27:16]为采样纠偏滤波器输出的上行数据； */
        unsigned int  reserved_1            : 2;  /* bit[30-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ulfe_tcxo_en_reg;
    struct
    {
        unsigned int  dsp_ulfe_tcxo_en : 1;  /* bit[0]   : W和C单TCXO使能信号：
                                                           1'b0:单TCXO使能关闭
                                                           1'b1:单TCXO使能打开
                                                           此使能信号只与时钟的选择有关系 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      sc_filter_offset_reg;
    struct
    {
        unsigned int  dsp_ul_sc_offset_en : 1;  /* bit[0]    : W:
                                                               双TCXO软件强配采样纠偏值使能信号 */
        unsigned int  dsp_sc_offset       : 11; /* bit[1-11] : W和C:
                                                               双TCXO软件强配采样纠偏值，和下行一致，范围为[-1023,0123]。 */
        unsigned int  dsp_td_sc_offset    : 9;  /* bit[12-20]: TDS:
                                                               软件强配采样纠偏值，有符号数 */
...
typedef union
{
    unsigned int      sc_cfg_ind_reg;
    struct
    {
        unsigned int  dsp_sc_offset_ind : 1;  /* bit[0]   : W和C:
                                                            双TCXO下软件强配采样纠偏值更新脉冲信号 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fifo_clr_en_reg;
    struct
    {
        unsigned int  dsp_fifo_clr_en : 1;  /* bit[0]   : W和C FIFO清零电平信号:
                                                          1'b0:FIFO不清零
                                                          1'b1:FIFO清零 */
        unsigned int  reserved        : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fifo_rpt_clr_reg;
    struct
    {
        unsigned int  dsp_fifo_rpt_clr_en : 1;  /* bit[0]   : W和C FIFO异常空满上报状态清零信号:
                                                              1'b0:FIFO异常空满不清零
                                                              1'b1:FIFO异常空满清零 */
        unsigned int  reserved            : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_atten_value_reg;
    struct
    {
        unsigned int  dsp_c_atten_value : 12; /* bit[0-11] : C模功率补偿值，无符号数 */
        unsigned int  reserved          : 20; /* bit[12-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      fifo_empty_full_reg;
    struct
    {
        unsigned int  fifo_empty_abn_dsp : 1;  /* bit[0]    : W和C FIFO 异常空标志:
                                                              1'b0:FIFO不异常空；
                                                              1'b1:FIFO异常空。 */
        unsigned int  reserved_0         : 3;  /* bit[1-3]  : 保留 */
        unsigned int  fifo_full_dsp      : 1;  /* bit[4]    : W和C FIFO 满标志:
                                                              1'b0:FIFO不满；
                                                              1'b1:FIFO满。 */
        unsigned int  reserved_1         : 3;  /* bit[5-7]  : 保留 */
        unsigned int  fifo_cnt_a         : 5;  /* bit[8-12] : W和C FIFO 写侧剩余数据个数 */
        unsigned int  reserved_2         : 3;  /* bit[13-15]: 保留 */
        unsigned int  fifo_cnt_b         : 5;  /* bit[16-20]: W和C FIFO 读侧剩余数据个数 */
        unsigned int  reserved_3         : 11; /* bit[21-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      c_phase_comp_reg;
    struct
    {
        unsigned int  dsp_cos_comp : 11; /* bit[0-10] : C模相位补偿值，有符号数 */
        unsigned int  dsp_sin_comp : 11; /* bit[11-21]: C模相位补偿值，有符号数 */
...
typedef union
{
    unsigned int      sc_cal_clr_reg;
    struct
    {
        unsigned int  dsp_ul_sc_cal_clr : 1;  /* bit[0]   : C模在打开上行时，sc_cal模块的积分清零使能信号
                                                            1'b1:在打开上行时，sc_cal模块的积分自动清零；
                                                            1'b0:在打开上行时，sc_cal模块的积分不清零；(W模不需要配置) */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      sc_cal_ul_ratio_reg;
    struct
    {
        unsigned int  dsp_ul_fc_ratio : 15; /* bit[0-14] : W和C Fc,up/Fc，无符号数 */
        unsigned int  reserved        : 17; /* bit[15-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      nco_ph_clr_reg;
    struct
    {
        unsigned int  dsp_nco_ph_clr : 1;  /* bit[0]   : W、C和TDS NCO模块在调试或出错的情况下，相位累加值清零脉冲信号 */
        unsigned int  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      nco_fw_en_reg;
    struct
    {
        unsigned int  dsp_nco_fw_cfg_en : 1;  /* bit[0]   : W/C NCO模块频偏输入强配使能信号 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      rrc_q_delay_reg;
    struct
    {
        unsigned int  dsp_c_q_delay_num : 2;  /* bit[0-1] : C RRC Q路延时样点数
                                                            2'b0：不延时
                                                            2'b1：延1样点
                                                            2'b10：延2样点
                                                            2'b11：延3样点
...
typedef union
{
    unsigned int      calbrt_mod_reg;
    struct
    {
        unsigned int  dsp_calbrt_mod : 1;  /* bit[0]   : W校准模式选择信号：
                                                         1'b1:校准单音信号立刻生效；
                                                         1'b0:校准单音信号在帧头生效。 */
        unsigned int  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_tds_sc_ult_fsmp_reg;
    struct
    {
        unsigned int  ul_tds_sc_dlt_fsmp : 17; /* bit[0-16] : tds模式下采样调整步长;17bit */
        unsigned int  reserved           : 15; /* bit[17-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_tds_sc_ult_vld_reg;
    struct
    {
        unsigned int  ul_tds_sc_dlt_vld : 1;  /* bit[0]   : tds dsp配置采样调整步长有效指示,为脉冲信号; */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_tds_sc_ini_vld_reg;
    struct
    {
        unsigned int  ul_tds_sc_ini_vld : 1;  /* bit[0]   : tds dsp配置采样初始偏差值有效指示,为脉冲信号; */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_tds_sc_mod_reg;
    struct
    {
        unsigned int  dsp_tds_sc_fsmp_mod : 1;  /* bit[0]   : tds模式下参数立即生效模式：
                                                              0：不立即生效，用da_on上升沿和*vld锁存参数；
                                                              1：立即生效，直接根据*vld去锁存参数； 
                                                              默认值为0。
                                                              (保留) */
...
typedef union
{
    unsigned int      ul_sc_abn_del_en_reg;
    struct
    {
        unsigned int  ul_sc_abn_del_en : 1;  /* bit[0]   : W、C和TDS sc_cal模块在fifo几乎空，几乎满的异常删加点使能：
                                                           0：关闭异常删加点功能；
                                                           1：打开异常删加点功能。
                                                           默认打开。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      ul_sc_cal_num_rpt_reg;
    struct
    {
        unsigned int  sc_add_num_rpt : 16; /* bit[0-15] : W、C和TDS sc_cal模块计算加点次数统计。 */
        unsigned int  sc_del_num_rpt : 16; /* bit[16-31]: W、C和TDS sc_cal模块计算删点次数统计。 */
    } reg;
...
typedef union
{
    unsigned int      ul_fifo_abn_num_reg;
    struct
    {
        unsigned int  fifo_alemp_num : 8;  /* bit[0-7]  : W、C和TDS FIFO将空次数上报 */
        unsigned int  fifo_alful_num : 8;  /* bit[8-15] : W、C和TDS FIFO将满次数上报 */
        unsigned int  fifo_emp_num   : 8;  /* bit[16-23]: W、C和TDS FIFO空次数上报 */
        unsigned int  fifo_ful_num   : 8;  /* bit[24-31]: W、C和TDS FIFO满次数上报 */
    } reg;
...
typedef union
{
    unsigned int      ul_sc_cal_num_rpt_clr_reg;
    struct
    {
        unsigned int  ul_sc_add_num_clr : 1;  /* bit[0]   : W、C和TDS sc_cal模块计算加点次数统计清零。 */
        unsigned int  ul_sc_del_num_clr : 1;  /* bit[1]   : W、C和TDS sc_cal模块计算删点次数统计清零。 */
        unsigned int  reserved          : 30; /* bit[2-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_sc_cal_para_en_reg;
    struct
    {
        unsigned int  dsp_sc_cal_para_en : 1;  /* bit[0]   : sc_filter模块的参数来源：
                                                             1'b1：参数来源于sc_cal；
                                                             1'b0：W参数来源于多径或强配，TDS和C模来源于强配。 */
        unsigned int  reserved           : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ul_del_add_en_reg;
    struct
    {
        unsigned int  dsp_del_add_en : 1;  /* bit[0]   : sc_cal模块积分功能使能信号，使能关闭情况下，删加点和offset都为0：
                                                         1'b0:积分使能关闭；
                                                         1'b1:积分使能打开。 */
        unsigned int  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_step1_reg;
    struct
    {
        unsigned int  dsp_dpd_step_v0 : 9;  /* bit[0-8]  : 表格0对应的量化参数step_v，无符号数 */
        unsigned int  dsp_dpd_step_v1 : 9;  /* bit[9-17] : 表格1对应的量化参数step_v，无符号数 */
        unsigned int  dsp_dpd_step_v2 : 9;  /* bit[18-26]: 表格2对应的量化参数step_v，无符号数 */
        unsigned int  reserved        : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_step2_reg;
    struct
    {
        unsigned int  dsp_dpd_step_v3 : 9;  /* bit[0-8]  : 表格3对应的量化参数step_v，无符号数 */
        unsigned int  dsp_dpd_step_v4 : 9;  /* bit[9-17] : 表格4对应的量化参数step_v，无符号数 */
        unsigned int  dsp_dpd_step_v5 : 9;  /* bit[18-26]: 表格5对应的量化参数step_v，无符号数 */
        unsigned int  reserved        : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_step3_reg;
    struct
    {
        unsigned int  dsp_dpd_step_v6 : 9;  /* bit[0-8]  : 表格6对应的量化参数step_v，无符号数 */
        unsigned int  dsp_dpd_step_v7 : 9;  /* bit[9-17] : 表格7对应的量化参数step_v，无符号数 */
        unsigned int  reserved        : 14; /* bit[18-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_comp_reg;
    struct
    {
        unsigned int  dsp_dpd_comp : 11; /* bit[0-10] : DPD功率的温补和频补的合并补偿值，有符号数 */
        unsigned int  reserved     : 21; /* bit[11-31]: 保留 */
    } reg;
} WBBP_DPD_COMP_UNION;
...
typedef union
{
    unsigned int      dpd_ram_table_sel_reg;
    struct
    {
        unsigned int  dsp_dpd_force_en : 1;  /* bit[0]   : DPD强制工作使能信号。在DPD使能打开情况下，不论实际发射功率是否达到门限值，若该信号为高，DPD补偿打开，每时隙都进行补偿，且512ram固定选择table0。供测试校准模式使用。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      tx_tone_freq_sel_reg;
    struct
    {
        unsigned int  dsp_tx_tone_freq_sel : 1;  /* bit[0]   : CPU接口配置的单音频率选择信号：
                                                               0:0.96M单音
                                                               1:0.48M单音，默认为0.96M单音，前向兼容原有版本。 */
        unsigned int  reserved             : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_dc_nco_ph_clr_reg;
    struct
    {
        unsigned int  dsp_dc_nco_ph_clr : 1;  /* bit[0]   : 上变频NCO模块在调试或出错的情况下，相位累加值清零脉冲信号 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dsp_conju_sel_reg;
    struct
    {
        unsigned int  dsp_conju_sel : 1;  /* bit[0]   : 主载波和辅载波上变频共轭选择信号，1'b0:辅载波虚部取反，主载波不取反；1'b1:主载波虚部取反，辅载波不取反 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dcupa_sw_ul_freq_mod_reg;
    struct
    {
        unsigned int  dsp_dcupa_sw_ul_freq_mod : 1;  /* bit[0]   : DSP是否强制配置NCO频点搬移、主辅载波相加：1表示强制NCO频点搬移、主辅载波相加；0表示该信号不起作用 */
        unsigned int  reserved_0               : 3;  /* bit[1-3] : 保留 */
        unsigned int  dsp_dcupa_act_disable    : 1;  /* bit[4]   : DSP强制配置NCO频点搬移、主辅载波相加是否随dcupa_active_flag指示生效。为1表示频点受dcupa_active_flag控制；为0表示不受dcupa_active_flag控制，默认为1。 */
        unsigned int  reserved_1               : 27; /* bit[5-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_bypass_reg;
    struct
    {
        unsigned int  dsp_tx_dpd_iqcomp_bypass : 1;  /* bit[0]   : DPD补偿通路bypass功能，当DPD使能打开并且配置了bypass功能时处理延时与DPD相同，但是不做数据通路补偿。 */
        unsigned int  reserved                 : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_lut_addr_sel_reg;
    struct
    {
        unsigned int  dsp_lut_addr_sel : 1;  /* bit[0]   : DPD LUT查找表的子表地址映射模式模式选择信号
                                                           1’b0：DPD LUT查找表的8档的8个子表采用绝对发送功率方式查表
                                                           1’b1：DPD LUT查找表的8档的8个子表采用根据射频增益衰减表查表地址值查表。 */
        unsigned int  reserved         : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_threshold0_1_reg;
    struct
    {
        unsigned int  dsp_dpd_threshold0 : 11; /* bit[0-10] : DPD生效功率门限0。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_dpd_threshold1 : 11; /* bit[16-26]: DPD生效功率门限1。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_threshold2_3_reg;
    struct
    {
        unsigned int  dsp_dpd_threshold2 : 11; /* bit[0-10] : DPD生效功率门限2。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_dpd_threshold3 : 11; /* bit[16-26]: DPD生效功率门限3。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_threshold4_5_reg;
    struct
    {
        unsigned int  dsp_dpd_threshold4 : 11; /* bit[0-10] : DPD生效功率门限4。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_dpd_threshold5 : 11; /* bit[16-26]: DPD生效功率门限5。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_threshold6_7_reg;
    struct
    {
        unsigned int  dsp_dpd_threshold6 : 11; /* bit[0-10] : DPD生效功率门限6。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_0         : 5;  /* bit[11-15]: 保留 */
        unsigned int  dsp_dpd_threshold7 : 11; /* bit[16-26]: DPD生效功率门限7。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved_1         : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      dpd_threshold8_reg;
    struct
    {
        unsigned int  dsp_dpd_threshold8 : 11; /* bit[0-10] : DPD生效功率门限8。当dpd_lut_addr_sel=1’b0时，对应功率门限，单位0.1dBm精度，当dpd_lut_addr_sel=1’b1，对应射频增益衰减表查表地址值 */
        unsigned int  reserved           : 21; /* bit[11-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com_cw_en_reg;
    struct
    {
        unsigned int  dsp_com_cw_en : 1;  /* bit[0]   : CFR前单/双音信号发送使能，在上行模式不为TDS-CDMA时配置有效。1’b1:U/X模CFR前单/双音信号发送使能，当该使能有效时，BBP根据软件配置的I/Q和周期点数周期发送单/双音信号。 */
        unsigned int  reserved      : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      com_cw_cnt_reg;
    struct
    {
        unsigned int  dsp_com_cw_cnt : 5;  /* bit[0-4] : CFR前单/双音信号正（余）弦波周期内的点数，默认为15，采用15.36M/(dsp_com_cw_cnt+1)得到需要发送信号的周期频率，例如默认15.36M/16=0.96M。 */
...
typedef union
{
    unsigned int      com_cw_tx_mode_reg;
    struct
    {
        unsigned int  dsp_com_cw_tx_mode : 1;  /* bit[0]   : CFR前单/双音信号发送模式选择信号
                                                             1’b0:按照软件配置的加权系数立即生效的加权模式
                                                             1’b1:BBP自动周期性循环发送模
                                                             。 */
        unsigned int  reserved           : 31; /* bit[1-31]: 保留 */
...
typedef union
{
    unsigned int      com_cw_tx_prd_reg;
    struct
    {
        unsigned int  dsp_com_cw_tx_prd : 2;  /* bit[0-1] : 当发送模式为自动发送时(dsp_cw_tx_mode=1’b1),每个周期发送的点数。
                                                            2’d0：每个周期发送1024个点
                                                            2’d1：每个周期发送2048个点
                                                            2’d2：每个周期发送3072个点
                                                            2’d3：每个周期发送4096个点 */
...
typedef union
{
    unsigned int      com_cw_coef_reg;
    struct
    {
        unsigned int  dsp_com_cw_coef : 24; /* bit[0-23] : 单/双音I/Q幅度加权系数，无符号数；
                                                           当dsp_com_cw_tx_mode=1’b0时，配置的补偿系数直接补偿立即生效
                                                           当dsp_com_cw_tx_mode=1’b1时，该系数作为初始发送的起始加权系数。
                                                           [23:12]:Q路的加权系数
                                                           [11:0]:I路的加权系数 */
...
typedef union
{
    unsigned int      com_cw_step_reg;
    struct
    {
        unsigned int  dsp_com_cw_step : 8;  /* bit[0-7] : 当dsp_com_cw_tx_mode=1'b1时,每个周期发送结束后将加权系数增加步长后发送下一个周期，无符号数 */
        unsigned int  reserved        : 24; /* bit[8-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      cw_pre_sel_reg;
    struct
    {
        unsigned int  dsp_cw_pre_sel : 1;  /* bit[0]   : nco模块da_on选择，用来做nco预充。当dsp_cw_pre_sel==1'b0时，选择rrc da_on信号，正常模式下使用；dsp_cw_pre_sel==1'b1时，选择dsp_nco_en信号，供削波前单音校准时使用。 */
        unsigned int  reserved       : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ulfe_dcupa_en_rpt_reg;
    struct
    {
        unsigned int  ulfe_dcupa_en_rpt : 1;  /* bit[0]   : 上行前端dcupa激活状态上报。 */
        unsigned int  reserved          : 31; /* bit[1-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      ulfe_dpd_rpt_reg;
    struct
    {
        unsigned int  uplink_tx_power_db_rpt : 11; /* bit[0-10] : 上行理论发射功率 */
        unsigned int  reserved_0             : 1;  /* bit[11]   : 保留 */
        unsigned int  act_nom_error_rpt      : 11; /* bit[12-22]: 上行最大理论发射功率修正值 */
        unsigned int  reserved_1             : 1;  /* bit[23]   : 保留 */
        unsigned int  dpd_table_sel_rpt      : 3;  /* bit[24-26]: 上行DPD子表选择，范围0~7 */
        unsigned int  reserved_2             : 5;  /* bit[27-31]: 保留 */
    } reg;
...
typedef union
{
    unsigned int      atten_table_addr_rpt_reg;
    struct
    {
        unsigned int  atten_table_addr_rpt : 9;  /* bit[0-8] : 上行前端RF回退索引值上报。 */
        unsigned int  reserved             : 23; /* bit[9-31]: 保留 */
    } reg;
...
/*****************************************************************************
 结构名    : WBBP_C_DPD_CONFIG1_UNION
 结构说明  : C_DPD_CONFIG1 寄存器结构定义。地址偏移量:0xAA0，初值:0x00000000，宽度:32
 寄存器说明: C模DPD相关配置1
*****************************************************************************/
typedef union
{
    unsigned int      c_dpd_config1_reg;
    struct
    {
        unsigned int  dsp_c_tx_power_db   : 11; /* bit[0-10] : C模上行理论发射功率，有符号数 */
        unsigned int  reserved_0          : 1;  /* bit[11]   : 保留 */
        unsigned int  dsp_c_act_nom_error : 11; /* bit[12-22]: C模上行最大理论发射功率修正值，有符号数 */
        unsigned int  reserved_1          : 9;  /* bit[23-31]: 保留 */
    } reg;
} WBBP_C_DPD_CONFIG1_UNION;
#define WBBP_C_DPD_CONFIG1_dsp_c_tx_power_db_START    (0)
#define WBBP_C_DPD_CONFIG1_dsp_c_tx_power_db_END      (10)
#define WBBP_C_DPD_CONFIG1_dsp_c_act_nom_error_START  (12)
#define WBBP_C_DPD_CONFIG1_dsp_c_act_nom_error_END    (22)


/*****************************************************************************
 结构名    : WBBP_C_DPD_CONFIG2_UNION
 结构说明  : C_DPD_CONFIG2 寄存器结构定义。地址偏移量:0xAA4，初值:0x00000000，宽度:32
 寄存器说明: C模DPD相关配置2
*****************************************************************************/
typedef union
{
    unsigned int      c_dpd_config2_reg;
    struct
    {
        unsigned int  dsp_c_atten_table_addr : 9;  /* bit[0-8]  : C模RF回退索引值，无符号数 */
        unsigned int  reserved_0             : 3;  /* bit[9-11] : 保留 */
        unsigned int  dsp_c_dpd_p_coef       : 12; /* bit[12-23]: C模DPD power_shift，无符号数 */
        unsigned int  reserved_1             : 8;  /* bit[24-31]: 保留 */
    } reg;
} WBBP_C_DPD_CONFIG2_UNION;
#define WBBP_C_DPD_CONFIG2_dsp_c_atten_table_addr_START  (0)
#define WBBP_C_DPD_CONFIG2_dsp_c_atten_table_addr_END    (8)
#define WBBP_C_DPD_CONFIG2_dsp_c_dpd_p_coef_START        (12)
#define WBBP_C_DPD_CONFIG2_dsp_c_dpd_p_coef_END          (23)
 ...
 ***======================================================================***/
/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_EN_UNION
 结构说明  : DLFE_SDR_EN 寄存器结构定义。地址偏移量:0x000，初值:0x01001000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_0                   : 3;  /* bit[1-3]  : 保留 */
        unsigned int  cpu_dlfe_sdr_start_stop_mode : 1;  /* bit[4]    : 下行SDR IF模块搬运启停模式。0，主分集都用主集启停信号；1，主分集用各自启停信号。 */
        unsigned int  reserved_1                   : 3;  /* bit[5-7]  : 保留 */
        unsigned int  cpu_dlfe_sdr_mode            : 2;  /* bit[8-9]  : 下行SDR IF模块工作模式。0表示W模；1表示无意义；2表示1X；3表示EVDO。 */
        unsigned int  reserved_2                   : 2;  /* bit[10-11]: 保留 */
        unsigned int  cpu_dlfe_sdr_rx_mode         : 1;  /* bit[12]   : 下行SDR IF模块接收分集模式。0，非接收分集；1，接收分集。 */
        ...
        unsigned int  cpu_dlfe_sdr_trans_mode      : 1;  /* bit[24]   : 下行搬运模式。0，DMA方式；1，BBPMaster方式。
                                                                        固定为1，使用BBPMaster方式，不再支持DMA方式。 */
        unsigned int  reserved_6                   : 3;  /* bit[25-27]: 保留 */
        unsigned int  cpu_dlfe_sdr_dma_mode        : 1;  /* bit[28]   : 下行DMA搬运模式。0，一次burst请求搬运32x32bit；1，一次burst请求搬运16x32bit。 */
        unsigned int  reserved_7                   : 2;  /* bit[29-30]: 保留 */
        unsigned int  cpu_dlfe_wc_dump_mode        : 1;  /* bit[31]   : 下行通道搬数模式。0，软基带搬数；1，前端AGC采数。 */
    } reg;
} WBBP_DLFE_SDR_EN_UNION;
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_en_START               (0)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_en_END                 (0)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_start_stop_mode_START  (4)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_start_stop_mode_END    (4)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_mode_START             (8)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_mode_END               (9)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_rx_mode_START          (12)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_rx_mode_END            (12)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_start_mode_START       (16)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_start_mode_END         (16)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_stop_mode_START        (20)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_stop_mode_END          (20)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_trans_mode_START       (24)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_trans_mode_END         (24)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_dma_mode_START         (28)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_sdr_dma_mode_END           (28)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_wc_dump_mode_START         (31)
#define WBBP_DLFE_SDR_EN_cpu_dlfe_wc_dump_mode_END           (31)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_CKG_BYS_UNION
...
} WBBP_DLFE_SDR_CKG_BYS_UNION;
#define WBBP_DLFE_SDR_CKG_BYS_cpu_dlfe_sdr_ckg_bypass_START  (0)
#define WBBP_DLFE_SDR_CKG_BYS_cpu_dlfe_sdr_ckg_bypass_END    (0)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_START_STOP_UNION
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_3                  : 3;  /* bit[13-15]: 保留 */
        unsigned int  cpu_dlfe_at2_sdr_start      : 1;  /* bit[16]   : 下行分集搬运启动脉冲。 */
        unsigned int  reserved_4                  : 3;  /* bit[17-19]: 保留 */
        unsigned int  cpu_dlfe_at2_sdr_stop       : 1;  /* bit[20]   : 下行分集搬运停止脉冲。 */
        unsigned int  reserved_5                  : 3;  /* bit[21-23]: 保留 */
        unsigned int  cpu_dlfe_at2_sdr_force_stop : 1;  /* bit[24]   : 下行分集搬运强制停止脉冲。在检测到下行分集搬运出错后配置。 */
        unsigned int  reserved_6                  : 3;  /* bit[25-27]: 保留 */
        unsigned int  cpu_dlfe_at2_sdr_trans_clr  : 1;  /* bit[28]   : 下行分集BBPMaster搬运复位脉冲。 */
        unsigned int  reserved_7                  : 3;  /* bit[29-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_START_STOP_UNION;
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_start_START       (0)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_start_END         (0)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_stop_START        (4)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_stop_END          (4)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_force_stop_START  (8)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_force_stop_END    (8)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_trans_clr_START   (12)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at1_sdr_trans_clr_END     (12)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_start_START       (16)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_start_END         (16)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_stop_START        (20)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_stop_END          (20)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_force_stop_START  (24)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_force_stop_END    (24)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_trans_clr_START   (28)
#define WBBP_DLFE_SDR_START_STOP_cpu_dlfe_at2_sdr_trans_clr_END     (28)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_UNION
...
typedef union
{
    ...
    struct
    {
        unsigned int  cpu_dlfe_sdr_buffer_depth : 12; /* bit[0-11] : 下行Buffer深度，单位32word，用于计算Buffer首地址的时间戳。
                                                                     ...
                                                                     G模软化含义无效。 */
        unsigned int  reserved_0                : 4;  /* bit[12-15]: 保留 */
        unsigned int  cpu_dlfe_slot_head_offset : 11; /* bit[16-26]: 下行时隙头相对sys_cnt时隙边界的提前量，单位chip。
                                                                     该参数仅在C模时有效。 */
        unsigned int  reserved_1                : 5;  /* bit[27-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_UNION;
#define WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_cpu_dlfe_sdr_buffer_depth_START  (0)
#define WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_cpu_dlfe_sdr_buffer_depth_END    (11)
#define WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_cpu_dlfe_slot_head_offset_START  (16)
#define WBBP_DLFE_SDR_OFFSET_BUFF_DEPTH_cpu_dlfe_slot_head_offset_END    (26)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_TIME_CTRL_UNION
...
} WBBP_DLFE_SDR_TIME_CTRL_UNION;
#define WBBP_DLFE_SDR_TIME_CTRL_cpu_dlfe_sdr_start_time_START  (0)
#define WBBP_DLFE_SDR_TIME_CTRL_cpu_dlfe_sdr_start_time_END    (21)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_TIME_CTRL2_UNION
...
} WBBP_DLFE_SDR_TIME_CTRL2_UNION;
#define WBBP_DLFE_SDR_TIME_CTRL2_cpu_dlfe_sdr_data_length_START  (0)
#define WBBP_DLFE_SDR_TIME_CTRL2_cpu_dlfe_sdr_data_length_END    (11)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_WORD_DEPTH_UNION
...
} WBBP_DLFE_SDR_WORD_DEPTH_UNION;
#define WBBP_DLFE_SDR_WORD_DEPTH_cpu_dlfe_sdr_word_depth_START  (0)
#define WBBP_DLFE_SDR_WORD_DEPTH_cpu_dlfe_sdr_word_depth_END    (15)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_dlfe_sdr_at1_rd_chip_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT1_RD_CHIP_RPT_dlfe_sdr_at1_rd_chip_cnt_cpu_END    (5)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_RD_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_RD_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_RD_RPT_dlfe_sdr_at1_rd_block_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT1_RD_RPT_dlfe_sdr_at1_rd_block_cnt_cpu_END    (11)
#define WBBP_DLFE_SDR_AT1_RD_RPT_dlfe_sdr_at1_rd_page_cnt_cpu_START   (12)
#define WBBP_DLFE_SDR_AT1_RD_RPT_dlfe_sdr_at1_rd_page_cnt_cpu_END     (31)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_UNION
 结构说明  : DLFE_SDR_AT2_RD_CHIP_RPT 寄存器结构定义。地址偏移量:0x048，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_rd_chip_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_rd_chip_cnt_cpu : 6;  /* bit[0-5] : 下行天线2当前搬运word数。 */
        unsigned int  reserved                     : 26; /* bit[6-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_dlfe_sdr_at2_rd_chip_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT2_RD_CHIP_RPT_dlfe_sdr_at2_rd_chip_cnt_cpu_END    (5)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_RD_RPT_UNION
 结构说明  : DLFE_SDR_AT2_RD_RPT 寄存器结构定义。地址偏移量:0x04C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_rd_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_rd_block_cnt_cpu : 12; /* bit[0-11] : 下行天线2当前搬运块数。根据cpu_dlfe_sdr_buffer_depth统计。 */
        unsigned int  dlfe_sdr_at2_rd_page_cnt_cpu  : 20; /* bit[12-31]: 下行天线2当前搬运页数。根据cpu_dlfe_sdr_buffer_depth统计。 */
    } reg;
} WBBP_DLFE_SDR_AT2_RD_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_RD_RPT_dlfe_sdr_at2_rd_block_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT2_RD_RPT_dlfe_sdr_at2_rd_block_cnt_cpu_END    (11)
#define WBBP_DLFE_SDR_AT2_RD_RPT_dlfe_sdr_at2_rd_page_cnt_cpu_START   (12)
#define WBBP_DLFE_SDR_AT2_RD_RPT_dlfe_sdr_at2_rd_page_cnt_cpu_END     (31)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_dlfe_sdr_at1_1st_stamp_cpu_START  (0)
#define WBBP_DLFE_SDR_AT1_1ST_STAMP_RPT_dlfe_sdr_at1_1st_stamp_cpu_END    (27)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_UNION
 结构说明  : DLFE_SDR_AT2_1ST_STAMP_RPT 寄存器结构定义。地址偏移量:0x054，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_1st_stamp_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_1st_stamp_cpu : 28; /* bit[0-27] : 下行天线2启动搬运后第一个数据对应base_cnt时间戳，单位chip。
                                                                      时间戳含义同上。 */
        unsigned int  reserved                   : 4;  /* bit[28-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_dlfe_sdr_at2_1st_stamp_cpu_START  (0)
#define WBBP_DLFE_SDR_AT2_1ST_STAMP_RPT_dlfe_sdr_at2_1st_stamp_cpu_END    (27)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_dlfe_sdr_at1_bufferhead_stamp_cpu_START  (0)
#define WBBP_DLFE_SDR_AT1_BUFFER_STAMP_RPT_dlfe_sdr_at1_bufferhead_stamp_cpu_END    (27)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_UNION
 结构说明  : DLFE_SDR_AT2_BUFFER_STAMP_RPT 寄存器结构定义。地址偏移量:0x05C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_buffer_stamp_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_bufferhead_stamp_cpu : 28; /* bit[0-27] : 下行天线2启动搬运后Buffer首地址数据对应时间戳，经过时隙头锁存。
                                                                             时间戳含义同上。
                                                                             时隙头含义同上。 */
        unsigned int  reserved                          : 4;  /* bit[28-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_dlfe_sdr_at2_bufferhead_stamp_cpu_START  (0)
#define WBBP_DLFE_SDR_AT2_BUFFER_STAMP_RPT_dlfe_sdr_at2_bufferhead_stamp_cpu_END    (27)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_dlfe_sdr_at1_dump_en_START  (0)
#define WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_dlfe_sdr_at1_dump_en_END    (0)
#define WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_dlfe_sdr_at1_err_sta_START  (4)
#define WBBP_DLFE_SDR_AT1_DUMP_EN_RPT_dlfe_sdr_at1_err_sta_END    (4)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_UNION
 结构说明  : DLFE_SDR_AT2_DUMP_EN_RPT 寄存器结构定义。地址偏移量:0x074，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_dump_en_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_dump_en : 1;  /* bit[0]   : 下行天线2搬运状态。 */
        unsigned int  reserved_0           : 3;  /* bit[1-3] : 保留 */
        unsigned int  dlfe_sdr_at2_err_sta : 1;  /* bit[4]   : 下行天线2搬运出错指示。 */
        unsigned int  reserved_1           : 27; /* bit[5-31]: 保留 */
    } reg;
} WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_dlfe_sdr_at2_dump_en_START  (0)
#define WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_dlfe_sdr_at2_dump_en_END    (0)
#define WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_dlfe_sdr_at2_err_sta_START  (4)
#define WBBP_DLFE_SDR_AT2_DUMP_EN_RPT_dlfe_sdr_at2_err_sta_END    (4)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT1_RD_WORD_RPT_UNION
...
} WBBP_DLFE_SDR_AT1_RD_WORD_RPT_UNION;
#define WBBP_DLFE_SDR_AT1_RD_WORD_RPT_dlfe_sdr_at1_rd_word_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT1_RD_WORD_RPT_dlfe_sdr_at1_rd_word_cnt_cpu_END    (15)
#define WBBP_DLFE_SDR_AT1_RD_WORD_RPT_dlfe_sdr_at1_rd_seg_cnt_cpu_START   (16)
#define WBBP_DLFE_SDR_AT1_RD_WORD_RPT_dlfe_sdr_at1_rd_seg_cnt_cpu_END     (31)


/*****************************************************************************
 结构名    : WBBP_DLFE_SDR_AT2_RD_WORD_RPT_UNION
 结构说明  : DLFE_SDR_AT2_RD_WORD_RPT 寄存器结构定义。地址偏移量:0x07C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dlfe_sdr_at2_rd_word_rpt_reg;
    struct
    {
        unsigned int  dlfe_sdr_at2_rd_word_cnt_cpu : 16; /* bit[0-15] : 下行天线2当前搬运字数。根据cpu_dlfe_sdr_word_depth统计。
                                                                        建议在G模软化时使用。 */
        unsigned int  dlfe_sdr_at2_rd_seg_cnt_cpu  : 16; /* bit[16-31]: 下行天线2当前搬运页数。根据cpu_dlfe_sdr_word_depth统计。
                                                                        建议在G模软化时使用。 */
    } reg;
} WBBP_DLFE_SDR_AT2_RD_WORD_RPT_UNION;
#define WBBP_DLFE_SDR_AT2_RD_WORD_RPT_dlfe_sdr_at2_rd_word_cnt_cpu_START  (0)
#define WBBP_DLFE_SDR_AT2_RD_WORD_RPT_dlfe_sdr_at2_rd_word_cnt_cpu_END    (15)
#define WBBP_DLFE_SDR_AT2_RD_WORD_RPT_dlfe_sdr_at2_rd_seg_cnt_cpu_START   (16)
#define WBBP_DLFE_SDR_AT2_RD_WORD_RPT_dlfe_sdr_at2_rd_seg_cnt_cpu_END     (31)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_EN_UNION
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int  reserved_0              : 3;  /* bit[1-3]  : 保留 */
        unsigned int  cpu_ulfe_sdr_da_on      : 1;  /* bit[4]    : 上行da_on，高电平有效。BBP经sys_cnt时隙边界同步再使用。
                                                                   该参数仅在C模时有效。 */
        unsigned int  reserved_1              : 3;  /* bit[5-7]  : 保留 */
        unsigned int  cpu_ulfe_sdr_mode       : 2;  /* bit[8-9]  : 上行SDR IF模块工作模式。0和1表示非C模；2表示1X；3表示EVDO。
                                                                   该参数仅在C模时有效。 */
        unsigned int  reserved_2              : 14; /* bit[10-23]: 保留 */
        unsigned int  cpu_ulfe_sdr_trans_mode : 1;  /* bit[24]   : 上行搬运模式。0，DMA方式；1，BBPMaster方式。
                                                                   建议采用BBPMaster方式。 */
        unsigned int  reserved_3              : 7;  /* bit[25-31]: 保留 */
    } reg;
} WBBP_ULFE_SDR_EN_UNION;
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_en_START          (0)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_en_END            (0)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_da_on_START       (4)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_da_on_END         (4)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_mode_START        (8)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_mode_END          (9)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_trans_mode_START  (24)
#define WBBP_ULFE_SDR_EN_cpu_ulfe_sdr_trans_mode_END    (24)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_CKG_UNION
...
} WBBP_ULFE_SDR_CKG_UNION;
#define WBBP_ULFE_SDR_CKG_cpu_ulfe_sdr_ckg_bypass_START  (0)
#define WBBP_ULFE_SDR_CKG_cpu_ulfe_sdr_ckg_bypass_END    (0)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_START_UNION
...
typedef union
{
    ...
    {
        unsigned int  reserved_0               : 4;  /* bit[0-3] : 保留 */
        unsigned int  cpu_ulfe_sdr_trans_start : 1;  /* bit[4]   : BBPMaster搬运方式下，C模数据搬运启动脉冲。 */
        unsigned int  cpu_ulfe_sdr_trans_clr   : 1;  /* bit[5]   : BBPMaster搬运复位脉冲。 */
        unsigned int  reserved_1               : 26; /* bit[6-31]: 保留 */
    } reg;
} WBBP_ULFE_SDR_START_UNION;
#define WBBP_ULFE_SDR_START_cpu_ulfe_sdr_trans_start_START  (4)
#define WBBP_ULFE_SDR_START_cpu_ulfe_sdr_trans_start_END    (4)
#define WBBP_ULFE_SDR_START_cpu_ulfe_sdr_trans_clr_START    (5)
#define WBBP_ULFE_SDR_START_cpu_ulfe_sdr_trans_clr_END      (5)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_OFFSET_UNION
...
} WBBP_ULFE_SDR_OFFSET_UNION;
#define WBBP_ULFE_SDR_OFFSET_cpu_ulfe_slot_offset_START  (0)
#define WBBP_ULFE_SDR_OFFSET_cpu_ulfe_slot_offset_END    (12)
#define WBBP_ULFE_SDR_OFFSET_cpu_ulfe_ach_offset_START   (16)
#define WBBP_ULFE_SDR_OFFSET_cpu_ulfe_ach_offset_END     (26)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_ACTIVE_TIME_UNION
...
} WBBP_ULFE_SDR_ACTIVE_TIME_UNION;
#define WBBP_ULFE_SDR_ACTIVE_TIME_cpu_ulfe_dbb_active_time_START         (0)
#define WBBP_ULFE_SDR_ACTIVE_TIME_cpu_ulfe_dbb_active_time_END           (12)
#define WBBP_ULFE_SDR_ACTIVE_TIME_cpu_ulfe_phase_comp_active_time_START  (16)
#define WBBP_ULFE_SDR_ACTIVE_TIME_cpu_ulfe_phase_comp_active_time_END    (28)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_DPD_ACTIVE_TIME_UNION
 结构说明  : ULFE_SDR_DPD_ACTIVE_TIME 寄存器结构定义。地址偏移量:0x114，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      ulfe_sdr_dpd_active_time_reg;
    struct
    {
        unsigned int  cpu_ulfe_dpd_active_time : 13; /* bit[0-12] : C模时，表示上行dpd功率提前时隙头生效位置，单位1/4chip。 */
        unsigned int  reserved                 : 19; /* bit[13-31]: 保留 */
    } reg;
} WBBP_ULFE_SDR_DPD_ACTIVE_TIME_UNION;
#define WBBP_ULFE_SDR_DPD_ACTIVE_TIME_cpu_ulfe_dpd_active_time_START  (0)
#define WBBP_ULFE_SDR_DPD_ACTIVE_TIME_cpu_ulfe_dpd_active_time_END    (12)


/*****************************************************************************
 结构名    : WBBP_ULFE_SDR_DUMP_EN_RPT_UNION
...
} WBBP_ULFE_SDR_DUMP_EN_RPT_UNION;
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_dump_en_START    (0)
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_dump_en_END      (0)
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_err_sta_START    (4)
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_err_sta_END      (4)
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_trans_sta_START  (8)
#define WBBP_ULFE_SDR_DUMP_EN_RPT_ulfe_sdr_trans_sta_END    (8)


/*****************************************************************************
 结构名    : WBBP_DUMP_START_TIME_W_UNION
 结构说明  : DUMP_START_TIME_W 寄存器结构定义。地址偏移量:0x204，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dump_start_time_w_reg;
    struct
    {
        unsigned int  dump_start_time_clk_w  : 4;  /* bit[0-3]  : WC前端AGC采数采数起始位置。全1表示立即启动。
                                                                  W模，chip内clk位置。 */
        unsigned int  dump_start_time_chip_w : 12; /* bit[4-15] : WC前端AGC采数采数起始位置。全1表示立即启动。
                                                                  W模，slot内chip位置。 */
        unsigned int  dump_start_time_slot_w : 4;  /* bit[16-19]: WC前端AGC采数采数起始位置。全1表示立即启动。
                                                                  W模，帧内slot位置。 */
        unsigned int  dump_start_time_cfn_w  : 12; /* bit[20-31]: WC前端AGC采数采数起始位置。全1表示立即启动。
                                                                  W模，CFN位置。
                                                                  C模，bit[28:0]表示c_base_cnt上的位置，bit[31:29]配置为0。 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : DUMP_START_W 寄存器结构定义。地址偏移量:0x208，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dump_start_w_reg;
    struct
    {
        unsigned int  dump_start_w : 1;  /* bit[0]   : WC前端AGC采数启动脉冲。 */
        unsigned int  reserved     : 31; /* bit[1-31]: 保留 */
    } reg;
...
/*****************************************************************************
 ...
 结构说明  : DUMP_STOP_W 寄存器结构定义。地址偏移量:0x20C，初值:0x00000000，宽度:32
 寄存器说明: 
*****************************************************************************/
typedef union
{
    unsigned int      dump_stop_w_reg;
    struct
    {
        unsigned int  dump_stop_w : 1;  /* bit[0]   : WC前端AGC采数停止脉冲。 */
        unsigned int  reserved    : 31; /* bit[1-31]: 保留 */
    } reg;
...
} WBBP_ULFE_SDR_RAM_UNION;
#define WBBP_ULFE_SDR_RAM_ulfe_sdr_data_i_START  (0)
#define WBBP_ULFE_SDR_RAM_ulfe_sdr_data_i_END    (11)
#define WBBP_ULFE_SDR_RAM_ulfe_sdr_data_q_START  (16)
#define WBBP_ULFE_SDR_RAM_ulfe_sdr_data_q_END    (27)
