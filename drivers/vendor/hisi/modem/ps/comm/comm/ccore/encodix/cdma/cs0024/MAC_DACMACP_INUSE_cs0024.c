...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_ACAck
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MAC_DACMACP_INUSE_ACAck (char* Buffer, ED_EXLONG BitOffset, const c_MAC_DACMACP_INUSE_ACAck* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
...
{
	return L3_ENCODE_c_MAC_DACMACP_INUSE_ACAck (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_ACAck
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MAC_DACMACP_INUSE_ACAck (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_ACAck
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MAC_DACMACP_INUSE_ACAck (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MAC_DACMACP_INUSE_ACAck* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MAC_DACMACP_INUSE_ACAck (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_AccessParameters
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MAC_DACMACP_INUSE_AccessParameters (char* Buffer, ED_EXLONG BitOffset, const c_MAC_DACMACP_INUSE_AccessParameters* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
	...
	/* Code for contents */
	{
		...
		Len = ENCODE_c_MAC_DACMACP_INUSE_AccessParameters_contents (Buffer, CurrOfs, (&(Source->contents)), pCSN1CContextSet);
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
		if (Len < 0) {
			ED_SIGNAL_ERROR ("<ERRID:481> Error encoding subfield contents\n");
			return -1;
		} 
#endif
		CurrOfs += Len;
...
{
	return L3_ENCODE_c_MAC_DACMACP_INUSE_AccessParameters (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_AccessParameters
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MAC_DACMACP_INUSE_AccessParameters (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MAC_DACMACP_INUSE_AccessParameters
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MAC_DACMACP_INUSE_AccessParameters (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MAC_DACMACP_INUSE_AccessParameters* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MAC_DACMACP_INUSE_AccessParameters (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...

{
	...
	RetLen = DECODE_c_MAC_DACMACP_INUSE_AccessParameters_contents (Buffer, CurrOfs, (&(Destin->contents)), Len, pCSN1CContextSet);
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
	if (RetLen != Len) {
		ED_SIGNAL_ERROR ("<ERRID:482> Error decoding subfield contents\n");
		ED_HANDLE_MANDATORY_IE_SYNTAX_ERROR ((-1), 482, (Destin->contents))
	} 
#endif
	CurrOfs += Len;
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
	if (CurrOfs > (BitOffset+Length)) {
		ED_SIGNAL_ERROR ("<ERRID:483> Message too short decoding subfield contents\n");
		ED_HANDLE_MANDATORY_IE_MSG_TOO_SHORT ((-1), 482, (Destin->contents))
	} 
#endif
