...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonRequest
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_XonRequest (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_XonRequest* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XonRequest (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonRequest
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XonRequest (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonRequest
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_XonRequest (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_XonRequest* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_XonRequest (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonResponse
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_XonResponse (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_XonResponse* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XonResponse (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonResponse
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XonResponse (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XonResponse
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_XonResponse (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_XonResponse* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_XonResponse (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffRequest
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffRequest (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_XoffRequest* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffRequest (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffRequest
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffRequest (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffRequest
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_XoffRequest (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_XoffRequest* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_XoffRequest (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffResponse
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffResponse (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_XoffResponse* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffResponse (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffResponse
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_XoffResponse (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_XoffResponse
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_XoffResponse (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_XoffResponse* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_XoffResponse (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReady
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReady (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_DataReady* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
	...
	/* Code for contents */
	{
		...
		Len = ENCODE_c_MFPAPP_FCP_INUSE_DataReady_contents (Buffer, CurrOfs, (&(Source->contents)), pCSN1CContextSet);
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReady (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReady
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReady (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReady
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_DataReady (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_DataReady* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_DataReady (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...

{
	...
	RetLen = DECODE_c_MFPAPP_FCP_INUSE_DataReady_contents (Buffer, CurrOfs, (&(Destin->contents)), Len, pCSN1CContextSet);
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
	if (RetLen != Len) {
...
#define THIS Source
/*-----------------------------------------------------------------
	ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReadyAck
	Mode: 
		0=normal (full message)
		1=body only
-----------------------------------------------------------------*/
ED_EXLONG L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReadyAck (char* Buffer, ED_EXLONG BitOffset, const c_MFPAPP_FCP_INUSE_DataReadyAck* Source, int Mode, struct SCSN1CContextSet* pCSN1CContextSet)
{
	int Len=0;
	ED_EXLONG CurrOfs = BitOffset+Len;
	ED_EXTRAPARAMS_CODE
	...
	{
		/* Append to buffer value part DEBUG FIELD=<Source->contents> DOT=<.> */
		Len = ENCODE_c_MFPAPP_FCP_INUSE_DataReadyAck_contents (Buffer, CurrOfs, (&(Source->contents)), pCSN1CContextSet);
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
		if (Len < 0) {
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReadyAck (Buffer, BitOffset, Source, 0, pCSN1CContextSet);
}

/*-----------------------------------------------------------------
	BODY-ONLY ENCODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReadyAck
-----------------------------------------------------------------*/
...
{
	return L3_ENCODE_c_MFPAPP_FCP_INUSE_DataReadyAck (Buffer, BitOffset, Source, 1, pCSN1CContextSet);
}

#undef THIS
#define THIS Destin
/*-----------------------------------------------------------------
	DECODING FUNCTION FOR GSM-L3 MESSAGE MFPAPP_FCP_INUSE_DataReadyAck
-----------------------------------------------------------------*/
ED_EXPORT_CALL (ED_EXLONG, DECODE_c_MFPAPP_FCP_INUSE_DataReadyAck (const char* ED_CONST Buffer, ED_CONST ED_EXLONG BitOffset, c_MFPAPP_FCP_INUSE_DataReadyAck* ED_CONST Destin, ED_EXLONG Length, struct SCSN1CContextSet* pCSN1CContextSet))
{
	...
	Len = DECODE_BODY_c_MFPAPP_FCP_INUSE_DataReadyAck (Buffer, (BitOffset+8), Destin, Length-(8), pCSN1CContextSet);
	if (Len < 0) return Len;
...

{
	...
	Len = Length + BitOffset - CurrOfs;
	/* Read value part LOCATOR_16 */
	RetLen = DECODE_c_MFPAPP_FCP_INUSE_DataReadyAck_contents (Buffer, CurrOfs, (&(Destin->contents)), Len, pCSN1CContextSet);
#ifndef ED_NO_EXTENSIVE_ERROR_LOGGING
	if (RetLen != Len) {
