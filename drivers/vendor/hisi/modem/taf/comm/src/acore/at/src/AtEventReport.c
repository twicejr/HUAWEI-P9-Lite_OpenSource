...
AT_ENCRYPT_VOICE_ERR_CODE_MAP_STRU                  g_astAtEncVoiceErrCodeMapTbl[] =
{
    ...
    { AT_ENCRYPT_VOICE_SO_NEGO_FAILURE,                     TAF_CALL_APP_ENCRYPT_VOICE_XSMS_SEND_RESULT_UNKNOWN},
...
static AT_PH_SYS_MODE_TBL_STRU g_astSysModeTbl[] =
{
    ...
    {MN_PH_SYS_MODE_EX_SVLTE_RAT    ,"CDMA1X+LTE(SVLTE)"}
};
    ...
    switch(pEvent->SsEvent)             /* 其它事件 */
    {
        ...
        case TAF_SS_EVT_PROCESS_USS_REQ_CNF:
            ...
            if((1 == pEvent->OP_UssdString) && (1 == pEvent->OP_DataCodingScheme))
            {
                ...
                if (pEvent->UssdString.usCnt > sizeof(pEvent->UssdString.aucUssdStr))
                {
                    AT_WARN_LOG1("At_SsIndProc: Invalid pEvent->UssdString.usCnt: ", pEvent->UssdString.usCnt);
                    pEvent->UssdString.usCnt = sizeof(pEvent->UssdString.aucUssdStr);
                }
    ...
    *pusLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,(TAF_CHAR *)pgucAtSndCodeAddr + *pusLength,"+CUSD: %d",ucTmp);
    ...
    if((0 == pEvent->OP_Error) && ((1 == pEvent->OP_UssdString) || (1 == pEvent->OP_USSData)) && (1 == pEvent->OP_DataCodingScheme))
    {
        *pusLength += (TAF_UINT16)At_sprintf(AT_CMD_MAX_LEN,(TAF_CHAR *)pgucAtSndCodeAddr,(TAF_CHAR *)pgucAtSndCodeAddr + *pusLength,",\"");
        ...
        if (pEvent->UssdString.usCnt > sizeof(pEvent->UssdString.aucUssdStr))
        {
            AT_WARN_LOG1("At_SsRspUssdProc: Invalid pEvent->UssdString.usCnt: ", pEvent->UssdString.usCnt);
            pEvent->UssdString.usCnt = sizeof(pEvent->UssdString.aucUssdStr);
        }
...
)
{
    ...
    ucIndex = 0;
    PS_MEM_SET(&stRatOrder, 0, sizeof(stRatOrder));
...
)
{
    ...
    switch (pstSysCfg->stRatOrder.aenRatOrder[0])
    {
        ...
        case TAF_MMA_RAT_1X:
            if (VOS_TRUE == AT_IsSupportHrpdMode(&pstSysCfg->stRatOrder))
            {
        ...
        case TAF_MMA_RAT_HRPD:
            if (VOS_TRUE == AT_IsSupport1XMode(&pstSysCfg->stRatOrder))
            {
                enAccessMode = AT_SYSCFG_RAT_1X_AND_HRPD;
            }
            else
...
)
{
    ...
    for (i = 0; i < pstRatOrder->ucRatOrderNum; i++)
    {
        ...
        {
             VOS_StrCpy((VOS_CHAR *)pucAcqOrder, "04");
             pucAcqOrder += AT_SYSCFGEX_RAT_MODE_STR_LEN;
        }
        else if (TAF_MMA_RAT_HRPD == pstRatOrder->aenRatOrder[i])
        {
             VOS_StrCpy((VOS_CHAR *)pucAcqOrder, "07");
             pucAcqOrder += AT_SYSCFGEX_RAT_MODE_STR_LEN;
        }
...
)
{
    ...
    /* 把上报的TAF_MMA_RAT_ORDER_STRU结构转换为acqorder字符串*/
...
)
{
    ...
    PS_MEM_SET(&stCsgListPara, 0, sizeof(stCsgListPara));
...
)
{
    ...
    PS_MEM_SET(&stListParm, 0, sizeof(MN_MSG_LIST_PARM_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCops, 0, sizeof(TAF_PH_NETWORKNAME_STRU));
...
)
{
    ...
    PS_MEM_SET(&stSysInfo, 0, sizeof(TAF_PH_SYSINFO_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCreg, 0, sizeof(TAF_PH_REG_STATE_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCgreg, 0, sizeof(TAF_PH_REG_STATE_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCereg, 0, sizeof(TAF_PH_REG_STATE_STRU));
...
)
{
    ...
    usLength = 0;
    PS_MEM_SET(&stIccId, 0, sizeof(TAF_PH_ICC_ID_STRU));
...
)
{
    ...
    /* 初始化 */
    ulResult   = AT_OK;
    PS_MEM_SET(&stAnqueryPara, 0, sizeof(MN_MMA_ANQUERY_PARA_STRU));
...
)
{
    ...
    PS_MEM_CPY(&stHplmn, pPara, sizeof(TAF_MMA_HPLMN_WITH_MNC_LEN_STRU));
...
)
{
    ...
    if (AT_IS_BROADCAST_CLIENT_INDEX(ucIndex))
    {
        ...
        if (( ID_EVT_TAF_PS_REPORT_DSFLOW_IND != pstEvt->ulEvtId )
         ...
         && (ID_EVT_TAF_PS_CGMTU_VALUE_CHG_IND != pstEvt->ulEvtId))
        {
...
)
{
    ...
    switch ( gastAtClientTab[ucIndex].UserType )
    {
        ...
        case AT_CTR_USER:
        case AT_NDIS_USER:
            if (VOS_TRUE == g_abAtNdisDialUp[ucIndex])
            {
                g_abAtNdisDialUp[ucIndex] = VOS_FALSE;
...
)
{
    ...
    {
        if (VOS_TRUE == g_abAtNdisDialDown[ucIndex])
        {
            g_abAtNdisDialDown[ucIndex] = VOS_FALSE;
...
)
{
    ...
    PS_MEM_SET(&stCgdcont, 0, sizeof(TAF_PRI_PDP_QUERY_INFO_STRU));
...
)
{
    ...
    PS_MEM_SET(&stSecPdpInfo, 0, sizeof(TAF_PDP_SEC_CONTEXT_STRU));
...
)
{
    ...
    PS_MEM_SET(aucLocalIpv4StrTmp, 0x00, sizeof(aucLocalIpv4StrTmp));
    PS_MEM_SET(aucLocalIpv6StrTmp, 0x00, sizeof(aucLocalIpv6StrTmp));
...
)
{
    ...
    PS_MEM_SET(&stCgeq, 0, sizeof(TAF_UMTS_QOS_QUERY_INFO_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCgact, 0, sizeof(TAF_CID_STATE_STRU));
...
)
{
    ...
    PS_MEM_SET(aucIPv6Str, 0x00, sizeof(aucIPv6Str));
    PS_MEM_SET(&stPdpAddrQuery, 0x00, sizeof(TAF_PDP_ADDR_QUERY_INFO_STRU));
...
)
{
    ...
    PS_MEM_SET(acIpv4StrTmp,  0, TAF_MAX_IPV4_ADDR_STR_LEN);
    PS_MEM_SET(aucIpv6StrTmp, 0, TAF_MAX_IPV6_ADDR_DOT_STR_LEN);
    PS_MEM_SET(&stCgdcont,    0, sizeof(TAF_PDP_DYNAMIC_PRIM_EXT_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCgdscont, 0, sizeof(TAF_PDP_DYNAMIC_SEC_EXT_STRU));
...
)
{
    ...
    PS_MEM_SET(&stCgeqos, 0, sizeof(TAF_EPS_QOS_EXT_STRU));
...
)
{
    ...
    PS_MEM_SET(&stPdpDns, 0, sizeof(TAF_DNS_QUERY_INFO_STRU));
...
)
{
    ...
    PS_MEM_SET(&stPdpAuthData, 0, sizeof(TAF_AUTHDATA_QUERY_INFO_STRU));
...
#if (FEATURE_ON == FEATURE_ECALL)
         || (AT_CMD_ECLSTOP_SET == gastAtClientTab[ucIndex].CmdCurrentOpt)
#endif
         )
        {
            ulResult = AT_OK;
