...
//0x10: command_mode,  0x03: video_mode
...
static struct dsi_cmd_desc lcd_display_command_cmds[] = {
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_MS,
		sizeof(command_mode), command_mode},
};

static struct dsi_cmd_desc lcd_display_video_cmds[] = {
	{DTYPE_DCS_WRITE1, 0, 10, WAIT_TYPE_MS,
...
static int mipi_jdi_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (is_mipi_cmd_panel(hisifd)) {
			mipi_dsi_cmds_tx(lcd_display_command_cmds, \
				ARRAY_SIZE(lcd_display_command_cmds), mipi_dsi0_base);
		} else {
			mipi_dsi_cmds_tx(lcd_display_video_cmds, \
				ARRAY_SIZE(lcd_display_video_cmds), mipi_dsi0_base);
		}
	...
	char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "jdi_NT35695 5.2' CMD TFT 1080 x 1920\n");
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	pinfo->mipi.rg_vrefsel_vcm_adjust = 6;
