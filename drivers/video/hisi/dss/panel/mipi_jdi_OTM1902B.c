...
static struct dsi_cmd_desc lcd_video_mode_display_cmds[] = {
	{DTYPE_DCS_WRITE1, 0,10, WAIT_TYPE_US,
...
static struct dsi_cmd_desc lcd_ifbc_orise2x_en_cmds[] = {
	{DTYPE_DCS_WRITE1, 0,10, WAIT_TYPE_US,
...
static int mipi_jdi_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		mipi_dsi_cmds_tx(lcd_display_effect_cmds, \
			ARRAY_SIZE(lcd_display_effect_cmds), mipi_dsi0_base);

		if (pinfo->ifbc_type == IFBC_TYPE_ORISE2X)
			mipi_dsi_cmds_tx(lcd_ifbc_orise2x_en_cmds, \
			ARRAY_SIZE(lcd_ifbc_orise2x_en_cmds), mipi_dsi0_base);

		if (!is_mipi_cmd_panel(hisifd))
			mipi_dsi_cmds_tx(lcd_video_mode_display_cmds, \
			ARRAY_SIZE(lcd_video_mode_display_cmds), mipi_dsi0_base);
