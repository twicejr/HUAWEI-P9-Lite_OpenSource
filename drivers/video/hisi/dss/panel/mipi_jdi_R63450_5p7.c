...
#define DTS_COMP_JDI_R63450_5P7 "hisilicon,mipi_jdi_R63450_5P7"
#define CABC_OFF	(0)
...
*/
static char manufacturer_command_on[] = {
	0xB0,
	0x04,
};
...
static char compression_method[] = {
	0xEB,
	0x00,
	0x83,
};
...
static char test_control[] = {
	0xD6,
	0x01,
};

static char manufacturer_command_off[] = {
	0xB0,
	0x03,
};
...
static struct dsi_cmd_desc jdi_display_on_cmds[] = {
	...
	{DTYPE_GEN_LWRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(manufacturer_command_on), manufacturer_command_on}
	,
	{DTYPE_GEN_LWRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(compression_method), compression_method}
	,
	{DTYPE_GEN_LWRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(compression_setting), compression_setting}
	,
	{DTYPE_GEN_LWRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(test_control), test_control}
	,
	{DTYPE_GEN_LWRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(manufacturer_command_off), manufacturer_command_off}
	,
	{DTYPE_DCS_WRITE, 0, 10, WAIT_TYPE_US,
	 sizeof(display_on), display_on}
	,
	{DTYPE_DCS_WRITE, 0, 90, WAIT_TYPE_MS,
...
/* set cabc pwm freq to 10KHz */
static char back_light_control[] = {
	0xCE,
	0x55, 0x40, 0x60, 0x78, 0x87,
	0x94, 0x9E, 0xAB, 0xB8, 0xC4,
	0xCD, 0xD6, 0xDE, 0xE5, 0xEC,
	0xF3, 0xFF, 0x09, 0x46, 0x04,
...
static char cabc_set_UI_para[] = {
	0xB8,
	0x2B, 0x4D, 0x05, 0x00, 0x08,
	0x00, 0x00,
};

static char cabc_set_still_para[] = {
	0xB9,
	0x6F, 0x3D, 0x28, 0x3C, 0x14,
	0xC8, 0xC8,
};

static char cabc_set_Moving_para[] = {
	0xBA,
	0x91, 0x53, 0x0A, 0x00, 0x0F,
	0x00, 0x00,
};

static char cabc_0xF9[] = {
	0xF9,
	0x64, 0x3F, 0xE0, 0xBE, 0xB6,
	0x99, 0x73, 0x30, 0x8D, 0xBF,
	0x80, 0x00,
};
...
static char color_enhancement[] = {
	0xCA,
	0x1D,0xFC,0xFC,0xFC,0x00,0x00,0x00,0x00,0x00,0x00,
	0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xED,
	0x00,0x00,0xEA,0x9E,0x86,0xE0,0xA7,0x90,0x48,0x00,
...
/* vcc get */
	...
	{DTYPE_VCC_GET, VCC_LCDANALOG_NAME, &vcc_lcdanalog, 0, 0, WAIT_TYPE_MS,
	 0},

/* vcc set voltage */
	{DTYPE_VCC_SET_VOLTAGE, VCC_LCDANALOG_NAME, &vcc_lcdanalog, 3100000,
	 3100000, WAIT_TYPE_MS, 0},
/* io set voltage */
...
/* vcc put */
	...
	{DTYPE_VCC_PUT, VCC_LCDANALOG_NAME, &vcc_lcdanalog, 0, 0, WAIT_TYPE_MS,
	 0},
};
...
/* lcd pinctrl normal */
		...
		if ((pinfo->bl_set_type & BL_SET_BY_BLPWM) || (pinfo->bl_set_type & BL_SET_BY_SH_BLPWM)) {
			mipi_dsi_cmds_tx(pwm_out_on_cmds,
					 ARRAY_SIZE(pwm_out_on_cmds),
					 mipi_dsi0_base);
		}

		/* lcd display on sequence */
		mipi_dsi_cmds_tx(jdi_display_on_cmds,
				 ARRAY_SIZE(jdi_display_on_cmds),
				 mipi_dsi0_base);
		g_cabc_mode = 1;
...
/* lcd display off sequence */
		mipi_dsi_cmds_tx(jdi_display_off_cmds,
				 ARRAY_SIZE(jdi_display_off_cmds),
				 mipi_dsi0_base);
...
/* lcd vcc disable */
				...
				HISI_FB_INFO
				    ("display off(regulator not need disabling).\n");

/* lcd gpio free */
				gpio_cmds_tx(lcd_gpio_sleep_free_cmds,
					     ARRAY_SIZE
					     (lcd_gpio_sleep_free_cmds));

/* lcd pinctrl normal */
				pinctrl_cmds_tx(pdev, lcd_pinctrl_normal_cmds,
						ARRAY_SIZE
						(lcd_pinctrl_normal_cmds));
			}
#ifdef CONFIG_HUAWEI_TS
...
*/
			...
			HISI_FB_INFO
			    ("display shutting down(regulator disabling).\n");
/* lcd gpio lowpower*/
			gpio_cmds_tx(asic_lcd_gpio_lowpower_cmds,
...
static ssize_t mipi_jdi_panel_lcd_gram_check_show(struct platform_device *pdev,
						  char *buf)
{
	ssize_t ret = 0;
						  ...
						  const char *buf, size_t count)
{
	...
	if (flag == CABC_OFF) {
		...
		mipi_dsi_cmds_tx(jdi_cabc_off_cmds,
				 ARRAY_SIZE(jdi_cabc_off_cmds), mipi_dsi0_base);
...
static ssize_t mipi_jdi_panel_lcd_check_reg_show(struct platform_device *pdev,
						 char *buf)
{
	ssize_t ret = 0;
	struct hisi_fb_data_type *hisifd = NULL;
	...
	char *reg_name[5] =
	    { "power mode", "MADCTR", "pixel format", "image mode",
		"mipi error"
	};
	...
	if (!mipi_dsi_read_compare(&data, mipi_dsi0_base)) {
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "P-0x0a:0x%x, 0x0b:0x%x, 0x0c:0x%x, 0x0d:0x%x, 0xab:0x%x\n",
			     data.read_value[0], data.read_value[1],
			     data.read_value[2], data.read_value[3],
			     data.read_value[4]);
	} else {
		ret =
		    snprintf(buf, PAGE_SIZE,
			     "F-0x0a:0x%x, 0x0b:0x%x, 0x0c:0x%x, 0x0d:0x%x, 0xab:0x%x\n",
			     data.read_value[0], data.read_value[1],
			     data.read_value[2], data.read_value[3],
			     data.read_value[4]);
	}
...
static ssize_t mipi_jdi_panel_lcd_mipi_detect_show(struct platform_device *pdev,
						   char *buf)
{
	ssize_t ret = 0;
	struct hisi_fb_data_type *hisifd = NULL;
...
static ssize_t mipi_jdi_panel_lcd_sleep_ctrl_show(struct platform_device *pdev,
						  char *buf)
{
	ssize_t ret = 0;
	struct hisi_fb_data_type *hisifd = NULL;
	...
	ret =
	    snprintf(buf, PAGE_SIZE,
		     ...
		     hisifd->panel_info.lcd_adjust_support);
	HISI_FB_DEBUG("fb%d, -.\n", hisifd->index);

	return ret;
}

static ssize_t mipi_jdi_panel_lcd_sleep_ctrl_store(struct platform_device *pdev,
						   const char *buf,
						   size_t count)
{
	ssize_t ret = 0;
						   ...
						   char *buf)
{
	if (!strncmp(lcd_cmd_now, "LCD_INCOME0", strlen("LCD_INCOME0")) ||	/* incoming sence0 config */
		...
		return snprintf(buf, PAGE_SIZE, "CURRENT1_0");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU1",
			       strlen("PROJECT_MENU1"))) {
		return snprintf(buf, PAGE_SIZE, "CURRENT1_1");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU2",
			       strlen("PROJECT_MENU2"))) {
		return snprintf(buf, PAGE_SIZE, "CURRENT1_2");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU3",
			       strlen("PROJECT_MENU3"))) {
		return snprintf(buf, PAGE_SIZE, "CURRENT1_3");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU4",
			       strlen("PROJECT_MENU4"))) {
		return snprintf(buf, PAGE_SIZE, "CURRENT1_4");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU5",
			       strlen("PROJECT_MENU5"))) {
		return snprintf(buf, PAGE_SIZE, "CURRENT1_5");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU6",
			       strlen("PROJECT_MENU6"))) {
		return snprintf(buf, PAGE_SIZE, "PCD");
		   ...
		   || !strncmp(lcd_cmd_now, "PROJECT_MENU7",
			       strlen("PROJECT_MENU7"))) {
		return snprintf(buf, PAGE_SIZE, "CHECKSUM");
	} else if (!strncmp(lcd_cmd_now, "CURRENT1_0", strlen("CURRENT1_0"))) {	/* current1 @ Color.WHITE */
		return snprintf(buf, PAGE_SIZE, "/sys/class/ina231/ina231_0/ina231_set," "/sys/class/ina231/ina231_0/ina231_value," "1,9999999,1,9999999,1,99999");	//vol_min,vol_max,power_min,power_max,curr_min,curr_max
...
static ssize_t mipi_jdi_panel_lcd_test_config_store(struct platform_device
						    *pdev, const char *buf,
						    size_t count)
{
	struct hisi_fb_data_type *hisifd = NULL;
...
static ssize_t mipi_jdi_panel_lcd_support_mode_show(struct platform_device
						    *pdev, char *buf)
{
	struct hisi_fb_data_type *hisifd = NULL;
	ssize_t ret = 0;
...
static ssize_t mipi_jdi_panel_lcd_support_mode_store(struct platform_device
						     *pdev, const char *buf,
						     size_t count)
{
	int ret = 0;
...
//Gama LCP
	...
	if (pinfo->gamma_support == 1) {
		...
		pinfo->gmp_lut_table_low32bit =
		    &gmp_lut_table_low32bit[0][0][0];
		pinfo->gmp_lut_table_high4bit =
		    &gmp_lut_table_high4bit[0][0][0];
