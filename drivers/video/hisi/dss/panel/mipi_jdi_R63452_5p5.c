...
#define DTS_COMP_JDI_R63452_5P5 "hisilicon,mipi_jdi_R63452_5P5"
...
static int mipi_jdi_panel_set_backlight(struct platform_device *pdev, uint32_t bl_level)
{
	...
	if (new_lcd_supported) {
		if (0x05 == panel_lcd_id) {
			...
			if (255 == hisifd->panel_info.bl_max) {
				if (bl_level > 187) {
					HISI_FB_INFO("bl_level over 187, set to 187\n");
					bl_level = 187;
				}
			} else {
				if (bl_level > 7288) {
					HISI_FB_INFO("bl_level over 7288, set to 7288\n");
					bl_level = 7288;
				}
			}
		}
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, DTS_COMP_JDI_R63452_5P5);
	if (!np) {
		HISI_FB_ERR("NOT FOUND device node %s!\n", DTS_COMP_JDI_R63452_5P5);
		goto err_return;
	}
	...
	HISI_FB_INFO("%s, lcd_id is 0x%x\n", DTS_COMP_JDI_R63452_5P5, panel_lcd_id);
...
static const struct of_device_id hisi_panel_match_table[] = {
	{
		.compatible = DTS_COMP_JDI_R63452_5P5,
		.data = NULL,
	},
	{},
};
