...
static struct dsi_cmd_desc lcd_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE, 0, 120, WAIT_TYPE_MS,  //10
...
static uint32_t gpio_lcd_vcc3v1 = 61;
...
static int mipi_jdi_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (!is_mipi_cmd_panel(hisifd))
			...
			ARRAY_SIZE(lcd_video_mode_display_cmds), mipi_dsi1_base);
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	if (g_lcd_fpga_flag == 1) {
		//ldi
		pinfo->ldi.h_back_porch = 46;//46; // 23
		pinfo->ldi.h_front_porch = 100;//100; // 50
		pinfo->ldi.h_pulse_width = 40;//30; // 20
		pinfo->ldi.v_back_porch = 12;
		pinfo->ldi.v_front_porch = 14;
		pinfo->ldi.v_pulse_width = 4;

		//mipi
		pinfo->mipi.dsi_bit_clk = 100;
		...
		//pinfo->mipi.clk_t_hs_trial_adjust = 200;
		pinfo->mipi.data_t_hs_trial_adjust = 20;
		
		pinfo->mipi.non_continue_en = 0;

		pinfo->pxl_clk_rate = 27 * 1000000; // 20 * 1000000
	} else {
