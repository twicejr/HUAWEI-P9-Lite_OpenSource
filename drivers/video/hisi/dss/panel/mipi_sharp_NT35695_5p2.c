...
static ssize_t mipi_sharp_panel_lcd_gram_check_show(struct platform_device *pdev, char *buf)
{
	...
	/* Checksum test only in factory mode */
	if (!runmode_is_factory()) {
		HISI_FB_INFO("Checksum test only in factory mode.\n");
		return ret;
	}
	...
	const char *buf, size_t count)
{
	...
	if(1 == val) {
		...
		/* checksum_pic_n 0~3,0:mmi,1,2,3:runningtest */
...
static int mipi_sharp_probe(struct platform_device *pdev)
{
	...
	HISI_FB_INFO("%s, lcd id is 0x%x\n", DTS_COMP_SHARP_NT35695_5P2, panel_lcd_id);
...
static const struct of_device_id hisi_panel_match_table[] = {
	{
		.compatible = DTS_COMP_SHARP_NT35695_5P2,
		.data = NULL,
	},
	{},
};
