...
static int proc_pid_readlink(struct dentry * dentry, char __user * buffer, int buflen)
{
	...
	if ( NULL==path.dentry || NULL==path.mnt ) {
		//reset error to eaccess, notify upper level that readlink fail
		error = -EACCES;
		goto null_out;//dput()/mntput() will check path.dentry & path.mnt's NULL ptr again
	}
