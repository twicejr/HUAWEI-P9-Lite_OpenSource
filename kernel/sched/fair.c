...
static u32 cpufreq_calc_scale(u32 min, u32 max, u32 curr)
{
	...
	/*
	 * make sure that curr is in the range
	 * when policy changed, there is a window that
	 * curr is not between min and max
	 */
	if (curr > (max << SCHED_FREQSCALE_SHIFT))
		curr = max << SCHED_FREQSCALE_SHIFT;
	else if (curr < (min << SCHED_FREQSCALE_SHIFT))
		curr = min << SCHED_FREQSCALE_SHIFT;
