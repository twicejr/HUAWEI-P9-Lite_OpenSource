/*
 * hi3630_srcup_dsp.c -- ALSA SoC HI3630 SRCUP DSP driver
 *
...
#define LOG_TAG "HI3630_SRCUP_DSP"
...
static const struct snd_pcm_hardware hi3630_srcup_dsp_hardware = {
...
static const struct of_device_id hi3630_srcup_dsp_match[] = {
	{ .compatible = "hisilicon,hi3630-srcup-dsp", },
	{},
};
MODULE_DEVICE_TABLE(of, hi3630_srcup_dsp_match);

static int hi3630_srcup_dsp_open(struct snd_pcm_substream *substream)
{
...
#endif
	ret = snd_soc_set_runtime_hwparams(substream, &hi3630_srcup_dsp_hardware);

	return ret;
}

static int hi3630_srcup_dsp_close(struct snd_pcm_substream *substream)
{
...
static struct snd_pcm_ops hi3630_srcup_dsp_ops = {
	.open	= hi3630_srcup_dsp_open,
	.close	= hi3630_srcup_dsp_close,
	.ioctl	= snd_pcm_lib_ioctl,
};

static unsigned long long hi3630_pcm_dmamask = DMA_BIT_MASK(32);

static int hi3630_srcup_dsp_new(struct snd_soc_pcm_runtime *rtd)
{
...
static void hi3630_srcup_dsp_free(struct snd_pcm *pcm)
{
	//do nothing
}

static struct snd_soc_platform_driver hi3630_srcup_dsp_platform = {
	.ops		= &hi3630_srcup_dsp_ops,
	.pcm_new	= hi3630_srcup_dsp_new,
	.pcm_free	= hi3630_srcup_dsp_free,
};

static int hi3630_srcup_dsp_probe(struct platform_device *pdev)
{
	int ret = -1;
	...
	pdata->regu.supply = "asp-srcup-dsp";
	...
	dev_set_name(dev, "hi3630-srcup-dsp");

	ret = snd_soc_register_platform(dev, &hi3630_srcup_dsp_platform);
	if (ret) {
...
static int hi3630_srcup_dsp_remove(struct platform_device *pdev)
{
...
#ifdef SRCUP_ENABLE
#ifdef CONFIG_PM_RUNTIME
int hi3630_srcup_dsp_runtime_suspend(struct device *dev)
{
...
int hi3630_srcup_dsp_runtime_resume(struct device *dev)
{
	struct hi3630_srcup_data *pdata = dev_get_drvdata(dev);
	int ret;
...
const struct dev_pm_ops hi3630_srcup_dsp_pm_ops = {
	.runtime_suspend	= hi3630_srcup_dsp_runtime_suspend,
	.runtime_resume		= hi3630_srcup_dsp_runtime_resume,
};
#endif
#endif

static struct platform_driver hi3630_srcup_dsp_driver = {
	.probe	= hi3630_srcup_dsp_probe,
	.remove	= hi3630_srcup_dsp_remove,
	.driver	= {
		.owner  = THIS_MODULE,
		.name	= "hi3630-srcup-dsp",
		.of_match_table = of_match_ptr(hi3630_srcup_dsp_match),
#ifdef SRCUP_ENABLE
#ifdef CONFIG_PM_RUNTIME
		.pm	= &hi3630_srcup_dsp_pm_ops,
#endif
#endif
	},
};

module_platform_driver(hi3630_srcup_dsp_driver);

MODULE_DESCRIPTION("ASoC hi3630 srcup dsp driver");
